...
#Verizon Functions
def check_datatype_for_HV(log_result,filename,directory,Datatype_result_for_HV,check_charles_timestamp_to_datetime):
...
def check_comment_to_option_for_HV(log_result,filename,directory,comment_to_option_result_for_HV):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    ...
    if (filename,directory) in inputFile:
        with open (os.path.join(inputFolderPath, filename), 'rb') as f:
            result = chardet.detect(f.read())
            e=result['encoding']
        with open (os.path.join(inputFolderPath, filename), "r",encoding=e) as fr:
            query = fr.read()
            fr.close()
...
def check_drop_raw_format_or_location_for_HV(log_result,filename,directory,drop_row_format_or_location_result_for_HV):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_create_table_external_for_HV(log_result,filename,directory,create_table_external_for_HV):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def No_Of_Statement_In_Script_Hive(log_result,filename,directory,Result_No_Of_Statement,fun_project_ID):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_cross_join_hive(log_result,filename,directory,JoinResult):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def PROJECT_ID_Hive(log_result,project_ID,filename,directory):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_colan_to_space(log_result,filename,directory,Result_check_colan_to_space):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    ...
    if (filename,directory) in outputFile:
        with open (os.path.join(OutputFolderPath, filename), 'rb') as f:
            result = chardet.detect(f.read())
            e=result['encoding']
        with open (os.path.join(OutputFolderPath, filename), "r",encoding=e) as fr:
            query = fr.read()
            fr.close()
...
def Check_map_to_array(log_result,filename,directory,Result_check_map_to_array):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_Check_par_col_datatype_change(log_result,filename,directory,result_check_par_col_datatype_change,table_column_list):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_Check_add_option_change(log_result,filename,directory,result_Check_add_option_change,table_option_list):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("check_par_col_datatype_change"))
    input_option=[]
    output_option=[]
...
def Check_PartitionBy_to_ClusterBy(log_result,filename,directory,result_Check_PartitionBy_to_ClusterBy):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    ...
    try:
        ...
        if (filename,directory) in outputFile:
            with open (os.path.join(OutputFolderPath, filename), 'rb') as f:
                result = chardet.detect(f.read())
                e=result['encoding']
            with open (os.path.join(OutputFolderPath, filename), "r",encoding=e) as finl:
                query = finl.read()
                finl.close()
...
def Check_clusterBy_create_cols(log_result,filename,directory,result_Check_clusterBy_create_cols):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Partitioned_BY_to_Partition_By(log_result,filename,directory,result_Check_Partitioned_BY_to_Partition_By):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Partition to cluster by check"))
...
def check_for_current_functions_CS(log_result,filename,directory,result_check_for_current_functions):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    ...
    try:
        ...
        # print("input_current_func=",filename,directory,input_current_func)
        # print("output_current_func=",filename,directory,output_current_func)
...
def Check_schema_table_case(log_result,filename,directory,result_schema_table_case):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_column_and_its_alias_check(log_result,filename,directory,result_column_and_its_alias_check):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_From_Unixtime(log_result,filename,directory,result_From_Unixtime):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_NVL_Columns(log_result,filename,directory,result_NVL_Columns):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_DATE_SUB_Columns(log_result,filename,directory,result_DATE_SUB_Columns):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Table Select From Timestamp"))
...
def Check_To_Date_Columns(log_result,filename,directory,result_To_Date_Columns):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Table Select From Timestamp"))
...
def Check_Date_Add_Columns(log_result,filename,directory,result_Date_Add_Columns):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Date_Diff_Columns(log_result,filename,directory,result_Date_Diff_Columns):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
#Flipkart project
def fkt_check_datatype_for_HV(log_result,filename,directory,Datatype_result_for_HV,fkt_check_timestamp_to_datetime):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Datatype Checking"))
    integer_column=[]
    float_column=[]
...
def fkt_check_struct_datatype(log_result,filename,directory,result_struct_datatype):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fkt_check_comment_to_option_for_HV(log_result,filename,directory,comment_to_option_result_for_HV):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Comment to option conversion check"))
    output_file_actual_option=[]
    Hive_comment_list_final=[]
...
def fkt_check_drop_raw_format_or_location_for_HV(log_result,filename,directory,drop_row_format_or_location_result_for_HV):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Drop_raw_format_location_from_hv"))
    drop_row_format = []
    find_option = []
...
def fkt_check_create_table_external_for_HV(log_result,filename,directory,create_table_external_for_HV):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Create Table To External Table Checking"))
    input_create_table_column=[]
    bq_equivalent_conversion=[]
...
def fkt_No_Of_Statement_In_Script(log_result,filename,directory,Result_No_Of_Statement,fun_project_ID):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Number of Statements in hive-BQ Script"))
    with_Append_project_id_Final_input_sql_statements=[]
    without_Append_project_id_Final_input_sql_statements=[]
...
def fkt_check_cross_join(log_result,filename,directory,JoinResult):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Verifying Join Conditions"))
    inputCrossJoin=[]
    outputCrossJoin=[]
...
def fkt_PROJECT_ID(log_result,project_ID,filename,directory):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Check Project ID"))
    try:
        if (filename,directory) in outputFile:
...
def fkt_Check_colan_to_space(log_result,filename,directory,Result_check_colan_to_space):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Check for Struct colan to space"))
    input_struct_raw=[]
    output_struct_raw=[]
...
def fkt_Check_map_to_array(log_result,filename,directory,Result_check_map_to_array):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Check for Map to Array"))
    # print(":inside input")
    input_map=[]
...
def fun_fkt_check_d_added_for_number_in_column(log_result,filename,directory,result_d_added_for_number_in_column):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_keyword_columns(log_result,filename,directory,result_keyword_columns):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_native_external_table(log_result,filename,directory,result_native_external_table,native_list):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fkt_check_external_table_path_for_HV(log_result,filename,directory,external_table_path_result_for_hv,external_table_path_dict):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_partition_col_datatype(log_result,filename,directory,result_partition_col_datatype,df):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fkt_Check_Partitioned_BY_to_Partition_By(log_result,filename,directory,result_Check_Partitioned_BY_to_Partition_By,native_list):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fkt_Check_PartitionBy_to_ClusterBy(log_result,filename,directory,result_Check_PartitionBy_to_ClusterBy,native_list):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Partition to cluster by check"))
...
def fkt_check_datetime_trunc_Granularity(log_result,filename,directory,result_Check_datetime_trunc_Granularity,df):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("check_partition_col_datatype"))
    input_list = []
    output_list = []
...
def fkt_partition_timestamp_to_date_function(log_result,filename,directory,result_partition_timestamp_to_date_function):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fkt_match_table_view_count(log_result,filename,directory,result_match_table_view_count):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_Match_join_count(log_result,filename,directory,result_Match_join_count):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_del_ins_sol_for_partition_tbl(log_result,filename,directory,result_del_ins_sol_for_partition_tbl):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_ins_sol_for_without_partition_tbl(log_result,filename,directory,result_ins_sol_for_without_partition_tbl):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fkt_Append_project_ID_to_udf_function(log_result,filename,directory,result_Append_project_ID_to_udf_function):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_safe_with_function(log_result,filename,directory,result_safe_with_function):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_lateral_view(log_result,filename,directory,result_lateral_view):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fkt_match_schema_table_case(log_result,filename,directory,result_match_schema_table_case):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fkt_Check_table_alias_column(log_result,filename,directory,result_check_table_alias):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Table Alias Column Checking"))
...
def fun_fkt_check_schema_in_seriliser_deser(log_result,filename,directory,result_check_schema_in_seriliser_deser):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_comparison_par_col_using_sheet(log_result,filename,directory,result_check_comparesion_par_col_using_sheet,table_column_dict):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_nullif_column(log_result,filename,directory,result_check_nullif_column):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_limit_count(log_result,filename,directory,result_check_limit_count):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_parse_date_to_case_when(log_result,filename,directory,result_check_parse_date_to_case_when):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_match_cte_table_count(log_result,filename,directory,result_fkt_match_cte_table_count):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_match_case_when_count(log_result, filename,directory, result_fkt_match_case_when_count):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_group_by_in_file(log_result, filename,directory, result_fkt_check_group_by_in_file):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_truncate_with_begin_transaction(log_result,filename,directory,result_fkt_check_truncate_with_begin_transaction):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_insert_column_names(log_result,filename,directory,result_check_insert_column_names):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("check_insert_column_names"))
    input_columns=[]
    output_columns=[]
...
def fun_fkt_match_aggregate_func_count(log_result,filename,directory,result_match_aggregate_func_count):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_match_clauses_count(log_result,filename,directory,result_match_clauses_count):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_date_sub_func(log_result,filename,directory,result_check_date_sub_func):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fkt_check_cast_count(log_result,filename,directory,result_check_cast_count):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_hash_parameter(log_result,filename,directory,result_check_hash_parameter,hash_parameter_csv_path):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_timezone(log_result,filename,directory,result_check_timezone):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def fun_fkt_check_safe_cast(log_result,filename,directory,result_check_safe_cast):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
