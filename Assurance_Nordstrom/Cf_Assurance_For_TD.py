...
def PROJECT_ID(log_result,project_ID,filename,directory):
...
def Data_Type(log_result,filename,directory,DataType_result,check_decimal_datatype_without_scale):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    ...
    try:
            ...
            with open (os.path.join(inputFolderPath, filename), "r",encoding=e) as f:
                ...
                for table_obj in obj:
                    ...
                    for ele in find_integer:
                        ...
                        replce_integer=re.sub(r"[\s]+INT,|[\s]+INT\)|[\s]+INT[\s]+|[\s]+INTEGER|[\s]+BIGINT","INT64",ele,flags=re.I)
                        # replce_integer = re.sub(r"[\s]+BIGINT", "INT64", replce_integer, flags=re.I)
        ...
        # print(input_all_datatype==output_all_datatype)
...
def fetch_bq_view_timestamp_column(TZ_result,file,source_table_name,all_Column_timestamp,file_name,location):
    d=[]
    print("Now checking all views")
    for next_file in file:
        with open(next_file, 'r') as nf:
            ...
            if output_from_table_name.lower()==source_table_name.lower():
                print("The process started for View checking Timestamp")
                Status='Y'
                ...
                if output_from_table_name and output_from_table_name != output_view_name:
                    ...
                    next_file_pattern = os.path.join(location, '*.sql')
                    next_files = glob.glob(next_file_pattern, recursive=True)

                    # With this
                    inner_result = fetch_bq_view_timestamp_column(TZ_result, next_files, output_view_name,
                                                                  all_Column_timestamp, os.path.basename(next_file),
                                                                  os.path.dirname(next_file))
                    if inner_result:
                        d.extend(inner_result)
    return d

def Timestamp_Check(log_result,filename,directory,inputFolderPath,TZ_result):
    same_file, OutputFolderPath, inputFolderPath, Append_project_id, inputFile, outputFile, cred_ValidateQueryOnBQ = Config_Setup(filename,directory)
    print("File name :",filename)
    print("Input file  :", inputFile)
    print("Input folder path  :", inputFolderPath)
    log_result.append(Common("TZ Column Checking"))
    input_column=[]
    # try:
    if (filename,directory) in inputFile:
        with open(os.path.join(inputFolderPath, filename), 'rb') as f:
        ...
        with open(os.path.join(inputFolderPath, filename), "r", encoding=e) as f:
            ...
            search_table_name = re.search(regex_for_table, file_content)
            table_name = search_table_name.group(2) if search_table_name else "None"

            all_Column_timestamp = re.findall(regex_for_timestamp, file_content)

            if (len(all_Column_timestamp) > 0 and table_name):
                print("It has timestamp column")

                with open(os.path.join(OutputFolderPath, filename) , 'r') as a:
                    py = a.read()
                    location = os.path.dirname(os.path.join(OutputFolderPath,filename))
                    file_name = os.path.basename(os.path.join(OutputFolderPath,filename))
                    print(location)
                    print(file_name)
                    regex_for_table = r"CREATE\s*(\w*\s+)*TABLE\s+([\w.]+)"
                    ...
                    if output_table_name.lower() == table_name.lower():
                        Status = 'Y'
                        for single_tmp in all_Column_timestamp:
                        ...
                        print("Length of:", len(input_column))
                        ...
                        log_result.append(Result_CM("Input and Output Table Name is different"))

                OutputFolderLSt = glob.glob(os.path.join(OutputFolderPath,"*.sql"),recursive=True)
                table_status = fetch_bq_view_timestamp_column(TZ_result,OutputFolderLSt,output_table_name, all_Column_timestamp, file_name, location)
                input_column.extend(table_status)
                print("Length of final result:",len(input_column))
                print(input_column)
...
def Verify_Index_And_Partition(log_result,filename,directory,partition_result,index,fun_project_ID):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def No_Of_Statement_In_Script(log_result,filename,directory,Result_No_Of_Statement,fun_project_ID):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    ...
    # if 1==1:
            ...
            with open (os.path.join(OutputFolderPath, filename), "r",encoding=e) as f:
                ...
                for element in file_obj:
                    ...
                    if fun_project_ID == "Y":
                        ...
                        output_sql_statements = re.findall(
                            r"TRUNCATE[\s]+TABLE[\s]*\`[^(\s]*\`|TRUNCATE[\s]+TABLE[\s]*\`[\w]+\`|TRUNCATE[\s]+TABLE[\s]*[\w]+\.[\w]+|TRUNCATE[\s]+TABLE[\s]*[^(\s]*|TRUNCATE[\s]+TABLE[\s]+\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\s*|TRUNCATE[\s]+TABLE[\s]+\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)\.[^(\s]*`\s*|"
                            r"CALL[\s]+\`?\{\{[\w]+\.\w+\}\}\`?\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\`?\.[^(\s]*\`?\s*|"
                            r"CREATE[\s]+OR[\s]+REPLACE[\s]+FUNCTION[\s]*[\w]+\.[\w]+|CREATE[\s]+OR[\s]+REPLACE[\s]+FUNCTION[\s]*\`[\w]+\.[\w]+\`|CREATE[\s]+OR[\s]+REPLACE[\s]+FUNCTION[\s]*\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\s*|CREATE[\s]+OR[\s]+REPLACE[\s]+FUNCTION[\s]*\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\`\s*|"
                            r"^WITH[\s]+[\w]+|"
                            r"CREATE[\s]+TEMPORARY[\s]+TABLE[\s]+IF[\s]+NOT[\s]+EXISTS[\s]*[^(\s]*|CREATE[\s]+TEMPORARY[\s]+TABLE[\s]+IF[\s]+NOT[\s]+EXISTS[\s]*\`[^(\s]*\`|CREATE[\s]+TEMPORARY[\s]+TABLE[\s]+IF[\s]+NOT[\s]+EXISTS[\s]*\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\s*|CREATE[\s]+TEMPORARY[\s]+TABLE[\s]+IF[\s]+NOT[\s]+EXISTS[\s]*\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\`\s*|"
                            r"CREATE[\s]+OR[\s]+REPLACE[\s]+VIEW[\s]*[\w]+\.[\w]+|CREATE[\s]+OR[\s]+REPLACE[\s]+VIEW[\s]*\`[\w]+\.[\w]+\`|CREATE[\s]+OR[\s]+REPLACE[\s]+VIEW[\s]*\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\s*|CREATE[\s]+OR[\s]+REPLACE[\s]+VIEW[\s]*\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\`\s*|"
                            r"CREATE[\s]+TABLE[\s]+IF[\s]+NOT[\s]+EXISTS[\s]*[\w]+\.[\w]+|CREATE[\s]+TABLE[\s]+IF[\s]+NOT[\s]+EXISTS[\s]*\`[\w]+\.[\w]+\`|CREATE[\s]+VIEW[\s]+IF[\s]+NOT[\s]+EXISTS[\s]*[\w]+\.[\w]+|CREATE[\s]+VIEW[\s]+IF[\s]+NOT[\s]+EXISTS[\s]*\`[\w]+\.[\w]+\`|CREATE[\s]+VIEW[\s]+IF[\s]+NOT[\s]+EXISTS[\s]*\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\s*|CREATE[\s]+VIEW[\s]+IF[\s]+NOT[\s]+EXISTS[\s]*\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\`\s*|"
                            r"INSERT[\s]+INTO[\s]*[\w]+\.[\w]+[\s]+|INSERT[\s]+INTO[\s]*\`[\w]+\.[\w]+\`[\s]+|"
                            r"INSERT[\s]+INTO[\s]+[\w]+[\s]+|INSERT[\s]+INTO[\s]+\`[\w]+\`[\s]+|"
                            r"INSERT[\s]+INTO[\s]+\{\{[\w]+\.\w+\}\}(?:_\w+)?\.\w+\s*|INSERT[\s]+INTO[\s]+\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\`\s*|"
                            r"INSERT[\s]+INTO[\s]+\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\s*|INSERT[\s]+INTO[\s]+\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\`\s*|"
                            r"UPDATE[\s]*[\w]+\.[\w]+|^UPDATE[\s]*\`[\w]+\.[\w]+\`|UPDATE[\s]*\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\s*|UPDATE[\s]*\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\`\s*|"
                            r"^UPDATE[\s]+[\w]+|^UPDATE[\s]+\`[\w]+\`|"
                            r"DELETE[\s]*FROM[\s]*[\w]+\.[\w]+|DELETE[\s]*FROM[\s]*\`[\w]+\.[\w]+\`|"
                            r"DELETE[\s]+FROM[\s]+\{\{[\w]+\.\w+\}\}\.\w+\s*|DELETE[\s]+FROM[\s]+\`\{\{[\w]+\.\w+\}\}\.[^(\s]*\`\s*|"
                            r"DELETE[\s]+FROM[\s]+\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.\w+\s*|DELETE[\s]+FROM[\s]+\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\`\s*"
                            r"|DELETE[\s]+FROM[\s]+[\w]+|DELETE[\s]+FROM[\s]+\`[\w]+\`|"
                            r"MERGE[\s]+INTO[\s]+\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\s*|MERGE[\s]+INTO[\s]+\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*`\s*"
                            r"DROP[\s]+TABLE[\s]+IF[\s]+EXISTS[\s]*[\w]+\.[\w]+|DROP[\s]+TABLE[\s]+IF[\s]+EXISTS[\s]*\`[\w]+\.[\w]+\`|DROP[\s]+TABLE[\s]+IF[\s]+EXISTS[\s]*\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\s*|DROP[\s]+TABLE[\s]+IF[\s]+EXISTS[\s]*\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\`\s*|"
                            r"DROP[\s]+TABLE[\s]+IF[\s]+EXISTS[\s]*[\w]+|DROP[\s]+TABLE[\s]+IF[\s]+EXISTS[\s]*\`[\w]+\`|^[\s]*SELECT[\s]+|"
                            r"CREATE[\s]+OR[\s]+REPLACE[\s]+TABLE[\s]*[^(\s]*|CREATE[\s]+OR[\s]+REPLACE[\s]+TABLE[\s]*\`[\w]+\.[\w]+\`|CREATE[\s]+OR[\s]+REPLACE[\s]+TABLE[\s]*\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\s*|CREATE[\s]+OR[\s]+TABLE[\s]+VIEW[\s]*\`\{\{[\w]+\.\w+\}\}\.\{\{[\w]+\.\w+\}\}(?:_\w+)?\.[^(\s]*\`\s*",
                            element, flags=re.I)

                        for item in output_sql_statements:                   # print(item)
                            remove_space=re.sub(r"[\s]+","",item)
                            # remove_space=re.sub(r"[\s]+"," ",item)
                            if remove_space.upper()=="SELECT":
                                all_output_file_statements.append(element)
                            ...
                            item = re.sub(r"[\{\{]+", "{", item)
                            item = re.sub(r"[\}\}]+", "}", item)
                            item = re.sub(r"params.", "",item)
                            item= re.sub(r"\{GCP_PROJECT_ID\}\.", "",item.upper())
                            item = re.sub(r"CREATEORREPLACETABLE", "CREATETABLEIFNOTEXISTS", item.upper())
        ...
        print(without_Append_project_id_Final_input_sql_statements==without_Append_project_id_Final_output_sql_statements)
        # print(filename)
            ...
            if fun_project_ID=="Y":
                ...
                if len(without_Append_project_id_Final_input_sql_statements) ==0 and len(without_Append_project_id_Final_output_sql_statements)==0:
                    ...
                    if without_Append_project_id_Final_input_sql_statements==without_Append_project_id_Final_output_sql_statements:
                        Result_No_Of_Statement.append("Y")
                        log_result.append(Result_Yes(all_input_file_statements,all_output_file_statements))
                    elif any('_VWS.' in s for s in without_Append_project_id_Final_input_sql_statements):
                        Result_No_Of_Statement.append("CM")
                        log_result.append(Result_No(all_input_file_statements, all_output_file_statements))
                    else:
                        Result_No_Of_Statement.append("N")
...
def Check_Pivot(log_result,filename,directory,Number_Pivot_in_Script):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Strtok_Function(log_result,filename,directory,strtok_final_result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Hashrow_equivalent(log_result,filename,directory,Hashrow_in_Script):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Checking_BT_And_ET(log_result,filename,directory,BT_ET_Result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Checking_sys_calender(log_result,filename,directory,sys_calender_result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_cross_join(log_result,filename,directory,JoinResult,fun_project_ID):
    # print("yyy")
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_comment_for_TD(log_result,filename,directory,comment_result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_View_Column_List(log_result,filename,directory,View_Cloumn_List_result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Hard_Coded_Value(log_result,filename,directory,Hard_Coded_Value_Result):
    #print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def not_between(log_result,filename,directory,not_between_result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Distinct(log_result,filename,directory,Check_Distinct_Result,DB_Conection,fun_project_ID):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Default_Coloumn_Values(log_result,filename,directory,Default_Coloumn_Values_Result,DB_Conection,fun_project_ID):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Unique_Primary_Index(log_result,filename,directory,Unique_Primary_Index_Result,fun_Unique_Primary_Index_Cloumn_FileList_Path):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def COALESCE(log_result,filename,directory,Coalesce_Result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Exceptions(log_result,filename,directory,check_Exception_Result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def System_Tables(log_result,filename,directory,check_System_Table_Result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Table_Column_Case_Check(log_result,filename,directory,result_CS_Col_Name_Case):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    ...
    try:
        ...
        if (filename,directory) in outputFile:
            ...
            with open (os.path.join(OutputFolderPath, filename), "r",encoding=e) as f:
                ...
                #with open (os.path.join(OutputFolderPath, filename), "w",encoding=e) as f
                # print(lstProcess)
                # for fileLine in fileLines:
...
def CS_View_Column_Case_Check(log_result,filename,directory,result_CS_View_Case_Check):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Table_DataType_Check(log_result,filename,directory,result_Cs_DataType_Check):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Table_Column_Names_In_Lower_Case(log_result,filename,directory,result_Cs_Lower_Check):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_star_columns(log_result,filename,directory,result_CS_star_columns):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_View_Source_Object_Alias_Check(log_result,filename,directory,result_CS_View_Source_Object_Alias_Check):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_View_Column_Names_Check(log_result, filename,directory, result_view_columns_order_in_input_and_output,result_view_columns_in_input_and_output,result_view_column_count):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    try:
        if (filename,directory) in inputFile:
...
def CS_View_Objects_Compare(log_result, filename,directory, result_from_and_join_in_view,result_from_and_join_count_in_view):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Join_Type_Count(log_result, filename,directory,result_inner_join_count,result_cross_join_count,result_left_join_count,result_right_join_count):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Check_Union_And_Union_All(log_result, filename,directory,result_union_and_union_all):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_No_Of_Lines_In_File(log_result, filename,directory,result_cs_file_line_count):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Compare_Hardcoded_Values(log_result, filename,directory,result_cs_compare_hardcoded_value):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Check_Group_By_In_File(log_result, filename,directory, result_cs_group_by):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Check_Aggregator_Functions(log_result, filename,directory, result_cs_aggregator_function):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Parenthesis_Count_In_Input_And_Output(log_result, filename,directory, result_cs_parenthesis_count):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_CLAUSES_Count_In_INPUT_And_Output(log_result, filename,directory, result_cs_clause_count):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Check_Date_Functions(log_result, filename,directory, result_cs_date_functions):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Check_Date_Format(log_result, filename,directory,result_cs_check_date_format):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Check_Backtick_In_BTEQ(log_result, filename,directory,result_cs_check_backtick_in_BTEQ):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Count_Doller_Variable(log_result, filename,directory,result_cs_count_doller_variable):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Check_IF_FALSE_In_Output(log_result, filename,directory,result_cs_check_if_false_in_output):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def CS_Check_Lower_Upper_Trim_In_Where(log_result, filename,directory,result_cs_check_lower_upper_trim_in_where,DB_Conection):
    # print(filename)
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def IF_Activity(log_result,filename,directory,result_for_if_activity):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def null_and_not_null(log_result,filename,directory,result_for_null,result_for_not_null):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def on_after_join(log_result,filename,directory,result_for_on_condition):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Default_Value_For_TD(log_result,filename,directory,result_for_default_value):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Timezone(log_result,filename,directory,result_Check_Timezone):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
#CSWG
def Primary_Index_To_Cluster_By_CSWG(log_result,filename,directory,result_Primary_Index_To_Cluster_By_CSWG):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Joins_CSWG(log_result,filename,directory,result_Joins_CSWG):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_No_Of_Statements_For_CSWG(log_result,filename,directory,result_no_of_statement_cswg):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Number of Statements in Script"))
    with_Append_project_id_Final_input_sql_statements=[]
    with_Append_project_id_Final_output_sql_statements=[]
            ...
            with open (os.path.join(inputFolderPath, filename), "r",encoding=e) as f:
                ...
                for element in file_obj:
                    ...
                    input_sql_statements=re.findall(r"CREATE[\s]+TABLE[\s]*[\w]+\.[\w]+|CREATE[\s]+MULTISET[\s]+VOLATILE[\s]+TABLE[\s]*[\w]+[\s]*|CREATE[\s]+VOLATILE[\s]+MULTISET[\s]+TABLE[\s]*[\w]+[\s]*|CREATE[\s]+SET[\s]+VOLATILE[\s]+TABLE[\s]*[\w]+[\s]*"
                                                    ...
                                                    r"|DELETE[\s]+FROM[\s]+[\w]+\.[\w]+|DELETE[\s]+FROM[\s]+[\w]+|DELETE[\s]+[\w]+\.[\w]+|DELETE[\s]+[\w]+[\s]*\;|DELETE[\s]+FROM[\s]+[\w]+[\s]+|"
                                                    r"DEL[\s]+FROM[\s]+[\w]+\.[\w]+|DEL[\s]+FROM[\s]+[\w]+|DEL[\s]+[\w]+\.[\w]+|DEL[\s]+[\w]+[\s]*\;|DEL[\s]+FROM[\s]+[\w]+[\s]+|DROP[\s]+TABLE[\s]*[\w]+\.[\w]+|DROP[\s]+TABLE[\s]*[\w]+[\s]+|DROP[\s]+TABLE[\s]*[\w]+[\s]*|"
                                                    r"MERGE[\s]+[\w]+\.[\w]+|MERGE[\s]+[\w]+\.[\w]+|CREATE[\s]+SET[\s]+TABLE[\s]+[\w]+\.[\w]+|"
                                                    r"CREATE[\s]+MULTISET[\s]+TABLE[\s]*[\w]+\.[\w]+|CREATE[\s]+PROCEDURE[\s]*[\w]+\.[\w]+|"
...
def Check_Table_And_View_In_Upper_Case(log_result,filename,directory,result_table_and_view_in_upper):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def BQ_Validation_For_CSWG(log_result,filename,directory,result_bq_validation_cswg,BQ_JSON_PATH_CSWG,Parameter_JSON_File_Path_CSWG,all_other_table):
...
def check_cast_to_safe_cast(log_result, filename,directory, result_check_cast_to_safe_cast):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_procedure_added_In_Output_File(log_result, filename,directory, result_check_procedure_added_In_Output_File):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    ...
    # if 1== 1:
        ...
        if (filename,directory) in inputFile:
            ...
            with open (os.path.join(inputFolderPath, filename), "r",encoding=e) as f:
                ...
                filename=re.sub(r"translated_|\.[\w]+$","",filename,directory,flags=re.I)
                final_parm_list=[]
                comment=0
                counter=0
                obj=''
...
def check_current_timestamp_to_current_datetime(log_result, filename,directory, result_check_current_timestamp_to_current_datetime):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Check Cast To Safe Cast"))
...
def Check_auther_comment_wells_fargo(log_result,filename,directory,result_auther_comment_wells_fargo):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Lower_For_Special_Characters(log_result,filename,directory,result_lower_for_special_character):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_DATEDIFF_Function_In_Output_File(log_result,filename,directory,result_datediff_function):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Group_By_Columns_In_Insert_Statement(log_result,filename,directory,result_group_by_columns):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
#For KSH Files
def check_trap_comment(log_result,filename,directory,result_of_trap_comment):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_bteq_comment(log_result,filename,directory,result_of_bteq_comment):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_error_check_added_or_not_in_file(log_result,filename,directory,result_for_error_check_added_or_not_in_file):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_sed_added_in_function_call_or_not(log_result,filename,directory,result_for_sed_added_in_function_call_or_not):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_create_table_added_after_os_and_export(log_result,filename,directory,result_for_create_table_added_after_os_and_export):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_email_logic(log_result,filename,directory,result_for_email_logic):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_f_get_aedw_open_batches(log_result,filename,directory,result_for_f_get_aedw_open_batches):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_f_execute_bteq_calling(log_result,filename,directory,result_for_f_execute_bteq_calling):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_fn_sql(log_result,filename,directory,result_for_fn_sql):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_Seq_MstSeq(log_result,filename,directory,result_for_Seq_MstSeq):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_comment_for_usrid_pwd_tdpid_logon(log_result,filename,directory,result_for_comment_for_usrid_pwd_tdpid_logon):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def cheak_fun_cheak_function_call(log_result,filename,directory,result_cheak_function_call):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_comment_present_in_echo(log_result,filename,directory,result_check_echo_comment):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
#Saadia
def check_execute_query_before_sql(log_result,filename,directory,result_execute_query_before_sql):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def check_bteq_logon_database(log_result,filename,directory,result_check_bteq_logon_database):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_auther_comment_sadiaa(log_result,filename,directory,result_Check_auther_comment_sadiaa):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Database_Values_Added_Or_NOT(log_result,filename,directory,fload_database_value_result,Schema_And_Value_Json):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Delimiter_Added_In_sed_Command(log_result,filename,directory,fload_delimiter_in_sed_result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Drop_And_Create_Table(log_result,filename,directory,fload_drop_and_create_table_result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Error1_Table_Statement(log_result,filename,directory,fload_error1_table_result):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Error2_Table_Statement(log_result,filename,directory,fload_error2_table_result,Translate_Table_Folder_Path):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def Check_Partition_By_And_Order_By_In_INSERT(log_result,filename,directory,fload_partition_and_order_by_result,Translate_Table_Folder_Path):
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
