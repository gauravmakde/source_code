...
def get_relative_path(file_path, var_Split):
    normalized_path = os.path.normpath(file_path)
    parts_after = normalized_path.split(var_Split)[1]
    filename = os.path.basename(normalized_path)

    return filename, parts_after

def Config_Setup(input_file_name="",input_directory=""):
    ...
    Input_Code=config['DEFAULT']['Input_Code']
    Output_Code = config['DEFAULT']['Output_Code']

    inputFile = glob.glob(inputFolderPath, recursive=True)
    outputFile = glob.glob(OutputFolderPath, recursive=True)

    print("Input file:",inputFile)
    print("Output file:", outputFile)
    inputFiles = {}
    for f in inputFile:
        filename, parts_after = get_relative_path(f, Input_Code)
        inputFiles[(filename, parts_after)] = f
        # print(inputFiles)

    outputFiles={}
    for f in outputFile:
        outputPath=os.path.dirname(os.path.normpath(f))
        filename,parts_after=get_relative_path(f, Output_Code)
        outputFiles[(filename,parts_after)]=f
        # print(outputFiles)
        try:
            fileName_O = filename
            if 'translated_' in filename.lower():
                filename = re.sub("translated_", "", filename, flags=re.I)
                os.rename(outputPath + fileName_O, outputPath + filename)
                outputFiles[(filename,parts_after)]=f
        except:
            pass

    same_file = {}
    print("#######Input Key#######",inputFiles.keys())
    print("#######Output Key#######", outputFiles.keys())
    # Iterate through the keys in the input files dictionary
    for key in inputFiles.keys():
        print("#######Input Key#######")
        print(key)
        if key in outputFiles.keys():
            print("#######Output Key#######")
            print(key)
            #     # Store the key along with both input and output file paths in the same_file dictionary
            same_file[key] = {
                'input_file': inputFiles[key],
                'output_file': outputFiles[key]
            }
            print(same_file)
    ...
    """
    # print("Input files are",finalinputFile)
    # # print("Output files are ",finaloutputFile)
    # print("file_name is",filename)
    # send_outputFile=finaloutputFile[filename]
    # print("Output files are ",send_outputFile)
    # return list_file_name, send_outputFile, finalinputFolderPath,Append_project_id, finalinputFile, finaloutputFile, cred_ValidateQueryOnBQ

    if input_directory and input_file_name:
        OutputFolderPath=os.path.normpath(os.path.dirname(outputFiles[(input_file_name,input_directory)]))
        inputFolderPath=os.path.normpath(os.path.dirname(inputFiles[(input_file_name,input_directory)]))
    else:
        OutputFolderPath = ""
        inputFolderPath = ""

    return same_file, OutputFolderPath, inputFolderPath, Append_project_id, inputFiles, outputFiles, cred_ValidateQueryOnBQ
    # else:
    #     print("Filename not found in output files.")
    #     return list_file_name, None, finalinputFolderPath, Append_project_id, finalinputFile, finaloutputFile, cred_ValidateQueryOnBQ


#Check file is validate_on_BQ
# def validate_on_BQ(log_result,filename,directory,BQ):
#     log_datetime=DateTime()
...
def validate_on_BQ(log_result,filename,directory,BQ, clientBQ):
    log_datetime=DateTime()
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    log_result.append(Common("Validate on BQ"))
    errorFileCounter = 0
    # clientBQ = bigquery.Client.from_service_account_json(json_credentials_path=cred_ValidateQueryOnBQ)
    try:
        fileReopen = open (os.path.join(OutputFolderPath,filename), "r")
        completeFileContent = fileReopen.read()
        job = clientBQ.query(completeFileContent)
        if job.result():
            BQ.append("Y")
...
def execute_on_BQ(log_result,filename,directory,executeOnBQ, clientBQ):
    log_datetime=DateTime()
    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
...
def get_relative_path_behind(file_path, var_Split):
    normalized_path = os.path.normpath(file_path)
    parts = normalized_path.split(var_Split)[0]
    return parts



def param_change_BQ(log_result,filename,directory,paramBQ, clientBQ,folder_configs,config_file_name,static_config_filename,folder_structure):
    log_datetime=DateTime()

    same_file,OutputFolderPath,inputFolderPath,Append_project_id,inputFile,outputFile,cred_ValidateQueryOnBQ=Config_Setup(filename,directory)
    print("Output folder from config",OutputFolderPath)

    log_result.append(Common("Param change on BQ"))
    dir_config_folder=get_relative_path_behind(os.path.join(OutputFolderPath, filename),folder_structure)
    config_folder=os.path.dirname(dir_config_folder)
    read_config=os.path.join(config_folder,folder_configs,config_file_name)
    static_config = os.path.join(config_folder, folder_configs, static_config_filename)
    # print(read_config)
    config = configparser.ConfigParser()
    config.read(read_config)
    sql_var = dict(config['environment_variables'])


    config.read(static_config)

    # print(config.read(static_config))
    static_var = dict(config['static'])
    sql_var.update(static_var)

    config_file=os.path.dirname(os.path.join(OutputFolderPath, filename))
    ...
    try:
        if (filename,directory) in inputFile:
            ...
            file_contain=""
            with open(os.path.join(OutputFolderPath, filename)) as fileReopen:
                ...
                for fileLine in fileLines:
                    ...
                    if "/*" in fileLine:
                        ...
                        completeFileContent = fileLine.replace('{{', '{').replace('}}', '}')
                        # Replace 'params.' with an empty string
                        completeFileContent = completeFileContent.replace('params.', '')
                        completeFileContent = completeFileContent.format(**sql_var)
                        file_contain = file_contain+completeFileContent

            job = clientBQ.query(file_contain)

            if job.result():
                paramBQ.append("Y")
                log_result.append(str(log_datetime) + " " + "Final_Result=Y\n\n")
                log_result.append(str(file_contain))
...
def Excel_Write_TZ(dic_Tz,Project_Name_For_Assurance_Report):
    print(dic_Tz)
    df = pd.DataFrame(dic_Tz)
    print(df)


    final_status=df['Status']

    value_counts = final_status.value_counts()
    yes_count = value_counts.get('Y', 0)  # Get the count of 'Y', default to 0 if not present
    no_count = value_counts.get('N', 0)  # Get the count of 'N', default to 0 if not present
    total_count = yes_count + no_count
    print(yes_count)
    get_time = datetime.datetime.now()
    get_time = get_time.strftime("%d-%m-%Y %H-%M-%S")
    ...
    excel_sheet_name = str(
        Project_Name_For_Assurance_Report + " - Timestamp Report " + get_time + ".xlsx")
    # excel_sheet_name=str(Project_Name_For_Assurance_Report+" - Code Conversion Assurance Report "+".xlsx")
    writer = pd.ExcelWriter(excel_sheet_name, engine='xlsxwriter')
    df.to_excel(writer, sheet_name='Assurance_Report_TZ_Report', startrow=8, header=False, index=False)

    workbook = writer.book
    worksheet = writer.sheets['Assurance_Report_TZ_Report']
    ...
    for col_num, value in enumerate(df.columns.values):
        # print(col_num, value)
        val = f"A8:{chr(65 + len(df.columns) - 1)}{len(df) + 9}"
        worksheet.write(7, col_num, value, header_format)
        worksheet.set_column(col_num, col_num, 25, format)
        worksheet.hide_gridlines(2)
        worksheet.set_default_row(25)
