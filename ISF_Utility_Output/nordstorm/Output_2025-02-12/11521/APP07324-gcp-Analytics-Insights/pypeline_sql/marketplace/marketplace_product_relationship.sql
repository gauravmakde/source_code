create table if not exists {hive_schema}.marketplace_product_relationship (
    webstyleid string, 
    marketcode string,
...
using PARQUET
location "s3://{s3_bucket_root_var}/marketplace_product_relationship"
partitioned by (join_date);

MSCK REPAIR table {hive_schema}.marketplace_product_relationship;

create or replace temporary view find_single_epmstyleid as
select webstyleid, marketcode, parentepmstyleid, rmsstylegroupid, brandlabelid, brandlabeldisplayname, sourcepublishtimestamp, partnerrelationshipid, partnerrelationshiptype, {start_date} as join_date
from
    (select *,
...
select marketcode, empstyleid, departmentdescription, departmentnumber, subclassdescription, subclassnumber, classdescription, classnumber, typelevel1description, typelevel2description, gender, agegroup, sourcepublishtimestamp, {start_date} as join_date
from 
	(select *,
...
insert
    OVERWRITE TABLE {hive_schema}.marketplace_product_relationship PARTITION (join_date)
select
    webstyleid, 
    marketcode,
    parentepmstyleid, 
