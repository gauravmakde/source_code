...
USING CSV 
OPTIONS(path "s3://{s3_bucket_root_var}/tpt_export/customer_cohort_trips/customer_cohort_trips.csv",
  sep  ",",
  header "false");



create table if not exists {hive_schema}.customer_cohort_trips
(
    source_platform_code string,
    SOURCE_channel_CODE string,
    business_unit_desc string, 
...
using PARQUET
location 's3://{s3_bucket_root_var}/customer_cohort_trips'
partitioned by (tran_date);




insert overwrite table {hive_schema}.customer_cohort_trips partition (tran_date)
select *
from user_trips;
