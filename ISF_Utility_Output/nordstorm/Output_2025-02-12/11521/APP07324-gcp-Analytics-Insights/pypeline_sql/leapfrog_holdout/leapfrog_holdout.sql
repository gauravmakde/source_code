...
create table if not exists {hive_schema}.leapfrog_holdout (
    shopper_id string,
...
) using ORC location 's3://{s3_bucket_root_var}/leapfrog_holdout' partitioned BY (activity_date_partition);
...
from
    (
        select
            ...
            max(
                case
                    when lower(variationname) in ('default', 'off') then 1
                    else 0
                end
            ) over (
...
where
    a.activity_date_partition between {start_date}
    and {end_date};
...
insert
    OVERWRITE TABLE {hive_schema}.leapfrog_holdout PARTITION (activity_date_partition)
select
    /*+ REPARTITION(100) */
    *
from
...
MSCK REPAIR table {hive_schema}.leapfrog_holdout;
