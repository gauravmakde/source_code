...
create table if not exists {hive_schema}.email_marketability
(
  acp_id                 string
, loyaltyid              string
...
using PARQUET
location 's3://{s3_bucket_root_var}/email_marketability/'
partitioned by (event_date_pacific);

--eligible_touches: creates a view of all touches over the eligible time period.
--This will filter out any arrivals that lack marketing information as we only want email Marketing touches.
        ...
        (
            ...
            from(
                select
                    ...
                    , parse_url(coalesce(requested_destination, actual_destination), 'QUERY', 'utm_medium') as utm_medium
                    , case
...
--final_touches_email: join utm_channel_lookup and marketing_channel_hierarchy to get mapped finance_detail on utm_channel
--only filtering for customer level email marketing touches
create  or replace temporary view cust_touches_email as
(
    select
          o.cust_id
        , o.iconid
...
create or replace temporary view market_email_touches_all as
(
SELECT
          o.cust_id
        , o.iconid
...
--cust_marketability_email: final mapping of arrival touches to conversions.
create or replace temporary view cust_marketability_email as
(
    SELECT
          a.cust_id
        , a.iconid
        ...
        , o.demand
    FROM market_email_touches_all a
...
insert overwrite table {hive_schema}.email_marketability partition (event_date_pacific)
 SELECT
...
MSCK REPAIR TABLE {hive_schema}.email_marketability;

-- Writing output to S3 CSV
-- This sould match the "sql_table_reference" indicated on the .json file.
insert overwrite table email_marketability_ldg_output
 SELECT
          acp_id
        , loyaltyid
        , iscardmember
