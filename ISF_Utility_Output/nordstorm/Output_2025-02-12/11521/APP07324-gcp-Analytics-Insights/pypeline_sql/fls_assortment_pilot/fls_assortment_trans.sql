...
create volatile multiset table trans as (
	...
	join prd_nap_usr_vws.retail_tran_hdr_fact h on 
		t.global_tran_id = h.global_tran_id
		and t.business_day_date = h.business_day_date
	where t.line_item_merch_nonmerch_ind = 'MERCH'
		and h.acp_id is not null
		and sale_date between (select min(day_date) from {shoe_categories_t2_schema}.pilot_day_dim) and (select max(day_date) from {shoe_categories_t2_schema}.pilot_day_dim)
...
create volatile multiset table loyalty_base as (
	...
	left join prd_nap_usr_vws.loyalty_level_lifecycle_fact_vw l on a.acp_loyalty_id = l.loyalty_id
	where l.start_day_date <= (select max(day_date) from {shoe_categories_t2_schema}.pilot_day_dim)
		and l.end_day_date >= (select min(day_date) from {shoe_categories_t2_schema}.pilot_day_dim)
...
create volatile multiset table customer_ntn as (
	...
	from trans t
	join prd_nap_usr_vws.customer_ntn_fact n on 
		t.acp_id = n.acp_id
...
create volatile multiset table shipped_orders as (
	...
	left join prd_nap_usr_vws.org_us_zip_dma d1 on 
		o.destination_country_code = 'US'
		and o.destination_zip_code = d1.us_zip_code
	left join prd_nap_usr_vws.org_dma d2 on 
		d1.us_dma_code = d2.dma_code
	left join prd_nap_usr_vws.org_ca_zip_dma d3 on 
		o.destination_country_code = 'CA'
	...
	) z on o.destination_country_code = z.country_code
		and o.destination_zip_code = z.zip_code
	where o.order_num in (select order_num from trans where order_num is not null)
...
create volatile multiset table wac as (
	...
	where eff_begin_dt <= (select max(day_date) from {shoe_categories_t2_schema}.pilot_day_dim)
	qualify eff_end_dt_mod >= (select min(day_date) from {shoe_categories_t2_schema}.pilot_day_dim)
)
with data
...
create multiset volatile table trans_base as (
	...
	from trans t
	join {shoe_categories_t2_schema}.pilot_sku_dim p on t.sku_num = p.rms_sku_num
	join {shoe_categories_t2_schema}.pilot_store_dim s on t.store_num = s.store_num

	--customer acquisition
	left join customer_ntn n on 
		t.trip_id = n.trip_id

	--customer loyalty
	left join loyalty l on t.trip_id = l.trip_id

	--customer segment (note: leaving T3 dependency for prod)
	left join t2dl_das_customber_model_attribute_productionalization.customer_prediction_core_target_segment seg on 
		t.acp_id = seg.acp_id

	left join shipped_orders o on 
...
DELETE FROM {shoe_categories_t2_schema}.fls_assortment_pilot_trans_base ALL;
INSERT INTO {shoe_categories_t2_schema}.fls_assortment_pilot_trans_base
...
collect stats
	...
	on {shoe_categories_t2_schema}.fls_assortment_pilot_trans_base;
