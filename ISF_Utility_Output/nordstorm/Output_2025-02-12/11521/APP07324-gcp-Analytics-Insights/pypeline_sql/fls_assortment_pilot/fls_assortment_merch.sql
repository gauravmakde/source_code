...
/*
T2/Table Names:
- t2dl_das_ccs_categories.fls_assortment_pilot_merch_base
- t2dl_das_ccs_categories.pilot_cc_dim
...
create volatile multiset table week_dim as (
	with week_dim_base as (
		select distinct
			...
			, year_label
			, true_week_num
		from {shoe_categories_t2_schema}.pilot_day_dim
	)

	select w.*
		, count(week_num) over (partition by month_num) as week_count
	from week_dim_base w
	where week_num in (select week_num from {shoe_categories_t2_schema}.pilot_day_dim)
)
with data
primary index (true_week_num)
...
/*create volatile multiset table month_dim as (
	...
	select *
	from month_dim_base
	where month_num in (select month_num from {shoe_categories_t2_schema}.pilot_day_dim)
)
with data
...
create volatile multiset table sales as (
	select
		...
		-- , cast(0 as decimal(20, 0)) as avg_inv_units
	...
	join {shoe_categories_t2_schema}.pilot_sku_dim p on 1=1
		and m.rms_sku_num = p.rms_sku_num
	join {shoe_categories_t2_schema}.pilot_store_dim s on 1=1
...
create volatile multiset table inv as (
	...
	where i.store_num IN (select distinct store_num from {shoe_categories_t2_schema}.pilot_store_dim s)
		and i.week_num IN (select DISTINCT true_week_num FROM week_dim)
	group by 1, 2, 3, 4
	having boh_units > 0
		or eoh_units > 0
		-- or avg_inv_units > 0
...
create volatile multiset table twist_base as (
	...
	join {shoe_categories_t2_schema}.pilot_sku_dim p on 1=1
		and t.rms_sku_num = p.rms_sku_num
	join {shoe_categories_t2_schema}.pilot_day_dim d on 1=1
		and t.day_date = d.day_date
	join {shoe_categories_t2_schema}.pilot_store_dim s on 1=1
		and t.store_num = s.store_num
	where 1=1
		and t.day_date between (select min(day_date) from {shoe_categories_t2_schema}.pilot_day_dim) and (select max(day_date) from {shoe_categories_t2_schema}.pilot_day_dim)
		and allocated_traffic > 0
)
with data and stats
primary index (cc_num, store_num, week_num, rp_flag)
on commit preserve rows;
...
where 1=1
	...
	and t.business_day_date between (select min(day_date) from {shoe_categories_t2_schema}.pilot_day_dim) and (select max(day_date) from {shoe_categories_t2_schema}.pilot_day_dim)
	and store_num in (select store_num from {shoe_categories_t2_schema}.pilot_store_dim)
)
with data and stats
...
DELETE FROM {shoe_categories_t2_schema}.fls_assortment_pilot_merch_base ALL;
INSERT INTO {shoe_categories_t2_schema}.fls_assortment_pilot_merch_base
	...
	select
		...
		, round(coalesce(
			--gross sales aur
			sum(sales_gross_retail) over (partition by s.channel_num, f.cc_num)
			/ nullif(sum(sales_gross_units) over (partition by s.channel_num, f.cc_num), 0)

			--avg_inv aur
			, sum(eoh_retail) over (partition by s.channel_num, f.cc_num)
			/ nullif(sum(eoh_units) over (partition by s.channel_num, f.cc_num), 0)

			--boh aur
			, sum(boh_retail) over (partition by s.channel_num, f.cc_num)
			/ nullif(sum(boh_units) over (partition by s.channel_num, f.cc_num), 0)

			--rcpt aur
			, sum(rcpt_po_retail + rcpt_ds_retail + tsfr_rs_in_retail + tsfr_rack_in_retail + tsfr_pah_in_retail) over (partition by s.channel_num, f.cc_num)
			/ nullif(sum(rcpt_po_units + rcpt_ds_units + tsfr_rs_in_units + tsfr_rack_in_units + tsfr_pah_in_units) over (partition by s.channel_num, f.cc_num), 0)

			--return aur
			, sum(return_retail) over (partition by s.channel_num, f.cc_num)
			/ nullif(sum(return_units) over (partition by s.channel_num, f.cc_num), 0)
		), 2) as aur_dim

		, sum(sales_units + eoh_units) over (partition by f.cc_num, f.store_num, f.week_num) as qual_ats_units
	...
	left join anniv a on 1=1 -- 5/6/24 added join to Anniversary CC lookup
		...
		and f.week_num = a.week_num
	join week_dim w on 1=1
		and f.week_num = w.week_num
	join {shoe_categories_t2_schema}.pilot_store_dim s
...
collect stats
	...
	, column (store_num)
	, column (week_num)
on {shoe_categories_t2_schema}.fls_assortment_pilot_merch_base ;



DELETE FROM {shoe_categories_t2_schema}.pilot_cc_dim ALL;
INSERT INTO {shoe_categories_t2_schema}.pilot_cc_dim
	...
	from {shoe_categories_t2_schema}.pilot_sku_dim
	where cc_num in (select distinct cc_num from {shoe_categories_t2_schema}.fls_assortment_pilot_merch_base)
	group by cc_num
;
