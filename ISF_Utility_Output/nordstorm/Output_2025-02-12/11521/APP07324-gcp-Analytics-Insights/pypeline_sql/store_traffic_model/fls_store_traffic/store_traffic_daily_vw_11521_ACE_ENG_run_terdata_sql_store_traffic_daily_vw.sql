...
REPLACE
VIEW T2DL_DAS_FLS_TRAFFIC_MODEL.store_traffic_daily_vw AS
...
from (
         ...
         from (select a.store_number
                    ...
                    , a.traffic_source
                    , dt.day_num
               from T2DL_DAS_FLS_TRAFFIC_MODEL.fls_traffic_daily a
         ...
         from (select a.store_number
                    ...
                    , a.traffic_source
                    , dt.day_num
               from T2DL_DAS_FLS_TRAFFIC_MODEL.nl_lc_traffic_daily a
                        left join PRD_NAP_USR_VWS.DAY_CAL dt on a.day_date = dt.day_date) ty
                  left join (select a.store_number
                                  , a.traffic
                                  , a.purchase_trips
         ...
         from (select a.store_number
                    ...
                    , a.traffic_source
                    , dt.day_num
               from T2DL_DAS_FLS_TRAFFIC_MODEL.rack_traffic_daily a
                        left join PRD_NAP_USR_VWS.DAY_CAL dt on a.day_date = dt.day_date) ty
                  left join (select a.store_number
                                  , a.traffic
                                  , a.purchase_trips
         ...
         left join (select store_number, min(day_date) msrmnt_start_date
                    from T2DL_DAS_FLS_TRAFFIC_MODEL.fls_traffic_daily
                    group by 1
                    union
                    select store_number, min(day_date) msrmnt_start_date
                    from T2DL_DAS_FLS_TRAFFIC_MODEL.rack_traffic_daily
                    group by 1
                    union
                    select store_number, min(day_date) msrmnt_start_date
                    from T2DL_DAS_FLS_TRAFFIC_MODEL.nl_lc_traffic_daily
