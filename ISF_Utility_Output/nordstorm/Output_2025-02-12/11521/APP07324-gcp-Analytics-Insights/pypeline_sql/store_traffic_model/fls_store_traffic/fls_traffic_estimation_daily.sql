...
(
   ...
   LEFT JOIN {fls_traffic_model_t2_schema}.fls_traffic_model_mlp v2 on tr.store_number=v2.store_number and tr.day_date = v2.day_date
)
WITH DATA
UNIQUE PRIMARY INDEX(store_number, day_date)
ON COMMIT PRESERVE ROWS;
...
DELETE
FROM {fls_traffic_model_t2_schema}.fls_traffic_estimation_daily
WHERE day_date BETWEEN {start_date} AND {end_date};

INSERT INTO {fls_traffic_model_t2_schema}.fls_traffic_estimation_daily
SELECT store_number
     , store_name
     , day_date
     , fiscal_week
...
--COLLECT STATS
COLLECT STATISTICS COLUMN (store_number), COLUMN (day_date) ON {fls_traffic_model_t2_schema}.fls_traffic_estimation_daily;
COLLECT STATISTICS COLUMN (PARTITION , day_date) ON {fls_traffic_model_t2_schema}.fls_traffic_estimation_daily;
COLLECT STATISTICS COLUMN (PARTITION) ON {fls_traffic_model_t2_schema}.fls_traffic_estimation_daily;
