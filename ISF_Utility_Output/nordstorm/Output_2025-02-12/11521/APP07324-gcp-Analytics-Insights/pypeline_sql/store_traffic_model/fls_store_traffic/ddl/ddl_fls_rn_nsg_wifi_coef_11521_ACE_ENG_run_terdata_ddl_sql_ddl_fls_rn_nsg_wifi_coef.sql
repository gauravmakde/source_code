...
create multiset table T2DL_DAS_FLS_TRAFFIC_MODEL.fls_rn_nsg_wifi_coef
    ,fallback
    ,no before journal
    ,no after journal
    ,checksum = default
...
COMMENT ON T2DL_DAS_FLS_TRAFFIC_MODEL.fls_rn_nsg_wifi_coef IS 'details on model coefficient for adjusting RetailNext wifi to align with new source of wifi data(NSG)';
