SET QUERY_BAND = 'App_ID=APP08737;
     DAG_ID=customer_sandbox_fact_11521_ACE_ENG;
     Task_Name=customer_sandbox_fact;'
...
Date Created/Modified: 11/14/2023

Notes:
-- Customer Sanbox fact table for microstrategy customer sandbox
*/
 ...
 ************************************************************************************/
--drop table cust_year_level_attributes;
...
select  a.acp_id
    ...
    ,max(case when channel = '7) JWN' then cust_chan_acquired_aare end) cust_jwn_acquired_aare
    ,max(case when channel = '7) JWN' then cust_chan_activated_aare end) cust_jwn_activated_aare
    ,max(case when channel = '7) JWN' then cust_chan_retained_aare end) cust_jwn_retained_aare
    ,max(case when channel = '7) JWN' then cust_chan_engaged_aare end) cust_jwn_engaged_aare
    ...
    ,max(case when cust_chan_return_items>0 then 1 else 0 end) cust_made_a_return
...
select distinct a.acp_id
    ...
    ,1 cancel_flag
from T2DL_DAS_STRATEGY.cco_order_item_cancels a 
    join PRD_NAP_USR_VWS.DAY_CAL b on a.order_date_pacific=b.day_date
where order_date_pacific between (select min(day_date) from PRD_NAP_USR_VWS.DAY_CAL dc where month_num = (select yr4_start_mo from month_lookup)) 

and (select max(day_date) from PRD_NAP_USR_VWS.DAY_CAL dc where month_num = (select yr1_end_mo from month_lookup))
) with data primary index(acp_id,reporting_year_shopped) on commit preserve rows;
...
--drop table ship2store_order_activity;
...
where business_day_date between (select min(day_date) from PRD_NAP_USR_VWS.DAY_CAL dc where month_num = (select yr4_start_mo from month_lookup))and (select max(day_date)+14 from PRD_NAP_USR_VWS.DAY_CAL dc where month_num = (select yr1_end_mo from month_lookup))
    and a.order_date_pacific between (select min(day_date) from PRD_NAP_USR_VWS.DAY_CAL dc where month_num = (select yr4_start_mo from month_lookup)) and (select max(day_date) from PRD_NAP_USR_VWS.DAY_CAL dc where month_num = (select yr1_end_mo from month_lookup))
...
--drop table cust_spend_div_price;
...
--drop table cust_year_everything;
...
select distinct a.acp_id
	...
	,a.cust_event_holiday
	,a.cust_event_anniversary
...
select
	   ...
	   -- ,b.cust_platform_APP
		...
		,b.cust_event_holiday
	    ,b.cust_event_anniversary
