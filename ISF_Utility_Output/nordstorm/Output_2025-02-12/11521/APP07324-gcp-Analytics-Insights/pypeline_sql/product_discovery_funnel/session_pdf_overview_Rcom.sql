...
-- Drop TABLE IF EXISTS {hive_schema}.session_pdf_overview_Rcom;
create table if not exists {hive_schema}.session_pdf_overview_Rcom (
    fiscal_year_num int, 
    quarter_abrv string, 
    ...
    week_partition date
) using ORC location 's3://{s3_bucket_root_var}/session_pdf_overview_Rcom' partitioned by (week_partition)
;
...
insert OVERWRITE TABLE {hive_schema}.session_pdf_overview_Rcom PARTITION (week_partition) 
select *
from final_view_overview
;

MSCK REPAIR table {hive_schema}.session_pdf_overview_Rcom
;
...
-- Drop TABLE IF EXISTS {hive_schema}.session_pdf_browse_dive_Rcom;
create table if not exists {hive_schema}.session_pdf_browse_dive_Rcom (
    fiscal_year_num int, 
    quarter_abrv string, 
    fiscal_month_num int, 
    month_abrv string, 
    ...
    ly_ttl_ord_demand decimal(36, 6),
    week_partition date
) using PARQUET location 's3://{s3_bucket_root_var}/session_pdf_browse_dive_Rcom' partitioned by (week_partition);
...
insert OVERWRITE TABLE {hive_schema}.session_pdf_browse_dive_Rcom PARTITION (week_partition) 
select *
from final_view_browse
;

MSCK REPAIR table {hive_schema}.session_pdf_browse_dive_Rcom
;
