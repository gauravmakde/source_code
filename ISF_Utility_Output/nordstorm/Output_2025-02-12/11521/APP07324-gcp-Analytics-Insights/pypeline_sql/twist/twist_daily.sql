...
create multiset volatile table product_views
  as (
    ...
    where event_date_pacific between {start_date} and {end_date} and site_source = 'MODERN'
    group by 1, 2, 3, 4
...
where 1=1
and year_num >= 2021
and stdn.day_date between {start_date} and {end_date}
group by 1,2,3,4
...
create multiset volatile table store_traffic
  as (
    ...
    where day_date between {start_date} and {end_date} and s.store_type_code in ('FL', 'RK')
    group by 1, 2, 3
) with data primary index (store_number, day_date) on commit preserve rows;
...
create multiset volatile table demand
  as (
    ...
    where business_day_date between cast({start_date} as date) - 60 and cast({end_date} as date)
...
create multiset volatile table rp_lkp
  as (
      ...
      inner join (
                ...
                where day_date between cast({start_date} as date) - 60 and cast({end_date} as date)
            ) d
...
create multiset volatile table eoh_base
  as (
      ...
      where snapshot_date between cast({start_date} as date) - 61 and cast({end_date} as date) - 1 -- go one day back for BOH casting
...
delete from {twist_t2_schema}.twist_daily where day_date between cast({start_date} as date) and cast({end_date} as date);

insert into {twist_t2_schema}.twist_daily
  select distinct
    day_date
    , country
    , banner
