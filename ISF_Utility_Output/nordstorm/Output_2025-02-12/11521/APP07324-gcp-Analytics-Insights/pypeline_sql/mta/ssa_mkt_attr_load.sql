...
create table if not exists {hive_schema}.ssa_mkt_attr_fact
(
...
using ORC
location 's3://{s3_bucket_root_var}/ssa_mkt_attr_fact/'
partitioned by (activity_date_pacific);

MSCK REPAIR TABLE {hive_schema}.ssa_mkt_attr_fact;
...
insert overwrite table {hive_schema}.ssa_mkt_attr_fact partition (activity_date_pacific)
