...
select
    ...
    from {mta_t2_schema}.mkt_utm_agg mua
    LEFT JOIN T2DL_DAS_MTA.utm_channel_lookup ucl
        ON ucl.utm_mkt_chnl =mua.utm_channel
where mua.activity_date_pacific between {start_date} and {end_date}
...
CREATE MULTISET VOLATILE TABLE mta_forecast_optimized AS (
    ...
    WHERE fcst."date" between {start_date} and {end_date}
    GROUP BY 1, 2, 3, 4, 5
)  with data
...
/*=======================================
 ...
 =======================================*/
delete from {mta_t2_schema}.MTA_SSA_COST_AGG
where activity_date_pacific between {start_date} and {end_date};

/*=======================================
    INSERT to final table
 =======================================*/
insert into {mta_t2_schema}.MTA_SSA_COST_AGG
-- EXPLAIN
select
    activity_date_pacific
    , order_channel
...
collect statistics column(partition) on {mta_t2_schema}.MTA_SSA_COST_AGG;
