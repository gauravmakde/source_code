...
create table if not exists {hive_schema}.mta_acp_scoring 
(
    acp_id string,
	order_number string,
...
using orc
location 's3://{s3_bucket_root_var}/mta/mta_acp_scoring/'
...
msck repair table {hive_schema}.mta_acp_scoring;
...
where   mta_tb.order_date_pacific between {start_date} and {end_date}
);
...
from    por_orders
where order_date_utc between {start_date} and {end_date}+1
...
insert overwrite table {hive_schema}.mta_acp_scoring partition (order_date_pacific)
