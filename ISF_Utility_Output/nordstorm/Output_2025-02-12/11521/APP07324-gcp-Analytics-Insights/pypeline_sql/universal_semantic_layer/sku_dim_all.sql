SET QUERY_BAND = 'App_ID=APP08818;
     DAG_ID=sku_dim_all_11521_ACE_ENG;
     Task_Name=sku_dim_all;'
...
-- What is the the purpose of the sql: refresh historical data to prevent any adverse effects on reclassification
-- What is the update cadence/lookback window: monthly refreshment, execute on the first day of each month at 9am UTC
*/

--drop table sku_dim_prep_all;
CREATE VOLATILE TABLE sku_dim_prep_all AS 
(select distinct sk.rms_sku_num AS sku_id
  ...
  join (select distinct rms_sku_num from prd_nap_usr_vws.order_line_detail_fact oldf where oldf.order_date_pacific between '2021-01-31' and current_date group by 1) as active_skus
...
DELETE FROM {usl_t2_schema}.sku_dim;
INSERT INTO {usl_t2_schema}.sku_dim
SELECT * FROM sku_dim_prep_all;
