...
CREATE  MULTISET VOLATILE TABLE styleboards AS (
	...
	WHERE CAST(BOARD_CREATED_PST AS DATE ) BETWEEN {start_date} AND {end_date}
) WITH DATA PRIMARY INDEX (board_created_pst) ON COMMIT PRESERVE ROWS;
...
FROM
    ...
    AND f.order_date_pacific >= {start_date}
    ) a
...
---------------------------------------------------
*/
DELETE FROM {remote_selling_t2_schema}.NORDSTROM_ON_DIGITAL_DAILY
WHERE board_created_date >= {start_date}
AND board_created_date <= {end_date};
INSERT INTO {remote_selling_t2_schema}.NORDSTROM_ON_DIGITAL_DAILY
...
ON {remote_selling_t2_schema}.NORDSTROM_ON_DIGITAL_DAILY;
