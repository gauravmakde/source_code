SET QUERY_BAND = 'App_ID=APP08176;
     DAG_ID=ddl_mothership_wbr_primary_11521_ACE_ENG;
     Task_Name=ddl_wbr_primary;'
...
--CALL SYS_MGMT.DROP_IF_EXISTS_SP ('t2dl_das_bie_dev', 'wbr_primary', OUT_RETURN_MSG);


CREATE MULTISET TABLE {mothership_t2_schema}.wbr_primary,
     FALLBACK ,
     NO BEFORE JOURNAL,
     NO AFTER JOURNAL,
     CHECKSUM = DEFAULT,
     ...
     MAP = TD_MAP1
     (
        week_num                    DECIMAL(6,0) DEFAULT 0.000000, 
     ...
     PRIMARY INDEX (week_num, business_unit_desc, feature_name);


-- table comment
COMMENT ON  {mothership_t2_schema}.wbr_primary IS 'primary table used to power the finance Weekly Business Review tableau dashboard: https://tableau.nordstrom.com/#/site/AS/views/WBRdash/DigitalWBR?:iid=2';
