...
--DROP TABLE IF EXISTS {hive_schema}.product_discovery_funnel_weekly_Ncom;
create table if not exists {hive_schema}.product_discovery_funnel_weekly_Ncom (
	fiscal_year_num int, 
	quarter_abrv string, 
...
) using ORC location 's3://{s3_bucket_root_var}/product_discovery_funnel_weekly_Ncom' partitioned by (week_start_day_date)
;
...
where activity_date_partition between ({start_date} - (dayofweek({start_date}) -1)) and ({end_date} - (dayofweek({end_date}) -1) + 6)
...
insert OVERWRITE TABLE {hive_schema}.product_discovery_funnel_weekly_Ncom PARTITION (week_start_day_date) 
select *
from weekly_Ncom
;

MSCK REPAIR table {hive_schema}.product_discovery_funnel_weekly_Ncom
;
...
--DROP TABLE IF EXISTS {hive_schema}.product_discovery_funnel_weekly_Rcom;
create table if not exists {hive_schema}.product_discovery_funnel_weekly_Rcom (
	fiscal_year_num int, 
	quarter_abrv string, 
	fiscal_month_num int, 
	month_abrv string, 
	...
	ttl_ord_demand decimal(36, 6),
	week_start_day_date date
) using ORC location 's3://{s3_bucket_root_var}/product_discovery_funnel_weekly_Rcom' partitioned by (week_start_day_date)
;
...
insert OVERWRITE TABLE {hive_schema}.product_discovery_funnel_weekly_Rcom PARTITION (week_start_day_date) 
select *
from weekly_Rcom
;

MSCK REPAIR table {hive_schema}.product_discovery_funnel_weekly_Rcom
;
