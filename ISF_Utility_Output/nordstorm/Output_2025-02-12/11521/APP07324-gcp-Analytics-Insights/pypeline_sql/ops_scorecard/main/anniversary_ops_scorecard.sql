SET QUERY_BAND = 'App_ID=APP08866;
     DAG_ID= anniversay_ops_scorecard_11521_ACE_ENG;
     Task_Name= anniversay_ops_scorecard;'
     FOR SESSION VOLATILE;

------------------------------------------------------------
-- ANNIVERSARY OPS SCORECARD --
-- Table Name: 
...
-- Note:
-- Purpose: To capture Anniversary Total Sales 
...
CREATE MULTISET VOLATILE TABLE ANNIVERSARY_OPS_1 AS (
SELECT
tran_date,
day_short_desc,
...
and tran_date between {start_date_anniversary} and ({start_date} - 1)
...
CREATE MULTISET VOLATILE TABLE ANNIVERSARY_OPS_2 AS (
SELECT
b.day_date,
day_short_desc,
...
and b.day_date between {start_date_anniversary} and {end_date}	-- 7/8 - 8/4
GROUP BY 1,2,3,4
...
-- Plan (Digital Demand)
CREATE MULTISET VOLATILE TABLE ANNIVERSARY_OPS_3 AS (
SELECT
b.day_date,
day_short_desc,
CASE WHEN business_unit_desc IN ('N.COM') THEN 'NORDSTROM'
...
CREATE MULTISET VOLATILE TABLE ANNIVERSARY_OPS_4 AS (
SELECT 
...
FROM ANNIVERSARY_OPS_1 A
FULL JOIN ANNIVERSARY_OPS_2 B ON A.TRAN_DATE = B.DAY_DATE AND A.BANNER = B.BANNER AND A.CHANNEL = B.CHANNEL
FULL JOIN ANNIVERSARY_OPS_3 C ON A.TRAN_DATE = C.DAY_DATE AND A.BANNER = C.BANNER AND A.CHANNEL = C.CHANNEL
JOIN PRD_NAP_BASE_VWS.DAY_CAL D ON COALESCE(A.TRAN_DATE,B.DAY_DATE,C.DAY_DATE) = D.DAY_DATE
WHERE COALESCE(A.BANNER, B.BANNER, C.BANNER) = 'NORDSTROM'
) WITH DATA PRIMARY INDEX (WEEK_NUM, BANNER, CHANNEL) ON COMMIT PRESERVE rows ;


INSERT INTO WEEKLY_OPS_STANDUP
SELECT
'ANNIVERSARY' AS OPS_NAME,
BANNER,
CHANNEL,
'Op GMV' AS METRIC_NAME,
NULL AS FISCAL_NUM,
day_short_desc AS FISCAL_DESC,
...
Null AS LY,
DAY_DATE
FROM ANNIVERSARY_OPS_4,CURR_WK
...
DAY_DATE
FROM ANNIVERSARY_OPS_4,CURR_WK
where channel in ('N.COM','MARKETPLACE')
GROUP BY 1,2,3,4,5,6,7,8,10,12,13;
...
WHERE OPS_NAME = 'ANNIVERSARY' -- Adding an extra filter since all Ops Scorecard will be using the same table
and day_date between {start_date_anniversary} and {end_date} ;
