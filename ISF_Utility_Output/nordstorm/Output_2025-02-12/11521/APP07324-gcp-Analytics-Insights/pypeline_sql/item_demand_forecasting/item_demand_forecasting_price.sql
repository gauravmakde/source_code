...
DELETE FROM {ip_forecast_t2_schema}.IDF_PRICE
WHERE (week_start_date >= {START_DATE} AND week_start_date <= {END_DATE});

INSERT INTO {ip_forecast_t2_schema}.IDF_PRICE
with wk as (
    SELECT distinct
...
all_data as (SELECT
    ...
    cast(max(price_fcst.regular_price_amt) as double precision) as regular_price_amt,
    cast(min(price_fcst.selling_retail_price_amt) as double precision) as current_price_amt,
    case
	    when max(price_fcst.regular_price_amt) = min(price_fcst.selling_retail_price_amt) then 'R' else 'C'
    end as current_price_type
    from wk
		join PRD_NAP_USR_VWS.PRODUCT_PRICE_TIMELINE_DIM as price_fcst
    on
...
epm_choice_num,
week_start_date
 from {ip_forecast_t2_schema}.IDF_ONLINE_INV
union 
select distinct channel_brand,
selling_channel,
epm_choice_num,
week_start_date
from {ip_forecast_t2_schema}.IDF_STORE_INV) a
join all_data  as b
...
on {ip_forecast_t2_schema}.IDF_PRICE;
