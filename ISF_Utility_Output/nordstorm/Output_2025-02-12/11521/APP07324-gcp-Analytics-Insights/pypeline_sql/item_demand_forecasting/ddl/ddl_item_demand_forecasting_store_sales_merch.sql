...
SET QUERY_BAND = 'App_ID=APP08073;
     ...
     Task_Name=create_store_sales_merch;'
...
Note:
This table all store sales data by Merch in-season team's standard by EPM_CHOICE_NUM X WEEK 

*/

--CALL SYS_MGMT.DROP_IF_EXISTS_SP ('t2dl_das_bie_dev', 'IDF_STORE_SALE_MERCH', OUT_RETURN_MSG);
create multiset table {ip_forecast_t2_schema}.IDF_STORE_SALE_MERCH
	,fallback
    ,no before journal
    ,no after journal
    ,checksum = default
...
COMMENT ON  {ip_forecast_t2_schema}.IDF_STORE_SALE_MERCH IS 'item demand forecasting store sales merch table - CC x week' ;
