...
*/
--CALL SYS_MGMT.DROP_IF_EXISTS_SP ('t2dl_das_bie_dev', 'IDF_EVENTS', OUT_RETURN_MSG);
create multiset table {ip_forecast_t2_schema}.IDF_EVENTS
	,fallback
    ,no before journal
    ,no after journal
    ,checksum = default
    ...
    regular_event_name  VARCHAR(100) CHARACTER SET UNICODE NOT CASESPECIFIC,
    loyalty_event VARCHAR(100) CHARACTER SET UNICODE NOT CASESPECIFIC,
    last_updated_utc DATE,
    dw_sys_load_tmstp  TIMESTAMP(6) DEFAULT CURRENT_TIMESTAMP(6) NOT NULL)
    PRIMARY INDEX ( channel_brand, selling_channel, week_start_date)
    partition by RANGE_N(week_start_date BETWEEN DATE '2019-01-01' AND DATE '2026-12-31' EACH INTERVAL '1' DAY);

-- Table Comment (STANDARD)
COMMENT ON  {ip_forecast_t2_schema}.IDF_EVENTS IS 'item demand forecasting event table - weekly' ;
