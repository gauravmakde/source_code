SET QUERY_BAND = 'App_ID=APP08154;
     DAG_ID=remote_sell_transactions_weekly_11521_ACE_ENG;
     Task_Name=remote_sell_transactions_weekly;'
	        ...
	        FROM
	            ...
	            LEFT JOIN  (
	                        ...
	                        FROM
	                                ...
	                                AND t.business_day_date >= '2020-01-01'
                                        AND t.business_day_date <= current_date()-1                   --DBA Recommended
	                        ...
	                        LEFT JOIN --prd_nap_usr_vws.retail_tran_hdr_fact c ON tndr.global_tran_id = c.global_tran_id
                                        ...
                                        WHERE business_day_date >= '2020-01-01'
                                                and business_day_date <= current_date()-1
							...
							WHERE --tndr.max_tender_cnt = 1 AND            --fy 2023 requirement change to NOT limit max tender count to 1
								...
								tndr.business_day_date >= '2020-01-01'
								and tndr.business_day_date <= current_date()-1
				GROUP BY 1,2
	                        ) tnd
	            ...
	            WHERE --UPPER(dtl.tran_type_code) = 'SALE'
		            ...
		            AND dtl.business_day_date >= '2020-01-01'
		            AND dtl.business_day_date <= current_date()-1
			) a
...
LEFT JOIN  (
            SELECT  sls.global_tran_id,
                    ...
                    WHERE sls.business_day_date >= '2020-01-01'
                    AND sls.business_day_date <= current_date()-1
...
CREATE  MULTISET VOLATILE TABLE hr_appended AS (
        ...
        (
	        ...
	        LEFT JOIN  HR_DAT r ON a.remote_sell_employee_id = r.worker_number
                ...
                WHERE a.business_day_date >= '2020-01-01'                               --DBA REC
                        and a.business_day_date <= current_date()-1
        ...
        WHERE b.business_day_date >= '2020-01-01'
        AND b.business_day_date <= current_date()-1
...
DELETE FROM T2DL_DAS_REMOTE_SELLING.remote_sell_transactions
WHERE business_day_date >= '2020-01-01'
AND business_day_date <= current_date()-1;

INSERT INTO T2DL_DAS_REMOTE_SELLING.remote_sell_transactions
...
ON T2DL_DAS_REMOTE_SELLING.remote_sell_transactions;
