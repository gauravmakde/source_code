SET QUERY_BAND = 'App_ID=APP08159;
     ...
     Task_Name=cust_service_3mo;'
     FOR SESSION VOLATILE;

/*
T2/Table Name: {service_eng_t2_schema}.cust_service_3mo
...
-- Create lookup table of start & end month for all 3-mo periods
--drop table fiscal_months;
create multiset volatile table fiscal_months as (
select 
   distinct 
   case when mod(cast(month_num as integer),100) in (1,2) then month_num - 90
        else month_num-2 end start_3mo
...
/*----------------------------------------------------------------------
--F-3) Create permanent customer/3-mo/service level table
----------------------------------------------------------------------*/ 
-- Aggregate data to a customer/3-mo/service-level
--drop table acp_id_service_3mo_temp;
create multiset volatile table acp_id_service_3mo_temp as (
select a.acp_id
  ,b.year_ending three_mo_ending
  ,a.service_name
...
from {service_eng_t2_schema}.cust_service_month a 
  join fiscal_months b on a.month_num between b.start_3mo and b.year_ending 
group by 1,2,3
) with data unique primary index (acp_id,three_mo_ending,service_name) on commit preserve rows;
...
--------------------------------------------
*/
delete from {service_eng_t2_schema}.cust_service_3mo;

insert into {service_eng_t2_schema}.cust_service_3mo
...
from  acp_id_service_3mo_temp a 
  join 
     (select distinct acp_id
      ,three_mo_ending
      from acp_id_service_3mo_temp
      where service_name='Z_) Nordstrom') b on a.acp_id=b.acp_id and a.three_mo_ending=b.three_mo_ending
;

COLLECT STATISTICS COLUMN(acp_id),
                   COLUMN(three_mo_ending),
                   COLUMN(service_name)
ON {service_eng_t2_schema}.cust_service_3mo;
