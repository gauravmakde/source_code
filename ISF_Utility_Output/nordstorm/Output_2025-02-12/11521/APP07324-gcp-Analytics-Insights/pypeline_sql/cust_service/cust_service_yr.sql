SET QUERY_BAND = 'App_ID=APP08159;
     ...
     Task_Name=cust_service_yr;'
     FOR SESSION VOLATILE;

/*
T2/Table Name: {service_eng_t2_schema}.cust_service_yr
...
-- Create lookup table of start & end month for all 12-mo periods
--drop table fiscal_months;
create multiset volatile table fiscal_months as (
select 
    distinct
    case when mod(cast(month_num as integer),100) = 12 then month_num - 11
         else month_num-99 end start_12mo
...
/*----------------------------------------------------------------------
--F-5) Create permanent customer/12-mo/service level table
----------------------------------------------------------------------*/
-- Aggregate data to a customer/year/service-level
--drop table acp_id_service_yr_temp; 
create multiset volatile table acp_id_service_yr_temp as (
select a.acp_id
  ,b.year_ending 
  ,a.service_name
  ...
  ,max(a.private_style) private_style
from {service_eng_t2_schema}.cust_service_month a 
  join fiscal_months b on a.month_num between b.start_12mo and b.year_ending 
group by 1,2,3
) with data unique primary index (acp_id,year_ending,service_name) on commit preserve rows;
...
--------------------------------------------
*/
delete from {service_eng_t2_schema}.cust_service_yr;

insert into {service_eng_t2_schema}.cust_service_yr
...
from  acp_id_service_yr_temp a 
  join 
     (select distinct acp_id
      ,year_ending
      from acp_id_service_yr_temp
      where service_name='Z_) Nordstrom') b on a.acp_id=b.acp_id and a.year_ending=b.year_ending
;

COLLECT STATISTICS COLUMN(acp_id),
                   COLUMN(year_ending),
                   COLUMN(service_name)
ON {service_eng_t2_schema}.cust_service_yr;
