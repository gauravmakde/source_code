SET QUERY_BAND = 'App_ID=APP08905;
     DAG_ID=ddl_techex_appid_airflow_dag_11521_ACE_ENG;
     Task_Name=techex_appid_airflow_dag_tpt_job_insert;'
     FOR SESSION VOLATILE;

DELETE FROM  {techex_t2_schema}.TPT_CONTROL_TBL where job_name = 'techex_appid_airflow_dag_ldg';

INSERT INTO {techex_t2_schema}.TPT_CONTROL_TBL (
  job_name,
  database_name,
  object_name,
  object_type,
  batchusername,
  truncateindicator,
  errorlimit,
  textdelimiter_hexa_flag,
  textdelimiter,
  AcceptExcessColumns,
  AcceptMissingColumns,
  QuotedData,
  OpenQuoteMark,
  s3bucket,
  s3prefix,
  s3singlepartfile,
  s3connectioncount,
  collist_indicator_flag,
  collist,
  rcd_load_tmstp,
  rcd_update_tmstp, 
  HeaderRcdIndicator,
  AllFilesHeaderRcdIndicator
)
VALUES (
	'techex_appid_airflow_dag_ldg',
	'{techex_t2_schema}',
	'techex_appid_airflow_dag_ldg',
	'T',
	 '*', 
	'Y',
	'1',
	'Y',
	'1F',
	'N',
	'N',
	'N',
	null,
	null,
	null,
	null,
	'20',
	'N',
	null,
	current_timestamp,
	current_timestamp,
	'N',
	'N'
 ); 

SET QUERY_BAND = NONE FOR SESSION;
