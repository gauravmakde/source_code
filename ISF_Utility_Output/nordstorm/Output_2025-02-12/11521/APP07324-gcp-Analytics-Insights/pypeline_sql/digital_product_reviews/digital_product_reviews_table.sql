...
create table if not exists {hive_schema}.digital_product_review_table
(
    channel                 string
    , month_idnt            string
    , quarter_idnt          string
...
using PARQUET
location 's3://{s3_bucket_root_var}/digital_product_review_table'
partitioned by (week_idnt);

-- sync partitions on hive table
MSCK REPAIR TABLE {hive_schema}.digital_product_review_table;
...
create or replace temporary view week_lkp as 
    ...
    where date_format(to_date(dt.day_date, 'MM/dd/yyyy'),'yyyy-MM-dd') between {start_date} and {end_date}
;
...
-- insert into table {hive_schema}.digital_product_review_table partition (week_idnt)
insert overwrite table {hive_schema}.digital_product_review_table partition (week_idnt)
select channel                 
    , month_idnt            
    , quarter_idnt          
    , fiscal_halfyear_num   
