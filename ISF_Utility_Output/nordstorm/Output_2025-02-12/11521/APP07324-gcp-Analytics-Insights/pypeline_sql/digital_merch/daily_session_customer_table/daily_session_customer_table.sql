...
-- DROP TABLE IF EXISTS {hive_schema}.daily_session_customer_table;
create table if not exists {hive_schema}.daily_session_customer_table (
	channel                         string
    ,experience                     string
    ...
    ,day_date                       date
) using PARQUET location 's3://{s3_bucket_root_var}/daily_session_customer_table' partitioned by (day_date);
;
 
-- sync partitions on hive table
MSCK REPAIR TABLE {hive_schema}.daily_session_customer_table;
...
create or replace temporary view marketplace_under100_tbl as
    ...
    where 1 = 1
        ...
        and cast(PDFI.activity_date_partition as date) between {start_date} and {end_date}
    group by 1,2,3,4
;



insert OVERWRITE TABLE {hive_schema}.daily_session_customer_table partition (day_date)
with session_acp_mapping as (
        ...
        (select xref.*,
         ...
         where 1 = 1
           and cast(xref.activity_date as date) between {start_date} and {end_date}
        )sub
        where rnk = 1
    )
select
...
where u.channelcountry = 'US'
  and cast(u.activity_date_partition as date)  between {start_date} and {end_date}
