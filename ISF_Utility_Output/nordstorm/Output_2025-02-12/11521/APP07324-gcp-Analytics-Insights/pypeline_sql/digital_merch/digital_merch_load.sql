...
USING csv 
OPTIONS (path "s3://{s3_bucket_root_var}/tpt_export/digital_merch_td_to_s3.csv",
        sep ",",
        header "false",
        dateFormat "yy/M/d",
        escape ""); 
...
-- DROP TABLE IF EXISTS {hive_schema}.digital_merch_table; 
create table if not exists {hive_schema}.digital_merch_table
(
	web_style_num       string
    ,selling_channel    string
    ,epm_style_num      string
...
USING PARQUET
location 's3://{s3_bucket_root_var}/digital_merch_table/'
partitioned by (day_date);


--msck repair runs a sync on the partitions so we can bring all data into the subsequent query
msck repair table {hive_schema}.digital_merch_table;


insert overwrite table {hive_schema}.digital_merch_table partition (day_date)
select
