...
USING csv 
OPTIONS (path "s3://{s3_bucket_root_var}/tpt_export/customer_attributes_table/*",
        sep ",",
        header "false",
        dateFormat "yy/M/d",
        escape "");
...
create table if not exists {hive_schema}.customer_attributes_table
(
	acp_id                  string
    ,session_id             string
    ,channel                string
...
USING PARQUET
location 's3://{s3_bucket_root_var}/customer_attributes_table/'
...
msck repair table {hive_schema}.customer_attributes_table;

insert overwrite table {hive_schema}.customer_attributes_table partition (activity_date_pacific)
select
     acp_id
    ,session_id
    ,channel 
