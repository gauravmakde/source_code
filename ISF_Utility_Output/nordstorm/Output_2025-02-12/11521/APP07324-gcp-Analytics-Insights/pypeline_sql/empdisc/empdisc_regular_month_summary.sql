SET QUERY_BAND = 'App_ID=APP02602;
     DAG_ID=empdisc_regular_month_summary_11521_ACE_ENG;
     Task_Name=empdisc_regular_month_summary;'
     FOR SESSION VOLATILE;

/*
T2/Table Name: T2DL_DAS_TRUST_EMP.empdisc_regular_month_summary{t2_test}
...
*/
-- regular calendar: first day of the month given a date
create multiset volatile table regular_first_day as (
    select
    -- first day of the month
    CAST((EXTRACT(YEAR FROM DATE '2024-02-24')) || '-' || (TO_CHAR(EXTRACT(MONTH FROM DATE '2024-02-24'), 'FM00')) || '-01' AS DATE) first_day
    -- four digits year
    ,CAST(EXTRACT(YEAR FROM DATE '2024-02-24') AS INTEGER) year_num
    -- two digits month number
    ,CAST(TO_CHAR(EXTRACT(MONTH FROM DATE '2024-02-24'), 'FM00') AS INTEGER) month_num
    -- six digits year and month, yyyymm
    ,CAST(EXTRACT(YEAR FROM DATE '2024-02-24')  || TO_CHAR(EXTRACT(MONTH FROM DATE '2024-02-24'), 'FM00') AS INTEGER) year_month_num
...
create multiset volatile table data as (
  SELECT
    (SELECT year_month_num from regular_first_day) year_month_num --YYYYMM
    ,(SELECT year_num from regular_first_day) year_num
    ,(SELECT month_num from regular_first_day) month_num
    ,e.data_source_code
  ...
  WHERE e.business_day_date BETWEEN (SELECT first_day from regular_first_day) AND  {execute_date}
...
DELETE FROM T2DL_DAS_TRUST_EMP.empdisc_regular_month_summary{t2_test}
WHERE year_month_num = (SELECT year_month_num from regular_first_day)
;

-- insert the data into the table
INSERT INTO T2DL_DAS_TRUST_EMP.empdisc_regular_month_summary{t2_test}
SELECT *
FROM data
;
...
COLLECT STATISTICS  COLUMN (year_month_num),
                    ...
                    COLUMN (ring_channel)
on T2DL_DAS_TRUST_EMP.empdisc_regular_month_summary{t2_test};
