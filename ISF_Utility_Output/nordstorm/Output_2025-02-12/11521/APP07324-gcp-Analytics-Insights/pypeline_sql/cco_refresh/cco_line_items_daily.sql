SET QUERY_BAND = 'App_ID=APP08240;
     DAG_ID=cco_line_items_table_11521_ACE_ENG;
 ...
 *
 * II) Set the date parameters. Only 60 days of data will be updated
 *
...
select min(case when year_num=2018 then day_date else null end) fy18_start_dt
    ...
    ,max(case when month_num < (select curr_mo from curr_mo_lkp) then day_date
              else null end) latest_mo_dt
    ,min(case when month_num = (select curr_mo-400 from curr_mo_lkp) then day_date
              else null end) r4yr1_start_dt
    ,min(case when month_num = (select curr_mo-500 from curr_mo_lkp) then day_date
              else null end) r4yr0_start_dt
    ,max(case when month_num = (select prior_mo-400 from curr_mo_lkp) then day_date
...
where day_date=(select latest_mo_dt from date_parameter_lookup)+1  --"2023-04-29"
...
select
     (select fy19_start_dt from date_parameter_lookup) yr1_start_dt  --"2019-02-03"
...
-- create multiset volatile table date_lookup as (
-- select
--    date'2024-03-10'    yr1_start_dt
--   ,date'2024-03-10'+1  yr1_end_dt
--   ,date'2024-03-10'+2  yr2_start_dt
--   ,date'2024-03-10'+3 yr2_end_dt
--   ,date'2024-03-10'+4 yr3_start_dt
--   ,date'2024-03-10'+5 yr3_end_dt
--   ,date'2024-03-10'+6 yr4_start_dt
--   ,date'2024-03-10'+7 yr4_end_dt
-- ) with data primary index(yr1_start_dt) on commit preserve rows;
...
(select distinct a.upc_num
                ...
                ,case when b.dept_num in (765) and trim(b.dept_desc)='GIFT WRAP SERVICES' then 1 else 0 end gift_wrap_service
...
where business_day_date between (CURRENT_DATE - INTERVAL '60' DAY) and CURRENT_DATE
    and a.order_date_pacific between (CURRENT_DATE - INTERVAL '60' DAY) and CURRENT_DATE
...
select distinct a.business_day_date
    ...
    ,case when st.business_unit_desc in ('FULL LINE','FULL LINE CANADA','N.CA','N.COM','TRUNK CLUB','MARKETPLACE')
                then 'NORDSTROM'
...
where business_day_date between (CURRENT_DATE - INTERVAL '60' DAY) and CURRENT_DATE
     and coalesce(dtl.order_date,dtl.tran_date) between (CURRENT_DATE - INTERVAL '60' DAY) and CURRENT_DATE
...
where remote_sell_swimlane in ('STYLEBOARD_ATTRIBUTED','STYLEBOARD_PRIVATE_ATTRIBUTED'
            ,'PERSONAL_REQUEST_A_LOOK_ATTRIB', 'PERSONAL_REQUEST_A_LOOK_ATTRIBUTED','REQUEST_A_LOOK_ATTRIBUTED'
            ,'CHAT_BOARD_ATTRIBUTED','STYLELINK_ATTRIBUTED','STYLEBOARD_PUBLIC_ATTRIBUTED'
            ,'PRIVATE_STYLING_ATTRIBUTED','ECF_ATTRIBUTED','TRUNK_CLUB')
  and upc_num is not null
  and global_tran_id is not null
  and business_day_date between (CURRENT_DATE - INTERVAL '60' DAY) and CURRENT_DATE
...
select a.day_date
  ...
  ,case when a.day_date between date'2018-07-11' and '2018-08-05' then 1
        ...
        when a.day_date between date'2024-07-09' and '2024-08-04' then 1
...
PARTITION BY Range_N(day_date BETWEEN DATE '2017-01-01' AND DATE '2025-12-31' EACH INTERVAL '1' DAY,UNKNOWN)
on commit preserve rows;
...
case when (st.business_unit_desc like '%CANADA' or st.business_unit_desc like '%.CA')
...
upc_num,
item_source
from T2DL_DAS_SALES_RETURNS.SALES_AND_RETURNS_FACT dtl
  join prd_nap_usr_vws.store_dim as ist on dtl.intent_store_num=ist.store_num
  join prd_nap_usr_vws.store_dim as st
    on (case
          when dtl.line_item_order_type in ('CustInitWebOrder','CustInitPhoneOrder') 
                  and ist.store_type_code='RK' then 828
          when dtl.line_item_order_type in ('CustInitWebOrder','CustInitPhoneOrder') 
 ...
 where business_day_date between (CURRENT_DATE - INTERVAL '60' DAY) and CURRENT_DATE
    and  coalesce( order_date, tran_date) between (select yr1_start_dt from date_lookup) and CURRENT_DATE
    and acp_id is not null
    and st.business_unit_desc in ('FULL LINE','FULL LINE CANADA','N.CA','N.COM','TRUNK CLUB','RACK','RACK CANADA'
           ,'OFFPRICE ONLINE','MARKETPLACE')
    and shipped_usd_sales >= 0
    and shipped_usd_sales is not null
    and coalesce( return_usd_amt,0) >= 0
)with data primary index(global_tran_id, line_item_seq_num)
PARTITION BY (Range_N(business_day_date BETWEEN DATE '2017-01-01' AND DATE '2025-12-31' EACH INTERVAL '1' DAY,UNKNOWN)
            , Range_N(tran_date BETWEEN DATE '2017-01-01' AND DATE '2025-12-31' EACH INTERVAL '1' DAY,UNKNOWN)
             )
on commit preserve rows;
...
where aare_status_date between (select yr1_start_dt from date_lookup) and (select yr4_end_dt+14 from date_lookup)
...
(
    ...
    ,store_region VARCHAR(30) compress ('FULL LINE CANADA','MIDWEST FLS','MIDWEST RACKS','N.CA','N.COM', 'NORTHEAST FLS','NORTHEAST RACKS','NORTHWEST FLS', 'NORTHWEST RACKS','OFFPRICE ONLINE','RACK CANADA','SCAL RACKS','SOUTHEAST FLS','SOUTHEAST RACKS','SOUTHERN CALIFORNIA FLS','SOUTHWEST FLS','SOUTHWEST RACKS','MARKETPLACE')
...
select distinct
   ...
   case
    ...
    ,case when rst.business_unit_desc in ('FULL LINE','FULL LINE CANADA','N.CA','N.COM','TRUNK CLUB','MARKETPLACE')
                then 'NORDSTROM'
    ...
    ,case when rst.business_unit_desc in ('FULL LINE','FULL LINE CANADA') then '1) Nordstrom Stores'
          when rst.business_unit_desc in ('N.CA','N.COM','TRUNK CLUB','MARKETPLACE') then '2) Nordstrom.com'
    ...
    ,case when coalesce(b.remote_sell_swimlane,'X') not in ('X','TRUNK_CLUB') --includes Style Boards, Style Links
                or trim(COALESCE(dtl.item_source,'XYZ')) = 'SB_SALESEMPINIT' --alternate Style Board logic
    ...
    ,case when substring(trim(ofa.item_delivery_method),1,8) in ('NEXT_DAY','FC_BOPUS') then 1
          else 0 end service_next_day_pickup
    ...
    ,case when trim(b.remote_sell_swimlane) in ('STYLEBOARD_ATTRIBUTED','STYLEBOARD_PRIVATE_ATTRIBUTED'
                ,'PERSONAL_REQUEST_A_LOOK_ATTRIB', 'PERSONAL_REQUEST_A_LOOK_ATTRIBUTED','REQUEST_A_LOOK_ATTRIBUTED'
                ,'CHAT_BOARD_ATTRIBUTED')
                or trim(COALESCE(dtl.item_source,'XYZ')) = 'SB_SALESEMPINIT'
            then 1
          else 0 end service_style_boards
    ,coalesce(upc.gift_wrap_service,0) service_gift_wrapping
    ,case when upc.dept_num = 587 then 1 else 0 end service_pop_in
    ,case when trim(dtl.business_unit_desc)='MARKETPLACE' then 1 else 0 end marketplace_flag
...
--     on dtl.intent_store_num = st.store_num
  ...
  and coalesce(TD_SYSFNLIB.lpad(trim(dtl.upc_num),15,'0'),'X'||substring(dtl.acp_id,12,11)) = b.upc_num
...
/* Delete 60 days transaction data */
DELETE FROM {cco_t2_schema}.cco_line_items
WHERE business_day_date between (CURRENT_DATE - INTERVAL '60' DAY) and CURRENT_DATE;

/* Delete NSEAM data */
DELETE FROM {cco_t2_schema}.cco_line_items
WHERE data_source = '2) NSEAM';
...
where st.business_unit_desc in ('FULL LINE','FULL LINE CANADA','N.CA','N.COM','TRUNK CLUB','RACK','RACK CANADA'
                                ,'OFFPRICE ONLINE','MARKETPLACE')
...
where dtl.date_shopped between (select yr1_start_dt from date_lookup) and  (select yr4_end_dt from date_lookup)
    ...
    and st.business_unit_desc
       in ('FULL LINE','FULL LINE CANADA','N.CA','N.COM','TRUNK CLUB','RACK','RACK CANADA'
            ,'OFFPRICE ONLINE','MARKETPLACE');
