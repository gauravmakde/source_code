...
select
  ...
  ,cust_chan_npg
  ,cust_chan_div_count
  ,lag(cust_chan_net_sales) over (partition by acp_id,channel order by reporting_year_shopped asc) as cust_chan_net_sales_ly
  ,lead(cust_chan_net_sales) over (partition by acp_id,channel order by reporting_year_shopped asc) as cust_chan_net_sales_ny
  ,lag(cust_chan_anniversary) over (partition by acp_id,channel order by reporting_year_shopped asc) as cust_chan_anniversary_ly
  ,lag(cust_chan_holiday) over (partition by acp_id,channel order by reporting_year_shopped asc) as cust_chan_holiday_ly
from
...
--insert into t2dl_das_strategy.CCO_cust_chan_yr_attributes
--explain

DELETE FROM {cco_t2_schema}.cco_cust_chan_yr_attributes ALL;

INSERT INTO {cco_t2_schema}.cco_cust_chan_yr_attributes 
select distinct
   a.acp_id
...
left join 
  {cco_t2_schema}.cco_buyer_flow b  
  on a.acp_id=b.acp_id 
  and a.channel=b.channel 
  and a.reporting_year_shopped=b.reporting_year_shopped
left join 
  {cco_t2_schema}.cco_customer_level_attributes c 
  on a.acp_id=c.acp_id 
  and a.reporting_year_shopped=c.reporting_year_shopped
left join
...
and a.reporting_year_shopped=k.reporting_year_shopped;
