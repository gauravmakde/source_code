...
DAG_ID=ddl_cco_cust_week_attributes_11521_ACE_ENG;
Task_Name=run_teradata_ddl_cco_cust_week_attributes;'
FOR SESSION VOLATILE;


/************************************************************************************/
/************************************************************************************
 * CCO Customer/Week-Level Attributes DDL file.
 * This file creates the production table T2DL_DAS_STRATEGY.cco_cust_week_attributes.
 ************************************************************************************/
/************************************************************************************/
...
CALL SYS_MGMT.DROP_IF_EXISTS_SP(
     '{cco_t2_schema}',
     'cco_cust_week_attributes',
     OUT_RETURN_MSG);
--*/


CREATE MULTISET TABLE {cco_t2_schema}.cco_cust_week_attributes,--NO FALLBACK ,
    CHECKSUM = DEFAULT,
...
(
    ...
    cust_tender_3rd_party_credit INTEGER COMPRESS (0, 1),
    cust_tender_debit_card INTEGER COMPRESS (0, 1),
    cust_tender_gift_card INTEGER COMPRESS (0, 1),
    cust_tender_cash INTEGER COMPRESS (0, 1),
    cust_tender_paypal INTEGER COMPRESS (0, 1),
    cust_tender_check INTEGER COMPRESS (0, 1),
    cust_event_holiday INTEGER COMPRESS (0, 1),
    cust_event_anniversary INTEGER COMPRESS (0, 1),
    cust_svc_group_exp_delivery INTEGER COMPRESS (0, 1),
    cust_svc_group_order_pickup INTEGER COMPRESS (0, 1),
    cust_svc_group_selling_relation INTEGER COMPRESS (0, 1),
    cust_svc_group_remote_selling INTEGER COMPRESS (0, 1),
    cust_svc_group_alterations INTEGER COMPRESS (0, 1),
    cust_svc_group_in_store INTEGER COMPRESS (0, 1),
    cust_svc_group_restaurant INTEGER COMPRESS (0, 1),
    cust_service_free_exp_delivery INTEGER COMPRESS (0, 1),
    cust_service_next_day_pickup INTEGER COMPRESS (0, 1),
    cust_service_same_day_bopus INTEGER COMPRESS (0, 1),
    cust_service_curbside_pickup INTEGER COMPRESS (0, 1),
    cust_service_style_boards INTEGER COMPRESS (0, 1),
    cust_service_gift_wrapping INTEGER COMPRESS (0, 1),
    cust_service_pop_in INTEGER COMPRESS (0, 1),
    cust_marketplace_flag INTEGER COMPRESS (0, 1),
...
PRIMARY INDEX (acp_id, week_idnt)
...
COLLECT STATISTICS COLUMN (acp_id) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (week_idnt) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_gender) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_age) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_lifestage) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_age_group) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_nms_market) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_dma) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_country) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_dma_rank) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_loyalty_type) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_loyalty_level) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (cust_employee_flag) ON {cco_t2_schema}.cco_cust_week_attributes;
COLLECT STATISTICS COLUMN (acp_id, week_idnt) ON {cco_t2_schema}.cco_cust_week_attributes;
