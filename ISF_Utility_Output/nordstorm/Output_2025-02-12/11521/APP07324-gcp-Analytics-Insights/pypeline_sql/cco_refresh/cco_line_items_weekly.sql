...
where business_day_date between (select yr1_start_dt from date_lookup) and CURRENT_DATE
    and a.order_date_pacific between (select yr1_start_dt from date_lookup) and CURRENT_DATE
...
where business_day_date between (select yr1_start_dt from date_lookup) and CURRENT_DATE
 and coalesce(dtl.order_date,dtl.tran_date) between (select yr1_start_dt from date_lookup) and CURRENT_DATE
...
where remote_sell_swimlane in ('STYLEBOARD_ATTRIBUTED','STYLEBOARD_PRIVATE_ATTRIBUTED'
  ...
  and business_day_date between (select yr1_start_dt from date_lookup) and CURRENT_DATE
...
where business_day_date between (select yr1_start_dt-14 from date_lookup) and CURRENT_DATE
...
where a.day_date between (select yr1_start_dt from date_lookup) and CURRENT_DATE
...
from T2DL_DAS_SALES_RETURNS.SALES_AND_RETURNS_FACT dtl
 ...
 where
    business_day_date between (select yr1_start_dt from date_lookup) and CURRENT_DATE
...
where aare_status_date between (select yr1_start_dt from date_lookup) and CURRENT_DATE
...
where st.business_unit_desc in ('FULL LINE','FULL LINE CANADA','N.CA','N.COM','TRUNK CLUB','RACK','RACK CANADA'
                                ,'OFFPRICE ONLINE','MARKETPLACE')
  and a.ticket_date between (select yr1_start_dt from date_lookup) and CURRENT_DATE
