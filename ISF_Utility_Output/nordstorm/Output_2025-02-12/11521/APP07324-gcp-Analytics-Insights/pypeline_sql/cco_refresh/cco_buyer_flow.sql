...
select max(case when month_num < (select curr_mo from curr_mo_lkp)
                then day_date
...
select dp.r4yr0_start_dt yr0_start_dt
     , dp.r4yr0_end_dt yr0_end_dt
     , dp.r4yr1_start_dt yr1_start_dt
     , dl.yr1_end_dt
     , dl.yr2_start_dt
     , dl.yr2_end_dt
     , dl.yr3_start_dt
...
cross join (
  select max(case when month_num = (select yr1_end_mo   from month_lookup) then day_date else null end) yr1_end_dt
...
/********* Step I-B-v: Generate names for the past 5 rolling-12-mo periods *********/
...
select 
  max(case when month_num=(select yr0_end_mo from month_lookup)
   ...
   yr4_name
from realigned_calendar
where month_num in ( (select yr0_end_mo from month_lookup)
