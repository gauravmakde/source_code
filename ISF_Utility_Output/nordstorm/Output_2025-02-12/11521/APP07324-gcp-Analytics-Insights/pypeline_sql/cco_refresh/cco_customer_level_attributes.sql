...
-- create MULTISET volatile table date_parameter_lookup as (
-- select date'2024-03-10'-2 fy18_start_dt
--     ,date'2024-03-10'-1 fy18_end_dt
--     ,date'2024-03-10' fy19_start_dt
--     ,date'2024-03-10'+7 latest_mo_dt
--     ,date'2024-03-10' r4yr1_start_dt
--     ,date'2024-03-10'-2 r4yr0_start_dt
--     ,date'2024-03-10'-1 r4yr0_end_dt
-- ) with data primary index(latest_mo_dt) on commit preserve rows;
...
/********* Step I-b-i: Find start & end month of the past 4 years *********/
...
where day_date = (select latest_mo_dt from date_parameter_lookup)+1
) with data primary index(current_mo) on commit preserve rows;


/********* Step I-b-ii: Create the "YE-MMMYY"-format "year names" for each of the past 4 years *********/
...
/********* Step I-b-iii: Create the date-lookup table *********/
...
group by 1
)with data primary index(reporting_year) on commit preserve rows;
...
from {cco_t2_schema}.cco_line_items a
  left join date_lookup b on a.reporting_year_shopped=b.reporting_year
...
select cust_dma
  ...
  ,RANK() OVER (PARTITION BY reporting_year_shopped ORDER BY dma_net_sales DESC) dma_rank
from customer_dma_prep
where coalesce(cust_dma,'Other') <> 'Other'
group by 1,2
)with data primary index(cust_dma,reporting_year_shopped) on commit preserve rows;
...
from customer_dma_prep a
  left join dma_ranking b on a.cust_dma=b.cust_dma and a.reporting_year_shopped=b.reporting_year_shopped
)with data primary index(acp_id,reporting_year_shopped) on commit preserve rows;

-- select count(*)
--   ,count(distinct acp_id||reporting_year_shopped)
-- from customer_dma
-- ;
...
create multiset volatile table anchor_brands as (
    ...
    where dtl.business_day_date >= (select fy19_start_dt from date_parameter_lookup)
...
select acp_id
  ...
  ,1 + (RANK() OVER (PARTITION BY reporting_year_shopped ORDER BY contribution_margin desc) - 1) * 10
          / COUNT(*) OVER (PARTITION BY reporting_year_shopped ) contribution_margin_decile
from
  (
  select dr.acp_id
...
from
  ...
  from customer_year_driver a
    left join experian_demos b on a.acp_id=b.acp_id and a.reporting_year_shopped=b.reporting_year_shopped
    left join modeled_ages c on a.acp_id=c.acp_id and a.reporting_year_shopped=c.reporting_year_shopped
  ) x
)with data primary index(acp_id,reporting_year_shopped) on commit preserve rows;
...
from customer_year_driver a
  left join cust_level_loyalty_cardmember b  on a.acp_id=b.acp_id and a.reporting_year_shopped=b.reporting_year_shopped
  left join cust_level_loyalty_member c on a.acp_id=c.acp_id and a.reporting_year_shopped=c.reporting_year_shopped
)with data primary index(acp_id,reporting_year_shopped) on commit preserve rows;
...
SELECT 
reporting_year as reporting_year_shopped,
...
group by 1
) with data primary index (reporting_year_shopped) on commit preserve rows;

 /************************************************************************************
 * PART I-j) Join all customer/year-level attributes together (& insert into final table)
 ************************************************************************************/

DELETE FROM {cco_t2_schema}.cco_customer_level_attributes ALL;

INSERT INTO {cco_t2_schema}.cco_customer_level_attributes
select distinct a.acp_id
  ,a.reporting_year_shopped
...
from customer_year_driver a
  left join both_ages b on a.acp_id=b.acp_id and a.reporting_year_shopped=b.reporting_year_shopped
  left join customer_NMS_market c on a.acp_id=c.acp_id
  left join customer_dma d on a.acp_id=d.acp_id and a.reporting_year_shopped=d.reporting_year_shopped
  left join cust_level_loyalty e on a.acp_id=e.acp_id and a.reporting_year_shopped=e.reporting_year_shopped
  --left join contribution_margin_deciles f on a.acp_id=f.acp_id and a.reporting_year_shopped=f.reporting_year_shopped
  left join customer_acquisition_tenure g on a.acp_id=g.acp_id and a.reporting_year_shopped=g.reporting_year_shopped
  left join customer_activation h on a.acp_id=h.acp_id and a.reporting_year_shopped=h.reporting_year_shopped
  left join derived_cust_attributes i on a.acp_id=i.acp_id and a.reporting_year_shopped=i.reporting_year_shopped
  left join clv_date cld on a.reporting_year_shopped = cld.reporting_year_shopped
