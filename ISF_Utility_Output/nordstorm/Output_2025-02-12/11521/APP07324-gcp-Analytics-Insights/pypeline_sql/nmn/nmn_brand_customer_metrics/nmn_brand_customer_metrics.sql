...
FROM PRD_NAP_USR_VWS.DAY_CAL_454_DIM 
where month_idnt between (select month_start from date_mapping where day_date= {start_date}) and (select month_end from date_mapping where day_date={end_date})
...
FROM PRD_NAP_USR_VWS.PRODUCT_SKU_DIM_VW p
  ...
  INNER JOIN PRD_NAP_USR_VWS.vendor_label_dim v
         ...
         AND (v.vendor_brand_code) IN (SELECT DISTINCT brand_id FROM {nmn_t2_schema}.nmn_head_brands)
UNION ALL
SELECT 
       p.rms_sku_num AS sku_num,
       v.vendor_brand_name AS brand_name,
...
FROM PRD_NAP_USR_VWS.PRODUCT_SKU_DIM_VW p
  ...
  INNER JOIN PRD_NAP_USR_VWS.vendor_label_dim v
          ON q.vendor_label_code = v.vendor_label_code
         AND (v.vendor_brand_code) IN (SELECT DISTINCT brand_id FROM {nmn_t2_schema}.nmn_torso_brands)
...
CREATE MULTISET VOLATILE TABLE brand_visits AS (
    ...
    ,Rank() Over(PARTITION BY x.acp_id, x.brand_name, x.brand_tier ORDER BY x.business_day_date DESC) AS visit_rank
    FROM (
        SELECT
             s.acp_id
             ,np.brand_name
...
FROM (SELECT 
      ...
      FROM brand_visits
      WHERE business_day_date BETWEEN (SELECT start_date FROM _variables) AND (SELECT end_date FROM _variables)group by 1,2 ) a group by 1,2 ) WITH DATA NO PRIMARY INDEX ON COMMIT PRESERVE ROWS;
...
DELETE from {nmn_t2_schema}.nmn_brand_customer_metrics where month_idnt>=(select month_idnt from _variables);
insert into {nmn_t2_schema}.nmn_brand_customer_metrics select a.*, Current_Timestamp AS dw_sys_load_tmstp from final_insert a;
