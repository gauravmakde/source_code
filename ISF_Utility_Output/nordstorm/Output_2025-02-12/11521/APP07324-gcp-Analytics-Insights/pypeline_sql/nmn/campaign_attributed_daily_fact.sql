...
WHERE 1=1
AND dd.day_date BETWEEN {start_date} AND {end_date}
GROUP BY
...
DELETE FROM {nmn_t2_schema}.CAMPAIGN_ATTRIBUTED_DAILY_FACT WHERE day_date BETWEEN {start_date} AND {end_date};

INSERT INTO {nmn_t2_schema}.CAMPAIGN_ATTRIBUTED_DAILY_FACT 
SELECT a.*, Current_Timestamp AS dw_sys_load_tmstp FROM cad_fact_stg a;

COLLECT STATISTICS COLUMN (DAY_DATE, CHANNEL_NUM, CHANNEL_COUNTRY, CHANNEL_BANNER, MKTG_TYPE, FINANCE_ROLLUP, FINANCE_DETAIL, UTM_SOURCE, UTM_CHANNEL,utm_campaign, utm_term, UTM_CONTENT, SUPPLIER_NAME, BRAND_NAME, DIVISION, SUBDIVISION, DEPARTMENT, campaign_id, campaign_brand_id, placementsio_id) ON {nmn_t2_schema}.CAMPAIGN_ATTRIBUTED_DAILY_FACT;
COLLECT STATISTICS COLUMN (DAY_DATE) ON {nmn_t2_schema}.CAMPAIGN_ATTRIBUTED_DAILY_FACT;
COLLECT STATISTICS COLUMN (BRAND_NAME ,SUBDIVISION) ON {nmn_t2_schema}.CAMPAIGN_ATTRIBUTED_DAILY_FACT;
