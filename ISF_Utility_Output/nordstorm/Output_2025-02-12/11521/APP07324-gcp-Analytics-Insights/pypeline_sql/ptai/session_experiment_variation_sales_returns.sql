create table if not exists {hive_schema}.session_experiment_variation_sales_returns (
    channel string,
...
using PARQUET
location "s3://{s3_bucket_root_var}/session_experiment_variation_sales_returns"
partitioned by (experimentname);

MSCK REPAIR table {hive_schema}.session_experiment_variation_sales_returns;
...
where last_activity_date_pst >= {end_date} - interval '90' day
group by 1 
;
...
from 
	...
	select     
	    ...
	    last_activity_date_pst,
		total_unique_sessions,
		row_number() over(partition by channel, experience, projectname, experimentname order by case when variationname like 'default' or variationname like 'off' then 0 else 1 end, variationname desc) variation_rk
	from ace_etl.session_experiment_variation_metadata
...
select
    ...
    case when variationname not in ('default', 'off') then RR_result_query else 'control arm' end as return_rate_results,
    ...
    (
        ...
        (
            select
				...
				control_returns,
				test_returns,
                case when variationname not in ('default', 'off') then (control_returns+test_returns)*1.00000000/(control_sales+test_sales)*1.00000000 end as RR_pool_p_query,
                1-case when variationname not in ('default', 'off') then (control_returns+test_returns)*1.00000000/(control_sales+test_sales)*1.00000000 end as RR_q_query,
            ...
            (
                select
                    ...
                    sum(case when base.variationname in ('default', 'off') then base.total_sales_units end) over (partition by  base.channel,base.experience,base.projectname,base.experimentname) as control_sales,
                    case when base.variationname not in ('default', 'off') then base.total_sales_units else null end test_sales,
                    sum(case when base.variationname in ('default', 'off') then  base.total_returns_units end) over (partition by  base.channel,base.experience,base.projectname,base.experimentname) as control_returns,
                    case when base.variationname not in ('default', 'off') then base.total_returns_units else null end test_returns
...
from
	...
	select
		...
		projectname,
		experimentname,
	    sum(case when variationname in ('default', 'off') then total_unique_sessions end)*1.000/sum(total_unique_sessions)*1.000 as control_total_sessions,
	    sum(case when variationname not in ('default', 'off') then total_unique_sessions end)*1.000/sum(total_unique_sessions)*1.000 as test_total_sessions,
	    sum(case when variationname in ('default', 'off') then unique_purchasing_session_ids end)*1.000/sum(unique_purchasing_session_ids)*1.000 as control_total_purchasing_sessions,
	    sum(case when variationname not in ('default', 'off') then unique_purchasing_session_ids end)*1.000/sum(unique_purchasing_session_ids)*1.000 as test_total_purchasing_sessions
	from stats_testing 
...
insert
    OVERWRITE TABLE {hive_schema}.session_experiment_variation_sales_returns PARTITION (experimentname)
select
    *
from
    output;