...
AS (
   SELECT DISTINCT
   ps.rms_sku_num
   FROM prd_nap_usr_vws.product_sku_dim ps
   WHERE ps.channel_country = 'US'
   AND ps.div_num IN (310, 345)
    )
...
DELETE FROM {shoe_categories_t2_schema}.shoe_size_performance_monthly_base WHERE week_num BETWEEN (Select start_week_num from variable_start_end_week) AND (Select end_week_num from variable_start_end_week);


INSERT INTO {shoe_categories_t2_schema}.shoe_size_performance_monthly_base
SELECT a.*, 
...
on {shoe_categories_t2_schema}.shoe_size_performance_monthly_base;
