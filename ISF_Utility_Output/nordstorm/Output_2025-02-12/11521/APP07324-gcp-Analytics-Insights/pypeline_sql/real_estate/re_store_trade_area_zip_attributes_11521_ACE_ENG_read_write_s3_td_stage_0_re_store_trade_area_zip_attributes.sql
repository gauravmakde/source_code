...
update T2DL_DAS_REAL_ESTATE.re_store_trade_area_zip_attributes_ldg
set zip_code = LPAD(TRIM(zip_code), 5, '0')
where REGEXP_SIMILAR(TRIM(zip_code), '[0-9]+') = 1;

-- delete data from a landing table
delete
from T2DL_DAS_REAL_ESTATE.re_store_trade_area_zip_attributes
;

-- insert data from a staging table to a landing table
insert into T2DL_DAS_REAL_ESTATE.re_store_trade_area_zip_attributes
    select
    store_number
    ,data_year
    ,zip_code
	...
	from T2DL_DAS_REAL_ESTATE.re_store_trade_area_zip_attributes_ldg
;
-- drop staging table
drop table T2DL_DAS_REAL_ESTATE.re_store_trade_area_zip_attributes_ldg;
