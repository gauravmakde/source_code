...
SET QUERY_BAND = 'App_ID=APP09117;
     DAG_ID="location_inventory_tracking_fls_11521_ACE_ENG";
     Task_Name=location_inventory_tracking_fls;'
     FOR SESSION VOLATILE;


/*
T2/Table Name: T2DL_DAS_LIT.LOCATION_INVENTORY_TRACKING_FLS
...
CREATE MULTISET VOLATILE TABLE store_climate AS (
    select
        ...
        WHERE peer_group_type_code = 'FPC'
)
WITH DATA
...
WHERE frcst.location_id IN (SELECT DISTINCT store_num FROM PRD_NAP_USR_VWS.STORE_DIM WHERE business_unit_desc IN ('FULL LINE', 'N.COM'))
...
,jdodv.dept_num
,jdodv.class_num
,CASE WHEN jdodv.fulfilled_from_location IN (210, 212, 209) THEN 210 ELSE jdodv.fulfilled_from_location END as loc_idnt
...
AND jdodv.fulfilled_from_location IN (SELECT sd.store_num FROM PRD_NAP_USR_VWS.STORE_DIM sd WHERE sd.channel_num IN ('110'))
...
WITH total_sff_u as
(SELECT month_idnt as month_num
,SUM(store_fulfill_u) as total_jwn_demand_store_fulfilled_units_ty
FROM store_fulfill_actuals
WHERE month_idnt IN (SELECT DISTINCT month_idnt FROM location_plans WHERE last_month_flag = 1)
GROUP BY 1
),
sff_splits as(
SELECT OREPLACE(CONCAT(w.class_num,w.dept_num,w.month_idnt,w.loc_idnt),' ','') as pkey
,w.class_num
,w.dept_num
,w.month_idnt as month_num
...
month_num,
supp_part_num
from PRD_NAP_VWS.MERCH_TRANSACTION_SBCLASS_STORE_WEEK_AGG_FACT_VW WHERE channel_num = 110
and week_num IN (SELECT DISTINCT week_IDNT
            FROM PRD_NAP_USR_VWS.DAY_CAL_454_DIM tdl
...
LEFT JOIN store_fulfill_actuals sfl 
ON sfl.month_idnt = l.month_idnt
AND sfl.dept_num = l.dept_idnt
AND sfl.class_num = l.class_idnt
AND sfl.loc_idnt = l.loc_idnt
)
WITH DATA
PRIMARY INDEX (pkey)
ON COMMIT PRESERVE ROWS;
...
DELETE FROM {location_inventory_tracking_t2_schema}.location_inventory_tracking_fls;



INSERT INTO {location_inventory_tracking_t2_schema}.location_inventory_tracking_fls (
pkey
,ab_key
,month_idnt
,month_desc
...
--SELECT * FROM T3DL_NAP_PLANNING.LIT_AGGREGATE_TABLE_FULL_LINE


COLLECT STATISTICS COLUMN (pkey) ON {location_inventory_tracking_t2_schema}.location_inventory_tracking_fls;
COLLECT STATISTICS COLUMN (class_idnt,dept_idnt,month_idnt,loc_idnt) ON {location_inventory_tracking_t2_schema}.location_inventory_tracking_fls;
COLLECT STATISTICS COLUMN (class_idnt,month_idnt,loc_idnt) ON {location_inventory_tracking_t2_schema}.location_inventory_tracking_fls;
COLLECT STATISTICS COLUMN (pkey,current_month_flag,last_month_flag) ON {location_inventory_tracking_t2_schema}.location_inventory_tracking_fls;
