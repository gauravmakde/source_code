...
create or replace temp view spv_add_to_bag as (
	...
	from
        ...
        and to_date(concat(caatb.year, '-', LPAD(caatb.month, 2, '0'), '-', LPAD(caatb.day, 2, '0')),'yyyy-MM-dd') between {start_date} and {end_date}
        and arrays_overlap(map_keys(headers), array('Nord-Load','nord-load','nord-test','Nord-Test', 'Sretest', 'sretest')) = False
        and (
            array_contains(map_keys(headers), 'identified-bot') = False
            or
...
create or replace temp view spv_remove_from_bag as (
	...
	from (
		...
		and customer.id is not null
		and trim(customer.id) <> ''
        and to_date(concat(carfb.year, '-', LPAD(carfb.month, 2, '0'), '-', LPAD(carfb.day, 2, '0')),'yyyy-MM-dd') between {start_date} and {end_date}
        and arrays_overlap(map_keys(headers), array('Nord-Load','nord-load','nord-test','Nord-Test', 'Sretest', 'sretest')) = False
        and (
            array_contains(map_keys(headers), 'identified-bot') = False
            or
...
where click_spv_date between {start_date} and {end_date}
;
