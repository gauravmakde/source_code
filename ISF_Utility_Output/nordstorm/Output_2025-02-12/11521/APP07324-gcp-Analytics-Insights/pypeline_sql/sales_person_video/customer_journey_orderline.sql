...
delete 
from    {spv_t2_schema}.customer_journey_orderline
where   click_spv_date between {start_date} and {end_date}
;

insert into {spv_t2_schema}.customer_journey_orderline
select  
    shopper_id,
    web_style_num,
    click_spv_country,
...
from {spv_t2_schema}.customer_journey_orderline_landing
where click_spv_date between {start_date} and {end_date}
;
...
collect statistics column (shopper_id)
		...
		, column (ordernumber)
		, column (orderlineid)
on {spv_t2_schema}.customer_journey_orderline
;
