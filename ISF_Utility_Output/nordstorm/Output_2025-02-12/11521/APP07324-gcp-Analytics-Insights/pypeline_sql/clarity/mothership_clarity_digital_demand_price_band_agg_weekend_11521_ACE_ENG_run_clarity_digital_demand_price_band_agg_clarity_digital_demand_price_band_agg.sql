...
where eff_end_tmstp >= cast(current_date()-60 as date) and eff_begin_tmstp <= cast(current_date() as date)
...
where 1=1
and demand_date between current_date()-60 and current_date()
...
DELETE FROM PRD_NAP_DSA_AI_BASE_VWS.digital_demand_price_bands_agg WHERE tran_date BETWEEN current_date()-60 AND current_date();
