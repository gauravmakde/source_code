...
and year_num in (select distinct year_num from PRD_NAP_BASE_VWS.DAY_CAL where day_date between current_date()-30 and current_date())
)
...
where 1=1
and execution_qtr_end_dt >= current_date()-30 and execution_qtr_start_dt <= current_date()
...
where business_day_date between current_date()-30 and current_date()
...
where activity_date_pacific between current_date()-30-30 and current_date() --need to account for lag between order and sale (for opgmv and margin)
...
where activity_date between current_date()-30-30 and current_date()
...
DELETE FROM PRD_NAP_DSA_AI_BASE_VWS.finance_margin_fact WHERE business_day_date BETWEEN current_date()-30 AND current_date();
