...
SET QUERY_BAND = 'App_ID=APP08073;
     DAG_ID=inventory_position_master_table_11521_ACE_ENG;
     Task_Name=inventory_position_master;'
...
SELECT  g.week_num
       ...
       , 0 as regular_sku_ct --COUNT(distinct CASE WHEN p.current_price_type = 'R' THEN liveonsite.rms_sku_num else null end) regular_sku_ct
       ,0 as skuday_ct --COUNT(distinct liveonsite.rms_sku_num || CAST((CAST(liveonsite.live_date AS FORMAT 'YYYY-MM-DD'))                                           AS char(10))) skuday_ct
       ,0 as regular_skuday_ct--COUNT(distinct CASE WHEN p.current_price_type = 'R' THEN liveonsite.rms_sku_num || CAST((CAST(liveonsite.live_date AS FORMAT 'YYYY-MM-DD')) AS char(10)) else null end) regular_skuday_ct
       , 0 as clearance_skuday_ct --COUNT(distinct CASE WHEN p.current_price_type = 'C' THEN liveonsite.rms_sku_num || CAST((CAST(liveonsite.live_date AS FORMAT 'YYYY-MM-DD')) AS char(10)) else null end) clearance_skuday_ct
       ,0 as promotion_skuday_ct --COUNT(distinct CASE WHEN p.current_price_type = 'P' THEN liveonsite.rms_sku_num || CAST((CAST(liveonsite.live_date AS FORMAT 'YYYY-MM-DD')) AS char(10)) else null end) promotion_skuday_ct
       ,0 as regular_price_amt --MAX(regular_price_amt) regular_price_amt
       ,0 as current_price_amt --MIN(current_price_amt) current_price_amt
FROM (
	SELECT  *
	FROM liveonsite
	WHERE live_date BETWEEN ({start_date}) AND current_date
) liveonsite
INNER JOIN (
	SELECT  day_date
	       ,week_num
...
FROM (
	...
	WHERE ORDER_DATE_PACIFIC >= {start_date}-- OMS started supporting Ship order 100% after 2021-04-29
...
ON trim(cast(coalesce(p.destination_zip_code, p.bill_zip_code) AS varchar(10))) = dma.us_zip_code
...
CREATE multiset volatile TABLE digital AS (
	...
	FROM (
		...
		INNER JOIN (
			...
			FROM sku where web_style_num is not null
		) AS a
...
CREATE multiset volatile TABLE events AS (
	...
	left join T2DL_DAS_STORE_SALES_FORECASTING.loyalty_events_sales_shipping AS op -- includes future data for 1-2 quarters
	on op.event_date = cal.day_date
	left join PRD_NAP_USR_VWS.SCALED_EVENTS_DATES_DIM AS fp
	on fp.day_date = cal.day_date
	where cal.day_date >= ({start_date})
	group by 1
...
DELETE FROM {ip_forecast_t2_schema}.inventory_position_master_table
...
INSERT INTO {ip_forecast_t2_schema}.inventory_position_master_table
...
WHERE (a.week_start_date >= {start_date} AND week_start_date < {end_date});
...
on {ip_forecast_t2_schema}.inventory_position_master_table;
