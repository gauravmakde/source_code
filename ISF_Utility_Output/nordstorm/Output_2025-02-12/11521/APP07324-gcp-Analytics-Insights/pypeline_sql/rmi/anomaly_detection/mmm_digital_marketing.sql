SET QUERY_BAND = 'App_ID=APP09044;
     DAG_ID=mmm_digital_marketing_11521_ACE_ENG;
     Task_Name=mmm_digital_marketing;'
...
FROM (
        ...
        where stats_date between (select start_date from _variables) and (select end_date from _variables)
        and country <> 'CA'
...
WHERE last_updated_date IS NOT NULL
	...
	AND   a.last_updated_date between (select start_date from _variables) and (select end_date from _variables)
...
when sourcename in ('Nordstrom Post Boost','Nordstrom Social Boosting') then 'BRAND_SOCIAL'
...
case when (campaign_type = 'DESERIALIZATION_DEFAULT_VALUE' and push_message_name not like '%Track The Truck Store Arrival%') 
then 'app push marketing planned'
when (campaign_type = 'MARKETING_TRIGGERED' or push_message_name like '%Track The Truck Store Arrival%')  then 'app push marketing trigerred'
end as platform,
...
and communication_send_date between (select start_date from _variables) and (select end_date from _variables) -- data is correct and reliable starting this date
-- removing all test pushes except some with "_prod" and "<blank space >prod"
and (push_message_name not like '% test%' 
AND push_message_name not like '%\_test%' ESCAPE '\'
and push_message_name not like 'test%' or push_message_name like '%\_prod%' escape '\' or push_message_name like '% prod%')
...
DELETE FROM {kpi_scorecard_t2_schema}.mmm_digital_marketing 
where stats_date between (select start_date from _variables) and (select end_date from _variables)
;


INSERT INTO {kpi_scorecard_t2_schema}.mmm_digital_marketing
select
stats_date,
platform,
banner,
case when campaign_name like '%CTV%' then 'CTV'
when finance_detail = 'DISPLAY'  and campaign_name like '%Video%' then 'VIDEO' 
when finance_detail = 'VIDEO' and campaign_name like '%Display%' then 'DISPLAY' 
--when sourcename = 'Nordstrom Levers' then 'DISPLAY' 
--when sourcename = 'Nordstrom Rack DPA' then 'DISPLAY' 
else finance_detail end as finance_detail,
campaign_name,
campaign_id,
adgroup_name,
...
ad_id,
device_type,
case when finance_detail = 'BRAND_SOCIAL' then 'UP' else funnel_type end as funnel_type,
funding,bar,
nmn_flag,
beauty_flag,
video_flag,
...
when finance_detail in ('VIDEO','DISPLAY') AND campaign_name like '%4538%' then 'StoreDriving'
when finance_detail in ('VIDEO','DISPLAY') AND campaign_name like '%4445%' then 'StoreDriving'
when finance_detail in ('VIDEO','DISPLAY') AND campaign_name like '%4390%' then 'StoreDriving'
when finance_detail in ('VIDEO','DISPLAY') AND campaign_name like '%4380%' then 'StoreDriving'
when finance_detail in ('VIDEO','DISPLAY') AND campaign_name like '%4576%' then 'StoreDriving'

when campaign_name like '%Store-%' then 'StoreDriving'
when campaign_name like '%Store%' then 'StoreDriving'
--when campaign_name like '%CTV%' then 'CTV'
--when banner = 'R.COM' AND campaign_name like '%CTR%' then 'CTR'
else 'other'
end as category,
