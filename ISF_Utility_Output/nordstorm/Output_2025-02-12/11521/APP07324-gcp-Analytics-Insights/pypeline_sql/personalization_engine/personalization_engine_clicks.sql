...
create table if not exists {hive_schema}.perso_engine_clicks_v5(
...
using PARQUET
location 's3://{perso_s3_bucket_root_var}/perso_engine_clicks_v5'
...
select distinct 
        ...
        where 
        activity_date_partition between {start_date} and {end_date}
...
insert OVERWRITE TABLE {hive_schema}.perso_engine_clicks_v5 PARTITION (interaction_date)
select * from output;
