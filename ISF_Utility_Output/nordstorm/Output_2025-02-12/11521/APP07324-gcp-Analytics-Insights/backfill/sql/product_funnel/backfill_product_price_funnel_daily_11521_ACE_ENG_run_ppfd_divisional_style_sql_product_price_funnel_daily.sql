...
create multiset volatile table pfd_base AS (
  ...
  where event_date_pacific between cast('2022-01-01'  as date) and cast('2022–01-31' as date) and site_source = 'MODERN'
  ...
  where event_date_pacific between cast('2022-01-01'  as date) and cast('2022–01-31' as date)
...
create multiset volatile table pfd_los as (
    ...
    left join (
        ...
        where day_date between cast('2022-01-01'  as date) and cast('2022–01-31' as date)
    ) l
...
create multiset volatile table twist as (
    ...
    where business_unit_desc IN ('N.COM', 'OFFPRICE ONLINE', 'N.CA')
        and product_views > 0
        and day_date between cast('2022-01-01' as date) and cast('2022–01-31' as date)
    group by 1,2,3,4
...
delete T2DL_DAS_PRODUCT_FUNNEL.product_price_funnel_daily where event_date_pacific between '2022-01-01' and '2022–01-31' ;

insert into T2DL_DAS_PRODUCT_FUNNEL.product_price_funnel_daily
    select
...
COLLECT STATS ON T2DL_DAS_PRODUCT_FUNNEL.product_price_funnel_daily  COLUMN( event_date_pacific ,rms_sku_num);
COLLECT STATS ON T2DL_DAS_PRODUCT_FUNNEL.product_price_funnel_daily  COLUMN( event_date_pacific );
COLLECT STATS ON T2DL_DAS_PRODUCT_FUNNEL.product_price_funnel_daily  COLUMN( rms_sku_num);
COLLECT STATS ON T2DL_DAS_PRODUCT_FUNNEL.product_price_funnel_daily  COLUMN( channelcountry);
