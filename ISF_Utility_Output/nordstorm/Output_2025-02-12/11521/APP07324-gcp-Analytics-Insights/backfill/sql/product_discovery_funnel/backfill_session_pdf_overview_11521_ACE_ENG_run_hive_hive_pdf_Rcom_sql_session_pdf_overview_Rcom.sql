-- Table Name:session_pdf_overview_Rcom, session_pdf_browse_dive_Rcom
-- Team/Owner: Customer Analytics/Cassie Zhang
-- Date Created/Modified: created on 3/27/2024, last modified on 5/08/2024
-- What is the update cadence/lookback window: refresh daily and 1 days lookback window 
 
-- Drop TABLE IF EXISTS ace_etl.session_pdf_overview_Rcom;
create table if not exists ace_etl.session_pdf_overview_Rcom (
    fiscal_year_num int, 
    quarter_abrv string, 
    ...
    week_partition date
) using ORC location 's3://ace-etl/session_pdf_overview_Rcom' partitioned by (week_partition)
;
...
select
    date'2024-04-14' as start_date,
    date'2024-04-20' as end_date,
    date'2024-04-14' - INTERVAL '1' DAY * (dayofweek(date'2024-04-14') - 1) AS TY_start_week,
    date'2024-04-20' + INTERVAL '1' DAY * (7 - dayofweek(date'2024-04-20')) AS TY_end_week,
    date'2024-04-14' - INTERVAL '1' DAY * (dayofweek(date'2024-04-14') - 1) - interval '364' day AS LY_start_week,
    date'2024-04-20' + INTERVAL '1' DAY * (7 - dayofweek(date'2024-04-20')) - interval '364' day AS LY_end_week
;
...
from ace_etl.product_discovery_funnel_daily_Rcom a
where 
	(
...
select cast(c.fiscal_year_num as int) as fiscal_year_num,
		...
		discovery_source, source_group, source_detail, source_pagetype, 
...
insert OVERWRITE TABLE ace_etl.session_pdf_overview_Rcom PARTITION (week_partition) 
select *
from final_view_overview
;

MSCK REPAIR table ace_etl.session_pdf_overview_Rcom
;
...
-- Drop TABLE IF EXISTS ace_etl.session_pdf_browse_dive_Rcom;
create table if not exists ace_etl.session_pdf_browse_dive_Rcom (
    fiscal_year_num int, 
    quarter_abrv string, 
    fiscal_month_num int, 
    month_abrv string, 
    ...
    ly_ttl_ord_demand decimal(36, 6),
    week_partition date
) using PARQUET location 's3://ace-etl/session_pdf_browse_dive_Rcom' partitioned by (week_partition);
...
insert OVERWRITE TABLE ace_etl.session_pdf_browse_dive_Rcom PARTITION (week_partition) 
select *
from final_view_browse
;

MSCK REPAIR table ace_etl.session_pdf_browse_dive_Rcom
;
