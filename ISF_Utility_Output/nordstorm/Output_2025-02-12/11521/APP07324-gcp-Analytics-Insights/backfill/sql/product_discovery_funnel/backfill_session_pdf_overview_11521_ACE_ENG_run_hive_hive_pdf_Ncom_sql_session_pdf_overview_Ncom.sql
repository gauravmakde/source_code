...
-- Drop TABLE IF EXISTS ace_etl.session_pdf_overview_Ncom;
create table if not exists ace_etl.session_pdf_overview_Ncom (
    fiscal_year_num int, 
    quarter_abrv string, 
    ...
    week_partition date
) using ORC location 's3://ace-etl/session_pdf_overview_Ncom' partitioned by (week_partition)
;
...
insert OVERWRITE TABLE ace_etl.session_pdf_overview_Ncom PARTITION (week_partition) 
select *
from final_view_overview
;

MSCK REPAIR table ace_etl.session_pdf_overview_Ncom
;
...
-- Drop TABLE IF EXISTS ace_etl.session_pdf_browse_dive_Ncom;
create table if not exists ace_etl.session_pdf_browse_dive_Ncom (
    fiscal_year_num int, 
    quarter_abrv string, 
    fiscal_month_num int, 
    month_abrv string, 
    ...
    ly_ttl_ord_demand decimal(36, 6),
    week_partition date
) using PARQUET location 's3://ace-etl/session_pdf_browse_dive_Ncom' partitioned by (week_partition);
...
insert OVERWRITE TABLE ace_etl.session_pdf_browse_dive_Ncom PARTITION (week_partition) 
select *
from final_view_browse
;

MSCK REPAIR table ace_etl.session_pdf_browse_dive_Ncom
;
