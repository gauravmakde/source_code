...
FROM
    ...
    left join (
        ...
        where
            activity_date_partition between date'2023-02-07'
            and date'2023-02-09'
...
where
    current.activity_date_partition between date'2023-02-07'
    and date'2023-02-09'
    and page_indicator = 'BROWSE';
...
from
    ...
    left join (
        ...
        where
            ...
            and atb_flag = 1
            and activity_date_partition between date'2023-02-07'
            and date'2023-02-09'
        group by
            1,
            2
...
CREATE TABLE if not exists ace_etl.browse_session_analytical (
...
) using ORC location "s3://ace-etl/browse_session_analytical" partitioned BY (activity_date_partition);

msck repair table ace_etl.browse_session_analytical;

insert
    OVERWRITE TABLE ace_etl.browse_session_analytical PARTITION (activity_date_partition)
select
    session_page_rank_id,
    session_id,
    channel,
