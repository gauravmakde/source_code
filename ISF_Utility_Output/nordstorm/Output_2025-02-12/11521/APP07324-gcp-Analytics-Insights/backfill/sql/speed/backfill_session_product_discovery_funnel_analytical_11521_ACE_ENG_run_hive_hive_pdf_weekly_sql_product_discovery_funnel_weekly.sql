-- Table Name:product_discovery_funnel_weekly_Ncom
...
--DROP TABLE IF EXISTS ace_etl.product_discovery_funnel_weekly_Ncom;
create table if not exists ace_etl.product_discovery_funnel_weekly_Ncom (
	fiscal_year_num int, 
	quarter_abrv string, 
...
) using ORC location 's3://ace-etl/product_discovery_funnel_weekly_Ncom' partitioned by (week_start_day_date)
;
...
/* 
Create weekly table for Ncom
*/
create or replace temp view weekly_Ncom as 
...
(select fiscal_year_num, quarter_abrv, fiscal_month_num, month_abrv, week_num_of_fiscal_month, fiscal_week_num, week_start_day_date, 
	channel, platform, 
	brand, division, subdivision, department, class, merch_themes, quantrix_category,
	price_range,
	discovery_source,
...
from ace_etl.session_product_discovery_funnel_analytical a
join OBJECT_MODEL.DAY_CAL_454_DIM cal 
on a.activity_date_partition = date_format(to_date(cal.day_date, 'MM/dd/yyyy'),'yyyy-MM-dd')
where activity_date_partition between (date'2024-10-08' - (dayofweek(date'2024-10-08') -1)) and (date'2024-10-09' - (dayofweek(date'2024-10-09') -1) + 6)
...
insert OVERWRITE TABLE ace_etl.product_discovery_funnel_weekly_Ncom PARTITION (week_start_day_date) 
select *
from weekly_Ncom
;

MSCK REPAIR table ace_etl.product_discovery_funnel_weekly_Ncom
;
...
-- Table Name:product_discovery_funnel_weekly_Rcom
-- Team/Owner: Digital Product Analytics/Cassie Zhang
-- Date Created/Modified: created on 8/29/2023, last modified on 3/14/2023
-- What is the update cadence/lookback window: refresh daily and 4 days lookback window 

--DROP TABLE IF EXISTS ace_etl.product_discovery_funnel_weekly_Rcom;
create table if not exists ace_etl.product_discovery_funnel_weekly_Rcom (
	fiscal_year_num int, 
	quarter_abrv string, 
	fiscal_month_num int, 
	month_abrv string, 
	...
	ttl_ord_demand decimal(36, 6),
	week_start_day_date date
) using ORC location 's3://ace-etl/product_discovery_funnel_weekly_Rcom' partitioned by (week_start_day_date)
;
...
/* 
Create weekly table for Rcom
*/
create or replace temp view weekly_Rcom as 
...
insert OVERWRITE TABLE ace_etl.product_discovery_funnel_weekly_Rcom PARTITION (week_start_day_date) 
select *
from weekly_Rcom
;

MSCK REPAIR table ace_etl.product_discovery_funnel_weekly_Rcom
;
