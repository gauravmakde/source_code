...
--DROP TABLE IF EXISTS ace_etl.session_product_discovery_funnel_analytical;
create table if not exists ace_etl.session_product_discovery_funnel_analytical (
    channel string, 
    platform string, 
...
) using ORC location 's3://ace-etl/session_product_discovery_funnel_analytical' partitioned by (activity_date_partition)
;
...
where day_date between date'2024-10-08' and date'2024-10-09'
;
...
insert OVERWRITE TABLE ace_etl.session_product_discovery_funnel_analytical PARTITION (activity_date_partition) 
select *
from final_view
;

MSCK REPAIR table ace_etl.session_product_discovery_funnel_analytical
;
