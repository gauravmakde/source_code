...
--DROP TABLE IF EXISTS ace_etl.product_discovery_funnel_daily_Ncom;
create table if not exists ace_etl.product_discovery_funnel_daily_Ncom (
	fiscal_year_num int, 
	quarter_abrv string, 
...
) using ORC location 's3://ace-etl/product_discovery_funnel_daily_Ncom' partitioned by (activity_date_partition)
;
...
where activity_date_partition between date'2024-10-08' and date'2024-10-09'
...
insert OVERWRITE TABLE ace_etl.product_discovery_funnel_daily_Ncom PARTITION (activity_date_partition) 
select *
from daily_Ncom
;

MSCK REPAIR table ace_etl.product_discovery_funnel_daily_Ncom
;
...
--DROP TABLE IF EXISTS ace_etl.product_discovery_funnel_daily_Rcom;
create table if not exists ace_etl.product_discovery_funnel_daily_Rcom (
	fiscal_year_num int, 
	quarter_abrv string, 
	fiscal_month_num int, 
	month_abrv string, 
	...
	ttl_ord_demand decimal(36, 6),
	activity_date_partition date
) using ORC location 's3://ace-etl/product_discovery_funnel_daily_Rcom' partitioned by (activity_date_partition)
;
...
insert OVERWRITE TABLE ace_etl.product_discovery_funnel_daily_Rcom PARTITION (activity_date_partition) 
select *
from daily_Rcom
;

MSCK REPAIR table ace_etl.product_discovery_funnel_daily_Rcom
;