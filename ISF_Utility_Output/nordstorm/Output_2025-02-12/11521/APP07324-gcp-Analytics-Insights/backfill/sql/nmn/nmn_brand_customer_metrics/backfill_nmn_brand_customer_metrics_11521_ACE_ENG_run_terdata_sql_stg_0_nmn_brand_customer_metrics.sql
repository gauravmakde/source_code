...
FROM PRD_NAP_USR_VWS.DAY_CAL_454_DIM 
where month_idnt between (select month_start from date_mapping where day_date= date'2024-11-03') and (select month_end from date_mapping where day_date=date'2024-11-03')
...
DELETE from T2DL_DAS_NMN.nmn_brand_customer_metrics where month_idnt=(select month_idnt from _variables);
insert into T2DL_DAS_NMN.nmn_brand_customer_metrics select a.*, Current_Timestamp AS dw_sys_load_tmstp from final_insert a;
