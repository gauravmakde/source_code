...
CREATE MULTISET VOLATILE TABLE date_variables AS (
    ...
    ON a.day_date = date'2023-02-07' AND b.day_date = date'2023-09-07'
...
DELETE T2DL_DAS_NMN.BRAND_WEEKLY_FACT WHERE week_idnt BETWEEN (SELECT start_week_idnt FROM date_variables) AND (SELECT end_week_idnt FROM date_variables);

INSERT INTO T2DL_DAS_NMN.BRAND_WEEKLY_FACT
SELECT
	 channel_num
	,channel_country
	,week_idnt 
