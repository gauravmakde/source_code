...
SET QUERY_BAND = 'App_ID=app08742;
     ...
     Task_Name=leapfrog_session_active_user;'
...
T2/Table Name: leapfrog_session_active_user
Team/Owner: Jin Liu
Date Created/Modified: 2023-08-10
...
-- What is the update cadence/lookback window: daily between current - 5 and current -4
...
on T2DL_DAS_DIGENG.leapfrog_session_active_user_ldg
;

delete
from    T2DL_DAS_DIGENG.leapfrog_session_active_user
where    activity_date_partition between date'2024-01-20' and date'2024-03-01'
;

insert into T2DL_DAS_DIGENG.leapfrog_session_active_user
select  
...
from    T2DL_DAS_DIGENG.leapfrog_session_active_user_ldg
where   activity_date_partition between date'2024-01-20' and date'2024-03-01'
;
...
COLLECT STATISTICS  COLUMN (PARTITION),
                    COLUMN (activity_date_partition,channel,experience,recognized_in_period,new_recognized_in_period,new_in_period, holdout_experimentname, holdout_variationname), -- column names used for primary index
                    COLUMN (activity_date_partition)  -- column names used for partition
on T2DL_DAS_DIGENG.leapfrog_session_active_user
;
