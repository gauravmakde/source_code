...
Note:
This table contains all online sales data based on Merch in-season team's standard by EPM_CHOICE_NUM X WEEK 

*/
DELETE FROM T2DL_DAS_INV_POSITION_FORECAST.IDF_ONLINE_SALE_MERCH
WHERE (week_start_date >= '2021-06-13' AND week_start_date <= '2021-06-27');


INSERT INTO T2DL_DAS_INV_POSITION_FORECAST.IDF_ONLINE_SALE_MERCH
with wk as (
    SELECT distinct
...
store as (
   SELECT DISTINCT
    ...
    WHERE 1=1
      ...
      AND channel_num IN ('120','250') --120 is N.COM, 250 is R.COM
)
   SELECT DISTINCT
        store.channel_brand
...
on T2DL_DAS_INV_POSITION_FORECAST.IDF_ONLINE_SALE_MERCH;
