...
CREATE  MULTISET VOLATILE TABLE styleboards AS (
	...
	WHERE CAST(BOARD_CREATED_PST AS DATE ) BETWEEN current_date()-999 AND current_date()-800
) WITH DATA PRIMARY INDEX (board_created_pst) ON COMMIT PRESERVE ROWS;
...
FROM
    ...
    AND f.order_date_pacific >= current_date()-999
    ) a
...
WHERE board_created_date >= current_date()-999
AND board_created_date <= current_date()-800;
