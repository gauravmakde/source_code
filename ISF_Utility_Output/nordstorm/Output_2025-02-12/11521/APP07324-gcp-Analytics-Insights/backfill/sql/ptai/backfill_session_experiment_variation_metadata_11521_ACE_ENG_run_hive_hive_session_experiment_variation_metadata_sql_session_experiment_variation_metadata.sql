create table if not exists ace_etl.session_experiment_variation_metadata (
    channel string,
...
using PARQUET
location "s3://ace-etl/session_experiment_variation_metadata"
partitioned by (experimentname);

MSCK REPAIR table ace_etl.session_experiment_variation_metadata;
...
where activity_date_pst between date'2024-02-17' and date'2024-04-08'
;
...
insert
    OVERWRITE TABLE ace_etl.session_experiment_variation_metadata PARTITION (experimentname)
select
    *
from
    metadata;