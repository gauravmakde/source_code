create table if not exists ace_etl.session_experiment_variation_exposed (
	cust_id string,
	bucketingid string,
	acp_id string,
	session_id string,
...
using PARQUET
location "s3://ace-etl/session_experiment_variation_exposed"
partitioned by (activity_date_pst,experimentname);

MSCK REPAIR table ace_etl.session_experiment_variation_exposed;
...
from
	...
	from acp_event_landing.customer_experiment_variation_exposed
	where
	    to_date(
        concat(
            year,
        ...
        'yyyy-MM-dd'
    ) between date'2024-03-22' -1 and date'2024-03-22' +1
	...
	and value.projectname in ('FULL_LINE_MOBILE', 'FULL_LINE_BACKEND_SERVICE', 'IOS_APP', 'FULL_LINE_DESKTOP', 'ANDROID_APP', 'DESKTOP')
...
from
	...
	where activity_date_partition between date'2024-03-22' and date'2024-03-22'
...
where activity_date_pst between date'2024-03-22' and date'2024-03-22'
;
...
insert
    OVERWRITE TABLE ace_etl.session_experiment_variation_exposed PARTITION (activity_date_pst,experimentname)
select
	/*+ REPARTITION(100) */
    *
from
...
create table if not exists ace_etl.session_experiment_variation_orders (
	session_id string,
	channel string, 
	channelcountry string,
	experience string, 
...
using PARQUET
location "s3://ace-etl/session_experiment_variation_orders"
partitioned by (activity_date_pst);

MSCK REPAIR table ace_etl.session_experiment_variation_orders;
...
where sess.activity_date_partition between date'2024-03-22' and date'2024-03-22'
...
insert
    OVERWRITE TABLE ace_etl.session_experiment_variation_orders PARTITION (activity_date_pst)
select
	/*+ REPARTITION(100) */
    *
from
