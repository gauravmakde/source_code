...
create table if not exists ace_etl.analytical_session_fact
(
...
using ORC
location 's3://ace-etl/analytical_session_fact/'
...
WHERE activity_date BETWEEN '2024-05-01' and '2024-05-31'
);
...
left join (select activity_date_partition, session_id from acp_event_intermediate.session_fact_attributes_parquet where activity_date_partition BETWEEN '2024-05-01' and '2024-05-31') c
on a.session_id = c.session_id
and a.activity_date_pacific = c.activity_date_partition
left join (select * from acp_event_intermediate.customer_digital_session_bot_fact_parquet where activity_date_partition BETWEEN '2024-05-01' and '2024-05-31') b
on a.activity_date_pacific = b.activity_date_partition
and a.session_id = b.session_id;


-- replace records from the days that the job is running
insert overwrite table ace_etl.analytical_session_fact partition (activity_date_pacific)
select /*+ REPARTITION(100) */ * from final_session_table_with_tstamp;

MSCK REPAIR TABLE ace_etl.analytical_session_fact;
