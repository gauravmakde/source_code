...
--T2/Table Name: T2DL_DAS_SESSIONS.customer_session_data
--Team/Owner: MOA
...
WHERE week_idnt = (select distinct week_idnt from PRD_NAP_USR_VWS.DAY_CAL_454_DIM where day_date=date'2022-12-25'))
...
FROM PRD_NAP_USR_VWS.DAY_CAL_454_DIM
WHERE week_idnt = (select distinct week_idnt from PRD_NAP_USR_VWS.DAY_CAL_454_DIM where day_date=date'2024-02-17'))
...
DELETE 
FROM    T2DL_DAS_SESSIONS.customer_session_data
where day_dt between (select start_date from _variables) and (select end_date from _variables);

INSERT INTO T2DL_DAS_SESSIONS.customer_session_data
