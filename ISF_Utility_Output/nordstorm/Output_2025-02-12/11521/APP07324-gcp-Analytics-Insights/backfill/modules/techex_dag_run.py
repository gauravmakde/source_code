...
from dag_run where date(execution_date) BETWEEN current_date-7 and current_date;
...
def get_stats_csv():
    ...
    try:
        ...
        s3_inst.load_file(filename='dr_techex.csv', key='tpt_load/techex_dag_run/result.csv', bucket_name=bucket, replace=True)
...
def callable_func_name(*arg, **kwargs):
    get_stats_csv()
    if 1 == 1:  
        return "tpt_jdbc_insert_tpt_stage_tpt_to_techex_ldg_control_tbl"
    else:
        return "tpt_jdbc_insert_tpt_stage_tpt_to_techex_ldg_control_tbl"