...
SET QUERY_BAND = '
App_ID=app06788;
Task_Name=credit_application_referred_kafka_to_teradata_stg_job;'
-- noqa: disable=all
FOR SESSION VOLATILE;
-- noqa: enable=all

-- Read Kafka into temp table
CREATE TEMPORARY VIEW temp_credit_application_referred
AS
SELECT * FROM kafka_credit_application_referred;

CREATE TEMPORARY VIEW credit_application_referred_temp
AS
SELECT
    eventtime AS event_time,
    applicationid AS application_id,
    'Referred' AS application_decision_status,
...
FROM temp_credit_application_referred;

INSERT INTO TABLE credit_application_referred_ldg
SELECT
    event_time,
    application_id,
    ...
    customer_id_type,
    client_reference_id
FROM credit_application_referred_temp
WHERE
