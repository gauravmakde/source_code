...
SET QUERY_BAND = 'App_ID=app06788; DAG_ID=credit_support_contacted_teradata; Task_Name=teradata_stg_to_teradata_fct_job;' -- noqa
FOR SESSION VOLATILE;

END TRANSACTION;

CREATE MULTISET VOLATILE TABLE CREDIT_SUPPORT_CONTACTED_LDG_TEMP AS (
    SELECT *
    FROM {db_env}_NAP_CREDIT_STG.CREDIT_SUPPORT_CONTACTED_LDG
    QUALIFY
        row_number() OVER (
...
-- Delete duplicates
DELETE FROM {db_env}_NAP_CREDIT_BASE_VWS.CREDIT_SUPPORT_CONTACTED_FACT
WHERE
    CREDIT_CUSTOMER_SUPPORT_CONTACTED_ID IN
    (SELECT CREDIT_CUSTOMER_SUPPORT_CONTACTED_ID FROM CREDIT_SUPPORT_CONTACTED_LDG_TEMP);

INSERT INTO {db_env}_NAP_CREDIT_BASE_VWS.CREDIT_SUPPORT_CONTACTED_FACT (
    EVENT_TIME,
    EVENT_DATE,
    CHANNEL_COUNTRY,
    CHANNEL_BRAND,
...
DELETE FROM {db_env}_NAP_CREDIT_STG.CREDIT_SUPPORT_CONTACTED_LDG;
