...
CREATE TEMPORARY VIEW temp_kafka_payment_refund_failed_v2 AS
SELECT * FROM kafka_payment_refund_failed_v2;

-- building the refund hdr table
--reading the refund failed hdr fields
CREATE TEMPORARY VIEW refund_failed_hdr_temp AS
SELECT
...
FROM temp_kafka_payment_refund_failed_v2;

--inserting into the teradata hdr stg table
INSERT INTO TABLE payment_refund_failed_hdr_stg
SELECT DISTINCT
    event_id,
    event_type,
    event_time,
...
--reading the refund failed tender - bankCardRefundResultV2s
CREATE TEMPORARY VIEW refund_failed_bankCardRefundV2 AS
WITH refund_failed_bankCardRefundV2_exploded AS (
    SELECT
        ELEMENT_AT(headers, 'Id') AS event_id,
        ELEMENT_AT(headers, 'Type') AS event_type,
        ELEMENT_AT(TRANSFORM_KEYS(headers, (k, v) -> LOWER(k)), 'nord-test') AS nord_test,
...
FROM refund_failed_bankCardRefundV2_exploded;

--reading the refund failed tender - afterPayVirtualCardRefundResults
CREATE TEMPORARY VIEW refund_failed_afterPayVirtualCard AS
WITH refund_failed_afterPayVirtualCard_exploded AS (
    SELECT
        ELEMENT_AT(headers, 'Id') AS event_id,
        ELEMENT_AT(headers, 'Type') AS event_type,
        ELEMENT_AT(TRANSFORM_KEYS(headers, (k, v) -> LOWER(k)), 'nord-test') AS nord_test,
...
SELECT
    ...
    NULL AS paypal_tndr_rslt_payer_account_country,
    NULL AS note_refund_reason
FROM refund_failed_afterPayVirtualCard_exploded;

--reading the refund failed tender - giftCardNoteIssuedResults
CREATE TEMPORARY VIEW refund_failed_giftCardNoteIssued AS
WITH refund_failed_giftCardNoteIssued_exploded AS (
    SELECT
        ELEMENT_AT(headers, 'Id') AS event_id,
        ELEMENT_AT(headers, 'Type') AS event_type,
        ELEMENT_AT(TRANSFORM_KEYS(headers, (k, v) -> LOWER(k)), 'nord-test') AS nord_test,
...
SELECT
    ...
    NULL AS paypal_tndr_rslt_payer_account_country,
    NULL AS note_refund_reason
FROM refund_failed_giftCardNoteIssued_exploded;

--reading the refund failed tender - payPalRefundResults
CREATE TEMPORARY VIEW refund_failed_payPalRefund AS
WITH refund_failed_payPalRefund_exploded AS (
    SELECT
        ELEMENT_AT(headers, 'Id') AS event_id,
        ELEMENT_AT(headers, 'Type') AS event_type,
        ELEMENT_AT(TRANSFORM_KEYS(headers, (k, v) -> LOWER(k)), 'nord-test') AS nord_test,
...
SELECT
    ...
    NULL AS paypal_tndr_rslt_payer_account_country,
    NULL AS note_refund_reason
FROM refund_failed_payPalRefund_exploded;

--reading the refund failed tender - payPalBillingAgreementRefundResults
CREATE TEMPORARY VIEW refund_failed_payPalBillingAgreement AS
WITH refund_failed_payPalBillingAgreement_exploded AS (
    SELECT
        ELEMENT_AT(headers, 'Id') AS event_id,
        ELEMENT_AT(headers, 'Type') AS event_type,
        ELEMENT_AT(TRANSFORM_KEYS(headers, (k, v) -> LOWER(k)), 'nord-test') AS nord_test,
...
SELECT
    ...
    NULL AS note_refund_reason
FROM refund_failed_payPalBillingAgreement_exploded;

--reading the refund failed tender - nordstromNoteRefundedResults
CREATE TEMPORARY VIEW refund_failed_nordstromNoteRefunded AS
WITH refund_failed_nordstromNoteRefunded_exploded AS (
    SELECT
        ELEMENT_AT(headers, 'Id') AS event_id,
        ELEMENT_AT(headers, 'Type') AS event_type,
        ELEMENT_AT(TRANSFORM_KEYS(headers, (k, v) -> LOWER(k)), 'nord-test') AS nord_test,
...
FROM refund_failed_nordstromNoteRefunded_exploded;

--unioning all the failed tender types
CREATE TEMPORARY VIEW refund_failed_tender_union
AS
SELECT DISTINCT * FROM refund_failed_bankCardRefundV2
UNION
SELECT DISTINCT * FROM refund_failed_afterPayVirtualCard
UNION
SELECT DISTINCT * FROM refund_failed_giftCardNoteIssued
UNION
SELECT DISTINCT * FROM refund_failed_payPalRefund
UNION
SELECT DISTINCT * FROM refund_failed_payPalBillingAgreement
UNION
SELECT DISTINCT * FROM refund_failed_nordstromNoteRefunded;

--inserting into the teradata tender stg table
INSERT INTO TABLE payment_refund_failed_tender_stg
SELECT DISTINCT
    event_id,
    event_type,
    event_time,
    ...
    note_refund_reason
FROM refund_failed_tender_union
WHERE
    -- Header filtering based on environment
    (
        ('{db_env}' = 'PRD' AND nord_test IS NULL AND nord_load IS NULL)
