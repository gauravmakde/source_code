...
CREATE MULTISET VOLATILE TABLE CARD_TO_WALLET_THRESHOLD_VIOLATED_TEMP AS (
    SELECT *
    FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.CARD_TO_WALLET_THRESHOLD_VIOLATED_LDG
    QUALIFY
        RANK() OVER (
...
DELETE FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.CARD_TO_WALLET_THRESHOLD_VIOLATED_FACT
WHERE CUSTOMER_ID IN (SELECT DISTINCT CUSTOMER_ID FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.CARD_TO_WALLET_THRESHOLD_VIOLATED_LDG);

INSERT INTO {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.CARD_TO_WALLET_THRESHOLD_VIOLATED_FACT
(
    EVENT_TIME,
    EVENT_DATE,
    CUSTOMER_ID,
...
DELETE FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.CARD_TO_WALLET_THRESHOLD_VIOLATED_LDG;
