...
CREATE MULTISET VOLATILE TABLE FRAUD_DECISION_LIFECYCLE_LDG_TEMP AS (
    SELECT DISTINCT *
    FROM {db_env}_NAP_PYMNT_FRAUD_STG.FRAUD_DECISION_LIFECYCLE_LDG
    QUALIFY
        row_number() OVER (
            PARTITION BY
                EVENT_ID, ORDER_ID
...
------------------------------------------------------------------------------------------------------------------------
DELETE FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.FRAUD_DECISION_LIFECYCLE_FACT
WHERE (
    EVENT_ID,
    ORDER_ID
    )
...
------------------------------------------------------------------------------------------------------------------------
INSERT INTO {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.FRAUD_DECISION_LIFECYCLE_FACT (
    ORDER_ID,
    ORDER_LINE_ID,
    AUTHORITY,
    FRAUD_DECISION,
...
------------------------------------------------------------------------------------------------------------------------
DELETE FROM {db_env}_NAP_PYMNT_FRAUD_STG.FRAUD_DECISION_LIFECYCLE_LDG;
------------------------------------------------------------------------------------------------------------------------
