...
CREATE MULTISET VOLATILE TABLE PAYMENT_SETTLEMENT_TENDER_TEMP AS (
    SELECT DISTINCT *
    FROM {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_INTENT_TENDER_V2_LDG
    QUALIFY
        RANK() OVER (
...
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_INTENT_TENDER_V2_FACT
WHERE TRANSACTION_ID IN (SELECT DISTINCT TRANSACTION_ID FROM {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_INTENT_TENDER_V2_LDG);

INSERT INTO {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_INTENT_TENDER_V2_FACT
(
  EVENT_TIME,
  ACTIVITY_DATE,
  TENDER_RECORD_NAME,
...
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_INTENT_TENDER_V2_LDG;
