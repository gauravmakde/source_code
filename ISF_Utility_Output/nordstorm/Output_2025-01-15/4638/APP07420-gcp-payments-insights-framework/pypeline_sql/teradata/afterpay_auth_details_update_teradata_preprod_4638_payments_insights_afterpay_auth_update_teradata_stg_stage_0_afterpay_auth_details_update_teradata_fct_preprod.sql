...
CREATE MULTISET VOLATILE TABLE TEMP_PAYMENT_AUTHORIZATION_RESULT_DETAILS_STG AS (
    ...
    FROM PREPROD_NAP_BASE_VWS.PAYMENT_AUTHORIZATION_RESULT_FACT AS PAR
    INNER JOIN PREPROD_NAP_BASE_VWS.PAYMENT_BANK_CARD_AUTHORIZATION_RESULT_FACT AS PBC
    ON PAR.AUTHORIZATION_RESULT_ID = PBC.AUTHORIZATION_RESULT_ID
    WHERE PAR.DW_SYS_UPDT_TMSTP > (SELECT coalesce(max(cast(DW_SYS_UPDT_TMSTP AS DATE)) - 2, cast('2023-11-01' AS DATE)) FROM PREPROD_NAP_BASE_VWS.PAYMENT_AUTHORIZATION_RESULT_DETAILS_FACT)
...
--Delete if exist
DELETE FROM PREPROD_NAP_BASE_VWS.PAYMENT_AUTHORIZATION_RESULT_DETAILS_FACT
WHERE
    AUTHORIZATION_RESULT_ID IN (SELECT AUTHORIZATION_RESULT_ID FROM TEMP_PAYMENT_AUTHORIZATION_RESULT_DETAILS_STG);

-- Insert into FACT table
INSERT INTO PREPROD_NAP_BASE_VWS.PAYMENT_AUTHORIZATION_RESULT_DETAILS_FACT (
    DW_SYS_LOAD_TMSTP,
    DW_SYS_UPDT_TMSTP,
    CHANNEL_COUNTRY,
    CHANNEL_BRAND,
