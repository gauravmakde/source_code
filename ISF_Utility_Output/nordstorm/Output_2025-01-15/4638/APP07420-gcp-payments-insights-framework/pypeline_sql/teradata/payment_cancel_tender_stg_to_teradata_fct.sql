...
CREATE MULTISET VOLATILE TABLE PAYMENT_CANCEL_TENDER_TEMP AS (
    SELECT *
    FROM {db_env}_NAP_BASE_VWS.PAYMENT_CANCEL_TENDER_LDG
    QUALIFY
        RANK() OVER (
...
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_CANCEL_TENDER_FACT
WHERE EVENT_ID IN (SELECT DISTINCT EVENT_ID FROM {db_env}_NAP_BASE_VWS.PAYMENT_CANCEL_TENDER_LDG);

INSERT INTO {db_env}_NAP_BASE_VWS.PAYMENT_CANCEL_TENDER_FACT
(
    EVENT_ID,
    ORDER_NUMBER,
    EVENT_TIME,
...
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_CANCEL_TENDER_LDG;
