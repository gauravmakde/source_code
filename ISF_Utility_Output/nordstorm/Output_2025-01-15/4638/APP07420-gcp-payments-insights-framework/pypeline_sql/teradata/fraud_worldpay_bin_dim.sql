...
DELETE FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.WORLDPAY_BIN_LDG
...
CREATE MULTISET VOLATILE TABLE WORLDPAY_BIN_LDG_TMP AS (
    ...
    FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.WORLDPAY_BIN_LDG
...
WHERE (
   ...
   IN (
    SELECT DISTINCT
       ...
       RELOAD_FLAG,
       CARDHOLDER_FUNDS_TRANSFER_FLAG
     FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.WORLDPAY_BIN_DIM
);

INSERT INTO {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.WORLDPAY_BIN_DIM
(
    BIN_LENGTH,
    PAN_LENGTH,
    BIN,
...
WHERE BIN_SIX IN (SELECT BIN_SIX FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.WORLDPAY_BIN_DIM WHERE RECENTLY_UPDATED = 'Y');
