...
CREATE MULTISET VOLATILE TABLE PAYMENT_AUTHORIZATION_TENDER_V2_LDG_TEMP AS (
    SELECT DISTINCT *
    FROM {db_env}_NAP_STG.PAYMENT_AUTHORIZATION_TENDER_V2_LDG
    QUALIFY
        row_number() OVER (
            PARTITION BY
...
------------------------------------------------------------------------------------------------------------------------
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_AUTHORIZATION_TENDER_V2_FACT
WHERE (
    EVENT_ID,
...
------------------------------------------------------------------------------------------------------------------------
INSERT INTO {db_env}_NAP_BASE_VWS.PAYMENT_AUTHORIZATION_TENDER_V2_FACT (
    DW_BATCH_DATE,
    DW_SYS_LOAD_TMSTP,
    DW_SYS_UPDT_TMSTP,
...
------------------------------------------------------------------------------------------------------------------------
DELETE FROM {db_env}_NAP_STG.PAYMENT_AUTHORIZATION_TENDER_V2_LDG;
------------------------------------------------------------------------------------------------------------------------
