-- noqa: disable=all
CREATE TABLE IF NOT EXISTS {delta_schema_name}.customer_activity_searchperformed_session_delta
(
  sessionId string,
...
LOCATION 's3://{delta_s3_bucket}/session_event_delta/customer_activity_searchperformed_session_delta/';
-- noqa: enable=all

CREATE TEMPORARY VIEW customer_activity_searchperformed_dedupe AS
...
SELECT
    ...
    as rownumber
FROM {event_parquet_schema_name}.customer_activity_searchperformed_parquet
...
MERGE INTO {delta_schema_name}.customer_activity_searchperformed_session_delta AS TGT
USING (
...
CREATE TEMPORARY VIEW customer_activity_searchperformed_session_delta_temp AS
WITH searchperformed_session AS (
    select *
    from {delta_schema_name}.session_analytical_object_flatten_delta
    where session_event_name = 'com.nordstrom.customer.SearchPerformed'
),

searchperformed_event_dedupe AS (
    select *
    from customer_activity_searchperformed_dedupe
    where rownumber = 1
)
...
from searchperformed_session AS a
left join searchperformed_event_dedupe AS b
    on
        a.session_event_id = b.eventId;

INSERT INTO TABLE {delta_schema_name}.customer_activity_searchperformed_session_delta PARTITION (activity_date)
SELECT
    sessionId,
    ...
    rcd_load_tmstp,
    rcd_update_tmstp
from customer_activity_searchperformed_session_delta_temp;

VACUUM {delta_schema_name}.customer_activity_searchperformed_session_delta;

-- noqa: disable=all
OPTIMIZE {delta_schema_name}.customer_activity_searchperformed_session_delta ZORDER BY (sessionid);
-- noqa: enable=all
