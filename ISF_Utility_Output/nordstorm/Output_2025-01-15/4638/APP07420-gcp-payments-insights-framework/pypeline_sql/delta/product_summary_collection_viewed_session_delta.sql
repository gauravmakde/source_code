-- noqa: disable=all
CREATE TABLE IF NOT EXISTS {delta_schema_name}.customer_activity_product_summary_collection_viewed_session_delta
(
  sessionId string,
...
LOCATION 's3://{delta_s3_bucket}/session_event_delta/customer_activity_product_summary_collection_viewed_session_delta/';
-- noqa: enable=all

CREATE TEMPORARY VIEW customer_activity_product_summary_collection_viewed_dedupe AS
WITH session_dates AS (
    ...
    where
        year = 2023 and month = 06 and day between 02 and 04
)
...
SELECT
    ...
    value.productSummaryCollection as productSummaryCollection,
    ...
    as rownumber
FROM {event_parquet_schema_name}.customer_activity_product_summary_collection_viewed_parquet
...
MERGE INTO {delta_schema_name}.customer_activity_product_summary_collection_viewed_session_delta AS TGT
USING (
...
CREATE TEMPORARY VIEW customer_activity_product_summary_collection_viewed_session_delta_temp AS
WITH product_summary_collection_viewed_session AS (
    select *
    from {delta_schema_name}.session_analytical_object_flatten_delta
    where
        session_event_name = 'com.nordstrom.event.customer.ProductSummaryCollectionViewed'
        and year = 2023 and month = 06 and day between 02 and 04
),

product_summary_collection_viewed_event_dedupe AS (
    select *
    from customer_activity_product_summary_collection_viewed_dedupe
    where rownumber = 1
)
...
from product_summary_collection_viewed_session AS a
left join product_summary_collection_viewed_event_dedupe AS b
    on
        a.session_event_id = b.eventId;

INSERT INTO TABLE {delta_schema_name}.customer_activity_product_summary_collection_viewed_session_delta PARTITION (activity_date)
SELECT
    sessionId,
    ...
    rcd_load_tmstp,
    rcd_update_tmstp
from customer_activity_product_summary_collection_viewed_session_delta_temp;

VACUUM {delta_schema_name}.customer_activity_product_summary_collection_viewed_session_delta;

-- noqa: disable=all
OPTIMIZE {delta_schema_name}.customer_activity_product_summary_collection_viewed_session_delta ZORDER BY (sessionid);
-- noqa: enable=all
