-- noqa: disable=all
CREATE TABLE IF NOT EXISTS {delta_schema_name}.customer_activity_removed_from_bag_session_delta
(
  sessionId string,
  ...
  credentials struct<iconid:string,shopperid:string,tokenizingcreditcardhvt:struct<value:string,authority:string,strategy:string,dataclassification:string>,creditcardhvt:struct<value:string,authority:string,dataclassification:string>,fraudtoken:string,useragent:string,tokenizingipaddress:struct<value:string,authority:string,strategy:string,dataclassification:string>,ipaddress:struct<value:string,authority:string,dataclassification:string>,experimentid:string,advertisingid:string,applevendorid:string,firebaseinstanceid:string>,
...
LOCATION 's3://{delta_s3_bucket}/session_event_delta/customer_activity_removed_from_bag_session_delta/';
-- noqa: enable=all

CREATE TEMPORARY VIEW customer_activity_removed_from_bag_dedupe AS
...
SELECT
    ...
    value.remainingquantity as remainingQuantity,
    ...
    as rownumber
FROM {event_parquet_schema_name}.customer_activity_removed_from_bag_parquet
...
MERGE INTO {delta_schema_name}.customer_activity_removed_from_bag_session_delta AS TGT
USING (
...
CREATE TEMPORARY VIEW customer_activity_removed_from_bag_session_delta_temp AS
WITH removed_from_bag_session AS (
    select *
    from {delta_schema_name}.session_analytical_object_flatten_delta
    where session_event_name = 'com.nordstrom.customer.RemovedFromBag'
),

removed_from_bag_event_dedupe AS (
    select *
    from customer_activity_removed_from_bag_dedupe
    where rownumber = 1
)
...
from removed_from_bag_session AS a
left join removed_from_bag_event_dedupe AS b
    on
        a.session_event_id = b.eventId;

INSERT INTO TABLE {delta_schema_name}.customer_activity_removed_from_bag_session_delta PARTITION (activity_date)
SELECT
    sessionId,
    ...
    rcd_load_tmstp,
    rcd_update_tmstp
from customer_activity_removed_from_bag_session_delta_temp;

VACUUM {delta_schema_name}.customer_activity_removed_from_bag_session_delta;

-- noqa: disable=all
OPTIMIZE {delta_schema_name}.customer_activity_removed_from_bag_session_delta ZORDER BY (sessionid);
-- noqa: enable=all
