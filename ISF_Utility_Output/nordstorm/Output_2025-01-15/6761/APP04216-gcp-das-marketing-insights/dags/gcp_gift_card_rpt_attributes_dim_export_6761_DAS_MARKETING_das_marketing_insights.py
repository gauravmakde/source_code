...
dag_id = 'gcp_gift_card_rpt_attributes_dim_export_6761_DAS_MARKETING_das_marketing_insights'
...
default_args = {
    ...
    'description': 'gift_card_rpt_attributes_dim_export_6761_DAS_MARKETING_das_marketing_insights DAG Description',
...
with DAG(
    ...
    launch_gc_rpt_attr_exp_s3_delete = launch_k8s_api_job_operator(
    dag=dag,
    connection_id='k8s_connection',
    namespace='app04216',
    task_id='gc_rpt_attr_exp_s3_delete',
    job_name='gc-rpt-attr-exp-s3-delete-gift-card-rpt-attributes-dim-export-6',
    macros=None,
    container_image='artifactory.nordstrom.com/docker/app04216/tm00989-app04216-das-marketing-insights/prod/python/utils:1.3.19',
    container_command=['/home/nonroot/docker-entrypoint.sh', 'python/utils/src/s3_delete.py', '--s3_source_bucket_env_name', 'S3_MKTG_BUCKET', '--s3_source_prefix_format', 'data/gift_card_static_export/stg/gift_card_rpt_attributes/'],
...
gift_card_rpt_attributes_dim_export_6761_DAS_MARKETING_das_marketing_insights_gc_rpt_attr_exp_collect_gc_data_for_tpt_export = open(
    os.path.join(sql_path, 'gift_card_rpt_attributes_dim_export_6761_DAS_MARKETING_das_marketing_insights_gc_rpt_attr_exp_collect_gc_data_for_tpt_export.sql'),
    "r").read()

    launch_gc_rpt_attr_exp_teradata_job = BigQueryInsertJobOperator(
        task_id = "gc_rpt_attr_exp_teradata_job",
        configuration = 
        {
            "query": 
            {
                "query": gift_card_rpt_attributes_dim_export_6761_DAS_MARKETING_das_marketing_insights_gc_rpt_attr_exp_collect_gc_data_for_tpt_export,
                "useLegacySql": False
            }
        },
...
tpt_jdbc_export_gc_rpt_attr_exp_tpt_export_control_tbl = BashOperator(
    task_id='tpt_jdbc_export_gc_rpt_attr_exp_tpt_export_control_tbl',
    bash_command=f'echo "Skipping Control Table Entry"')

    tpt_gc_rpt_attr_exp_tpt_export_export_tbl = SSHOperator(
    task_id='tpt_gc_rpt_attr_exp_tpt_export_export_tbl',
    command="/db/teradata/bin/tpt_export.sh  -e {database_env}_NAP_UTL -s s3://tf-nap-prod-mkt-etl/apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/6761/DAS_MARKETING/das_marketing_insights/sql/gift_card_rpt_attributes_dim_export_6761_DAS_MARKETING_das_marketing_insights_gc_rpt_attr_exp_gift_card_rpt_attributes_dim_export.sql -h tdnapprod.nordstrom.net -l 'N' -u SCH_NAP_MKTG_BATCH -p \"\$tdwallet(SCH_NAP_MKTG_BATCH_PWD)\"  -t s3://tf-das-marketing-prod-teradata-stage/data/gift_card_static_export/stg/gift_card_rpt_attributes/gift_card_rpt_export_{year}_{month}_{day}.csv -a N -d , -c 1 -q Y -f \\\" -j {database_env}_NAP_BASE_VWS_GIFT_CARD_RPT_ATTRIBUTES_DIM_JOB".format(database_env=eval('{"nonprod": "DEV","test": "DEV", "prod": "PRD"}[env]'),year=eval('str(datetime.now().strftime("%Y"))'),month=eval('str(datetime.now().strftime("%m"))'),day=eval('str(datetime.now().strftime("%d"))')),
    timeout=1800,
    ssh_conn_id="teradata_ssh_connection_to_ec2_isf")

    launch_gc_rpt_attr_exp_s3_copy = launch_k8s_api_job_operator(
    dag=dag,
    connection_id='k8s_connection',
    namespace='app04216',
    task_id='gc_rpt_attr_exp_s3_copy',
    job_name='gc-rpt-attr-exp-s3-copy-gift-card-rpt-attributes-dim-export-676',
    macros=None,
    ...
    container_command=['/home/nonroot/docker-entrypoint.sh', 'python/utils/src/s3_copy.py', '--s3_source_bucket_env_name', 'S3_MKTG_BUCKET', '--s3_target_bucket_env_name', 'S3_MKTG_BUCKET', '--s3_source_prefix_format', 'data/gift_card_static_export/stg/gift_card_rpt_attributes/', '--s3_target_prefix_format', 'data/gift_card_static_export/hist/gift_card_rpt_attributes/year=<year>/month=<month>/day=<day>/'],
    resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    common_envs=[],
    task_envs=[{'name': 'S3_MKTG_BUCKET', 'value': 'tf-das-marketing-prod-teradata-stage'}, {'name': 'S3_MMM_BUCKET', 'value': 'prod-mmm-marketing-mix-model'}, {'name': 'S3_OPTIMINE_BUCKET', 'value': 'prod-mft-optimine'}, {'name': 'S3_ICON_LOUNGE_BUCKET', 'value': 'prod-icon-lounge'}, {'name': 'ENV', 'value': 'production'}, {'name': 'TD_ENV', 'value': 'PRD'}, {'name': 'TD_HOST', 'value': 'tdnapprod.nordstrom.net'}, {'name': 'TD_VAULT_PATH', 'value': 'data/application/APP04216/6761/das_marketing/das-marketing-insights/prod/teradata/teradta_db_credentials'}, {'name': 'NR_VAULT_PATH', 'value': 'data/application/APP04216/6761/das_marketing/das-marketing-insights/prod/newrelic/newrelic_ingest_key'}, {'name': 'MAIL_SA_VAULT_PATH', 'value': 'data/application/APP04216/6761/das_marketing/das-marketing-insights/prod/service_account/sa_credentials'}, {'name': 'VAULT_PATH', 'value': 'vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com'}, {'name': 'VAULT_PORT', 'value': '8200'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}],
    metadata={'annotations': {'kube2iam.beta.nordstrom.net/role': 'arn:aws:iam::699296113972:role/k8s/tf-k8s-controlled-role'}, 'labels': {'nap_domain': 'onehop-etl', 'nap_project': 'insights-framework', 'customer_team_name': 'DAS_MARKETING', 'customer_project_name': 'das_marketing_insights'}},
...
launch_gc_rpt_attr_exp_send_slack_notification = launch_k8s_api_job_operator(
    dag=dag,
    connection_id='k8s_connection',
    namespace='app04216',
    task_id='gc_rpt_attr_exp_send_slack_notification',
    job_name='gc-rpt-attr-exp-send-slack-notification-gift-card-rpt-attribute',
    macros=None,
    container_image='artifactory.nordstrom.com/docker/app04216/tm00989-app04216-das-marketing-insights/prod/python/utils:1.3.18',
    container_command=['/home/nonroot/docker-entrypoint.sh', 'python/utils/src/send_email.py', '--vault_host_env_var', 'VAULT_PATH', '--vault_port_env_var', 'VAULT_PORT', '--k8s_vault_role_id_env_var', 'VAULT_ROLE_ID', '--k8s_vault_secret_id_env_var', 'VAULT_SECRET', '--vault_email_key_env_var', 'MAIL_SA_VAULT_PATH', '--vault_email_value_user_key', 'user', '--vault_email_value_password_key', 'password', '--mail_subject', '[<month>-<day>-<year>] Gift Cards RPT_ATTRIBUTES export - Successful', '--mail_body', 'Exported data location: s3://<bucket>/data/gift_card_static_export/hist/gift_card_rpt_attributes/year=<year>/month=<month>/day=<day>/gift_card_rpt_export_<year>_<month>_<day>.csv', '--from_address', 'das_marketing_isf_mb_auto@nordstrom.com', '--to_addresses', '<to_addresses>', '--replace_mapping', '{{"year": "{year}", "month": "{month}", "day": "{day}", "bucket": "{bucket}", "to_addresses": "{to_addresses}"}}'.format(bucket=eval('{"test": "tf-das-marketing-nonprod-teradata-stage","nonprod": "tf-das-marketing-nonprod-teradata-stage", "prod": "tf-das-marketing-prod-teradata-stage"}[env]'),to_addresses=eval('{"test": "notification-test-cha-aaaaj3bbztfzioluiilnrmvf2y@nordstrom.slack.com","nonprod": "notification-test-cha-aaaaj3bbztfzioluiilnrmvf2y@nordstrom.slack.com", "prod": "gift-card-notificatio-aaaakastua7yjrcq26s6uh63ai@nordstrom.slack.com"}[env]'),year=eval('str(datetime.now().strftime("%Y"))'),month=eval('str(datetime.now().strftime("%m"))'),day=eval('str(datetime.now().strftime("%d"))'))],
    resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    common_envs=[],
    task_envs=[{'name': 'S3_MKTG_BUCKET', 'value': 'tf-das-marketing-prod-teradata-stage'}, {'name': 'S3_MMM_BUCKET', 'value': 'prod-mmm-marketing-mix-model'}, {'name': 'S3_OPTIMINE_BUCKET', 'value': 'prod-mft-optimine'}, {'name': 'S3_ICON_LOUNGE_BUCKET', 'value': 'prod-icon-lounge'}, {'name': 'ENV', 'value': 'production'}, {'name': 'TD_ENV', 'value': 'PRD'}, {'name': 'TD_HOST', 'value': 'tdnapprod.nordstrom.net'}, {'name': 'TD_VAULT_PATH', 'value': 'data/application/APP04216/6761/das_marketing/das-marketing-insights/prod/teradata/teradta_db_credentials'}, {'name': 'NR_VAULT_PATH', 'value': 'data/application/APP04216/6761/das_marketing/das-marketing-insights/prod/newrelic/newrelic_ingest_key'}, {'name': 'MAIL_SA_VAULT_PATH', 'value': 'data/application/APP04216/6761/das_marketing/das-marketing-insights/prod/service_account/sa_credentials'}, {'name': 'VAULT_PATH', 'value': 'vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com'}, {'name': 'VAULT_PORT', 'value': '8200'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}],
    metadata={'annotations': {'kube2iam.beta.nordstrom.net/role': 'arn:aws:iam::699296113972:role/k8s/tf-k8s-controlled-role'}, 'labels': {'nap_domain': 'onehop-etl', 'nap_project': 'insights-framework', 'customer_team_name': 'DAS_MARKETING', 'customer_project_name': 'das_marketing_insights'}},
...
creds_setup >> fetch_batch_run_id_task >> launch_gc_rpt_attr_exp_s3_delete >>[launch_gc_rpt_attr_exp_teradata_job]
    launch_gc_rpt_attr_exp_teradata_job >>[tpt_jdbc_export_gc_rpt_attr_exp_tpt_export_control_tbl]
    tpt_jdbc_export_gc_rpt_attr_exp_tpt_export_control_tbl >>[tpt_gc_rpt_attr_exp_tpt_export_export_tbl]
    tpt_gc_rpt_attr_exp_tpt_export_export_tbl >>[launch_gc_rpt_attr_exp_s3_copy]
    launch_gc_rpt_attr_exp_s3_copy >>[launch_gc_rpt_attr_exp_send_slack_notification]
    launch_gc_rpt_attr_exp_send_slack_notification >> pelican_validator >> pelican_sensor