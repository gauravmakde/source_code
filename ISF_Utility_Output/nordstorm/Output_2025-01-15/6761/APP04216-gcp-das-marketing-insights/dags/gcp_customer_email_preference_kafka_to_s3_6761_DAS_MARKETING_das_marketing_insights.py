...
dag_id = 'gcp_customer_email_preference_kafka_to_s3_6761_DAS_MARKETING_das_marketing_insights'
...
default_args = {
    ...
    'description': 'customer_email_preference_kafka_to_s3_6761_DAS_MARKETING_das_marketing_insights DAG Description',
...
session_name=get_batch_id(dag_id=dag_id,task_id="load_customer_preference_object_model_kafka_to_s3")   
    load_customer_preference_object_model_kafka_to_s3 = DataprocCreateBatchOperator(
            task_id = "load_customer_preference_object_model_kafka_to_s3",
           ...
           #op_args = { "version":"1.1"},
            batch=
            {    
                "runtime_config": {"version": "1.1","properties":{ 'spark.sql.legacy.avro.datetimeRebaseModeInWrite': 'LEGACY', 'spark.yarn.maxAppAttempts': '1', 'spark.speculation': 'false', 'spark.airflow.run_id':"{{ ti.xcom_pull(task_ids='fetch_batch_run_id_task')}}", 'spark.sql.autoBroadcastJoinThreshold': '-1', 'spark.sql.legacy.parquet.int96RebaseModeInWrite': 'CORRECTED', 'spark.sql.mapKeyDedupPolicy': 'LAST_WIN'}},
                "spark_batch": 
                    {
                    ...
                    "args": [f"{user_config_path}/argument_customer_email_preference_kafka_to_s3_6761_DAS_MARKETING_das_marketing_insights_load_customer_preference_object_model_kafka_to_s3.json",
...
session_name=get_batch_id(dag_id=dag_id,task_id="load_customer_tokenized_object_model_kafka_to_s3")   
    load_customer_tokenized_object_model_kafka_to_s3 = DataprocCreateBatchOperator(
            task_id = "load_customer_tokenized_object_model_kafka_to_s3",
            project_id = project_id,
            region = region,
            gcp_conn_id = gcp_conn,
           #op_args = { "version":"1.1"},
            ...
            {    
                ...
                "spark_batch": 
                    ...
                    "main_class": "com.nordstrom.nap.onehop.etl.app.ExecSQLWrapper",
                    "jar_file_uris": [f"{spark_jar_path}/{delta_core_jar}",f"{spark_jar_path}/uber-onehop-etl-pipeline-{etl_jar_version}.jar"],
                    "args": [f"{user_config_path}/argument_customer_email_preference_kafka_to_s3_6761_DAS_MARKETING_das_marketing_insights_load_customer_tokenized_object_model_kafka_to_s3.json",
                    '--aws_user_role_external_id', Variable.get('aws_role_externalid')],
                    },
                    "environment_config": 
                                        {
...
launch_extracted_and_audit_files_copy_to_s3_path_folder = launch_k8s_api_job_operator(
    ...
    job_name='extracted-and-audit-files-copy-to-s3-path-folder-customer-email',
    ...
    container_command=['/home/nonroot/docker-entrypoint.sh', 'python/utils/src/bulk_s3_copy.py', '--s3_source_bucket_env_name', 'S3_MKTG_BUCKET', '--s3_target_bucket_env_name', 'S3_MKTG_BUCKET', '--source_to_target_path_pair', "{'source': 'isf/data/customer_preference/customer_preference_data_extract/', 'target': 'isf/data/customer_preference/incremental/year=<year>/month=<month>/day=<day>/hour=<hour>/'}", '--source_to_target_path_pair', "{'source': 'isf/data/customer_email/customer_tokenized_data_extract/', 'target': 'isf/data/customer_email/incremental/year=<year>/month=<month>/day=<day>/hour=<hour>/'}", '--source_to_target_path_pair', "{'source': 'isf/audit/customer_preference/customer_preference_final_count/', 'target': 'isf/audit/customer_preference/incremental/year=<year>/month=<month>/day=<day>/hour=<hour>/'}", '--source_to_target_path_pair', "{'source': 'isf/audit/customer_email/customer_tokenized_final_count/', 'target': 'isf/audit/customer_email/incremental/year=<year>/month=<month>/day=<day>/hour=<hour>/'}", '--base_date_iso_env_name', 'DAG_EXECUTION_DATE'],
...
creds_setup >> fetch_batch_run_id_task >>[load_customer_preference_object_model_kafka_to_s3, load_customer_tokenized_object_model_kafka_to_s3]
    load_customer_preference_object_model_kafka_to_s3 >>[launch_extracted_and_audit_files_copy_to_s3_path_folder]
    load_customer_tokenized_object_model_kafka_to_s3 >>[launch_extracted_and_audit_files_copy_to_s3_path_folder]
