...
dag_id = 'gcp_loyalty_activity_consistency_check_6761_DAS_MARKETING_das_marketing_insights'
...
from modules.utils.src.branching import loyalty_activity_missing_ids_notification_branching
...
default_args = {
    ...
    'description': 'loyalty_activity_consistency_check_6761_DAS_MARKETING_das_marketing_insights DAG Description',
           ...
           #op_args = { "version":"1.1"},
            ...
            {    
                ...
                "spark_batch": 
                    ...
                    "args": [f"{user_config_path}/argument_loyalty_activity_consistency_check_6761_DAS_MARKETING_das_marketing_insights_refresh_om_table_attach_partitions.json",
           ...
           #op_args = { "version":"1.1"},
            ...
            {    
                ...
                "spark_batch": 
                    ...
                    "main_class": "com.nordstrom.nap.onehop.etl.app.ExecSQLWrapper",
                    "jar_file_uris": [f"{spark_jar_path}/{delta_core_jar}",f"{spark_jar_path}/uber-onehop-etl-pipeline-{etl_jar_version}.jar"],
                    "args": [f"{user_config_path}/argument_loyalty_activity_consistency_check_6761_DAS_MARKETING_das_marketing_insights_refresh_dlq_table_attach_partitions.json",
                    '--aws_user_role_external_id', Variable.get('aws_role_externalid')],
                    },
                    "environment_config": 
                                        {
           ...
           #op_args = { "version":"1.1"},
            ...
            {    
                ...
                "spark_batch": 
                    ...
                    "main_class": "com.nordstrom.nap.onehop.etl.app.ExecSQLWrapper",
                    "jar_file_uris": [f"{spark_jar_path}/{delta_core_jar}",f"{spark_jar_path}/uber-onehop-etl-pipeline-{etl_jar_version}.jar"],
                    "args": [f"{user_config_path}/argument_loyalty_activity_consistency_check_6761_DAS_MARKETING_das_marketing_insights_compare_om_and_dlq_tables_job.json",
                    '--aws_user_role_external_id', Variable.get('aws_role_externalid')],
                    },
                    "environment_config": 
                                        {
...
check_missing_ids_presence_branching_operator = BranchPythonOperator(
    ...
    python_callable=loyalty_activity_missing_ids_notification_branching.check_missing_ids_presence)
    ...
    container_command=['/home/nonroot/docker-entrypoint.sh', 'python/utils/src/bulk_s3_copy.py', '--s3_source_bucket_env_name', 'S3_MKTG_BUCKET', '--s3_target_bucket_env_name', 'S3_MKTG_BUCKET', '--source_to_target_path_pair', "{'source': 'missing_ids/loyalty_activity/missing_create_ids/', 'target': 'missing_ids/loyalty_activity/incremental/year=<year>/month=<month>/day=<day>/'}"],
...
launch_process_notification_send_notification_to_slack = launch_k8s_api_job_operator(
    ...
    job_name='process-notification-send-notification-to-slack-loyalty-activit',
    macros=None,
    ...
    container_command=['/home/nonroot/docker-entrypoint.sh', 'python/utils/src/send_email.py', '--vault_host_env_var', 'VAULT_PATH', '--vault_port_env_var', 'VAULT_PORT', '--k8s_vault_role_id_env_var', 'VAULT_ROLE_ID', '--k8s_vault_secret_id_env_var', 'VAULT_SECRET', '--vault_email_key_env_var', 'MAIL_SA_VAULT_PATH', '--vault_email_value_user_key', 'user', '--vault_email_value_password_key', 'password', '--mail_subject', 'Missing LoylatyActivity Create Event IDs notification', '--mail_body', 'Missing LoylatyActivity Create detected! The CSV file with IDs could be found in the following folder: s3://<bucket>/missing_ids/loyalty_activity/incremental/year=<year>/month=<month>/day=<day>', '--from_address', 'das_marketing_isf_mb_auto@nordstrom.com', '--to_addresses', '<to_addresses>', '--replace_mapping', '{{"year": "{year}", "month": "{month}", "day": "{day}", "bucket": "{bucket}", "to_addresses": "{to_addresses}"}}'.format(bucket=eval('{"test": "mock","nonprod": "tf-das-marketing-nonprod-teradata-stage", "prod": "tf-das-marketing-prod-teradata-stage"}[env]'),year=eval('str(datetime.now().strftime("%Y"))'),month=eval('str(datetime.now().strftime("%m"))'),day=eval('str(datetime.now().strftime("%d"))'),to_addresses=eval('{"test": "mock","nonprod": "test-ooo-notification-aaaamfdfmbl3eawwoaybm5atpa@nordstrom.slack.com", "prod": "das-pod-crashing-loya-aaaaiak743vndt4xz4g5l2wzqe@nordstrom.slack.com"}[env]'))],
    resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    common_envs=[],
    task_envs=[{'name': 'S3_MKTG_BUCKET', 'value': 'tf-das-marketing-prod-teradata-stage'}, {'name': 'S3_MMM_BUCKET', 'value': 'prod-mmm-marketing-mix-model'}, {'name': 'S3_OPTIMINE_BUCKET', 'value': 'prod-mft-optimine'}, {'name': 'S3_ICON_LOUNGE_BUCKET', 'value': 'prod-icon-lounge'}, {'name': 'ENV', 'value': 'production'}, {'name': 'TD_ENV', 'value': 'PRD'}, {'name': 'TD_HOST', 'value': 'tdnapprod.nordstrom.net'}, {'name': 'TD_VAULT_PATH', 'value': 'data/application/APP04216/6761/das_marketing/das-marketing-insights/prod/teradata/teradta_db_credentials'}, {'name': 'NR_VAULT_PATH', 'value': 'data/application/APP04216/6761/das_marketing/das-marketing-insights/prod/newrelic/newrelic_ingest_key'}, {'name': 'MAIL_SA_VAULT_PATH', 'value': 'data/application/APP04216/6761/das_marketing/das-marketing-insights/prod/service_account/sa_credentials'}, {'name': 'VAULT_PATH', 'value': 'vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com'}, {'name': 'VAULT_PORT', 'value': '8200'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}],
    metadata={'annotations': {'kube2iam.beta.nordstrom.net/role': 'arn:aws:iam::699296113972:role/k8s/tf-k8s-controlled-role'}, 'labels': {'nap_domain': 'onehop-etl', 'nap_project': 'insights-framework', 'customer_team_name': 'DAS_MARKETING', 'customer_project_name': 'das_marketing_insights'}},
...
launch_end_dummy = launch_k8s_api_job_operator(
    ...
    namespace='app04216',
    task_id='end_dummy',
    job_name='end-dummy-loyalty-activity-consistency-check-6761-das-marketing',
    macros=None,
    container_image='artifactory.nordstrom.com/docker/app04216/tm00989-app04216-das-marketing-insights/prod/python/utils:1.3.22',
