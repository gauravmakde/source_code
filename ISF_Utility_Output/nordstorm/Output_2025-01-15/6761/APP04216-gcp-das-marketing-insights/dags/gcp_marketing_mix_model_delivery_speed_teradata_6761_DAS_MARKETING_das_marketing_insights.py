...
dag_id = 'gcp_marketing_mix_model_delivery_speed_teradata_6761_DAS_MARKETING_das_marketing_insights'
...
default_args = {
    ...
    'description': 'marketing_mix_model_delivery_speed_teradata_6761_DAS_MARKETING_das_marketing_insights DAG Description',
...
with DAG(
    ...
    conn_id='okta_connection_app02560',
    external_dag_id='gcp_order_line_detail_17421_DAS_SC_OUTBOUND_APP02560_insights_v2',
    ...
    marketing_mix_model_delivery_speed_teradata_6761_DAS_MARKETING_das_marketing_insights_marketing_mix_model_delivery_speed_marketing_mix_model_delivery_speed_teradata = open(
    os.path.join(sql_path, 'marketing_mix_model_delivery_speed_teradata_6761_DAS_MARKETING_das_marketing_insights_marketing_mix_model_delivery_speed_marketing_mix_model_delivery_speed_teradata.sql'),
    "r").read()

    launch_marketing_mix_model_delivery_speed_teradata_job = BigQueryInsertJobOperator(
        task_id = "marketing_mix_model_delivery_speed_teradata_job",
        configuration = 
        {
            "query": 
            {
                "query": marketing_mix_model_delivery_speed_teradata_6761_DAS_MARKETING_das_marketing_insights_marketing_mix_model_delivery_speed_marketing_mix_model_delivery_speed_teradata,
                "useLegacySql": False
            }
        },
...
tpt_jdbc_export_marketing_mix_model_delivery_speed_job_export_control_tbl = BashOperator(
    task_id='tpt_jdbc_export_marketing_mix_model_delivery_speed_job_export_control_tbl',
    bash_command=f'echo "Skipping Control Table Entry"')

    tpt_marketing_mix_model_delivery_speed_job_export_export_tbl = SSHOperator(
    task_id='tpt_marketing_mix_model_delivery_speed_job_export_export_tbl',
    command="/db/teradata/bin/tpt_export.sh  -e PRD_NAP_UTL -s s3://tf-nap-prod-mkt-etl/apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/6761/DAS_MARKETING/das_marketing_insights/sql/marketing_mix_model_delivery_speed_teradata_6761_DAS_MARKETING_das_marketing_insights_marketing_mix_model_delivery_speed_marketing_mix_model_delivery_speed_tpt_export.sql -h tdnapprod.nordstrom.net -l 'N' -u SCH_NAP_MMM_BATCH -p \"\$tdwallet(SCH_NAP_MMM_BATCH_PWD)\"  -t s3://prod-mmm-marketing-mix-model/Nordstrom/Outbound/delta_load/weekly_refresh/year={year}/month={month}/day={day}/mmm_delivery_speed_export_{year}_{month}_{day}.csv -a N -d , -c 1 -q N -f '' -j {database_env}_MMM_DELIVERY_SPEED_LDG_JOB".format(database_env=eval('{"nonprod": "PROTO_DSA_AI_DB","test": "PROTO_DSA_AI_DB", "prod": "PRD_NAP_DSA_AI_STG"}[env]'),year=eval('str(datetime.now().strftime("%Y"))'),month=eval('str(datetime.now().strftime("%m"))'),day=eval('str(datetime.now().strftime("%d"))')),
    timeout=1800,
    ssh_conn_id="teradata_ssh_connection_to_ec2_isf")

    creds_setup >> fetch_batch_run_id_task >> launch_other_appId_upstream_dependencies_sensor_0 >>[launch_marketing_mix_model_delivery_speed_teradata_job]
    launch_marketing_mix_model_delivery_speed_teradata_job >>[tpt_jdbc_export_marketing_mix_model_delivery_speed_job_export_control_tbl]
    tpt_jdbc_export_marketing_mix_model_delivery_speed_job_export_control_tbl >>[tpt_marketing_mix_model_delivery_speed_job_export_export_tbl] >> pelican_validator >> pelican_sensor