...
Task_Name=wfm_scheduling_metric_load_0_stg_table;'
FOR SESSION VOLATILE;

-- Reading from kafka:
create
temporary view wfm_scheduling_metric_input AS
select *
from kafka_wfm_scheduling_metric_input;


-- Writing Kafka Data to S3 Path
insert into table wfm_scheduling_metric_orc_output partition(year, month, day)
select *, current_date() as process_date, year(current_date()) as year,month(current_date()) as month,day(current_date()) as day
from wfm_scheduling_metric_input;


-- Writing Kafka to Semantic Layer:
insert
overwrite table wfm_scheduling_metric_stg_table
select id as ID,
       name as NAME,
       deleted                 as DELETED,
