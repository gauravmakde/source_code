...
drop table if exists {hive_schema}.by_allocation_npi_partitioned_op;


create table if not exists {hive_schema}.by_allocation_npi_partitioned_op
(
    npi_month string,
    dept_num integer,
    class_num integer,
...
STORED AS PARQUET
location 's3://{s3_bucket_by}/raw/npi_by_allocation/business_unit=op/'
TBLPROPERTIES ('parquet.compress' = 'SNAPPY');


MSCK REPAIR TABLE {hive_schema}.by_allocation_npi_partitioned_op;


--- Writing Data to AWS S3
insert overwrite table npi_by_allocation_op_result PARTITION (locations)
select
...
from {hive_schema}.by_allocation_npi_partitioned_op b
