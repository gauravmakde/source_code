SET QUERY_BAND = '
App_ID=APP04070;
DAG_ID=nap_to_allocation_sales_extracts_weekly;
Task_Name=sales_fp_extracts;'
...
create temporary view sales_fp_extracts as 
SELECT
             alloc.store_num
            ,alloc.rms_sku_num
            ,alloc.week_num
...
LEFT OUTER JOIN (select inv.* 
       ...
       WHERE inv.snapshot_date= (select dw_batch_dt from etl_batch_dt_lkup_vw where interface_code='MERCH_ALLOCATION_DLY')
         AND sd.business_unit_num IN ('1000')) inv_curr
...
LEFT OUTER JOIN (select oo.store_num,	
   ...
   WHERE alloc.business_unit_num = '1000'  
GROUP BY alloc.store_num,
         alloc.rms_sku_num,
         alloc.week_num;

--- Writing Data to s3 on a single partition
insert overwrite table sales_fp_tab
select /*+ COALESCE(1) */ * from  sales_fp_extracts ;

insert overwrite table sales_fp_audit select concat(date_format(from_utc_timestamp(current_timestamp,'PST'),'yyyyMMddHHmmss'),' ',format_number(count(1),'000000000000')) from sales_fp_extracts;
