...
create temporary view oo_fp_extracts as 
select oo.store_num,	
       oo.rms_sku_num,
 ...
 WHERE oo.quantity_open>0 
   ...
   AND sd.business_unit_num IN ('1000');
 

--- Writing Data to s3 on a single partition
insert overwrite table oo_fp_tab
select /*+ COALESCE(1) */ * from  oo_fp_extracts ;

insert overwrite table oo_fp_audit select concat(date_format(from_utc_timestamp(current_timestamp,'PST'),'yyyyMMddHHmmss'),' ',format_number(count(1),'000000000000')) from oo_fp_extracts;
