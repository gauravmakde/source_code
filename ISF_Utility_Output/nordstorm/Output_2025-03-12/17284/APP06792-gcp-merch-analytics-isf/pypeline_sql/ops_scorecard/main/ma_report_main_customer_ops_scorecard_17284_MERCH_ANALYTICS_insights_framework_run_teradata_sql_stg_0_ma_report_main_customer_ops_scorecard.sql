...
WHERE DTL.business_day_date BETWEEN CURRENT_DATE - 90 AND CURRENT_DATE
...
WHERE CASE WHEN LINE_NET_USD_AMT > 0 THEN COALESCE(DTL.ORDER_DATE,DTL.TRAN_DATE) ELSE DTL.TRAN_DATE END BETWEEN (CURRENT_DATE-28) AND (SELECT WEEK_END_DAY_DATE FROM CURR_WK)
...
WHERE tran_date BETWEEN (CURRENT_DATE-30) AND (SELECT WEEK_END_DAY_DATE FROM CURR_WK)
...
where business_day_date BETWEEN (CURRENT_DATE-30) AND (SELECT WEEK_END_DAY_DATE FROM CURR_WK)
group by 1,2) b
...
where rttf.business_day_date BETWEEN (CURRENT_DATE-30) AND (SELECT WEEK_END_DAY_DATE FROM CURR_WK)
