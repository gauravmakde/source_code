...
APPID-Name:					APP09125
Purpose:					Creates table in {environment_schema} for Merch KPI Scorecard
...
CALL SYS_MGMT.DROP_IF_EXISTS_SP ('{environment_schema}', 'kpi_scorecard{env_suffix}', OUT_RETURN_MSG);
CREATE MULTISET TABLE {environment_schema}.kpi_scorecard{env_suffix}
    ,FALLBACK
	,NO BEFORE JOURNAL
	,NO AFTER JOURNAL
	,CHECKSUM = DEFAULT
...
;
GRANT SELECT ON {environment_schema}.kpi_scorecard{env_suffix} TO PUBLIC
;