-- Views to fold in forward looking plans 

CALL SYS_MGMT.DROP_IF_EXISTS_SP ('t2dl_das_apt_cost_reporting', 'current_month', OUT_RETURN_MSG);
CREATE MULTISET TABLE t2dl_das_apt_cost_reporting.current_month,
    FALLBACK,
    NO BEFORE JOURNAL,
    NO AFTER JOURNAL,
    CHECKSUM = DEFAULT,
...
REPLACE VIEW t2dl_das_apt_cost_reporting.merch_assortment_category_cluster_plan_fact_vw 
AS
LOCK ROW FOR ACCESS

SELECT *
FROM t2dl_das_apt_cost_reporting.merch_assortment_category_cluster_plan_fact
WHERE fiscal_month_idnt < (SELECT DISTINCT month_idnt FROM t2dl_das_apt_cost_reporting.current_month)

UNION 

SELECT
     month_id
    ,CAST(CONCAT('20',RIGHT(month_id,2),
...
WHERE CAST(CONCAT('20',RIGHT(month_id,2),
           CASE WHEN LEFT(month_id,3)= 'FEB' THEN '01'
                WHEN LEFT(month_id,3)= 'MAR' THEN '02'
                WHEN LEFT(month_id,3)= 'APR' THEN '03'
                WHEN LEFT(month_id,3)= 'MAY' THEN '04'
                ...
                WHEN LEFT(month_id,3)= 'DEC' THEN '11'
                WHEN LEFT(month_id,3)= 'JAN' THEN '12'
              END) AS INTEGER) >= (SELECT DISTINCT month_idnt FROM t2dl_das_apt_cost_reporting.current_month)
;


REPLACE VIEW t2dl_das_apt_cost_reporting.merch_assortment_category_country_plan_fact_vw 
AS
LOCK ROW FOR ACCESS

SELECT *
FROM t2dl_das_apt_cost_reporting.merch_assortment_category_country_plan_fact
WHERE fiscal_month_idnt < (SELECT DISTINCT month_idnt FROM t2dl_das_apt_cost_reporting.current_month)
...
REPLACE VIEW t2dl_das_apt_cost_reporting.merch_assortment_supplier_cluster_plan_fact_vw 
AS
LOCK ROW FOR ACCESS

SELECT *
FROM t2dl_das_apt_cost_reporting.merch_assortment_supplier_cluster_plan_fact
WHERE fiscal_month_idnt < (SELECT DISTINCT month_idnt FROM t2dl_das_apt_cost_reporting.current_month)
