...
WHERE CAST(CONCAT('20',LEFT(strtok(month_id,'FY',2),2),
           CASE WHEN LEFT(month_id,3)= 'FEB' THEN '01'
                ...
                WHEN LEFT(month_id,3)= 'DEC' THEN '11'
                WHEN LEFT(month_id,3)= 'JAN' THEN '12'
              END) AS INTEGER)  >= (SELECT DISTINCT month_idnt FROM prd_nap_usr_vws.day_cal_454_dim WHERE day_date = current_date)
)
WITH DATA
...
DELETE FROM {environment_schema}.merch_assortment_category_cluster_plan_history{env_suffix} WHERE snapshot_plan_month_idnt = (SELECT DISTINCT month_idnt FROM prd_nap_usr_vws.day_cal_454_dim WHERE day_date = current_date);

INSERT INTO {environment_schema}.merch_assortment_category_cluster_plan_history{env_suffix}
SELECT * 
...
DELETE FROM {environment_schema}.merch_assortment_category_country_plan_history{env_suffix} WHERE snapshot_plan_month_idnt = (SELECT DISTINCT month_idnt FROM prd_nap_usr_vws.day_cal_454_dim WHERE day_date = current_date);

INSERT INTO {environment_schema}.merch_assortment_category_country_plan_history{env_suffix}
SELECT * 
...
DELETE FROM {environment_schema}.merch_assortment_supplier_cluster_plan_history{env_suffix} WHERE snapshot_plan_month_idnt = (SELECT DISTINCT month_idnt FROM prd_nap_usr_vws.day_cal_454_dim WHERE day_date = current_date);

INSERT INTO {environment_schema}.merch_assortment_supplier_cluster_plan_history{env_suffix}
SELECT * 
