CALL SYS_MGMT.DROP_IF_EXISTS_SP ('{environment_schema}', 'merch_assortment_category_cluster_plan_fact{env_suffix}', OUT_RETURN_MSG);
CREATE MULTISET TABLE {environment_schema}.merch_assortment_category_cluster_plan_fact{env_suffix} ,FALLBACK , 
	NO BEFORE JOURNAL,
	NO AFTER JOURNAL,
	CHECKSUM = DEFAULT,
...
PRIMARY INDEX(fiscal_month_idnt, cluster_name, dept_idnt, category)
-- PARTITION BY RANGE_N(snapshot_start_day_date BETWEEN DATE '2011-01-01' AND DATE '2031-12-31' EACH INTERVAL '1' DAY , NO RANGE)
;


CALL SYS_MGMT.DROP_IF_EXISTS_SP ('{environment_schema}', 'merch_assortment_category_country_plan_fact{env_suffix}', OUT_RETURN_MSG);
CREATE MULTISET TABLE {environment_schema}.merch_assortment_category_country_plan_fact{env_suffix} ,FALLBACK , 
	NO BEFORE JOURNAL,
	NO AFTER JOURNAL,
	CHECKSUM = DEFAULT,
	DEFAULT MERGEBLOCKRATIO,
...
PRIMARY INDEX(fiscal_month_idnt, selling_country, selling_brand, dept_idnt, price_band)
-- PARTITION BY RANGE_N(snapshot_start_day_date BETWEEN DATE '2011-01-01' AND DATE '2031-12-31' EACH INTERVAL '1' DAY , NO RANGE)
;

-- Supplier Clust Plans

CALL SYS_MGMT.DROP_IF_EXISTS_SP ('{environment_schema}', 'merch_assortment_supplier_cluster_plan_fact{env_suffix}', OUT_RETURN_MSG);
CREATE MULTISET TABLE {environment_schema}.merch_assortment_supplier_cluster_plan_fact{env_suffix} ,FALLBACK , 
	NO BEFORE JOURNAL,
	NO AFTER JOURNAL,
	CHECKSUM = DEFAULT,
	DEFAULT MERGEBLOCKRATIO,
...
(
    ...
    ,dept_idnt VARCHAR(40) CHARACTER SET UNICODE NOT CASESPECIFIC 
...
PRIMARY INDEX(fiscal_month_idnt, selling_country, selling_brand, dept_idnt, category, supplier_group)
-- PARTITION BY RANGE_N(snapshot_start_day_date BETWEEN DATE '2011-01-01' AND DATE '2031-12-31' EACH INTERVAL '1' DAY , NO RANGE)
;

CALL SYS_MGMT.DROP_IF_EXISTS_SP ('{environment_schema}', 'size_sku_loc_weekly_test', OUT_RETURN_MSG);
CREATE MULTISET TABLE {environment_schema}.current_month,
    FALLBACK,
    NO BEFORE JOURNAL,
    NO AFTER JOURNAL,
    CHECKSUM = DEFAULT,
