...
where s.year_num in (select fiscal_year_num from prd_nap_usr_vws.day_cal_454_dim group by 1 where fiscal_year_num >= (select max(fiscal_year_num) - 1 as ty_fiscal_yr from prd_nap_usr_vws.day_cal_454_dim where week_end_day_date < current_date))
...
as (
	...
	from prd_nap_usr_vws.merch_sale_return_sku_store_week_fact_vw as sales
...
collect stats primary index(week_idnt, channel_num, supplier_number, dept_idnt),
	...
	column(week_idnt),
	column(dept_idnt),
	column(channel_num) on sales_stg;

create volatile multiset table sales_final
as (
	select
		ty_ly_ind,
		week_idnt,
...
collect stats primary index(week_idnt, channel_num, supplier_number, dept_idnt),
	...
	column(week_idnt),
	column(dept_idnt),
	column(channel_num) on sales_final;
...
as (
	...
	from prd_nap_usr_vws.merch_poreceipt_sku_store_week_fact_vw as receipts
	inner join date_lkup as dt
...
as (
	...
	from prd_nap_usr_vws.merch_transfer_sku_loc_week_agg_fact_vw transfer
	inner join date_lkup as dt
...
as (
    ...
    cast(0 as decimal(10,0)) as dropship_net_sales_tot_units,
	  ...
	  cast(0 as decimal(12,2)) as mos_total_cost_amt,
		...
		cast(0 as decimal(12,2)) as rtv_total_retail,
		cast(0 as decimal(10,0)) as rtv_total_units
	from prd_nap_usr_vws.merch_disc_terms_sku_loc_week_agg_fact_vw as terms
	inner join date_lkup as dt
...
as (
	...
	from prd_nap_usr_vws.merch_vendor_funds_sku_store_week_fact_vw vfm
	inner join date_lkup as dt
...
as (
	...
	from prd_nap_usr_vws.merch_inventory_sku_store_week_fact_vw inventory
	inner join date_lkup as dt
...
as (
    ...
    cast(0 as decimal(10,0)) as dropship_net_sales_tot_units,
	  ...
	  cast(0 as decimal(10,0)) as eoh_regular_units,
		...
		cast(0 as decimal(12,2)) as rtv_total_retail,
		cast(0 as decimal(10,0)) as rtv_total_units
	from prd_nap_usr_vws.merch_sp_mos_adjusted_sku_store_week_agg_fact mos
	inner join date_lkup as dt
...
as (
	...
	from prd_nap_usr_vws.merch_return_to_vendor_sku_loc_week_agg_fact_vw as rtv
	inner join date_lkup as dt
...
delete from {environment_schema}.supplier_profitability_expectation_channel_week_dept all;
insert into {environment_schema}.supplier_profitability_expectation_channel_week_dept
	select
		base.ty_ly_ind,
		base.week_idnt,
		d.fiscal_year_num ,
...
collect statistics primary index (week_idnt, channel_num, supplier_number, dept_idnt )
		on {environment_schema}.supplier_profitability_expectation_channel_week_dept;
