...
# Run command: pipenv run python selection_planning_backfill.py prod 'prod'
...
ddl_filepath = 'ma_pipeline_prod_ddl_selection_planning.sql'
main_filepath = 'ma_pipeline_prod_main_selection_planning.sql'

env = sys.argv[1] # position [0] is script name
if env == 'prod':
    connection_id = 'prod-selection-nap' # T2DL_NAP_SEL_BATCH
    auth_mech = None
    environment_schema = 'T2DL_DAS_SELECTION'
    env_suffix = ''
else:
    connection_id = 'nonprod-selection-t3dl-clhq' # CLHQ
    auth_mech = 'LDAP'
    environment_schema = 'T3DL_ACE_ASSORTMENT'
    env_suffix = ''
...
def make_logger(name: str = 'query_exec'):
...
def execute_sql_statements_with_splitter(sql_string: str, splitter: str = ';', file_path: str = 'query_exec') -> None:
...
if __name__ == "__main__":
    ...
    print('Running loop for daily base table...')
    date_sql =  """
                SELECT 
                ...
                WHERE fiscal_year_num in (2023,2022,2021,2020,2019)
                    AND fiscal_month_num in (1,6,7,12) -- 6 month chunks
                ORDER BY 1
                """
    date_ranges = [ # 2019
                    ("DATE '2019-02-03'", "DATE '2019-08-03'"),
                    ("DATE '2019-08-04'", "DATE '2020-02-01'"),
                    # 2020
                    ("DATE '2020-02-02'", "DATE '2020-08-01'"),
                    ("DATE '2020-08-02'", "DATE '2021-01-30'"),
                    # 2021
                    ("DATE '2021-01-31'", "DATE '2021-07-31'"),
                    ("DATE '2021-08-01'", "DATE '2022-01-29'"),
                    # 2022
                    ("DATE '2022-01-30'", "DATE '2022-07-30'"),
                    ("DATE '2022-07-31'", "DATE '2023-01-28'"), 
                    # 2023
                    ("DATE '2023-01-28'", "CURRENT_DATE - 1"), 
                    ]
    ...
    for i,(start_date,end_date) in enumerate(date_ranges):
        ...
        print(f'{i} Running chunk for {start_date} to {end_date}.')
        my_params = {'environment_schema': environment_schema
                    ,'env_suffix': env_suffix
    ...
    print('Daily base loops complete.')
    
    # build daily and monthly tables
    print('Building daily and monthly tables...')
    execute_sql_statements_with_splitter(sql_daily_monthly.format(**params))
    print('Daily and monthly tables complete.')
