...
AS (
    ...
    FROM (
        SELECT
            ...
            END AS date_event
            , CASE
                WHEN pfd.channel = 'FULL_LINE' THEN '808'
...
CREATE VOLATILE MULTISET TABLE prod_hier AS (
    ...
    LEFT JOIN T2DL_DAS_SCALED_EVENTS.AN_STRATEGIC_BRANDS_2024 anchor
...
-- merch clk
    -- Part 1 - From start_date to end_date_middle
...
AS (
    WITH date_filter AS (
        SELECT
            ...
            WHERE day_dt BETWEEN {start_date} AND {end_date_middle}
    ...
    FROM(
        ...
        INNER JOIN date_filter df
            ON clk.day_idnt = df.day_idnt
        WHERE clk.day_dt BETWEEN {start_date} AND {end_date_middle}  --Diff from Delta DAG
        UNION ALL
        --OO
        SELECT
...
------------------------------------------------------------- END TEMPORARY TABLES PT.1 -----------------------------------------------------------------------


---------------------------------------------------------------- START MAIN QUERY - PT.1 -------------------------------------------------------------------------
...
SELECT
    ...
    , pm.promo_grouping -- 'NA' AS promo_grouping  (comment price match for AN 2025)
    , sku.bipoc_ind
    , um.udig
    , CASE
        --Anniv Event
...
-- price match (uncomment for Cyber 2024)
...
---------------------------------------------------------- START TEMPORARY TABLES PT.2 ------------------------------------------------------------------

-- 2022.08.22 - Purge/Drop merch_clk for re-build to clear out resources and start over
DELETE FROM MERCH_CLK;
DROP TABLE MERCH_CLK;

-- merch clk
    -- Part 2 - From start_date_middle to end_date
CREATE MULTISET VOLATILE TABLE merch_clk
AS (
    WITH date_filter AS (
        SELECT
                ...
                , wk_idnt
            FROM {environment_schema}.SCALED_EVENT_DATES
            WHERE day_dt BETWEEN {start_date_middle} AND {end_date}
                AND date_event_type = '{date_event_type}'
    )
    SELECT
        base.sku_idnt
    ...
    FROM(
        ...
        INNER JOIN date_filter df
            ON clk.day_idnt = df.day_idnt
        WHERE clk.day_dt BETWEEN {start_date_middle} AND {end_date}  --Diff from Delta DAG
        UNION ALL
        --OO
        SELECT
            base.rms_sku_num AS sku_idnt
...
------------------------------------------------------------- END TEMPORARY TABLES PT.2 -----------------------------------------------------------------------




---------------------------------------------------------------- START MAIN QUERY - PT.2 -------------------------------------------------------------------------
