...
if Enable_to_run_specific_file == "Y":
    ...
    file_df = pd.read_csv(os.path.join( Input_directory,csv_file_name))
    print(file_df)
    PyFolderLst = []
    for single_file in file_df['file_name']:
        ...
        if single_file != 'nan':
            singleFileLst = glob.glob(os.path.join(Single_input_Path, '**', 'dags' , str(single_file)), recursive=True)
...
if __name__ == "__main__":
    ...
    for file in PyFolderLst:
        ...
        try:
            ...
            all_json_sql_details,error=multiple_utility.list_json_from_livy(file,operator_list,location,error)
        ...
        # print(all_json_sql_details)
            data.extend(all_json_sql_details)
