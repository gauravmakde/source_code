...
SELECT DISTINCT
-- loyalty IDs has 'lyltyact::' prefix that are removed by using the 'substr' command
    substr(
            decode(dlq.headers['usid'],'UTF-8'), 11
    ) AS missing_id
FROM acp_etl_landing.loyaltyactivity_object_model_dlq AS dlq
WHERE (
  ...
  AND NOT EXISTS (
    SELECT 1
    FROM object_model.loyaltyactivity_object_model_avro AS om
