...
SET QUERY_BAND = '
App_ID=app06788;
Task_Name=credit_application_failed_kafka_to_teradata_stg_job;'
-- noqa: disable=all
FOR SESSION VOLATILE;
-- noqa: enable=all

-- Read Kafka into temp table
CREATE TEMPORARY VIEW temp_credit_application_failed
AS
SELECT * FROM kafka_credit_application_failed;

CREATE TEMPORARY VIEW credit_application_failed_temp
AS
SELECT
    eventtime AS event_time,
    applicationid AS application_id,
    'Failed' AS application_decision_status,
...
FROM temp_credit_application_failed;

INSERT INTO TABLE credit_application_failed_ldg
SELECT
    event_time,
    application_id,
    ...
    client_reference_id,
    failure_reason
FROM credit_application_failed_temp
WHERE
