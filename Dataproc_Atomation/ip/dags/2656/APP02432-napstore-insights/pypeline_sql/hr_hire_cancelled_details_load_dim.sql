...
Task_Name=hire_cancelled_details_stage_load_10_worker_hire_cancelled_dim_tables;'
...
CREATE VOLATILE MULTISET TABLE HR_HIRE_CANCELLED_EMPLOYEE_DETAILS_TEMP AS (
  	select distinct
    worker_number,
	worker_type,
    employment_status_last_updated,
    employment_status_details_transaction_id,
    NULL as contingent_worker_projected_end_date,
    NULL as hire_date,
    ...
    from {db_env}_NAP_HR_STG.HR_WORKER_V1_LDG
       ...
       QUALIFY Row_Number() Over( PARTITION BY worker_number,worker_type,employment_status_details_transaction_id
       ORDER BY employment_status_last_updated DESC) =1
...
USING  HR_HIRE_CANCELLED_EMPLOYEE_DETAILS_TEMP src
