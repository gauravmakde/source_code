DELETE FROM  {db_env}_nap_fct.ecf_salesreporting_batch_v2_fact WHERE dw_sys_end_tmstp is null;
ET;

INSERT INTO {db_env}_nap_fct.ecf_salesreporting_batch_v2_fact(business_day_start_date, business_day_end_date)
WITH start_ly as (
    SELECT day_date as start_date_ly
    FROM {db_env}_NAP_USR_VWS.DAY_CAL_SALES_DIM
    ...
    FROM {db_env}_NAP_USR_VWS.RETAIL_TRAN_DETAIL_FACT, start_ly
    ...
    FROM {db_env}_NAP_USR_VWS.RETAIL_TRAN_HDR_FACT, start_ly
    WHERE cast(dw_sys_updt_tmstp as date) >= CURRENT_DATE - 1
    AND business_day_date >= start_date_ly
    ),
    max_batch_end_date as (
    SELECT MAX(business_day_end_date) + 1 as max_batch_end_date
    FROM {db_env}_NAP_FCT.ecf_salesreporting_batch_v2_fact
...
(
    ...
    FROM
        (
            ...
            FROM
            {db_env}_NAP_USR_VWS.PRODUCT_UPC_DIM
            WHERE
        ...
        JOIN (
            SELECT
                ...
                dept_num
            FROM
                {db_env}_NAP_USR_VWS.product_sku_dim_vw psdv
            WHERE
                channel_country = 'US' QUALIFY ROW_NUMBER() OVER (
        ...
        JOIN {db_env}_NAP_USR_VWS.DEPARTMENT_DIM dpt ON sku.dept_num = dpt.dept_num
)
WITH DATA
...
CREATE VOLATILE MULTISET TABLE tran_filtered AS
(
SELECT
    rthf.business_day_date,
       COALESCE(LPAD(rtdf.commission_slsprsn_num, 15, '0'), '0') as commission_slsprsn_num,
       CASE WHEN rtdf.item_source IN ('SB_SALESEMPINIT',
                     'STYLE_BOARDS'
        ) THEN 1 ELSE 0 END as digital_ind,
                    CASE
                     ...
                     THEN 1
                 ELSE 0 END as original_curr_code_ind,
            CASE
                WHEN (
                                rthf.tran_type_code = 'VOID'
                              AND reversal_flag = 'N'
                         )
                    OR (
                                rthf.tran_type_code <> 'VOID'
                     ...
                     THEN -1
                 ELSE 1 END AS reversal_multiplier,
       rtdf.line_item_activity_type_code,
       rtdf.original_line_item_usd_amt,
       rtdf.line_net_usd_amt,
       NVL(rthf.ownership_model, 'ERTM') as ownership_model
FROM
    {db_env}_NAP_USR_VWS.RETAIL_TRAN_HDR_FACT rthf
    JOIN {db_env}_NAP_USR_VWS.RETAIL_TRAN_DETAIL_FACT rtdf
    ...
    AND rthf.data_source_code NOT IN ('TRUNK') -- Modified to include NRHL data and ECONCESSION
    AND (rthf.sa_tran_status_code NOT IN ('D', 'O')  OR  (rthf.sa_tran_status_code is NULL and rthf.ownership_model = 'ECONCESSION'))
    AND (
           ...
           OR rtdf.item_source in ( 'SB_SALESEMPINIT',
                                     'STYLE_BOARDS',
                                      'POINT_OF_SALE'
                                  )
                          )
...
SELECT
    ...
    ownership_model,
    SUM( CASE WHEN digital_ind = 1 THEN reversal_multiplier * sale_amt ELSE 0 END) as digital_gross_amt,
    SUM( CASE WHEN digital_ind = 1 THEN reversal_multiplier * return_amt ELSE 0 END) as digital_return_amt,
    SUM( CASE WHEN digital_ind = 0 THEN reversal_multiplier * sale_amt ELSE 0 END) as store_gross_amt,
    SUM( CASE WHEN digital_ind = 0 THEN reversal_multiplier * return_amt ELSE 0 END) as store_return_amt,
    SUM(reversal_multiplier * sale_amt) as gross_amt,
    SUM(reversal_multiplier * return_amt) as return_amt
FROM (
SELECT business_day_date,
       ...
       digital_ind,
       reversal_multiplier,
       CASE WHEN line_item_activity_type_code = 'S'
                 THEN CASE WHEN original_curr_code_ind = 1 THEN  original_line_item_usd_amt ELSE line_net_usd_amt END
            ELSE 0
       END  as sale_amt,
       CASE WHEN line_item_activity_type_code = 'R'
                 THEN CASE WHEN original_curr_code_ind = 1 THEN  -1 * original_line_item_usd_amt ELSE line_net_usd_amt END
            ELSE 0
       END  as return_amt
      FROM tran_filtered
      ) as t
...
DELETE FROM {db_env}_NAP_FCT.ECF_SALESREPORTING_V2_FACT
...
INSERT INTO {db_env}_NAP_FCT.ECF_SALESREPORTING_V2_FACT
(
    commission_slsprsn_num,
    business_day_date,
    ownership_model,
...
SELECT
    ...
    digital_gross_amt,
    digital_return_amt,
    digital_gross_amt + digital_return_amt as digital_net_amt,
    store_gross_amt,
    store_return_amt,
    store_gross_amt + store_return_amt as store_net_amt,
    gross_amt,
    return_amt,
    gross_amt + return_amt as net_amt,
    batch_id as dw_batch_id,
    batch_date as dw_batch_date,
    current_timestamp as dw_sys_load_tmstp,
    current_timestamp as dw_sys_upd_tmstp
FROM tran_table t
         JOIN {db_env}_nap_fct.ecf_salesreporting_batch_v2_fact b
ON dw_sys_end_tmstp is null
;

UPDATE {db_env}_nap_fct.ecf_salesreporting_batch_v2_fact
