...
Task_Name=wfm_scheduling_metric_load_1_fct_table;'
...
-- Create temp table
CREATE
VOLATILE MULTISET TABLE WFM_SCHEDULING_METRIC_TEMP AS (
    SELECT DISTINCT * FROM {db_env}_NAP_STG.SCHEDULING_METRIC_STG
...
-- Merge and update:
MERGE INTO {db_env}_NAP_FCT.SCHEDULING_METRIC_FACT tgt
    USING WFM_SCHEDULING_METRIC_TEMP src
    ON (src.ID = tgt.ID)
    WHEN MATCHED THEN
UPDATE
SET
