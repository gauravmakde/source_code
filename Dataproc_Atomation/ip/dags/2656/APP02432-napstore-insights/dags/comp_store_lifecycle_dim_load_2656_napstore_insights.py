...
default_args = {
    'retries': 3,
    'description': 'comp_store_lifecycle_dim_load_2656_napstore_insights DAG Description',
...
with DAG(
    ...
    parent_dag_name="comp_store_lifecycle_dim_load_2656_napstore_insights",
    ...
    livy_s3_json_load_0_stg_table = LivyOperator(
    task_id='s3_json_load_0_stg_table',
    ...
    app_args=['@s3://tf-nap-prod-airflow-nsk-insights/apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/2656/napstore/insights/user_config/argument_comp_store_lifecycle_dim_load_2656_napstore_insights_s3_json_load_0_stg_table.json', '--aws_user_role_external_id', Variable.get('aws_role_externalid')],
    driver_memory='12g',
    driver_cores=2,
    executor_memory='9g',
    executor_cores=5,
    session_name='comp_store_lifecycle_dim_load_2656_napstore_insights_s3_json_load_0_stg_table_'+CURRENT_TIMESTAMP.strftime('%Y%m%d_%H%M%S'),
    ...
    launch_s3_json_load_fct_table_sensor = monitor_k8s_api_job_status(
    dag=dag,
    connection_id='TECH_ISF_NAPSTORE_INSIGHTS_CONN_PROD',
    namespace='app02432',
    task_id='s3_json_load_fct_table_sensor',
    poke_interval=100,
    time_out=1800,
    job_name='s3-json-load-fct-table-comp-store-lifecycle-dim-load-2656-napst',
    nsk_cluster='nsk-miso-prod')
    ...
    livy_s3_json_load_orc_table = LivyOperator(
    task_id='s3_json_load_orc_table',
    app_file="s3a://tf-nap-prod-airflow-nsk-insights/apps/etl_framework/prod/jars/uber-onehop-etl-pipeline-2.0.1.jar",
    conn_id='TECH_ISF_NAP_STORE_TEAM_NSP_ETL_LIVY_PROD',
    class_name="com.nordstrom.nap.onehop.etl.app.ExecSQLWrapper",
    app_args=['@s3://tf-nap-prod-airflow-nsk-insights/apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/2656/napstore/insights/user_config/argument_comp_store_lifecycle_dim_load_2656_napstore_insights_s3_json_load_orc_table.json', '--aws_user_role_external_id', Variable.get('aws_role_externalid')],
    driver_memory='12g',
    driver_cores=2,
    executor_memory='9g',
    executor_cores=5,
    session_name='comp_store_lifecycle_dim_load_2656_napstore_insights_s3_json_load_orc_table_'+CURRENT_TIMESTAMP.strftime('%Y%m%d_%H%M%S'),
    spark_conf={'spark.sql.legacy.avro.datetimeRebaseModeInWrite': 'LEGACY', 'spark.yarn.maxAppAttempts': '1', 'spark.speculation': 'false', 'spark.airflow.run_id': '{{ ts_nodash }}', 'spark.dynamicAllocation.enabled': 'true', 'spark.dynamicAllocation.shuffleTracking.enabled': 'true', 'spark.dynamicAllocation.minExecutors': '5', 'spark.dynamicAllocation.maxExecutors': '30', 'spark.dynamicAllocation.initialExecutors': '10', 'spark.dynamicAllocation.executorIdleTimeout': '240'})


    livy_s3_json_load_orc_table_sensor = LivySensor(
    conn_id='TECH_ISF_NAP_STORE_TEAM_NSP_ETL_LIVY_PROD',
    task_id='s3_json_load_orc_table_sensor',
    batch_id="{{ task_instance.xcom_pull(task_ids='s3_json_load_orc_table') }}")

    create_persistent_cluster_nsp_etl >> [livy_s3_json_load_0_stg_table]
    livy_s3_json_load_0_stg_table >> [livy_s3_json_load_0_stg_table_sensor]
    livy_s3_json_load_0_stg_table_sensor >> [launch_s3_json_load_fct_table]
    launch_s3_json_load_fct_table >> [launch_s3_json_load_fct_table_sensor]
    launch_s3_json_load_fct_table_sensor >> [livy_s3_json_load_orc_table]
    livy_s3_json_load_orc_table >> [livy_s3_json_load_orc_table_sensor]
