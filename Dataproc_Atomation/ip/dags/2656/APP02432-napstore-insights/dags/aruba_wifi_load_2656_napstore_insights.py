...
default_args = {
    'retries': 3,
    'description': 'aruba_wifi_load_2656_napstore_insights DAG Description',
...
with DAG(
    ...
    parent_dag_name="aruba_wifi_load_2656_napstore_insights",
    ...
    livy_s3_avro_to_orc_load_0 = LivyOperator(
    task_id='s3_avro_to_orc_load_0',
    ...
    app_args=['@s3://tf-nap-prod-airflow-nsk-insights/apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/2656/napstore/insights/user_config/argument_aruba_wifi_load_2656_napstore_insights_s3_avro_to_orc_load_0.json', '--aws_user_role_external_id', Variable.get('aws_role_externalid')],
    driver_memory='12g',
    driver_cores=2,
    executor_memory='9g',
    executor_cores=5,
    session_name='aruba_wifi_load_2656_napstore_insights_s3_avro_to_orc_load_0_'+CURRENT_TIMESTAMP.strftime('%Y%m%d_%H%M%S'),
    ...
    livy_s3_avro_to_orc_load_0_sensor = LivySensor(
    conn_id='TECH_ISF_NAP_STORE_TEAM_NSP_ETL_LIVY_PROD',
    task_id='s3_avro_to_orc_load_0_sensor',
    batch_id="{{ task_instance.xcom_pull(task_ids='s3_avro_to_orc_load_0') }}")

    create_persistent_cluster_nsp_etl >> [livy_s3_avro_to_orc_load_0]
    livy_s3_avro_to_orc_load_0 >> [livy_s3_avro_to_orc_load_0_sensor]
