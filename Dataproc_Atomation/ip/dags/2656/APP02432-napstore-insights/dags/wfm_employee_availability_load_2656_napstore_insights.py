...
default_args = {
    'retries': 3,
    'description': 'wfm_employee_availability_load_2656_napstore_insights DAG Description',
...
with DAG(
    ...
    parent_dag_name="wfm_employee_availability_load_2656_napstore_insights",
    ...
    livy_wfm_employee_availability_load_0_stg_table = LivyOperator(
    task_id='wfm_employee_availability_load_0_stg_table',
    ...
    app_args=['@s3://tf-nap-prod-airflow-nsk-insights/apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/2656/napstore/insights/user_config/argument_wfm_employee_availability_load_2656_napstore_insights_wfm_employee_availability_load_0_stg_table.json', '--aws_user_role_external_id', Variable.get('aws_role_externalid')],
    driver_memory='9g',
    driver_cores=2,
    executor_memory='6g',
    executor_cores=5,
    num_executors=6,
    session_name='wfm_employee_availability_load_2656_napstore_insights_wfm_employee_availability_load_0_stg_table_'+CURRENT_TIMESTAMP.strftime('%Y%m%d_%H%M%S'),
    ...
    livy_wfm_employee_availability_load_0_stg_table_sensor = LivySensor(
    conn_id='TECH_ISF_NAP_STORE_TEAM_NSP_ETL_LIVY_PROD',
    task_id='wfm_employee_availability_load_0_stg_table_sensor',
    batch_id="{{ task_instance.xcom_pull(task_ids='wfm_employee_availability_load_0_stg_table') }}")

    launch_wfm_employee_availability_load_1_fct_table = launch_k8s_api_job_operator(
    dag=dag,
    connection_id='TECH_ISF_NAPSTORE_INSIGHTS_CONN_PROD',
    namespace='app02432',
    task_id='wfm_employee_availability_load_1_fct_table',
    job_name='wfm-employee-availability-load-1-fct-table-wfm-employee-availab',
    macros=None,
    container_image='artifactory.nordstrom.com/docker/app08499/insights-framework/etl-executor:0.4.1',
    container_command=['python', '/home/nonroot/etl_executor/main.py', '--sql_files', 'apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/2656/napstore/insights/sql/wfm_employee_availability_load_2656_napstore_insights_wfm_employee_availability_wfm_employee_availability_load_fact.sql', '--bucket', 'tf-nap-prod-airflow-nsk-insights', '--executor', 'teradata', '--host', 'tdnapprod.nordstrom.net', '--database', 'PRD_NAP_STG', '--vault_path', 'data/application/APP02432/2656/napstore/APP02432-napstore-insights/prod/teradata/teradata_db_credentials', '--vault_host', 'https://vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com:8200', '--mount_point', 'nordsecrets'],
    resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    common_envs=[],
    task_envs=[{'name': 'TIER', 'value': '3'}, {'name': 'STATSD_HOST', 'value': 'statsd.k8s-newrelic.svc.cluster.local'}, {'name': 'STATSD_PORT', 'value': '8128'}, {'name': 'APP_ID', 'value': 'APP02432'}, {'name': 'NEW_RELIC_APP_NAME', 'value': 'onehop-insights-framework-APP02432-PROD'}, {'name': 'ENV', 'value': 'PROD'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}, {'name': 'DAG_ID', 'value': 'wfm_employee_availability_load_2656_napstore_insights'}],
    ...
    launch_wfm_employee_availability_load_1_fct_table_sensor = monitor_k8s_api_job_status(
    dag=dag,
    connection_id='TECH_ISF_NAPSTORE_INSIGHTS_CONN_PROD',
    namespace='app02432',
    task_id='wfm_employee_availability_load_1_fct_table_sensor',
    poke_interval=100,
    time_out=1800,
    job_name='wfm-employee-availability-load-1-fct-table-wfm-employee-availab',
    nsk_cluster='nsk-miso-prod')
    ...
    create_persistent_cluster_nsp_etl >> [livy_wfm_employee_availability_load_0_stg_table]
    livy_wfm_employee_availability_load_0_stg_table >> [livy_wfm_employee_availability_load_0_stg_table_sensor]
    livy_wfm_employee_availability_load_0_stg_table_sensor >> [launch_wfm_employee_availability_load_1_fct_table]
    launch_wfm_employee_availability_load_1_fct_table >> [launch_wfm_employee_availability_load_1_fct_table_sensor]
