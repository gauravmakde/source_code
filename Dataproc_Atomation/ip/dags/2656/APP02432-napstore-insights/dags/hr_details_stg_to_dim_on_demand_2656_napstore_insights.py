...
cron = None
dag_id = 'hr_details_stg_to_dim_on_demand_2656_napstore_insights'
...
with DAG(
    ...
    launch_hr_details_events_load_1_details_dim_tables = launch_k8s_api_job_operator(
    dag=dag,
    connection_id='TECH_ISF_NAPSTORE_INSIGHTS_CONN_PROD',
    namespace='app02432',
    task_id='hr_details_events_load_1_details_dim_tables',
    job_name='hr-details-events-load-1-details-dim-tables-hr-details-stg-to-d',
    macros=None,
    container_image='artifactory.nordstrom.com/docker/app08499/insights-framework/etl-executor:0.4.1',
    container_command=['python', '/home/nonroot/etl_executor/main.py', '--sql_files', 'apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/2656/napstore/insights/sql/hr_details_stg_to_dim_on_demand_2656_napstore_insights_hr_details_events_hr_employee_status_details_load_dim.sql,apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/2656/napstore/insights/sql/hr_details_stg_to_dim_on_demand_2656_napstore_insights_hr_details_events_hr_job_details_load_dim.sql,apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/2656/napstore/insights/sql/hr_details_stg_to_dim_on_demand_2656_napstore_insights_hr_details_events_hr_line_position_details_load_dim.sql,apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/2656/napstore/insights/sql/hr_details_stg_to_dim_on_demand_2656_napstore_insights_hr_details_events_hr_org_details_load_dim.sql,apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/2656/napstore/insights/sql/hr_details_stg_to_dim_on_demand_2656_napstore_insights_hr_details_events_hr_work_contact_details_load_dim.sql', '--bucket', 'tf-nap-prod-airflow-nsk-insights', '--executor', 'teradata', '--host', 'tdnapprod.nordstrom.net', '--database', 'PRD_NAP_HR_STG', '--vault_path', 'data/application/APP02432/2656/napstore/APP02432-napstore-insights/prod/teradata/teradata_hr_db_credentials', '--vault_host', 'https://vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com:8200', '--mount_point', 'nordsecrets'],
    resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    common_envs=[],
    task_envs=[{'name': 'TIER', 'value': '3'}, {'name': 'STATSD_HOST', 'value': 'statsd.k8s-newrelic.svc.cluster.local'}, {'name': 'STATSD_PORT', 'value': '8128'}, {'name': 'APP_ID', 'value': 'APP02432'}, {'name': 'NEW_RELIC_APP_NAME', 'value': 'onehop-insights-framework-APP02432-PROD'}, {'name': 'ENV', 'value': 'PROD'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}, {'name': 'DAG_ID', 'value': 'hr_details_stg_to_dim_on_demand_2656_napstore_insights'}],
    ...
    launch_hr_details_events_load_1_details_dim_tables_sensor = monitor_k8s_api_job_status(
    dag=dag,
    connection_id='TECH_ISF_NAPSTORE_INSIGHTS_CONN_PROD',
    namespace='app02432',
    task_id='hr_details_events_load_1_details_dim_tables_sensor',
    poke_interval=100,
    time_out=1800,
    job_name='hr-details-events-load-1-details-dim-tables-hr-details-stg-to-d',
    nsk_cluster='nsk-miso-prod')
    ...
    launch_hr_details_events_load_1_details_dim_tables >> [launch_hr_details_events_load_1_details_dim_tables_sensor]
