...
************************************************************************************/
LOCK TABLE {db_env}_NAP_STG.INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_LDG FOR ACCESS
DELETE FROM {db_env}_NAP_STG.INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_LDG
WHERE (
  -- Primary key null check
  return_to_vendor_num is null
  or external_reference_num is null
  or sku_num is null
  or sku_type is null
...
-- Description             : write data from INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_LDG to INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_FACT table
-- Reference Documentation : https://confluence.nordstrom.com/pages/viewpage.action?pageId=1391364079
--/***********************************************************************************
---- Merge into final INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_FACT fact table
--************************************************************************************/

LOCK TABLE {db_env}_NAP_FCT.INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_FACT FOR ACCESS
MERGE INTO {db_env}_NAP_FCT.INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_FACT fact
USING (
        SELECT
        ...
        FROM (SELECT * 
                FROM {db_env}_NAP_BASE_VWS.INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_LDG
            QUALIFY ROW_NUMBER() OVER(PARTITION BY return_to_vendor_num, external_reference_num, sku_num, sku_type, location_num ORDER BY last_updated_sys_tmstp desc) = 1) as l
        LEFT JOIN (SELECT batch_id, batch_date 
                    FROM {db_env}_NAP_BASE_VWS.ETL_BATCH_INFO
                    WHERE interface_code = 'INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_FACT' 
                    AND dw_sys_end_tmstp IS NULL) bi
            ON 1=1
            ) as ldg
     ON fact.return_to_vendor_num = ldg.return_to_vendor_num
    AND fact.external_reference_num = ldg.external_reference_num
...
************************************************************************************/
COLLECT STATS ON {db_env}_NAP_FCT.INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_FACT
INDEX (return_to_vendor_num, external_reference_num, sku_num, location_num);
