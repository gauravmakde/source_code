...
SELECT batch_id
     , batch_date
     , '{db_env}_NAP_BASE_VWS.INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_FACT' as source_table
...
FROM (
  SELECT COUNT(DISTINCT return_to_vendor_num || external_reference_num || sku_num || sku_type || location_num) as fct_cnt
   FROM {db_env}_NAP_BASE_VWS.INVENTORY_DEXTR_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_FACT
   JOIN TMP_ETL_BATCH_INFO
     ON dw_batch_id = batch_id
 ) as F
CROSS JOIN (
   SELECT COUNT(distinct return_to_vendor_num || external_reference_num || sku_num || sku_type || location_num) as ldg_cnt
