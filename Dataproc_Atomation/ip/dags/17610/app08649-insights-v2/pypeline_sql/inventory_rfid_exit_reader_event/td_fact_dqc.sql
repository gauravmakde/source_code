...
SELECT BATCH_ID
 , BATCH_DATE
 , '{db_env}_NAP_BASE_VWS.INVENTORY_RFID_EXIT_READER_EVENT_FACT' as SOURCE_TABLE
...
FROM (SELECT COUNT(1) as FCT_CNT, BATCH_ID, BATCH_DATE
        FROM {db_env}_NAP_BASE_VWS.INVENTORY_RFID_EXIT_READER_EVENT_FACT
        JOIN #TMP_ETL_BATCH_INFO as b ON DW_BATCH_ID = BATCH_ID
       GROUP BY BATCH_ID, BATCH_DATE
 ) as F
CROSS JOIN (
 SELECT COUNT(1) as LDG_CNT
   FROM {db_env}_NAP_BASE_VWS.INVENTORY_RFID_EXIT_READER_EVENT_LDG
 ) as S
;
ET;
