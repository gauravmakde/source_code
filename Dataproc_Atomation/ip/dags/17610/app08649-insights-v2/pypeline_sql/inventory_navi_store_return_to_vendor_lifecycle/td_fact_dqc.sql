...
SELECT batch_id
     , batch_date
     , '{db_env}_NAP_BASE_VWS.INVENTORY_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_FACT' as source_table
...
FROM (
  SELECT COUNT(DISTINCT return_to_vendor_num || sku_num || sku_type || location_num) as fct_cnt
   FROM {db_env}_NAP_BASE_VWS.INVENTORY_NAVI_STORE_RETURN_TO_VENDOR_LIFECYCLE_FACT
