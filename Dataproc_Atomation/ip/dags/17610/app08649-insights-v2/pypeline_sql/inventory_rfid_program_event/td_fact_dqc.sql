...
*/
LOCK {db_env}_NAP_BASE_VWS.INVENTORY_RFID_PROGRAM_EVENT_LDG FOR ACCESS
INSERT INTO TMP_ETL_BATCH_DQC_LOG (batch_id, batch_date, source_table, error_status, error_message, dw_sys_tmstp)
SELECT batch_id
     , batch_date
     , '{db_env}_NAP_BASE_VWS.INVENTORY_RFID_PROGRAM_EVENT_FACT' as source_table
     ...
     , CURRENT_TIMESTAMP(6)
FROM (
  SELECT COUNT(DISTINCT rms_sku_id || event_type_code || CAST(event_tmstp AS VARCHAR(23))) as fct_cnt
   FROM {db_env}_NAP_BASE_VWS.INVENTORY_RFID_PROGRAM_EVENT_FACT
   JOIN TMP_ETL_BATCH_INFO
     ON dw_batch_id = batch_id
 ) as F
CROSS JOIN (
   SELECT COUNT(distinct rmsSkuId || eventType || eventTime) as ldg_cnt
   FROM {db_env}_NAP_BASE_VWS.INVENTORY_RFID_PROGRAM_EVENT_LDG
 ) as S
