...
SELECT batch_id
     , batch_date
     , '{db_env}_NAP_BASE_VWS.INVENTORY_STORE_NON_CUSTOMER_ORDER_SHIPPING_LABEL_GENERATED_FACT' as source_table
     ...
     , CURRENT_TIMESTAMP(6)
FROM (
  SELECT COUNT(DISTINCT carton_number || tracking_number || product_sku_id || CAST(event_tmstp AS VARCHAR(23)))
  as fct_cnt
   FROM {db_env}_NAP_BASE_VWS.INVENTORY_STORE_NON_CUSTOMER_ORDER_SHIPPING_LABEL_GENERATED_FACT
   JOIN TMP_ETL_BATCH_INFO
     ON dw_batch_id = batch_id
 ) as F
CROSS JOIN (
   SELECT COUNT(distinct cartonNumber || shippingLabelDetails_trackingNumber || itemDetails_productSku_id || eventTime)
   as ldg_cnt
   FROM {db_env}_NAP_BASE_VWS.INVENTORY_STORE_NON_CUSTOMER_ORDER_SHIPPING_LABEL_GENERATED_LDG
 ) as S
