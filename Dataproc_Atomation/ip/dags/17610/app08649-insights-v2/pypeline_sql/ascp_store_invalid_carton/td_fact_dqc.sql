...
(
  ...
  WHERE subject_area_nm = 'NAP_ASCP_STORE_INVALID_CARTON'
)
...
*/
LOCK {db_env}_NAP_BASE_VWS.STORE_INVALID_CARTON_FACT FOR ACCESS
INSERT INTO TMP_ETL_BATCH_DQC_LOG (batch_id, batch_date, source_table, error_status, error_message, dw_sys_tmstp)
SELECT batch_id
     , batch_date
     , '{db_env}_NAP_BASE_VWS.STORE_INVALID_CARTON_FACT' as source_table
     ...
     , CURRENT_TIMESTAMP(6)
FROM (
  SELECT COUNT(DISTINCT carton_num) as fct_cnt
   FROM {db_env}_NAP_BASE_VWS.STORE_INVALID_CARTON_FACT
   JOIN TMP_ELT_CONTROL
     ON dw_batch_id = batch_id
 ) as F
CROSS JOIN (
   SELECT COUNT(distinct carton_num) as ldg_cnt
   FROM {db_env}_NAP_BASE_VWS.STORE_INVALID_CARTON_LDG
 ) as S
