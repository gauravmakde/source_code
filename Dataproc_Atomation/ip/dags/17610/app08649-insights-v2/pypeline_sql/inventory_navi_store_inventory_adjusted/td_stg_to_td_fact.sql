...
-- Script Details
--*************************************************************************************************************************************
-- File                    : inventory_navi_store_inventory_adjusted/td_stg_to_td_fact.sql
-- Author                  : Dogukan Ulu
-- Description             : ETL to write data from staging table <DBENV>_NAP_STG.INVENTORY_NAVI_STORE_INVENTORY_ADJUSTED_LDG into fact table <DBENV>_NAP_FCT.INVENTORY_NAVI_STORE_INVENTORY_ADJUSTED_FACT.
-- Data Source             : Staging table <DBENV>_NAP_STG.INVENTORY_NAVI_STORE_INVENTORY_ADJUSTED_LDG
-- ETL Run Frequency       : Daily
--*************************************************************************************************************************************
-- Create temporary table to get latest data from staging table without duplicates of records
CREATE VOLATILE MULTISET TABLE  #NAVI_STORE_INVENTORY_ADJUSTED_EVENT_DETAIL_LDG
AS
    (    SELECT
              locationId
            , id
            ...
            , eventTime
           FROM {db_env}_NAP_BASE_VWS.INVENTORY_NAVI_STORE_INVENTORY_ADJUSTED_LDG
...
COLLECT STATISTICS INDEX (inventoryAdjustmentId, adjustmentDetails_sku_Id) ON #NAVI_STORE_INVENTORY_ADJUSTED_EVENT_DETAIL_LDG;
ET;

--Create temporary table with all of transformation and renaming all columns to target table names
CREATE VOLATILE MULTISET TABLE  #NAVI_STORE_INVENTORY_ADJUSTED_EVENT_DETAIL_LDG_FINAL
AS 
(WITH  ETL_BATCH_INFO AS
    (SELECT BATCH_ID, BATCH_DATE 
       FROM {db_env}_NAP_BASE_VWS.ETL_BATCH_INFO 
      WHERE INTERFACE_CODE = 'INVENTORY_NAVI_STORE_INVENTORY_ADJUSTED_FACT'
      ...
      SELECT
        ...
        FROM (
            ...
            FROM #NAVI_STORE_INVENTORY_ADJUSTED_EVENT_DETAIL_LDG
...
COLLECT STATISTICS INDEX (adjustment_id,sku_num) ON #NAVI_STORE_INVENTORY_ADJUSTED_EVENT_DETAIL_LDG_FINAL;
ET;

LOCK TABLE {db_env}_NAP_FCT.INVENTORY_NAVI_STORE_INVENTORY_ADJUSTED_FACT FOR ACCESS
MERGE INTO {db_env}_NAP_FCT.INVENTORY_NAVI_STORE_INVENTORY_ADJUSTED_FACT fct
USING
    (
        SELECT
              location_id
            ...
            , dw_sys_load_tmstp
            , dw_sys_updt_tmstp
        FROM #NAVI_STORE_INVENTORY_ADJUSTED_EVENT_DETAIL_LDG_FINAL
    ) stg
