...
(
  ...
  WHERE subject_area_nm = 'NAP_ASCP_STORE_RETURN_TO_VENDOR_DETAIL_FACT'
)
...
*/
LOCK {db_env}_NAP_BASE_VWS.STORE_RETURN_TO_VENDOR_DETAIL_FACT FOR ACCESS
INSERT INTO TMP_ETL_BATCH_DQC_LOG (batch_id, batch_date, source_table, error_status, error_message, dw_sys_tmstp)
SELECT batch_id
     , batch_date
     , '{db_env}_NAP_BASE_VWS.STORE_RETURN_TO_VENDOR_DETAIL_FACT' as source_table
     ...
     , CURRENT_TIMESTAMP(6)
FROM (
  SELECT COUNT(DISTINCT return_to_vendor_num || sku_num || sku_type || location_num) as fct_cnt
   FROM {db_env}_NAP_BASE_VWS.STORE_RETURN_TO_VENDOR_DETAIL_FACT
   JOIN TMP_ELT_CONTROL
     ON dw_batch_id = batch_id
 ) as F
CROSS JOIN (
   SELECT COUNT(distinct return_to_vendor_num || sku_num || sku_type || location_num) as ldg_cnt
   FROM {db_env}_NAP_BASE_VWS.STORE_RETURN_TO_VENDOR_DETAIL_LDG
 ) as S
