...
CREATE MULTISET VOLATILE TABLE PAYMENT_REFUND_HDR_TEMP AS (
    SELECT *
    FROM {db_env}_NAP_BASE_VWS.PAYMENT_REFUND_HDR_LDG
    QUALIFY
        RANK() OVER (
...
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_REFUND_HDR_FACT
WHERE EVENT_ID IN (SELECT DISTINCT EVENT_ID FROM {db_env}_NAP_BASE_VWS.PAYMENT_REFUND_HDR_LDG);

INSERT INTO {db_env}_NAP_BASE_VWS.PAYMENT_REFUND_HDR_FACT
(
  EVENT_ID,
  EVENT_TYPE,
  EVENT_TIME,
...
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_REFUND_HDR_LDG;
