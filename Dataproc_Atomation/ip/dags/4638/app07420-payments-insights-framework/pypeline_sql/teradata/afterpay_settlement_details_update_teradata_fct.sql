...
CREATE MULTISET VOLATILE TABLE TEMP_PAYMENT_SETTLEMENT_RESULT_DETAILS_STG AS (
    ...
    FROM {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_RESULT_FACT AS PAR
    INNER JOIN {db_env}_NAP_BASE_VWS.PAYMENT_BANK_CARD_SETTLEMENT_RESULT_FACT AS PBC
    ON PAR.SETTLEMENT_RESULT_RECEIVED_ID = PBC.SETTLEMENT_RESULT_RECEIVED_ID
    WHERE PAR.DW_SYS_UPDT_TMSTP > (SELECT coalesce(max(cast(DW_SYS_UPDT_TMSTP AS DATE)) - 2, cast('2023-11-01' AS DATE)) FROM {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_RESULT_DETAILS_FACT)
...
--Delete if exist
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_RESULT_DETAILS_FACT
WHERE
...
INSERT INTO {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_RESULT_DETAILS_FACT (
    DW_SYS_LOAD_TMSTP,
    DW_SYS_UPDT_TMSTP,
