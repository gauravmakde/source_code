...
CREATE MULTISET VOLATILE TABLE SETTLEMENT_INTENT_STG_TEMP AS (
    SELECT *
    FROM {db_env}_NAP_STG.PAYMENT_SETTLEMENT_INTENT_LDG
    QUALIFY
        row_number() OVER (
...
CREATE MULTISET VOLATILE TABLE BANK_CARD_SETTLEMENT_RESULTS_STG_TEMP AS (
    SELECT *
    FROM {db_env}_NAP_STG.BANKCARD_SETTLEMENT_RESULTS_LDG
    QUALIFY
        row_number() OVER (
            PARTITION BY
...
CREATE MULTISET VOLATILE TABLE GIFTCARD_NOTE_TENDER_RESULTS_STG_TEMP AS (
    SELECT *
    FROM {db_env}_NAP_STG.GIFTCARD_NOTE_TENDER_RESULTS_LDG
    QUALIFY
        row_number() OVER (
            PARTITION BY
                TRANSACTION_IDENTIFIER_ID
...
PAYPAL_BILLING_AGREEMENT_SETTLEMENT_RESULTS_STG_TEMP AS (
    SELECT *
    FROM {db_env}_NAP_STG.PAYPAL_BILLING_AGREEMENT_SETTLEMENT_RESULTS_LDG
    QUALIFY
        row_number() OVER (
            PARTITION BY
                TRANSACTION_IDENTIFIER_ID
...
CREATE MULTISET VOLATILE TABLE PAYPAL_SETTLEMENT_RESULT_STG_TEMP AS (
    SELECT *
    FROM {db_env}_NAP_STG.PAYPAL_SETTLEMENT_RESULT_LDG
    QUALIFY
        row_number() OVER (
            PARTITION BY
                TRANSACTION_IDENTIFIER_ID
...
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_INTENT_FACT
WHERE
    TRANSACTION_IDENTIFIER_ID IN (SELECT TRANSACTION_IDENTIFIER_ID FROM SETTLEMENT_INTENT_STG_TEMP);

-- Insert new event data
INSERT INTO {db_env}_NAP_BASE_VWS.PAYMENT_SETTLEMENT_INTENT_FACT (
    DW_SYS_LOAD_TMSTP,
    DW_SYS_UPDT_TMSTP,
    DW_BATCH_DATE,
    EVENT_TIME,
...
-- Purge and reprocess data to account for updates
DELETE FROM {db_env}_NAP_BASE_VWS.BANKCARD_SETTLEMENT_RESULTS_FACT
WHERE
    TRANSACTION_IDENTIFIER_ID IN (SELECT TRANSACTION_IDENTIFIER_ID FROM BANK_CARD_SETTLEMENT_RESULTS_STG_TEMP);

-- Insert new event data
INSERT INTO {db_env}_NAP_BASE_VWS.BANKCARD_SETTLEMENT_RESULTS_FACT (
    DW_SYS_LOAD_TMSTP,
    DW_SYS_UPDT_TMSTP,
    DW_BATCH_DATE,
    TRANSACTION_TIME,
...
-- Purge and reprocess data to account for updates
DELETE FROM {db_env}_NAP_BASE_VWS.GIFTCARD_NOTE_TENDER_RESULTS_FACT
WHERE
    TRANSACTION_IDENTIFIER_ID IN (SELECT TRANSACTION_IDENTIFIER_ID FROM GIFTCARD_NOTE_TENDER_RESULTS_STG_TEMP);

-- Insert new event data
INSERT INTO {db_env}_NAP_BASE_VWS.GIFTCARD_NOTE_TENDER_RESULTS_FACT (
    DW_SYS_LOAD_TMSTP,
    DW_SYS_UPDT_TMSTP,
    DW_BATCH_DATE,
    TRANSACTION_TIME,
...
-- Purge and reprocess data to account for updates
DELETE FROM {db_env}_NAP_BASE_VWS.PAYPAL_BILLING_AGREEMENT_SETTLEMENT_RESULTS_FACT
WHERE
...
-- Insert new event data
INSERT INTO {db_env}_NAP_BASE_VWS.PAYPAL_BILLING_AGREEMENT_SETTLEMENT_RESULTS_FACT (
    DW_SYS_LOAD_TMSTP,
    DW_SYS_UPDT_TMSTP,
    DW_BATCH_DATE,
    TRANSACTION_TIME,
...
-- Purge and reprocess data to account for updates
DELETE FROM {db_env}_NAP_BASE_VWS.PAYPAL_SETTLEMENT_RESULT_FACT
WHERE
    TRANSACTION_IDENTIFIER_ID IN (SELECT TRANSACTION_IDENTIFIER_ID FROM PAYPAL_SETTLEMENT_RESULT_STG_TEMP);

-- Insert new event data
INSERT INTO {db_env}_NAP_BASE_VWS.PAYPAL_SETTLEMENT_RESULT_FACT (
    DW_SYS_LOAD_TMSTP,
    DW_SYS_UPDT_TMSTP,
    DW_BATCH_DATE,
    TRANSACTION_TIME,
...
DELETE FROM {db_env}_NAP_STG.BANKCARD_SETTLEMENT_RESULTS_LDG;
DELETE FROM {db_env}_NAP_STG.GIFTCARD_NOTE_TENDER_RESULTS_LDG;
DELETE FROM {db_env}_NAP_STG.PAYMENT_SETTLEMENT_INTENT_LDG;
DELETE FROM {db_env}_NAP_STG.PAYPAL_BILLING_AGREEMENT_SETTLEMENT_RESULTS_LDG;
DELETE FROM {db_env}_NAP_STG.PAYPAL_SETTLEMENT_RESULT_LDG;
