...
CREATE MULTISET VOLATILE TABLE PAYMENT_RESULT_BIN_V2_LDG_TEMP AS (
    SELECT DISTINCT *
    FROM {db_env}_NAP_PYMNT_FRAUD_STG.PAYMENT_RESULT_BIN_V2_LDG
    QUALIFY
        ROW_NUMBER() OVER (
...
DELETE FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.PAYMENT_RESULT_BIN_V2_FACT
WHERE SOURCE_ID IN (SELECT DISTINCT SOURCE_ID FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.PAYMENT_RESULT_BIN_V2_LDG);

INSERT INTO {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.PAYMENT_RESULT_BIN_V2_FACT
(
    DW_SYS_LOAD_TMSTP,
    DW_SYS_UPDT_TMSTP,
    SOURCE_ID,
...
DELETE FROM {db_env}_NAP_PYMNT_FRAUD_BASE_VWS.PAYMENT_RESULT_BIN_V2_LDG;
