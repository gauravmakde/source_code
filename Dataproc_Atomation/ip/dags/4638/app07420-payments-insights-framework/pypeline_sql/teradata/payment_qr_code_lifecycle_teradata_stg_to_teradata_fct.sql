...
CREATE MULTISET VOLATILE TABLE QR_CODE_LIFECYCLE_STG_TEMP AS (
    SELECT *
    FROM {db_env}_NAP_STG.PAYMENT_QR_CODE_LIFECYCLE_LDG
    QUALIFY
        RANK() OVER (
...
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_QR_CODE_LIFECYCLE_FACT
WHERE
    QR_CODE_ID IN (SELECT DISTINCT QR_CODE_ID FROM QR_CODE_LIFECYCLE_STG_TEMP);

-- Insert new event data
INSERT INTO {db_env}_NAP_BASE_VWS.PAYMENT_QR_CODE_LIFECYCLE_FACT (
    QR_CODE_ID,
    LAST_UPDATED_TIME,
    CREATED_TIMESTAMP,
    CREATED_DATE,
...
UPDATE TGT
FROM {db_env}_NAP_BASE_VWS.PAYMENT_QR_CODE_LIFECYCLE_FACT TGT,
 (
  SELECT DISTINCT
            QR_CODE_ID,
            LAST_UPDATED_TIME
...
UPDATE {db_env}_NAP_BASE_VWS.PAYMENT_QR_CODE_LIFECYCLE_FACT
...
DELETE FROM {db_env}_NAP_BASE_VWS.PAYMENT_QR_CODE_LIFECYCLE_LDG;
