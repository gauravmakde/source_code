...
CREATE MULTISET VOLATILE TABLE TEMP_PAYMENT_AUTHORIZATION_STG AS (
    SELECT DISTINCT *
    FROM PREPROD_NAP_STG.PAYMENT_AUTHORIZATION_LDG
    QUALIFY
        row_number() OVER (
            PARTITION BY
                ID
...
--Delete if exist
DELETE FROM PREPROD_NAP_BASE_VWS.PAYMENT_AUTHORIZATION_FACT
WHERE
    ID IN (SELECT ID FROM TEMP_PAYMENT_AUTHORIZATION_STG);

-- Insert into FACT table
INSERT INTO PREPROD_NAP_BASE_VWS.PAYMENT_AUTHORIZATION_FACT (
    DW_SYS_LOAD_TMSTP,
    DW_SYS_UPDT_TMSTP,
    ID,
    LAST_UPDATED_TIME,
...
DELETE FROM PREPROD_NAP_STG.PAYMENT_AUTHORIZATION_LDG;
