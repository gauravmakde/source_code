...
--reading the cancel failed hdr fields
CREATE TEMPORARY VIEW cancel_failed_hdr_temp AS
SELECT
...
INSERT INTO TABLE payment_cancel_failed_hdr_stg
SELECT DISTINCT
    event_id,
    order_number,
    event_time,
...
--reading the cancel failed dtl fields exploding orderLineId
CREATE TEMPORARY VIEW cancel_failed_dtl_temp1 AS
WITH cancel_failed_orderLineId_exploded AS (
    SELECT
        element_at(headers, 'Id') AS event_id,
        element_at(headers, 'Type') AS event_type,
        element_at(transform_keys(headers, (k, v) -> lower(k)), 'nord-test') AS nord_test,
...
FROM cancel_failed_orderLineId_exploded;

--reading the cancel failed dtl fields exploding serviceLineId
CREATE TEMPORARY VIEW cancel_failed_dtl_temp2 AS
WITH cancel_failed_serviceLineId_exploded AS (
    SELECT
        element_at(headers, 'Id') AS event_id,
        element_at(headers, 'Type') AS event_type,
        element_at(transform_keys(headers, (k, v) -> lower(k)), 'nord-test') AS nord_test,
...
SELECT
    ...
    service_line_id
FROM cancel_failed_serviceLineId_exploded;

-- union of above two temp views
CREATE TEMPORARY VIEW cancel_failed_dtl_temp_union
AS
SELECT DISTINCT * FROM cancel_failed_dtl_temp1
UNION
SELECT DISTINCT * FROM cancel_failed_dtl_temp2;


--inserting into the teradata dtl stg table
INSERT INTO TABLE payment_cancel_failed_dtl_stg
SELECT DISTINCT
    event_id,
    event_time,
    event_type,
    ...
    order_line_id,
    service_line_id
FROM cancel_failed_dtl_temp_union
WHERE
    -- Header filtering based on environment
    (
        ('{db_env}' = 'PRD' AND nord_test IS NULL AND nord_load IS NULL)
...
--reading the cancel failed tender - giftCardRefundResults
CREATE TEMPORARY VIEW cancel_failed_giftCardRefund AS
WITH cancel_failed_giftCardRefund_exploded AS (
    SELECT
        cast(cast(element_at(headers, 'EventTime') AS BIGINT) / 1e3 AS TIMESTAMP) AS event_time,
...
FROM cancel_failed_giftCardRefund_exploded;

--reading the cancel failed tender - giftCardNoteUnlockResults
CREATE TEMPORARY VIEW cancel_failed_giftCardNoteUnlock AS
WITH cancel_failed_giftCardNoteUnlock_exploded AS (
    SELECT
        cast(cast(element_at(headers, 'EventTime') AS BIGINT) / 1e3 AS TIMESTAMP) AS event_time,
...
SELECT
    ...
    NULL AS paypal_payer_account_country,
    NULL AS note_refund_reason
FROM cancel_failed_giftCardNoteUnlock_exploded;

--reading the cancel failed tender - nordstromNoteRefundedResults
CREATE TEMPORARY VIEW cancel_failed_nordstromNoteRefunded AS
WITH cancel_failed_nordstromNoteRefunded_exploded AS (
    SELECT
        cast(cast(element_at(headers, 'EventTime') AS BIGINT) / 1e3 AS TIMESTAMP) AS event_time,
...
FROM cancel_failed_nordstromNoteRefunded_exploded;

--reading the cancel failed tender - bankCardOrderCanceledResultV2
CREATE TEMPORARY VIEW cancel_failed_bankCardOrderCanceledResultV2 AS
WITH cancel_failed_bankCardOrderCanceledV2 AS (
    SELECT
        cast(cast(element_at(headers, 'EventTime') AS BIGINT) / 1e3 AS TIMESTAMP) AS event_time,
...
SELECT
    ...
    NULL AS paypal_payer_account_country,
    NULL AS note_refund_reason
FROM cancel_failed_bankCardOrderCanceledV2;

--reading the cancel failed tender - afterPayCanceledResult
CREATE TEMPORARY VIEW cancel_failed_afterPayCanceledResult AS
WITH cancel_failed_afterPayCanceled AS (
    SELECT
        cast(cast(element_at(headers, 'EventTime') AS BIGINT) / 1e3 AS TIMESTAMP) AS event_time,
...
SELECT
    ...
    NULL AS paypal_payer_account_country,
    NULL AS note_refund_reason
FROM cancel_failed_afterPayCanceled;

--reading the cancel failed tender - payPalOrderCanceledResult
CREATE TEMPORARY VIEW cancel_failed_payPalOrderCanceledResult AS
WITH cancel_failed_payPalOrderCanceled AS (
    SELECT
        cast(cast(element_at(headers, 'EventTime') AS BIGINT) / 1e3 AS TIMESTAMP) AS event_time,
...
SELECT
    ...
    NULL AS paypal_payer_account_country,
    NULL AS note_refund_reason
FROM cancel_failed_payPalOrderCanceled;

--reading the cancel failed tender - payPalBillingAgreementOrderCanceledResult
CREATE TEMPORARY VIEW cancel_failed_payPalBillingAgreementOrderCanceledResult AS
WITH cancel_failed_payPalBillingAgreementOrderCanceled AS (
    SELECT
        cast(cast(element_at(headers, 'EventTime') AS BIGINT) / 1e3 AS TIMESTAMP) AS event_time,
...
SELECT
    ...
    NULL AS note_refund_reason
FROM cancel_failed_payPalBillingAgreementOrderCanceled;

--unioning all failed tender types
CREATE TEMPORARY VIEW cancel_failed_tender_union
AS
SELECT DISTINCT * FROM cancel_failed_giftCardRefund
UNION
SELECT DISTINCT * FROM cancel_failed_giftCardNoteUnlock
UNION
SELECT DISTINCT * FROM cancel_failed_nordstromNoteRefunded
UNION
SELECT DISTINCT * FROM cancel_failed_bankCardOrderCanceledResultV2
UNION
SELECT DISTINCT * FROM cancel_failed_afterPayCanceledResult
UNION
SELECT DISTINCT * FROM cancel_failed_payPalOrderCanceledResult
UNION
SELECT DISTINCT * FROM cancel_failed_payPalBillingAgreementOrderCanceledResult;

--inserting into the teradata tender stg table
INSERT INTO TABLE payment_cancel_failed_tender_stg
SELECT DISTINCT
    event_id,
    order_number,
    event_time,
    ...
    note_refund_reason
FROM cancel_failed_tender_union
WHERE
    -- Header filtering based on environment
    (
        ('{db_env}' = 'PRD' AND nord_test IS NULL AND nord_load IS NULL)
