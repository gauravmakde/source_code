...
CREATE TEMPORARY VIEW temp_kafka_removed_from_fraud_blocklist AS
SELECT * FROM kafka_removed_from_fraud_blocklist;

-- building the fraud_blocklist_activity table
--reading the fraud_blocklist_activity fields
CREATE TEMPORARY VIEW removed_from_fraud_blocklist_temp AS
SELECT
    cast(eventTime AS TIMESTAMP) AS event_time,
    'RemovedFromFraudBlocklist' AS activity_type,
    ...
    reason AS reason
FROM temp_kafka_removed_from_fraud_blocklist;

--inserting into the teradata stg table
INSERT INTO TABLE removed_from_fraud_blocklist_stg
SELECT DISTINCT
    event_time,
    activity_type,
    customer_id,
    customer_id_type,
    reason
FROM removed_from_fraud_blocklist_temp
WHERE
