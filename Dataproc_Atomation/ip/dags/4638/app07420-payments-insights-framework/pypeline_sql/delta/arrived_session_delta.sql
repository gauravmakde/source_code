-- noqa: disable=all
CREATE TABLE IF NOT EXISTS {delta_schema_name}.customer_activity_arrived_session_delta
(
  sessionId string,
  ...
  credentials struct<iconid:string,shopperid:string,tokenizingcreditcardhvt:struct<value:string,authority:string,strategy:string,dataclassification:string>,creditcardhvt:struct<value:string,authority:string,dataclassification:string>,fraudtoken:string,useragent:string,tokenizingipaddress:struct<value:string,authority:string,strategy:string,dataclassification:string>,ipaddress:struct<value:string,authority:string,dataclassification:string>,experimentid:string,adtrackingstatus:string,advertisingid:string,applevendorid:string,firebaseinstanceid:string,locationtracking:string,isnotificationsenabled:string,ispreciselocationenabled:string>,
  ...
  screendimensions struct<screenwidth:struct<unit:string,quantity:int>,screenheight:struct<unit:string,quantity:int>,windowwidth:struct<unit:string,quantity:int>,windowheight:struct<unit:string,quantity:int>,scrollwidth:struct<unit:string,quantity:int>>,
...
LOCATION 's3://{delta_s3_bucket}/session_event_delta/customer_activity_arrived_session_delta/';
-- noqa: enable=all

CREATE TEMPORARY VIEW customer_activity_arrived_dedupe AS
...
SELECT
    ...
    value.requestedDestination as requestedDestination,
    value.actualDestination as actualDestination,
    value.credentials as credentials,
    value.context as context,
    value.screenDimensions as screenDimensions,
    ...
    as rownumber
FROM {event_parquet_schema_name}.customer_activity_arrived_parquet
...
MERGE INTO {delta_schema_name}.customer_activity_arrived_session_delta AS TGT
USING (
...
CREATE TEMPORARY VIEW customer_activity_arrived_session_delta_temp AS
WITH arrived_session AS (
    select *
    from {delta_schema_name}.session_analytical_object_flatten_delta
    where session_event_name = 'com.nordstrom.customer.Arrived'
),

arrived_event_dedupe AS (
    select *
    from customer_activity_arrived_dedupe
    where rownumber = 1
)
...
from arrived_session AS a
left join arrived_event_dedupe AS b
    on
        a.session_event_id = b.eventId;

INSERT INTO TABLE {delta_schema_name}.customer_activity_arrived_session_delta PARTITION (activity_date)
SELECT
    sessionId,
    ...
    rcd_load_tmstp,
    rcd_update_tmstp
from customer_activity_arrived_session_delta_temp;

VACUUM {delta_schema_name}.customer_activity_arrived_session_delta;

-- noqa: disable=all
OPTIMIZE {delta_schema_name}.customer_activity_arrived_session_delta ZORDER BY (sessionid);
-- noqa: enable=all
