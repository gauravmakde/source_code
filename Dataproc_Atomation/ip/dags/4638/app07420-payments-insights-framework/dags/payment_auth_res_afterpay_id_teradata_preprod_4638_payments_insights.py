...
cron = None
dag_id = 'payment_auth_res_afterpay_id_teradata_preprod_4638_payments_insights'
...
default_args = {
    'retries': 5,
    'description': 'payment_auth_res_afterpay_id_teradata_preprod_4638_payments_insights DAG Description',
...
with DAG(
    ...
    launch_payment_auth_res_afterpay_id_teradata_stg_to_teradata_fct_stage_0_teradata_stg_to_teradata_fct_job = launch_k8s_api_job_operator(
    dag=dag,
    connection_id='TECH_ISF_NAP_PAYMENTS_K8S_NSK_CONN_V2_PROD',
    namespace='app07420',
    task_id='payment_auth_res_afterpay_id_teradata_stg_to_teradata_fct_stage_0_teradata_stg_to_teradata_fct_job',
    job_name='payment-auth-res-afterpay-id-teradata-stg-to-teradata-fct-stage',
    macros=None,
    container_image='artifactory.nordstrom.com/docker/app08499/insights-framework/etl-executor:0.4.1',
    container_command=['python', '/home/nonroot/etl_executor/main.py', '--sql_files', 'apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/4638/payments/insights/sql/teradata/payment_auth_res_afterpay_id_teradata_preprod_4638_payments_insights_payment_auth_res_afterpay_id_teradata_stg_to_teradata_fct_stage_0_payment_auth_result_afterpay_id_teradata_stg_to_teradata_fct_preprod.sql', '--bucket', 'tf-nap-prod-credit-payments-isf-airflow', '--executor', 'teradata', '--host', 'tdnapprodcop1.nordstrom.net', '--database', 'PREPROD_NAP_FCT', '--vault_path', 'data/application/APP07420/4638/payments/insights/preprod/teradata/teradata_db_credentials', '--vault_host', 'https://vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com:8200', '--mount_point', 'nordsecrets'],
    resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    common_envs=[],
    task_envs=[{'name': 'TIER', 'value': '3'}, {'name': 'STATSD_HOST', 'value': 'gostatsd.kube-system.svc.cluster.local.'}, {'name': 'STATSD_PORT', 'value': '8128'}, {'name': 'APP_ID', 'value': 'app07420'}, {'name': 'NEW_RELIC_APP_NAME', 'value': 'onehop-insights-framework-app07420-PROD'}, {'name': 'ENV', 'value': 'PROD'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}, {'name': 'DAG_ID', 'value': 'payment_auth_res_afterpay_id_teradata_preprod_4638_payments_insights'}],
    ...
    launch_payment_auth_res_afterpay_id_teradata_stg_to_teradata_fct_stage_0_teradata_stg_to_teradata_fct_job_sensor = monitor_k8s_api_job_status(
    dag=dag,
    connection_id='TECH_ISF_NAP_PAYMENTS_K8S_NSK_CONN_V2_PROD',
    namespace='app07420',
    task_id='payment_auth_res_afterpay_id_teradata_stg_to_teradata_fct_stage_0_teradata_stg_to_teradata_fct_job_sensor',
    poke_interval=100,
    time_out=1800,
    job_name='payment-auth-res-afterpay-id-teradata-stg-to-teradata-fct-stage',
    nsk_cluster='nsk-gumbo-prod')
    ...
    launch_payment_auth_res_afterpay_id_teradata_stg_to_teradata_fct_stage_0_teradata_stg_to_teradata_fct_job >> [launch_payment_auth_res_afterpay_id_teradata_stg_to_teradata_fct_stage_0_teradata_stg_to_teradata_fct_job_sensor]
