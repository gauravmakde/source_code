...
cron = '0 10 * * 3'
dag_id = 'fraud_worldpay_bin_4638_payments_insights'
...
default_args = {
    'retries': 3,
    'description': 'fraud_worldpay_bin_4638_payments_insights DAG Description',
...
with DAG(
    ...
    command="/db/teradata/bin/tpt_load.sh  -e PRD_NAP_PYMNT_FRAUD_UTL -j WORLDPAY_BIN_LDG_TPT_ENGINE_JOB -h tdnapprod.nordstrom.net -l 'N' -u SCH_NAP_PYMNT_FRAUD_BATCH -p \"\$tdwallet(SCH_NAP_PYMNT_FRAUD_BATCH_PWD)\" -f s3://tf-nap-prod-master-merchant-bin-files/raw/* -o UPDATE -r TD-UTILITIES-EC2",
    timeout=1800,
    ...
    parent_dag_name="fraud_worldpay_bin_4638_payments_insights",
    ...
    app_args=['@s3://tf-nap-prod-credit-payments-isf-airflow/apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/4638/payments/insights/user_config/argument_fraud_worldpay_bin_4638_payments_insights_read_td_write_td_stage_td_dim_to_td_stg_job.json', '--aws_user_role_external_id', Variable.get('aws_role_externalid')],
    driver_memory='12g',
    driver_cores=2,
    executor_memory='9g',
    executor_cores=5,
    session_name='fraud_worldpay_bin_4638_payments_insights_read_td_write_td_stage_td_dim_to_td_stg_job_'+CURRENT_TIMESTAMP.strftime('%Y%m%d_%H%M%S'),
    ...
    macros=None,
    container_image='artifactory.nordstrom.com/docker/app08499/insights-framework/etl-executor:0.4.1',
    container_command=['python', '/home/nonroot/etl_executor/main.py', '--sql_files', 'apps/etl_framework/airflow_nsk_dag_job_artifacts/prod/4638/payments/insights/sql/teradata/fraud_worldpay_bin_4638_payments_insights_read_td_write_td_stage_fraud_worldpay_rollup_dim.sql', '--bucket', 'tf-nap-prod-credit-payments-isf-airflow', '--executor', 'teradata', '--host', 'tdnapprodcop1.nordstrom.net', '--database', 'PRD_NAP_PYMNT_FRAUD_BASE_VWS', '--vault_path', 'data/application/APP07420/4638/payments/insights/prod/teradata/payment_fraud/teradata_db_credentials', '--vault_host', 'https://vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com:8200', '--mount_point', 'nordsecrets'],
    resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    common_envs=[],
    task_envs=[{'name': 'TIER', 'value': '3'}, {'name': 'STATSD_HOST', 'value': 'gostatsd.kube-system.svc.cluster.local.'}, {'name': 'STATSD_PORT', 'value': '8128'}, {'name': 'APP_ID', 'value': 'app07420'}, {'name': 'NEW_RELIC_APP_NAME', 'value': 'onehop-insights-framework-app07420-PROD'}, {'name': 'ENV', 'value': 'PROD'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}, {'name': 'DAG_ID', 'value': 'fraud_worldpay_bin_4638_payments_insights'}],
    metadata={'annotations': {'kube2iam.beta.nordstrom.net/role': 'arn:aws:iam::594668836985:role/k8s/tf-nap-prod-payments-k8s'}, 'labels': {'nap_domain': 'onehop-etl', 'nap_project': 'insights-framework', 'customer_team_name': 'payments', 'customer_project_name': 'insights'}},
