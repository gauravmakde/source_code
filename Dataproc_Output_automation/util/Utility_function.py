...
class Util:
    ...
    def fetch_json_sql(self,file,error):

        # print(file,error)
        dag_dir_name=os.path.dirname(file)
        base_dir=os.path.dirname(dag_dir_name)
        details_dataproc=[]
        # print(base_dir)
        with open(file, "r") as f:
            py = f.read()
        # print(py)
        regex_pattern = r"(\w+)\s*=\s*DataprocCreateBatchOperator\s*\(\s*([\s\S]*?dag,*)\s+\)"
        regex_json=r"\w+.json"
        regex_sql=r"sql_file_location\s(.*).sql"
        regex_sql_folder_name=r"(sql/.*)"
        regex_dag_id="dag_id\s*=\s*[\"']([^\"']+)[\"']"
        matches = re.findall(regex_pattern, py,re.DOTALL)
        dag_id_match = re.search(regex_dag_id, py)

        if dag_id_match:
            dag_id= dag_id_match.group(1) if dag_id_match else None
            for task_name, full_dataproc in matches:
                task = task_name
                json_name_matches = re.search(regex_json, full_dataproc, re.DOTALL)
                print("It got the dataproc")
                if json_name_matches:
                    print(f"It has json name {json_name_matches.group(0) }")
                    json_name = json_name_matches.group(0) if json_name_matches else None
                    print(json_name)
                    json_location=os.path.join(base_dir,kafka_json_folder)
                    print(json_name)
                    json_file=glob.glob(os.path.join(json_location,"**",json_name),recursive=True)
                    print(json_file)
                    ...
                    for single_json in json_file:
                        ...
                        with open(single_json, "r") as f:
                            file_json = f.read()
                        # print(file_json)
                        sql_name_match= re.search(regex_sql,file_json)
                        # print(sql_name_match)
                        if sql_name_match:
                            full_sql_name=sql_name_match.group(1)
                            print(full_sql_name)
                            sql_folder_name_match= re.search(regex_sql_folder_name,full_sql_name)
                            if sql_folder_name_match:
                                # print(sql_folder_name_match)
                                sql_folder_name= sql_folder_name_match.group(0) if sql_folder_name_match else None

                                local_sql=os.path.normpath(os.path.join(base_dir,sql_folder_name+".sql"))
                                local_sql=os.path.join("dags",local_sql.split("\\dags\\")[1])
                                single_json=os.path.normpath(single_json)
                                print("single_json:", single_json)
                                print(single_json.split("\\dags\\")[1])
                                local_json=os.path.join("dags",single_json.split("\\dags\\")[1])
                                print(f"dag name is :{dag_id}")
                                print(f"dataproc_task_id :{task}")
                                print(f"sql_location :{local_sql}")
                                print(f"single_json {single_json}")
                                cloud_sql=full_sql_name+".sql"
                                print(f"cloud sql {cloud_sql}")
                                details_dataproc.append({
                                    'dag':dag_id,
                                    'dataproc_task_id':task,
                                    'local_sql':common_obj.get_normalized_path(local_sql,os_type),
                                    'cloud_sql': common_obj.get_normalized_path(cloud_sql,os_type),
                                    'local_json':common_obj.get_normalized_path(local_json,os_type),
                                    'cloud_json': common_obj.get_normalized_path(os.path.join(testing_json_directory,json_name),os_type)

                                })
                                print(details_dataproc)
                else:
                    error.append({
                        'location': file,
                        'dag_file_name': os.path.basename(file),
                        ...
                        'missing_file_name': "",
                        'json_file_location': "",
                        'error': f"It has some issue with json"
                    })
        else:
            print("Dag_id issue")
            error.append({
                ...
                'missing_file_name': "",
                'json_file_location': "",
                'error': f"It has some issue with dag_id"
            })

        return details_dataproc, error
