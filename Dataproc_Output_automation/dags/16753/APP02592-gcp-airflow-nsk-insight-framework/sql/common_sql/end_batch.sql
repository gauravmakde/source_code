...
UPDATE `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_utl.isf_dag_control_log
 SET
    metric_value = CAST(DATETIME_ADD(CAST(FORMAT_TIMESTAMP('%F %H:%M:%S', CURRENT_DATETIME('GMT')) AS DATETIME), INTERVAL 2 MINUTE) AS STRING),
    metric_tmstp = CAST(CAST(FORMAT_TIMESTAMP('%F %H:%M:%S', CURRENT_DATETIME('GMT')) AS DATETIME) AS TIMESTAMP),
    metric_tmstp_tz = 'GMT' --added tz for GMT timezone
WHERE LOWER(isf_dag_nm) = LOWER('{{params.dag_name}}')
 AND LOWER(metric_nm) = LOWER('ISF_DAG_END_TMSTP') 
