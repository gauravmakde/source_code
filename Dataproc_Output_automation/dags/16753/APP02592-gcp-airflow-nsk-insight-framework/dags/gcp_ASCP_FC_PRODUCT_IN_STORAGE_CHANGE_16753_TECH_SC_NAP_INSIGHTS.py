...
dag_id = "gcp_ascp_fc_product_in_storage_change_16753_TECH_SC_NAP_insights"
...
default_args = {
    ...
    "description": "ascp_fc_product_in_storage_change_16753_TECH_SC_NAP_insights DAG Description",
...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region=region,
        topic_name="inventory-fulfillment-center-product-in-storage-change-v2-analytical-avro",
        consumer_group_name="onix-inventory-fulfillment-center-product-in-storage-change-v2-analytical-avro_ascp_fc_product_in_storage_change_16753_TECH_SC_NAP_insights_kafka_load_02_kafka",
    ...
    common_sql_start_batch = open(
        os.path.join(sql_path, "common_sql/start_batch.sql"), "r"
    ).read()

    launch_start_batch_load_01_start_batch = BigQueryInsertJobOperator(
        task_id="start_batch_load_01_start_batch",
        configuration={
            "query": {"query": common_sql_start_batch, "useLegacySql": False}
        },
    ...
    ascp_fc_product_in_storage_change_load_metrics = open(
        os.path.join(sql_path, "ascp_fc_product_in_storage_change/load_metrics.sql"),
        "r",
    ).read()
    ...
    launch_metrics_load_metrics = BigQueryInsertJobOperator(
        ...
        configuration={
            "query": {
                "query": ascp_fc_product_in_storage_change_load_metrics,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    kafka_load_02_kafka = DataprocCreateBatchOperator(
        ...
        batch={
            ...
            "spark_batch": {
                ...
                "args": [
                    f"{user_config_path}/argument_ascp_fc_product_in_storage_change_16753_TECH_SC_NAP_insights_kafka_load_02_kafka.json",
    ...
    tpt_jdbc_insert_tpt_load_load_03_tpt_control_tbl = BashOperator(
        task_id="tpt_jdbc_insert_tpt_load_load_03_tpt_control_tbl",
        bash_command=f'echo "Skipping Control Table Entry"',
    )

    # tpt_tpt_load_load_03_tpt_load_tbl = SSHOperator(
    # task_id='tpt_tpt_load_load_03_tpt_load_tbl',
    # command="/db/bigquery/bin/tpt_load.sh  -e PRD_NAP_UTL -j PRD_NAP_STG_FC_PRODUCT_IN_STORAGE_CHANGE_LDG_JOB -h tdnapprod.nordstrom.net -l 'N' -u SCH_NAP_ASCP_BATCH_PRD -p \"\$tdwallet(SCH_NAP_ASCP_BATCH_PWD)\" -f s3://tf-nap-scwhandinv-prod-ascp-extract-csv/data/fc_product_in_storage_change_ldg_table_csv/*.csv.gz -o UPDATE",
    # timeout=1800,
    # ssh_conn_id="TECH_SC_NAP_bigquery_ssh")

    ascp_fc_product_in_storage_change_fc_product_in_storage_change_fact_table = open(
        os.path.join(
            sql_path,
            "ascp_fc_product_in_storage_change/fc_product_in_storage_change_fact_table.sql",
        ),
        "r",
    ).read()
    ...
    launch_bigquery_load_04_bigquery = BigQueryInsertJobOperator(
        ...
        configuration={
            "query": {
                "query": ascp_fc_product_in_storage_change_fc_product_in_storage_change_fact_table,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    launch_send_new_relic_metric_send_newrelic_metric = PythonOperator(
        ...
        op_kwargs={
            ...
            "metrics_yaml_file": os.path.join(
                module_path, "yaml/FC_PRODUCT_IN_STORAGE_CHANGE_METRICS.yaml"
            ),
        },
    )
    ...
    (
        ...
        >> [launch_metrics_load_metrics]
    )
    launch_metrics_load_metrics >> [
        kafka_load_02_kafka,
        launch_kafka_start_cntl_track_kafka_start,
    ]

    kafka_load_02_kafka >> [
        launch_kafka_end_cntl_track_kafka_end,
        tpt_jdbc_insert_tpt_load_load_03_tpt_control_tbl,
    ]

    tpt_jdbc_insert_tpt_load_load_03_tpt_control_tbl >> [
        launch_bigquery_load_04_bigquery
    ]
    launch_bigquery_load_04_bigquery >> [
        launch_send_new_relic_metric_send_newrelic_metric
    ]
    launch_send_new_relic_metric_send_newrelic_metric >> [
        launch_end_batch_load_05_end_batch
    ]
