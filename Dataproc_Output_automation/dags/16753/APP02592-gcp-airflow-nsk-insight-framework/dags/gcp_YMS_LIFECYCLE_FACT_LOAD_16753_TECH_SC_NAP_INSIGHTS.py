...
dag_id = "gcp_yms_lifecycle_fact_load_16753_TECH_SC_NAP_insights"
...
default_args = {
    ...
    "description": "yms_lifecycle_fact_load_16753_TECH_SC_NAP_insights DAG Description",
...
with DAG(
    ...
    yms_v3_yms_lifecycle_agg_tables_load = open(
        os.path.join(
            sql_path,
            "yms_v3/yms_lifecycle_agg_tables_load.sql",
        ),
        "r",
    ).read()

    launch_yms_lifecycle_fact_load_yms_teradata_consume = BigQueryInsertJobOperator(
        task_id="yms_lifecycle_fact_load_yms_teradata_consume",
        configuration={
            "query": {
                "query": yms_v3_yms_lifecycle_agg_tables_load,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    yms_v3_end_batch = open(
        os.path.join(
            sql_path,
            "yms_v3/end_batch.sql",
        ),
        "r",
    ).read()
    ...
    (
        creds_setup
        >> launch_start_batch_load
        >> [launch_yms_lifecycle_fact_load_yms_teradata_consume]
    )
    launch_yms_lifecycle_fact_load_yms_teradata_consume >> [
