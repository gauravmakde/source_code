...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region = region,
        topic_name = 'customer-payment-authorization-analytical-avro',
        consumer_group_name = 'onix-customer-payment-authorization-analytical-avro_payment_auth_teradata_v2_4638_payments_insights_payment_auth_kafka_to_teradata_stg_stage_kafka_to_teradata_stg_job',
    ...
    session_name=get_batch_id(dag_id=dag_id,task_id="payment_auth_kafka_to_bigquery_stg_stage_kafka_to_bigquery_stg_job")   
    payment_auth_kafka_to_bigquery_stg_stage_kafka_to_bigquery_stg_job = DataprocCreateBatchOperator(
            task_id = "payment_auth_kafka_to_bigquery_stg_stage_kafka_to_bigquery_stg_job",
            project_id = project_id,
            region = region,
            gcp_conn_id = gcp_conn,
            batch=
            {    
                "runtime_config": {"version": "1.1","properties":{ 'spark.sql.legacy.avro.datetimeRebaseModeInWrite': 'LEGACY', 'spark.sql.parquet.writeLegacyFormat': 'true', 'spark.speculation': 'false', 'spark.airflow.run_id':"{{ ti.xcom_pull(task_ids='fetch_batch_run_id_task')}}"}},
                "spark_batch": 
                    {
                    ...
                    "args": [f"{user_config_path}/argument_payment_auth_bigquery_v2_4638_payments_insights_payment_auth_kafka_to_bigquery_stg_stage_kafka_to_bigquery_stg_job.json",
    ...
    bigquery_payment_auth_bigquery_hdr_stg_to_bigquery_hdr_fct_v2 = open( os.path.join(sql_path, 'bigquery/payment_auth_bigquery_hdr_stg_to_bigquery_hdr_fct_v2.sql'),"r").read()

    launch_payment_auth_bigquery_hdr_stg_to_bigquery_hdr_fct_stage_bigquery_hdr_stg_to_bigquery_hdr_fct_job = BigQueryInsertJobOperator(
        task_id = "payment_auth_bigquery_hdr_stg_to_bigquery_hdr_fct_stage_bigquery_hdr_stg_to_bigquery_hdr_fct_job",
        configuration = 
        {
            "query": 
            {
                "query": bigquery_payment_auth_bigquery_hdr_stg_to_bigquery_hdr_fct_v2,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    bigquery_payment_auth_bigquery_tender_stg_to_bigquery_tender_fct_v2 = open( os.path.join(sql_path, 'bigquery/payment_auth_bigquery_tender_stg_to_bigquery_tender_fct_v2.sql'),"r").read()

    launch_payment_auth_bigquery_tender_stg_to_bigquery_tender_fct_stage_bigquery_tender_stg_to_bigquery_tender_fct_job = BigQueryInsertJobOperator(
        task_id = "payment_auth_bigquery_tender_stg_to_bigquery_tender_fct_stage_bigquery_tender_stg_to_bigquery_tender_fct_job",
        configuration = 
        {
            "query": 
            {
                "query": bigquery_payment_auth_bigquery_tender_stg_to_bigquery_tender_fct_v2,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    bigquery_payment_auth_bigquery_hdr_fct_update_payment_id = open( os.path.join(sql_path, 'bigquery/payment_auth_bigquery_hdr_fct_update_payment_id.sql'),"r").read()

    launch_payment_auth_bigquery_hdr_fct_update_payment_id_stage_bigquery_hdr_fct_update_job = BigQueryInsertJobOperator(
        task_id = "payment_auth_bigquery_hdr_fct_update_payment_id_stage_bigquery_hdr_fct_update_job",
        configuration = 
        {
            "query": 
            {
                "query": bigquery_payment_auth_bigquery_hdr_fct_update_payment_id,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    creds_setup >> fetch_batch_run_id_task >> payment_auth_kafka_to_bigquery_stg_stage_kafka_to_bigquery_stg_job
    payment_auth_kafka_to_bigquery_stg_stage_kafka_to_bigquery_stg_job >>[launch_payment_auth_bigquery_hdr_stg_to_bigquery_hdr_fct_stage_bigquery_hdr_stg_to_bigquery_hdr_fct_job, launch_payment_auth_bigquery_tender_stg_to_bigquery_tender_fct_stage_bigquery_tender_stg_to_bigquery_tender_fct_job]
    launch_payment_auth_bigquery_hdr_stg_to_bigquery_hdr_fct_stage_bigquery_hdr_stg_to_bigquery_hdr_fct_job >>launch_payment_auth_bigquery_hdr_fct_update_payment_id_stage_bigquery_hdr_fct_update_job
    launch_payment_auth_bigquery_tender_stg_to_bigquery_tender_fct_stage_bigquery_tender_stg_to_bigquery_tender_fct_job >>launch_payment_auth_bigquery_hdr_fct_update_payment_id_stage_bigquery_hdr_fct_update_job
    launch_payment_auth_bigquery_hdr_fct_update_payment_id_stage_bigquery_hdr_fct_update_job >> pelican_validator >> pelican_sensor