...
root_path = (
    path.dirname(__file__).split("APP07420-gcp-payments-insights-framework")[0]
    + "APP07420-gcp-payments-insights-framework/"
)
...
dag_id = "gcp_fraud_decision_lifecycle_teradata_4638_payments_insights"
...
cron = None if config_env in ["nonprod", "development"] else config.get(dag_id, "cron")
...
default_args = {
    ...
    "email": (
        None
        if config_env in ["nonprod", "development"]
...
with DAG(
    ...
    schedule_interval=(
        None if config_env in ["nonprod", "development"] else config.get(dag_id, "cron")
    ),
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region=region,
        topic_name="customer-fraud-decision-lifecycle-analytical-avro",
        consumer_group_name="onix-customer-fraud-decision-lifecycle-analytical-avro_fraud_decision_lifecycle_teradata_4638_payments_insights_fraud_decision_lifecycle_kafka_to_teradata_stg_stage_0_kafka_to_teradata_stg_job",
    ...
    session_name = get_batch_id(
        dag_id=dag_id,
        task_id="fraud_decision_lifecycle_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job",
    )
    livy_fraud_decision_lifecycle_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job = DataprocCreateBatchOperator(
        task_id="fraud_decision_lifecycle_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job",
        ...
        batch={
            ...
            "spark_batch": {
                ...
                "args": [
                    f"{user_config_path}/argument_fraud_decision_lifecycle_bigquery_4638_payments_insights_fraud_decision_lifecycle_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job.json",
    ...
    bigquery_fraud_decision_lifecycle_bigquery_stg_to_bigquery_fct = open(
        os.path.join(
            sql_path,
            "bigquery/fraud_decision_lifecycle_bigquery_stg_to_bigquery_fct.sql",
        ),
        "r",
    ).read()

    launch_fraud_decision_lifecycle_kafka_to_bigquery_stg_stage_0_bigquery_stg_to_bigquery_fct_job = BigQueryInsertJobOperator(
        task_id="fraud_decision_lifecycle_kafka_to_bigquery_stg_stage_0_bigquery_stg_to_bigquery_fct_job",
        configuration={
            "query": {
                "query": bigquery_fraud_decision_lifecycle_bigquery_stg_to_bigquery_fct,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    (
        ...
        >> livy_fraud_decision_lifecycle_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job
        >> launch_fraud_decision_lifecycle_kafka_to_bigquery_stg_stage_0_bigquery_stg_to_bigquery_fct_job
        >> pelican_validator
        >> pelican_sensor
    )
