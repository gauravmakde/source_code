...
root_path = (
    path.dirname(__file__).split("APP02432-gcp-napstore-insights")[0]
    + "APP02432-gcp-napstore-insights/"
)
...
dag_id = "gcp_hr_details_stg_to_dim_on_demand_2656_napstore_insights"
...
default_args = {
    ...
    "description": "hr_details_stg_to_dim_on_demand_2656_napstore_insights DAG Description",
...
with DAG(
    ...
    hr_employee_status_details_load_dim = open(
        os.path.join(sql_path, "hr_employee_status_details_load_dim.sql"), "r"
    ).read()

    launch_hr_details_events_load_1_details_dim_tables = BigQueryInsertJobOperator(
        task_id="hr_details_events_load_1_details_dim_tables",
        configuration={
            "query": {
    ...
    hr_job_details_load_dim = open(
        os.path.join(sql_path, "hr_job_details_load_dim.sql"), "r"
    ).read()

    launch_hr_details_events_load_2_details_dim_tables = BigQueryInsertJobOperator(
        task_id="launch_hr_details_events_load_2_details_dim_tables",
        configuration={
            "query": {"query": hr_job_details_load_dim, "useLegacySql": False}
        },
        project_id=project_id,
        gcp_conn_id=gcp_conn,
        params=sql_config_dict,
    ...
    hr_line_position_details_load_dim = open(
        os.path.join(sql_path, "hr_line_position_details_load_dim.sql"), "r"
    ).read()

    launch_hr_details_events_load_3_details_dim_tables = BigQueryInsertJobOperator(
        task_id="launch_hr_details_events_load_3_details_dim_tables",
        configuration={
            "query": {"query": hr_line_position_details_load_dim, "useLegacySql": False}
        },
        project_id=project_id,
        gcp_conn_id=gcp_conn,
        params=sql_config_dict,
    ...
    hr_org_details_load_dim = open(
        os.path.join(sql_path, "hr_org_details_load_dim.sql"), "r"
    ).read()

    launch_hr_details_events_load_4_details_dim_tables = BigQueryInsertJobOperator(
        task_id="launch_hr_details_events_load_4_details_dim_tables",
        configuration={
            "query": {"query": hr_org_details_load_dim, "useLegacySql": False}
        },
        project_id=project_id,
        gcp_conn_id=gcp_conn,
        params=sql_config_dict,
    ...
    hr_work_contact_details_load_dim = open(
        os.path.join(sql_path, "hr_work_contact_details_load_dim.sql"), "r"
    ).read()

    launch_hr_details_events_load_5_details_dim_tables = BigQueryInsertJobOperator(
        task_id="launch_hr_details_events_load_5_details_dim_tables",
        configuration={
            "query": {"query": hr_work_contact_details_load_dim, "useLegacySql": False}
        },
        project_id=project_id,
        gcp_conn_id=gcp_conn,
        params=sql_config_dict,
    ...
    (
        creds_setup
        >> launch_hr_details_events_load_1_details_dim_tables
        >> launch_hr_details_events_load_2_details_dim_tables
        >> launch_hr_details_events_load_3_details_dim_tables
        >> launch_hr_details_events_load_4_details_dim_tables
        >> launch_hr_details_events_load_5_details_dim_tables
        >> pelican_validator
        >> pelican_sensor
    )
