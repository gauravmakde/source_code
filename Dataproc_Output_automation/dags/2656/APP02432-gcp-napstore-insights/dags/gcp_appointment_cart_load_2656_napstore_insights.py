...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region=region,
        topic_name="customer-appointment-cart-analytical-avro",
        consumer_group_name="onix-customer-appointment-cart-analytical-avro_appointment_cart_load_2656_napstore_insights_appointment_cart_v1_load_orc_td_stg",
    ...
    session_name = get_batch_id(
        dag_id=dag_id, task_id="appointment_cart_v1_load_orc_bq_stg"
    )
    appointment_cart_v1_load_orc_bq_stg = DataprocCreateBatchOperator(
        task_id="appointment_cart_v1_load_orc_bq_stg",
        ...
        batch={
            ...
            "spark_batch": {
                ...
                "args": [
                    f"{user_config_path}/argument_appointment_cart_load_2656_napstore_insights_appointment_cart_v1_load_orc_bq_stg.json",
    ...
    appointment_cart_engagement_load_bigquery_stg_to_dim = open(
        os.path.join(
            sql_path, "appointment_cart_engagement_load_bigquery_stg_to_dim.sql"
        ),
        "r",
    ).read()

    launch_appointment_cart_v1_load_bq_stg_to_dims = BigQueryInsertJobOperator(
        task_id="appointment_cart_v1_load_bq_stg_to_dims",
        configuration={
            "query": {
                "query": appointment_cart_engagement_load_bigquery_stg_to_dim,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    creds_setup >> fetch_batch_run_id_task >> [appointment_cart_v1_load_orc_bq_stg]
    appointment_cart_v1_load_orc_bq_stg >> [
        launch_appointment_cart_v1_load_bq_stg_to_dims
    ]
    (
        launch_appointment_cart_v1_load_bq_stg_to_dims
        >> pelican_validator
