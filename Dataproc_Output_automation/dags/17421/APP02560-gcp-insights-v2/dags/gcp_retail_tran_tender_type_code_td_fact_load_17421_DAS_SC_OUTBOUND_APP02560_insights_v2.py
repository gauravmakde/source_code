...
dag_id = 'gcp_retail_tran_tender_type_code_td_fact_load_17421_DAS_SC_OUTBOUND_APP02560_insights_v2'
...
default_args = {
    ...
    'description': 'retail_tran_tender_type_code_td_fact_load_17421_DAS_SC_OUTBOUND_APP02560_insights_v2 DAG Description',
...
with DAG(
    ...
    batch_start = open( os.path.join(sql_path, 'retail_tran_tender_type_code/batch_start.sql'),"r").read()
    ...
    td_fact_load = open( os.path.join(sql_path, 'retail_tran_tender_type_code/td_fact_load.sql'),"r").read()

    launch_main_job_2_td_fact_load = BigQueryInsertJobOperator(
        task_id = "main_job_2_td_fact_load",
        configuration = 
        {
            "query": 
            {
    ...
    batch_end = open( os.path.join(sql_path, 'retail_tran_tender_type_code/batch_end.sql'),"r").read()

    launch_main_job_3_td_batch_end = BigQueryInsertJobOperator(
        task_id = "main_job_3_td_batch_end",
        configuration = 
        {
            "query": 
            {
    ...
    creds_setup >> launch_main_job_1_td_batch_start >>[launch_main_job_2_td_fact_load]
    launch_main_job_2_td_fact_load >>[launch_main_job_3_td_batch_end]
    launch_main_job_3_td_batch_end >> pelican_validator >> pelican_sensor