...
root_path = (
    path.dirname(__file__).split("APP02560-gcp-insights-v2")[0]
    + "APP02560-gcp-insights-v2/"
)
...
dag_id = (
    "gcp_sco_order_line_routing_assignment_17421_DAS_SC_OUTBOUND_APP02560_insights_v2"
)
...
default_args = {
    ...
    "description": "sco_order_line_routing_assignment_17421_DAS_SC_OUTBOUND_APP02560_insights_v2 DAG Description",
...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region=region,
        topic_name="inventory-routing-decision-lifecycle-analytical-avro",
        consumer_group_name="onix-inventory-routing-decision-lifecycle-analytical-avro_sco_order_line_routing_assignment_17421_DAS_SC_OUTBOUND_APP02560_insights_v2_kafka_job_20_kafka_avro_to_td_stg",
    ...
    order_line_routing_assignment_10_bq_start_batch = open(
        os.path.join(sql_path, "order_line_routing_assignment/10_bq_start_batch.sql"),
        "r",
    ).read()

    launch_kafka_job_10_bq_start_batch = BigQueryInsertJobOperator(
        task_id="kafka_job_10_bq_start_batch",
        configuration={
            "query": {
                "query": order_line_routing_assignment_10_bq_start_batch,
                "useLegacySql": False,
            }
        },
    ...
    session_name = get_batch_id(
        dag_id=dag_id, task_id="kafka_job_20_kafka_avro_to_bq_stg"
    )
    kafka_job_20_kafka_avro_to_bq_stg = DataprocCreateBatchOperator(
        task_id="kafka_job_20_kafka_avro_to_bq_stg",
        project_id=project_id,
        region=region,
        gcp_conn_id=gcp_conn,
        # op_args = { "version":"1.1"},
        batch={
            ...
            "spark_batch": {
                ...
                "args": [
                    f"{user_config_path}/argument_sco_order_line_routing_assignment_17421_DAS_SC_OUTBOUND_APP02560_insights_v2_kafka_job_20_kafka_avro_to_bq_stg.json",
    ...
    order_line_routing_assignment_25_bq_stg_to_bq_fact = open(
        os.path.join(
            sql_path, "order_line_routing_assignment/25_bq_stg_to_bq_fact.sql"
        ),
        "r",
    ).read()

    launch_kafka_job_25_bq_stg_to_bq_fact = BigQueryInsertJobOperator(
        task_id="kafka_job_25_bq_stg_to_bq_fact",
        configuration={
            "query": {
                "query": order_line_routing_assignment_25_bq_stg_to_bq_fact,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    order_line_routing_assignment_30_bq_fact_dqc = open(
        os.path.join(sql_path, "order_line_routing_assignment/30_bq_fact_dqc.sql"), "r"
    ).read()

    launch_kafka_job_30_bq_fact_dqc = BigQueryInsertJobOperator(
        task_id="kafka_job_30_bq_fact_dqc",
        configuration={
            "query": {
                "query": order_line_routing_assignment_30_bq_fact_dqc,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    order_line_routing_assignment_40_bq_end_batch = open(
        os.path.join(sql_path, "order_line_routing_assignment/40_bq_end_batch.sql"), "r"
    ).read()

    launch_kafka_job_40_bq_end_batch = BigQueryInsertJobOperator(
        task_id="kafka_job_40_bq_end_batch",
        configuration={
            "query": {
                "query": order_line_routing_assignment_40_bq_end_batch,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    (
        ...
        >> launch_kafka_job_10_bq_start_batch
        >> [kafka_job_20_kafka_avro_to_bq_stg]
    )
    kafka_job_20_kafka_avro_to_bq_stg >> [launch_kafka_job_25_bq_stg_to_bq_fact]
    launch_kafka_job_25_bq_stg_to_bq_fact >> [launch_kafka_job_30_bq_fact_dqc]
    launch_kafka_job_30_bq_fact_dqc >> [launch_kafka_job_40_bq_end_batch]
    launch_kafka_job_40_bq_end_batch >> pelican_validator >> pelican_sensor
