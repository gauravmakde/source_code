...
root_path = (
    path.dirname(__file__).split("APP06788-gcp-credit-insights-framework")[0]
    + "APP06788-gcp-credit-insights-framework/"
)
...
dag_id = "gcp_credit_account_teradata_3678_credit_insights"
...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region=region,
        topic_name="customer-credit-account-analytical-avro",
        consumer_group_name="onix-customer-credit-account-analytical-avro_credit_account_teradata_3678_credit_insights_credit_account_kafka_to_teradata_stg_stage_0_kafka_to_teradata_stg_job",
    ...
    session_name = get_batch_id(
        dag_id=dag_id,
        task_id="credit_account_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job",
    )
    credit_account_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job = DataprocCreateBatchOperator(
        task_id="credit_account_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job",
        ...
        batch={
            ...
            "spark_batch": {
                ...
                "args": [
                    f"{user_config_path}/argument_credit_account_bigquery_3678_credit_insights_credit_account_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job.json",
    ...
    bigquery_credit_account_bigquery_stg_to_bigquery_fct = open(
        os.path.join(
            sql_path, "bigquery/credit_account_bigquery_stg_to_bigquery_fct.sql"
        ),
        "r",
    ).read()

    launch_credit_account_kafka_to_bigquery_stg_stage_0_bigquery_stg_to_bigquery_fct_job = BigQueryInsertJobOperator(
        task_id="credit_account_kafka_to_bigquery_stg_stage_0_bigquery_stg_to_bigquery_fct_job",
        configuration={
            "query": {
                "query": bigquery_credit_account_bigquery_stg_to_bigquery_fct,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    creds_setup >> fetch_batch_run_id_task >>[credit_account_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job]
    credit_account_kafka_to_bigquery_stg_stage_0_kafka_to_bigquery_stg_job >>[launch_credit_account_kafka_to_bigquery_stg_stage_0_bigquery_stg_to_bigquery_fct_job]
    launch_credit_account_kafka_to_bigquery_stg_stage_0_bigquery_stg_to_bigquery_fct_job >> pelican_validator >> pelican_sensor