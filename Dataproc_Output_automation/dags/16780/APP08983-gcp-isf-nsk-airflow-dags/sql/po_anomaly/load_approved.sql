...
INSERT INTO
  ...
  SELECT
    ...
    new_po_latest_status.diff_event_count AS new_po_approval_count,
    CASE
      WHEN new_po_latest_status.epo_purchase_order_id IS NULL THEN 'Is not received from source'
      WHEN LOWER(new_po_latest_status.epo_most_recent_status) = LOWER('APPROVED')
    AND new_po_latest_status.diff_event_count = 0 THEN 'No Approved events received'
      WHEN LOWER(all_rows_new_po.status) <> LOWER('APPROVED') THEN 'Actual status <> APPROVED'
      WHEN CAST(all_rows_new_po.first_approval_event_tmstp_pacific AS DATE) <> DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 1 DAY) THEN 'New POs have different timestamps (potentially caused by lag)'
      ELSE NULL
  END
  ...
  FROM (
    ...
    WHERE
      ...
      WHERE
        LOWER(UPPER(status)) = LOWER('APPROVED')
  ...
  LEFT JOIN (
    SELECT
      ...
      SUM(CASE
          WHEN LOWER(event_name) = LOWER('PurchaseOrderApproved') THEN 1
          ELSE 0
      END
        ) OVER (PARTITION BY epo_purchase_order_id RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS diff_event_count
    FROM
