...
dag_id = 'gcp_ascp_po_receipt_v1_16780_TECH_SC_NAP_insights'
...
default_args = {
    ...
    'description': 'ascp_po_receipt_v1_16780_TECH_SC_NAP_insights DAG Description',
    'retry_delay': timedelta(minutes = 10),
    # 'email': None if config.get(dag_id, "email").lower() == "none" else config.get(dag_id, "email"),
...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region = region,
        topic_name = 'ascp-rms-shipment-po-receipt-v1-object-model-avro',
        consumer_group_name = 'onix-ascp-rms-shipment-po-receipt-v1-object-model-avro_ascp_po_receipt_v1_16780_TECH_SC_NAP_insights_po_receipt_load_load_02_kafka',
    ...
    common_sql_load_metrics = open( os.path.join(sql_path, 'common_sql/load_metrics.sql'),"r").read()
    ...
    session_name = get_batch_id(dag_id = dag_id,task_id = "po_receipt_load_load_02_kafka")   
    po_receipt_load_load_02_kafka = DataprocCreateBatchOperator(
            task_id = "po_receipt_load_load_02_kafka",
            project_id = project_id,
            region = region,
            gcp_conn_id = gcp_conn,
            batch=
            {    
                ...
                "spark_batch": 
                    ...
                    "args": [f"{user_config_path}/argument_ascp_po_receipt_v1_16780_TECH_SC_NAP_insights_po_receipt_load_load_02_kafka.json",
    ...
    tpt_jdbc_insert_tpt_load_03_tpt_control_tbl = BashOperator(
    task_id = 'tpt_jdbc_insert_tpt_load_03_tpt_control_tbl',
    bash_command = f'echo "Skipping Control Table Entry"')
        
    ascp_po_receipt_v1_po_receipt_load_fact_table_full = open( os.path.join(sql_path, 'ascp_po_receipt_v1/po_receipt_load_fact_table_full.sql'),"r").read()

    launch_bigquery_load_04_bigquery = BigQueryInsertJobOperator(
        task_id = "bigquery_load_04_bigquery",
        configuration = 
        {
            "query": 
            {
                "query": ascp_po_receipt_v1_po_receipt_load_fact_table_full,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    launch_send_new_relic_metric_send_newrelic_metric = PythonOperator(
        ...
        op_kwargs={
            ...
            "hook": BigQueryHook(gcp_conn_id = gcp_conn, location = sql_config_dict['region'], use_legacy_sql = False),
            "bigquery_environment": sql_config_dict["dbenv"],
            "metrics_yaml_file": os.path.join(module_path, "yaml/ASCP_COMMON_METRICS.yaml"),
    ...
    launch_end_batch_load_05_end_batch = BigQueryInsertJobOperator(
        task_id = "end_batch_load_05_end_batch",
        configuration = 
        {
            "query": 
            {
    ...
    launch_metrics_load_metrics >> [launch_kafka_start_cntl_track_kafka_start, po_receipt_load_load_02_kafka]
    po_receipt_load_load_02_kafka >>[launch_kafka_end_cntl_track_kafka_end, tpt_jdbc_insert_tpt_load_03_tpt_control_tbl]
    tpt_jdbc_insert_tpt_load_03_tpt_control_tbl >>[launch_bigquery_load_04_bigquery]
    launch_bigquery_load_04_bigquery >>[launch_send_new_relic_metric_send_newrelic_metric]
    launch_send_new_relic_metric_send_newrelic_metric >>[launch_end_batch_load_05_end_batch]
    launch_end_batch_load_05_end_batch >> pelican_validator >> pelican_sensor
