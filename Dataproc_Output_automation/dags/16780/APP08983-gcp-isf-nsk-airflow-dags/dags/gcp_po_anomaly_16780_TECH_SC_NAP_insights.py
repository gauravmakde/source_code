...
dag_id = 'gcp_po_anomaly_16780_TECH_SC_NAP_insights'
...
default_args = {
    ...
    'description': 'po_anomaly_16780_TECH_SC_NAP_insights DAG Description',
...
with DAG(
    ...
    po_anomaly_load_approved= open( os.path.join(sql_path, 'po_anomaly/load_approved.sql'),"r").read()
    launch_main_load_po_anomaly_table = BigQueryInsertJobOperator(
        task_id = "main_load_po_anomaly_table",
        configuration = 
        {
            "query": 
            {
                "query": po_anomaly_load_approved,
                "useLegacySql": False
            }
        },
    ...
    po_anomaly_load_closed= open( os.path.join(sql_path, 'po_anomaly/load_closed.sql'),"r").read()
    launch_main_load_closed_po_anomaly_table = BigQueryInsertJobOperator(
        task_id = "main_load_closed_po_anomaly_table",
        configuration = 
        {
            "query": 
            {
                "query": po_anomaly_load_closed,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    po_anomaly_load_worksheet= open( os.path.join(sql_path, 'po_anomaly/load_worksheet.sql'),"r").read()
    launch_main_load_worksheet_po_anomaly_table = BigQueryInsertJobOperator(
        task_id = "main_load_worksheet_po_anomaly_table",
        configuration = 
        {
            "query": 
            {
                "query": po_anomaly_load_worksheet,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    po_anomaly_validate_diff = open( os.path.join(sql_path, 'po_anomaly/validate_diff.sql'),"r").read()

    launch_main_validate_diff = BigQueryInsertJobOperator(
        task_id = "main_validate_diff",
        configuration = 
        {
            "query": 
            {
                "query": po_anomaly_validate_diff,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    creds_setup >> launch_check_external_upstream_dependencies_sensor_0 >>[launch_main_load_po_anomaly_table]
    launch_main_load_po_anomaly_table >>[launch_main_load_closed_po_anomaly_table]
    launch_main_load_closed_po_anomaly_table >>[launch_main_load_worksheet_po_anomaly_table] 
    launch_main_load_worksheet_po_anomaly_table >>[launch_main_validate_diff]
    launch_main_validate_diff >> pelican_validator >> pelican_sensor