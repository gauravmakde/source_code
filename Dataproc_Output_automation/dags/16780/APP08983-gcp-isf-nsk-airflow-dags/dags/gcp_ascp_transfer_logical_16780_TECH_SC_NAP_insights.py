...
dag_id = 'gcp_ascp_transfer_logical_16780_TECH_SC_NAP_insights'
...
default_args = {
    ...
    'description': 'ascp_transfer_logical_16780_TECH_SC_NAP_insights DAG Description',
...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region = region,
        topic_name = 'ascp-rms-transfer-object-model-avro',
        consumer_group_name = 'onix-ascp-rms-transfer-object-model-avro_ascp_transfer_logical_16780_TECH_SC_NAP_insights_kafka_load_02_kafka',
    ...
    common_sql_load_metrics_4 = open( os.path.join(sql_path, 'common_sql/load_metrics_4.sql'),"r").read()
    ...
    launch_metrics_load_metrics = BigQueryInsertJobOperator(
        ...
        configuration = 
        {
            "query": 
            {
                "query": common_sql_load_metrics_4,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    kafka_load_02_kafka = DataprocCreateBatchOperator(
            ...
            {    
                ...
                "spark_batch": 
                    ...
                    "args": [f"{user_config_path}/argument_ascp_transfer_logical_16780_TECH_SC_NAP_insights_kafka_load_02_kafka.json",
    ...
    common_sql_track_kafka_start = open( os.path.join(sql_path, 'common_sql/track_kafka_start.sql'),"r").read()
    ...
    launch_kafka_start_cntl_track_kafka_start = BigQueryInsertJobOperator(
        ...
        configuration = 
        {
            "query": 
            {
                "query": common_sql_track_kafka_start,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    common_sql_track_kafka_end = open( os.path.join(sql_path, 'common_sql/track_kafka_end.sql'),"r").read()
    ...
    launch_kafka_end_cntl_track_kafka_end = BigQueryInsertJobOperator(
        ...
        configuration = 
        {
            "query": 
            {
                "query": common_sql_track_kafka_end,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    tpt_jdbc_insert_tpt_1_load_03_tpt_control_tbl = BashOperator(
    task_id='tpt_jdbc_insert_tpt_1_load_03_tpt_control_tbl',
    bash_command=f'echo "Skipping Control Table Entry"')

    # tpt_tpt_1_load_03_tpt_load_tbl = SSHOperator(
    # task_id='tpt_tpt_1_load_03_tpt_load_tbl',
    # command="/db/teradata/bin/tpt_load.sh  -e PRD_NAP_UTL -j PRD_NAP_STG_TRANSFER_SHIPMENT_RECEIPT_LOGICAL_LDG_JOB -h tdnapprod.nordstrom.net -l 'N' -u SCH_NAP_SCINV_BATCH_PRD -p \"\$tdwallet(SCH_NAP_SCINV_BATCH_PRD_PWD)\" -f s3://tf-nap-scinv-prod-ascp-extract-csv/data/transfer_shipment_receipt_logical_ldg_table_csv/*.csv.gz -o UPDATE",
    # timeout=1800,
    # ssh_conn_id="TECH_ISF_NAP_SCINV_TDUTIL_SERVER_S3EXT_PROD")

    tpt_jdbc_insert_tpt_2_load_04_tpt_control_tbl = BashOperator(
    task_id='tpt_jdbc_insert_tpt_2_load_04_tpt_control_tbl',
    bash_command=f'echo "Skipping Control Table Entry"')

    # tpt_tpt_2_load_04_tpt_load_tbl = SSHOperator(
    # task_id='tpt_tpt_2_load_04_tpt_load_tbl',
    # command="/db/teradata/bin/tpt_load.sh  -e PRD_NAP_UTL -j PRD_NAP_STG_TRANSFER_CREATED_LOGICAL_LDG_JOB -h tdnapprod.nordstrom.net -l 'N' -u SCH_NAP_SCINV_BATCH_PRD -p \"\$tdwallet(SCH_NAP_SCINV_BATCH_PRD_PWD)\" -f s3://tf-nap-scinv-prod-ascp-extract-csv/data/transfer_created_logical_ldg_table_csv/*.csv.gz -o UPDATE",
    # timeout=1800,
    # ssh_conn_id="TECH_ISF_NAP_SCINV_TDUTIL_SERVER_S3EXT_PROD")

    
        
    ascp_transfer_logical_transfer_shipment_receipt_logical_fact_load = open( os.path.join(sql_path, 'ascp_transfer_logical/transfer_shipment_receipt_logical_fact_load.sql'),"r").read()

    launch_teradata_1_load_05_teradata = BigQueryInsertJobOperator(
        task_id = "teradata_1_load_05_teradata",
        configuration = 
        {
            "query": 
            {
                "query": ascp_transfer_logical_transfer_shipment_receipt_logical_fact_load,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    ascp_transfer_logical_transfer_created_logical_fact_load = open( os.path.join(sql_path, 'ascp_transfer_logical/transfer_created_logical_fact_load.sql'),"r").read()

    launch_teradata_2_load_06_teradata = BigQueryInsertJobOperator(
        task_id = "teradata_2_load_06_teradata",
        configuration = 
        {
            "query": 
            {
                "query": ascp_transfer_logical_transfer_created_logical_fact_load,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    launch_send_new_relic_metric_send_newrelic_metric = PythonOperator(
        ...
        op_kwargs={
            ...
            "metrics_yaml_file": os.path.join(
                module_path, "yaml/ASCP_TRANSFER_LOGICAL_METRICS.yaml"
            ),
        },
    )
    ...
    launch_end_batch_load_07_end_batch = BigQueryInsertJobOperator(
        task_id = "end_batch_load_07_end_batch",
        configuration = 
        {
            "query": 
            {
    ...
    launch_metrics_load_metrics >>[launch_kafka_start_cntl_track_kafka_start]
    launch_metrics_load_metrics >> kafka_load_02_kafka
    kafka_load_02_kafka >>[launch_kafka_end_cntl_track_kafka_end, tpt_jdbc_insert_tpt_1_load_03_tpt_control_tbl, tpt_jdbc_insert_tpt_2_load_04_tpt_control_tbl]
    tpt_jdbc_insert_tpt_1_load_03_tpt_control_tbl >>[launch_teradata_1_load_05_teradata]
    tpt_jdbc_insert_tpt_2_load_04_tpt_control_tbl >>[launch_teradata_2_load_06_teradata]
    launch_teradata_1_load_05_teradata >>[launch_send_new_relic_metric_send_newrelic_metric]
    launch_teradata_2_load_06_teradata >>[launch_send_new_relic_metric_send_newrelic_metric]
    launch_send_new_relic_metric_send_newrelic_metric >>[launch_end_batch_load_07_end_batch]
    launch_end_batch_load_07_end_batch >> pelican_validator >> pelican_sensor