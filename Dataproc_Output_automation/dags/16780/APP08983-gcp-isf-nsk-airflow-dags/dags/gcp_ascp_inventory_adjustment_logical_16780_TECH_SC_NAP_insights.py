...
dag_id = "gcp_ascp_inventory_adjustment_logical_16780_TECH_SC_NAP_insights"
...
default_args = {
    ...
    "retries": 0 if config_env in ["nonprod", "development"] else 3,
    "description": "ascp_inventory_adjustment_logical_16780_TECH_SC_NAP_insights DAG Description",
...
with DAG(
        ...
        schedule_interval = None if config_env == "development" else config.get(dag_id, "cron"),
    ...
    session_name = get_batch_id(
        dag_id=dag_id, task_id="inventory_adjustment_logical_load_load_02_kafka"
    )
    internal_shipment_load_02_kafka = DataprocCreateBatchOperator(
        task_id="inventory_adjustment_logical_load_load_02_kafka",
        project_id=project_id,
        region=region,
        gcp_conn_id=gcp_conn,
        batch={
            "runtime_config": {
                "version": "1.1",
                "properties": {"spark.sql.legacy.avro.datetimeRebaseModeInWrite": "LEGACY", "spark.sql.legacy.parquet.datetimeRebaseModeInWrite": "LEGACY", "spark.speculation": "false","spark.airflow.run_id": "{{ ti.xcom_pull(task_ids='fetch_batch_run_id_task')}}"
                },
            },
            "spark_batch": {
                ...
                "args": [
                    f"{user_config_path}/argument_ascp_inventory_adjustment_logical_16780_TECH_SC_NAP_insights_inventory_adjustment_logical_load_load_02_kafka.json",
    ...
    inventory_adjustment_logical_load_fact_table = open( os.path.join(sql_path, "ascp_inventory_adjustment_logical/inventory_adjustment_logical_load_fact_table.sql"),"r").read()
    ...
    launch_bigquery_load_04_bigquery = BigQueryInsertJobOperator(
        ...
        configuration = 
        {
            "query": 
            {
                "query": inventory_adjustment_logical_load_fact_table,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    launch_kafka_start_cntl_track_kafka_start >> launch_kafka_end_cntl_track_kafka_end >> [launch_bigquery_load_04_bigquery]
