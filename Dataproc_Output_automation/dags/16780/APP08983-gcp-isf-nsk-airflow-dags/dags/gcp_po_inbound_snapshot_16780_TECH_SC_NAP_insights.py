...
dag_id = 'gcp_po_inbound_snapshot_16780_TECH_SC_NAP_insights'
...
sys.path.append(module_path + 'date_functions')
...
dag_sla = timedelta(minutes=int(30)) if int(30) > 1 else None
...
default_args = {
    ...
    'description': 'po_inbound_snapshot_16780_TECH_SC_NAP_insights DAG Description',
...
with DAG(
    ...
    po_inbound_snapshot_save_snapshot = open( os.path.join(sql_path, 'po_inbound_snapshot/save_snapshot.sql'),"r").read()

    launch_main_save_snapshot = BigQueryInsertJobOperator(
        task_id = "main_save_snapshot",
        configuration = 
        {
            "query": 
            {
                "query": po_inbound_snapshot_save_snapshot,
                "useLegacySql": False
            }
        },
    ...
    creds_setup >> launch_other_appId_upstream_dependencies_sensor_0 >>[launch_main_save_snapshot]
    launch_main_save_snapshot >> pelican_validator >> pelican_sensor