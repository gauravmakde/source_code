...
dag_id = "gcp_compliance_date_lifecycle_kafka_to_orc_td_16780_TECH_SC_NAP_insights"
...
dag_sla = timedelta(minutes=int(45)) if int(45) > 1 else None
...
default_args = {
    ...
    "description": "compliance_date_lifecycle_kafka_to_orc_td_16780_TECH_SC_NAP_insights DAG Description",
...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region=region,
        topic_name="vendor-compliance-date-lifecycle-analytical-avro",
        consumer_group_name="onix-vendor-compliance-date-lifecycle-analytical-avro_compliance_date_lifecycle_kafka_to_orc_td_16780_TECH_SC_NAP_insights_compliance_date_lifecycle_v2_kafka_to_orc",
    ...
    session_name = get_batch_id(
        dag_id=dag_id, task_id="compliance_date_lifecycle_v2_kafka_to_orc"
    )
    compliance_date_lifecycle_v2_kafka_to_orc = DataprocCreateBatchOperator(
        task_id="compliance_date_lifecycle_v2_kafka_to_orc",
        ...
        batch={
            ...
            "spark_batch": {
                ...
                "args": [
                    f"{user_config_path}/argument_compliance_date_lifecycle_kafka_to_bq_16780.json",
    ...
    compliance_date_lifecycle_compliance_date_lifecycle_bq_to_bq_16780 = open(
        os.path.join(
            sql_path, "compliance_date_lifecycle/compliance_date_lifecycle_bq_to_bq_16780.sql"
        ),
        "r",
    ).read()

    launch_compliance_date_lifecycle_v2_kafka_to_bq = BigQueryInsertJobOperator(
        task_id="compliance_date_lifecycle_v2_kafka_to_bq",
        configuration={
            "query": {
                "query": compliance_date_lifecycle_compliance_date_lifecycle_bq_to_bq_16780,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    compliance_date_lifecycle_compliance_date_event_to_fact = open(
        os.path.join(
            sql_path, "compliance_date_lifecycle/compliance_date_event_to_fact.sql"
        ),
        "r",
    ).read()

    launch_compliance_date_lifecycle_v2_ldg_to_fact = BigQueryInsertJobOperator(
        task_id="compliance_date_lifecycle_v2_ldg_to_fact",
        configuration={
            "query": {
                "query": compliance_date_lifecycle_compliance_date_event_to_fact,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    (
        ...
        >> [compliance_date_lifecycle_v2_kafka_to_orc]
    )
    compliance_date_lifecycle_v2_kafka_to_orc >> [
        launch_compliance_date_lifecycle_v2_kafka_to_bq
    ]
    launch_compliance_date_lifecycle_v2_kafka_to_bq >> [
        launch_compliance_date_lifecycle_v2_ldg_to_fact
    ]
    (
        launch_compliance_date_lifecycle_v2_ldg_to_fact
        >> pelican_validator
