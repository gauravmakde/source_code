...
dag_id = 'gcp_phase_zero_td_v1_16780_TECH_SC_NAP_insights'
...
default_args = {
    ...
    'description': 'phase_zero_td_v1_16780_TECH_SC_NAP_insights DAG Description',
...
with DAG(
    ...
    external_dag_id='gcp_nrlt_wm_inbound_carton_lifecycle_16753_TECH_SC_NAP_insights',
    ...
    external_dag_id='gcp_po_legacy_kafka_to_orc_td_16780_TECH_SC_NAP_insights',
    date_query_fn = upstream_dependency.hours12_query_function,
    timeout = 3600,
    retries = 1,
    poke_interval = 120)
    ...
    external_dag_id='gcp_vendor_asn_kafka_to_orc_td_17268_REDRIVER_SCOT',
    date_query_fn = upstream_dependency.hours12_query_function,
    timeout = 3600,
    retries = 1,
    poke_interval = 120)
    ...
    phase_zero_v1_po_inbound_tableau_extract_stg_ld = open( os.path.join(sql_path, 'phase_zero_v1/po_inbound_tableau_extract_stg_ld.sql'),"r").read()

    launch_main_load_staging = BigQueryInsertJobOperator(
        task_id = "main_load_staging",
        configuration = 
        {
            "query": 
            {
                "query": phase_zero_v1_po_inbound_tableau_extract_stg_ld,
                "useLegacySql": False
            }
        },
    ...
    phase_zero_v1_po_inbound_tableau_extract_fact_ld = open( os.path.join(sql_path, 'phase_zero_v1/po_inbound_tableau_extract_fact_ld.sql'),"r").read()

    launch_main_load_extract = BigQueryInsertJobOperator(
        task_id = "main_load_extract",
        configuration = 
        {
            "query": 
            {
                "query": phase_zero_v1_po_inbound_tableau_extract_fact_ld,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    creds_setup >> [launch_other_appId_upstream_dependencies_sensor_0,launch_other_appId_upstream_dependencies_sensor_1,launch_other_appId_upstream_dependencies_sensor_2]
    launch_other_appId_upstream_dependencies_sensor_0 >> [launch_main_load_staging]
    launch_other_appId_upstream_dependencies_sensor_1 >> [launch_main_load_staging]
    launch_other_appId_upstream_dependencies_sensor_2 >> [launch_main_load_staging]
    launch_main_load_staging >> [launch_main_load_extract]
    launch_main_load_extract >> pelican_validator >> pelican_sensor