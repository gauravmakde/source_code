...
dag_id = "gcp_po_discrepancy_lifecycle_kafka_to_orc_td_16780_TECH_SC_NAP_insights"
...
default_args = {
    ...
    "description": "po_discrepancy_lifecycle_kafka_to_orc_td_16780_TECH_SC_NAP_insights DAG Description",
...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region=region,
        topic_name="inventory-purchase-order-shipment-discrepancy-lifecycle-analytical-avro",
        consumer_group_name="onix-inventory-purchase-order-shipment-discrepancy-lifecycle-analytical-avro_po_discrepancy_lifecycle_kafka_to_orc_td_16780_TECH_SC_NAP_insights_kafka_to_orc_load",
    ...
    common_sql_load_metrics_no_csv = open(
        os.path.join(sql_path, "common_sql/load_metrics_no_csv.sql"), "r"
    ).read()

    launch_metrics_load_metrics = BigQueryInsertJobOperator(
        task_id="metrics_load_metrics",
        configuration={
            "query": {"query": common_sql_load_metrics_no_csv, "useLegacySql": False}
        },
        project_id=project_id,
        gcp_conn_id=gcp_conn,
        params=sql_config_dict,
    ...
    session_name = get_batch_id(dag_id=dag_id, task_id="kafka_to_orc_load")
    kafka_to_orc_load = DataprocCreateBatchOperator(
        task_id="kafka_to_orc_load",
        project_id=project_id,
        region=region,
        gcp_conn_id=gcp_conn,
        batch={
            ...
            "spark_batch": {
                ...
                "args": [
                    f"{user_config_path}/argument_po_discrepancy_lifecycle_kafka_to_orc_td_16780_TECH_SC_NAP_insights_kafka_to_orc_load.json",
    ...
    po_discrepancy_lifecycle_po_discrepancy_orc_to_ldg = open(
        os.path.join(
            sql_path, "po_discrepancy_lifecycle/po_discrepancy_orc_to_ldg.sql"
        ),
        "r",
    ).read()

    launch_orc_to_ldg_load = BigQueryInsertJobOperator(
        task_id="orc_to_ldg_load",
        configuration={
            "query": {
                "query": po_discrepancy_lifecycle_po_discrepancy_orc_to_ldg,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    po_discrepancy_lifecycle_po_discrepancy_ldg_to_fact = open(
        os.path.join(
            sql_path, "po_discrepancy_lifecycle/po_discrepancy_ldg_to_fact.sql"
        ),
        "r",
    ).read()

    launch_ldg_to_fact_load = BigQueryInsertJobOperator(
        task_id="ldg_to_fact_load",
        configuration={
            "query": {
                "query": po_discrepancy_lifecycle_po_discrepancy_ldg_to_fact,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    launch_send_new_relic_metric_send_newrelic_metric = PythonOperator(
        ...
        op_kwargs={
            ...
            "hook": BigQueryHook(
                gcp_conn_id=gcp_conn, location=sql_config_dict['region'], use_legacy_sql=False
            ),
            "bigquery_environment": sql_config_dict["dbenv"],
            "metrics_yaml_file": os.path.join(
                module_path, "yaml/ASCP_PO_DISCREPANCY_LIFECYCLE_METRICS.yaml"
            ),
        },
    )
    ...
    launch_metrics_load_metrics >> [
        kafka_to_orc_load,
        launch_orc_to_ldg_load,
        launch_kafka_start_cntl_track_kafka_start,
    ]
    kafka_to_orc_load >> [launch_kafka_end_cntl_track_kafka_end, launch_orc_to_ldg_load]
    launch_orc_to_ldg_load >> [launch_ldg_to_fact_load]
    launch_ldg_to_fact_load >> [launch_end_batch_load_06_end_batch]
