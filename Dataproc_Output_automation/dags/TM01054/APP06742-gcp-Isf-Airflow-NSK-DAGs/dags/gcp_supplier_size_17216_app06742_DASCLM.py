...
root_path = path.dirname(__file__).split('APP06742-gcp-Isf-Airflow-NSK-DAGs')[0] + 'APP06742-gcp-Isf-Airflow-NSK-DAGs/'
...
dag_id = 'gcp_supplier_size_17216_app06742_DASCLM'
...
# Added a pelican flag from config
...
default_args = {
    ...
    'description': 'supplier_size_17216_app06742_DASCLM DAG Description',
...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region = region,
        topic_name = 'product-item-supplier-size-analytical-avro',
        consumer_group_name = 'onix-product-item-supplier-size-analytical-avro_supplier_size_17216_app06742_DASCLM_load_kafka_to_landing',
    ...
    supplier_size_supplier_size_supplier_size_17216_app06742_DASCLM_load_elt_control_start_updt = open( os.path.join(sql_path, 'supplier_size/elt_control_start_updt.sql'),"r").read()

    launch_load_elt_control_start_updt = BigQueryInsertJobOperator(
        task_id = "load_elt_control_start_updt",
        configuration = 
        {
            "query": 
            {
                "query": supplier_size_supplier_size_supplier_size_17216_app06742_DASCLM_load_elt_control_start_updt,
                "useLegacySql": False
            }
        },
    ...
    supplier_size_supplier_size_supplier_size_17216_app06742_DASCLM_load_clean_staging_table = open( os.path.join(sql_path, 'supplier_size/clean_staging_table.sql'),"r").read()

    launch_load_clear_ldg = BigQueryInsertJobOperator(
        task_id = "load_clear_ldg",
        configuration = 
        {
            "query": 
            {
                "query": supplier_size_supplier_size_supplier_size_17216_app06742_DASCLM_load_clean_staging_table,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    session_name = get_batch_id(dag_id = dag_id,task_id = "load_kafka_to_landing")   
    load_kafka_to_landing = DataprocCreateBatchOperator(
            task_id = "load_kafka_to_landing",
            project_id = project_id,
            region = region,
            gcp_conn_id = gcp_conn,
            batch=
            {    
                "runtime_config": {
            "version": "1.1",
            "properties": {'spark.airflow.run_id': "{{ ti.xcom_pull(task_ids = 'fetch_batch_run_id_task')}}",'spark.sql.mapKeyDedupPolicy': 'LAST_WIN'}
        },
                "spark_batch": 
                    {
                    ...
                    "args": [f"{user_config_path}/argument_supplier_size_17216_app06742_DASCLM_load_kafka_to_landing.json",
    ...
    supplier_size_supplier_size_supplier_size_17216_app06742_DASCLM_load_ldg_to_dim = open( os.path.join(sql_path, 'supplier_size/ldg_to_dim.sql'),"r").read()

    launch_load_ldg_to_dim = BigQueryInsertJobOperator(
        task_id = "load_ldg_to_dim",
        configuration = 
        {
            "query": 
            {
                "query": supplier_size_supplier_size_supplier_size_17216_app06742_DASCLM_load_ldg_to_dim,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    apps_etl_framework_airflow_nsk_dag_job_artifacts_prod_17216_app06742_DASCLM_sql_supplier_size_supplier_size_17216_app06742_DASCLM_load_elt_control_end_updt = open( os.path.join(sql_path, 'supplier_size/elt_control_end_updt.sql'),"r").read()

    launch_load_elt_control_end_updt = BigQueryInsertJobOperator(
        task_id = "load_elt_control_end_updt1",
        configuration = 
        {
            "query": 
            {
                "query": apps_etl_framework_airflow_nsk_dag_job_artifacts_prod_17216_app06742_DASCLM_sql_supplier_size_supplier_size_17216_app06742_DASCLM_load_elt_control_end_updt,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    creds_setup >> fetch_batch_run_id_task >> launch_load_elt_control_start_updt >> [launch_load_clear_ldg]
    launch_load_clear_ldg >> [load_kafka_to_landing]
    load_kafka_to_landing >> [launch_load_ldg_to_dim]
    launch_load_ldg_to_dim >> [launch_load_elt_control_end_updt]
    launch_load_elt_control_end_updt >> pelican_validator >> pelican_sensor
