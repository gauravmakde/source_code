...
dag_id = "gcp_disc_terms_agg_fact_final_data_load_10976_tech_nap_merch"
...
default_args = {
    ...
    "description": "disc_terms_agg_fact_final_data_load_10976_tech_nap_merch DAG Description",
...
with DAG(
    ...
    disc_terms_agg_final_fact_data_load = open(
        os.path.join(
            sql_path,
            "disc_terms_agg_final_fact_data_load.sql",
        ),
        "r",
    ).read()

    launch_disc_terms_agg_fact_data_load_job_00 = BigQueryInsertJobOperator(
        task_id="disc_terms_agg_fact_data_load_job_00",
        configuration={
            "query": {
                "query": disc_terms_agg_final_fact_data_load,
                "useLegacySql": False,
            }
        },
    ...
    disc_terms_batch_date_update = open(
        os.path.join(
            sql_path,
            "disc_terms_batch_date_update.sql",
        ),
        "r",
    ).read()

    launch_disc_terms_agg_fact_data_load_job_01 = BigQueryInsertJobOperator(
        task_id="disc_terms_agg_fact_data_load_job_01",
        configuration={
            "query": {
                "query": disc_terms_batch_date_update,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    (
        ...
        >> [launch_disc_terms_agg_fact_data_load_job_00]
    )
    launch_disc_terms_agg_fact_data_load_job_00 >> [
        launch_disc_terms_agg_fact_data_load_job_01
    ]
    launch_disc_terms_agg_fact_data_load_job_01 >> pelican_validator >> pelican_sensor
