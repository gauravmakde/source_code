# -*- coding: utf-8 -*-
# DAG version:          default
# CI commit sha:        b4c01d6207fd9b489a11c61efc919fd28fca9993
# CI pipeline URL:      https://git.jwn.app/TM00593/das_merch/APP04070-gcp-MFP_kafka_to_TD_IF/-/pipelines/6791855
# CI commit timestamp:  2024-08-23T10:22:16+00:00
...
dag_id = 'gcp_mfp_costvarianceledger_kafka_to_teradata_10976_tech_nap_merch'
...
default_args = {
    ...
    'description': 'mfp_costvarianceledger_kafka_to_teradata_10976_tech_nap_merch DAG Description',
...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region = region,
        topic_name = 'inventory-cost-variance-ledger-analytical-avro',
        consumer_group_name = 'onix-inventory-cost-variance-ledger-analytical-avro_mfp_costvarianceledger_kafka_to_teradata_10976_tech_nap_merch_topic_to_s3_read_from_kafka_write_to_s3',
    ...
    session_name=get_batch_id(dag_id=dag_id,task_id="topic_to_s3_read_from_kafka_write_to_s3")   
    topic_to_s3_read_from_kafka_write_to_s3 = DataprocCreateBatchOperator(
            task_id = "topic_to_s3_read_from_kafka_write_to_s3",
            ...
            {    
                ...
                "spark_batch": 
                    ...
                    "args": [f"{user_config_path}/argument_mfp_costvarianceledger_kafka_to_teradata_10976_tech_nap_merch_topic_to_s3_read_from_kafka_write_to_s3.json",
    ...
    tpt_jdbc_insert_topic_to_s3_load_to_landing_table_using_tpt_control_tbl = BashOperator(
    task_id='tpt_jdbc_insert_topic_to_s3_load_to_landing_table_using_tpt_control_tbl',
    bash_command=f'echo "Skipping Control Table Entry"')

    # tpt_topic_to_s3_load_to_landing_table_using_tpt_load_tbl = SSHOperator(
    # task_id='tpt_topic_to_s3_load_to_landing_table_using_tpt_load_tbl',
    # command="/db/teradata/bin/tpt_load.sh  -e PRD_NAP_UTL -j PRD_NAP_STG_MERCH_COST_VARIANCE_LEDGER_LDG_JOB -h tdnapprod.nordstrom.net -l 'N' -u SCH_NAP_MERCH_BATCH -p \"\$tdwallet(SCH_NAP_MERCH_BATCH_PWD)\" -f s3://nap-das-merch-tpt-prod-v1/costvarianceledger/*/*/*/* -o UPDATE",
    # timeout=1800,
    # ssh_conn_id="TECH_ISF_NAP_MERCH_MFP_SECURITY_TDUTIL_SERVER_S3EXT_PROD")

    
    costvarianceledger_rtv_td_fact_load = open(
    os.path.join(sql_path, 'costvarianceledger_rtv_td_fact_load.sql'),
    "r").read()

    launch_costvarianceledger_rtv_load_to_fact = BigQueryInsertJobOperator(
        task_id = "costvarianceledger_rtv_load_to_fact",
        configuration = 
        {
            "query": 
            {
                "query": costvarianceledger_rtv_td_fact_load,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    costvarianceledger_allocation_td_fact_load = open(
    os.path.join(sql_path, 'costvarianceledger_allocation_td_fact_load.sql'),
    "r").read()

    launch_costvarianceledger_allocation_load_to_fact = BigQueryInsertJobOperator(
        task_id = "costvarianceledger_allocation_load_to_fact",
        configuration = 
        {
            "query": 
            {
                "query": costvarianceledger_allocation_td_fact_load,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    costvarianceledger_transfer_td_fact_load = open(
    os.path.join(sql_path, 'costvarianceledger_transfer_td_fact_load.sql'),
    "r").read()

    launch_costvarianceledger_transfer_load_to_fact = BigQueryInsertJobOperator(
        task_id = "costvarianceledger_transfer_load_to_fact",
        configuration = 
        {
            "query": 
            {
                "query": costvarianceledger_transfer_td_fact_load,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    costvarianceledger_purchase_order_td_fact_load = open(
    os.path.join(sql_path, 'costvarianceledger_purchase_order_td_fact_load.sql'),
    "r").read()

    launch_costvarianceledger_po_load_to_fact = BigQueryInsertJobOperator(
        task_id = "costvarianceledger_po_load_to_fact",
        configuration = 
        {
            "query": 
            {
                "query": costvarianceledger_purchase_order_td_fact_load,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    costvarianceledger_cost_variance_td_fact_load = open(
    os.path.join(sql_path, 'costvarianceledger_cost_variance_td_fact_load.sql'),
    "r").read()

    launch_cost_variance_load_to_fact = BigQueryInsertJobOperator(
        task_id = "cost_variance_load_to_fact",
        configuration = 
        {
            "query": 
            {
                "query": costvarianceledger_cost_variance_td_fact_load,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    # launch_post_run_s3_cleanup_stage_v2 = launch_k8s_api_job_operator(
    # dag=dag,
    # connection_id='TECH_Merchandising_NAP_Engineering_K8S_CONN_PROD',
    # namespace='app04070',
    # task_id='post_run_s3_cleanup_stage_v2',
    # job_name='post-run-s3-cleanup-stage-v2-mfp-costvarianceledger-kafka-to-te',
    # macros=None,
    ...
    # container_command=['python', '/home/nonroot/python/s3cleanup/main.py', 'costvarianceledger'],
    ...
    creds_setup >> fetch_batch_run_id_task >>[topic_to_s3_read_from_kafka_write_to_s3]
    topic_to_s3_read_from_kafka_write_to_s3 >>[tpt_jdbc_insert_topic_to_s3_load_to_landing_table_using_tpt_control_tbl]
    tpt_jdbc_insert_topic_to_s3_load_to_landing_table_using_tpt_control_tbl >>[launch_cost_variance_load_to_fact, launch_costvarianceledger_allocation_load_to_fact, launch_costvarianceledger_po_load_to_fact, launch_costvarianceledger_rtv_load_to_fact, launch_costvarianceledger_transfer_load_to_fact]
    launch_costvarianceledger_rtv_load_to_fact >> pelican_validator
    launch_costvarianceledger_allocation_load_to_fact >> pelican_validator
    launch_costvarianceledger_transfer_load_to_fact >> pelican_validator
    launch_costvarianceledger_po_load_to_fact >> pelican_validator
    launch_cost_variance_load_to_fact >> pelican_validator
