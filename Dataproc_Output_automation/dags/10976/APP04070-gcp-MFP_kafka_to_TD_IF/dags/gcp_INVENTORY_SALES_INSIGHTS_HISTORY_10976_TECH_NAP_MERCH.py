# -*- coding: utf-8 -*-
# DAG version:          default
# CI commit sha:        7a165d10fce1fd8ecc7d0b77efc9f079f6a2b07d
# CI pipeline URL:      https://git.jwn.app/TM00593/das_merch/APP04070-gcp-MFP_kafka_to_TD_IF/-/pipelines/7294703
# CI commit timestamp:  2024-10-22T16:08:12+00:00
...
dag_id = 'gcp_inventory_sales_insights_history_10976_tech_nap_merch'
...
from modules.sensor_utils_modules import sensor_utils
...
default_args = {
    ...
    'description': 'inventory_sales_insights_history_10976_tech_nap_merch DAG Description',
...
with DAG(
    ...
    # date_query_fn=sensor_utils.get_dag_run_date_range,
    # timeout=3600,
    # retries=3,
    # poke_interval=60)
    ...
    inventory_sales_insights_history_etl_batch_dt_start_updt = open( os.path.join(sql_path, 'inventory_sales_insights_history/etl_batch_dt_start_updt.sql'),"r").read()
    ...
    launch_execute_elt_control_start_update = BigQueryInsertJobOperator(
        ...
        configuration = 
        {
            "query": 
            {
                "query": inventory_sales_insights_history_etl_batch_dt_start_updt,
                "useLegacySql": False
            }
        },
    ...
    inventory_sales_insights_history_delete_metrics = open( os.path.join(sql_path, 'inventory_sales_insights_history/delete_metrics.sql'),"r").read()
    ...
    launch_execute_delete_metrics = BigQueryInsertJobOperator(
        ...
        configuration = 
        {
            "query": 
            {
                "query": inventory_sales_insights_history_delete_metrics,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    inventory_sales_insights_history_load_inventory = open( os.path.join(sql_path, 'inventory_sales_insights_history/load_inventory.sql'),"r").read()
    ...
    launch_execute_load_inventory = BigQueryInsertJobOperator(
        ...
        configuration = 
        {
            "query": 
            {
                "query": inventory_sales_insights_history_load_inventory,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    inventory_sales_insights_history_load_sales = open( os.path.join(sql_path, 'inventory_sales_insights_history/load_sales.sql'),"r").read()
    ...
    launch_execute_load_sales = BigQueryInsertJobOperator(
        ...
        configuration = 
        {
            "query": 
            {
                "query": inventory_sales_insights_history_load_sales,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    launch_other_appId_upstream_dependencies_sensor_1 >>[launch_execute_elt_control_start_update]
    ...
    launch_execute_load_sales >> pelican_validator >> pelican_sensor