...
dag_id = "gcp_merch_nap_mri_transaction_fact_to_day_aggregate_daily_load_10976_tech_nap_merch"
...
default_args = {
    ...
    "description": "merch_nap_mri_transaction_fact_to_day_aggregate_daily_load_10976_tech_nap_merch DAG Description",
...
with DAG(
    ...
    mric_day_batch_update_dly = open(
        os.path.join(sql_path, "mric_day_batch_update_dly.sql"), "r"
    ).read()

    launch_mric_mric_day_agg_dly_bf_date_roll = BigQueryInsertJobOperator(
        task_id="mric_mric_day_agg_dly_bf_date_roll",
        configuration={
            "query": {"query": mric_day_batch_update_dly, "useLegacySql": False}
        },
    ...
    merch_transaction_sku_store_day_agg_wrk_truncate_daily = open(
        os.path.join(
            sql_path, "merch_transaction_sku_store_day_agg_wrk_truncate_daily.sql"
        ),
        "r",
    ).read()
    ...
    launch_mric_merch_transaction_sku_store_day_agg_wrk_truncate = (
        BigQueryInsertJobOperator(
            ...
            configuration={
                "query": {
                    "query": merch_transaction_sku_store_day_agg_wrk_truncate_daily,
                    "useLegacySql": False,
                }
            },
            project_id=project_id,
    ...
    merch_inventory_sku_store_day_agg_wrk_daily_load = open(
        os.path.join(sql_path, "merch_inventory_sku_store_day_agg_wrk_daily_load.sql"),
        "r",
    ).read()
    ...
    launch_mric_merch_inventory_sku_store_day_agg_wrk_load = BigQueryInsertJobOperator(
        ...
        configuration={
            "query": {
                "query": merch_inventory_sku_store_day_agg_wrk_daily_load,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    merch_jwn_demand_sku_store_day_agg_wrk_daily_load = open(
        os.path.join(sql_path, "merch_jwn_demand_sku_store_day_agg_wrk_daily_load.sql"),
        "r",
    ).read()
    ...
    launch_mric_merch_jwn_demand_sku_store_day_agg_wrk_load = BigQueryInsertJobOperator(
        ...
        configuration={
            "query": {
                "query": merch_jwn_demand_sku_store_day_agg_wrk_daily_load,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    merch_jwn_sale_sku_store_day_agg_wrk_daily_load = open(
        os.path.join(sql_path, "merch_jwn_sale_sku_store_day_agg_wrk_daily_load.sql"),
        "r",
    ).read()
    ...
    launch_mric_merch_jwn_sale_sku_store_day_agg_wrk_load = BigQueryInsertJobOperator(
        ...
        configuration={
            "query": {
                "query": merch_jwn_sale_sku_store_day_agg_wrk_daily_load,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    merch_transaction_sku_store_day_agg_fact_daily_load = open(
        os.path.join(
            sql_path, "merch_transaction_sku_store_day_agg_fact_daily_load.sql"
        ),
        "r",
    ).read()
    ...
    launch_mric_merch_transaction_sku_store_day_agg_fact_load = (
        BigQueryInsertJobOperator(
            ...
            configuration={
                "query": {
                    "query": merch_transaction_sku_store_day_agg_fact_daily_load,
                    "useLegacySql": False,
                }
            },
            project_id=project_id,
    ...
    launch_other_appId_upstream_dependencies_sensor_1 >> [
        launch_mric_mric_day_agg_dly_bf_date_roll
    ]
    launch_mric_mric_day_agg_dly_bf_date_roll >> [
        launch_mric_merch_transaction_sku_store_day_agg_wrk_truncate
    ]
    launch_mric_merch_transaction_sku_store_day_agg_wrk_truncate >> [
        launch_mric_merch_inventory_sku_store_day_agg_wrk_load
    ]
    launch_mric_merch_inventory_sku_store_day_agg_wrk_load >> [
        launch_mric_merch_jwn_demand_sku_store_day_agg_wrk_load
    ]
    launch_mric_merch_jwn_demand_sku_store_day_agg_wrk_load >> [
        launch_mric_merch_jwn_sale_sku_store_day_agg_wrk_load
    ]
    launch_mric_merch_jwn_sale_sku_store_day_agg_wrk_load >> [
        launch_mric_merch_transaction_sku_store_day_agg_fact_load
    ]
