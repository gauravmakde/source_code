...
dag_id = "gcp_merch_clearance_markdown_receipt_date_week_load_10976_tech_nap_merch"
...
default_args = {
    ...
    "description": "merch_clearance_markdown_receipt_date_week_load_10976_tech_nap_merch DAG Description",
...
with DAG(
    ...
    launch_other_appId_upstream_dependencies_sensor_0 = ApiDagRunLatestSensor(
        ...
        external_dag_id="gcp_merch_clearance_markdown_receipt_date_load_10976_tech_nap_merch",
    ...
    smart_markdown_insights_merch_receipt_date_reset_applied_week_fact_load = open(
        os.path.join(
            sql_path,
            "smart_markdown_insights/merch_receipt_date_reset_applied_week_fact_load.sql",
        ),
        "r",
    ).read()

    launch_run_receipt_date_reset_applied_week_fact_load = BigQueryInsertJobOperator(
        task_id="run_receipt_date_reset_applied_week_fact_load",
        configuration={
            "query": {
                "query": smart_markdown_insights_merch_receipt_date_reset_applied_week_fact_load,
                "useLegacySql": False,
            }
        },
    ...
    smart_markdown_insights_merch_receipt_date_batch_date_updt = open(
        os.path.join(
            sql_path, "smart_markdown_insights/merch_receipt_date_batch_date_updt.sql"
        ),
        "r",
    ).read()

    launch_run_receipt_date_batch_date_updt = BigQueryInsertJobOperator(
        task_id="run_receipt_date_batch_date_updt",
        configuration={
            "query": {
                "query": smart_markdown_insights_merch_receipt_date_batch_date_updt,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    (
        ...
        >> [launch_run_receipt_date_reset_applied_week_fact_load]
    )
    launch_run_receipt_date_reset_applied_week_fact_load >> [
        launch_run_receipt_date_batch_date_updt
    ]
    launch_run_receipt_date_batch_date_updt >> pelican_validator >> pelican_sensor
