...
dag_id = 'gcp_spe_ap_supp_grp_fct_wkly_10976_tech_nap_merch'
...
default_args = {
    ...
    'description': 'spe_ap_supp_grp_fct_wkly_10976_tech_nap_merch DAG Description',
...
#     conn_id='okta_conn_to_APP04070_team',
#     external_dag_id='gcp_spe_mfp_dept_banner_10976_tech_nap_merch',
...
# )
    ...
    merch_spe_ap_suppgrp_bnr_yr_fact_load = open(
    os.path.join(sql_path, 'merch_spe_ap_suppgrp_bnr_yr_fact_load.sql'),
    "r").read()

    launch_SPE_AP_SUPP_GRP_FACT_t1_spe_ap_supgrp_bnr_fact_load = BigQueryInsertJobOperator(
        task_id = "SPE_AP_SUPP_GRP_FACT_t1_spe_ap_supgrp_bnr_fact_load",
        configuration = 
        {
            "query": 
            {
                "query": merch_spe_ap_suppgrp_bnr_yr_fact_load,
                "useLegacySql": False
            }
        },
    ...
    merch_spe_dly_batch_updt = open(
    os.path.join(sql_path, 'merch_spe_dly_batch_updt.sql'),
    "r").read()

    launch_SPE_AP_SUPP_GRP_FACT_t2_spe_batch_updt = BigQueryInsertJobOperator(
        task_id = "SPE_AP_SUPP_GRP_FACT_t2_spe_batch_updt",
        configuration = 
        {
            "query": 
            {
                "query": merch_spe_dly_batch_updt,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    # launch_other_appId_upstream_dependencies_sensor_1 >>[launch_SPE_AP_SUPP_GRP_FACT_t1_spe_ap_supgrp_bnr_fact_load]
    # launch_SPE_AP_SUPP_GRP_FACT_t1_spe_ap_supgrp_bnr_fact_load >>[launch_SPE_AP_SUPP_GRP_FACT_t2_spe_batch_updt]
    # launch_SPE_AP_SUPP_GRP_FACT_t2_spe_batch_updt >> pelican_validator >> pelican_sensor

    creds_setup >>[launch_SPE_AP_SUPP_GRP_FACT_t1_spe_ap_supgrp_bnr_fact_load]
    launch_SPE_AP_SUPP_GRP_FACT_t1_spe_ap_supgrp_bnr_fact_load >>[launch_SPE_AP_SUPP_GRP_FACT_t2_spe_batch_updt]
    launch_SPE_AP_SUPP_GRP_FACT_t2_spe_batch_updt >> pelican_validator >> pelican_sensor 