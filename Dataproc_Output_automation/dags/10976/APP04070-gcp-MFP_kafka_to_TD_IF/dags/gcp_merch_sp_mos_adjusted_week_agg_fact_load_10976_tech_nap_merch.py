...
dag_id = "gcp_merch_sp_mos_adjusted_week_agg_fact_load_10976_tech_nap_merch"
...
default_args = {
    ...
    "description": "merch_sp_mos_adjusted_week_agg_fact_load_10976_tech_nap_merch DAG Description",
...
with DAG(
    ...
    launch_other_appId_upstream_dependencies_sensor_0 = ApiDagRunLatestSensor(
        ...
        external_dag_id="gcp_merch_mos_sp_adjusted_cost_td_ledger_load_10976_tech_nap_merch",
    ...
    merch_sp_mos_adjusted_week_fact_load = open(
        os.path.join(sql_path, "merch_sp_mos_adjusted_week_fact_load.sql"), "r"
    ).read()

    launch_mos_agg_fact_load_job_2 = BigQueryInsertJobOperator(
        task_id="mos_agg_fact_load_job_2",
        configuration={
            "query": {
                "query": merch_sp_mos_adjusted_week_fact_load,
                "useLegacySql": False,
            }
        },
    ...
    merch_sp_mos_adjusted_batch_date_update = open(
        os.path.join(sql_path, "merch_sp_mos_adjusted_batch_date_update.sql"), "r"
    ).read()

    launch_mos_agg_fact_load_job_3 = BigQueryInsertJobOperator(
        task_id="mos_agg_fact_load_job_3",
        configuration={
            "query": {
                "query": merch_sp_mos_adjusted_batch_date_update,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    (
        ...
        >> [launch_mos_agg_fact_load_job_2]
    )
    launch_mos_agg_fact_load_job_2 >> [launch_mos_agg_fact_load_job_3]
    launch_mos_agg_fact_load_job_3 >> pelican_validator >> pelican_sensor
