...
dag_id = 'gcp_rtv_week_agg_fact_load_10976_tech_nap_merch'
...
default_args = {
    ...
    'description': 'rtv_week_agg_fact_load_10976_tech_nap_merch DAG Description',
...
with DAG(
    ...
    external_dag_id='gcp_mfpc_return_to_vendor_kafka_to_teradata_10976_tech_nap_merch',
    ...
    rtv_agg_final_fact_data_load = open(
    os.path.join(sql_path, 'rtv_agg_final_fact_data_load.sql'),
    "r").read()

    launch_rtv_agg_fact_load_job_2 = BigQueryInsertJobOperator(
        task_id = "rtv_agg_fact_load_job_2",
        configuration = 
        {
            "query": 
            {
                "query": rtv_agg_final_fact_data_load,
                "useLegacySql": False
            }
        },
    ...
    rtv_batch_date_update = open(
    os.path.join(sql_path, 'rtv_batch_date_update.sql'),
    "r").read()

    launch_rtv_agg_fact_load_job_3 = BigQueryInsertJobOperator(
        task_id = "rtv_agg_fact_load_job_3",
        configuration = 
        {
            "query": 
            {
                "query": rtv_batch_date_update,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    creds_setup >> launch_other_appId_upstream_dependencies_sensor_0 >>[launch_rtv_agg_fact_load_job_2]
    launch_rtv_agg_fact_load_job_2 >>[launch_rtv_agg_fact_load_job_3]
    launch_rtv_agg_fact_load_job_3 >> pelican_validator >> pelican_sensor