...
dag_id = "gcp_merch_on_order_fact_snapshot_load_10976_tech_nap_merch"
...
default_args = {
    ...
    "description": "merch_on_order_fact_snapshot_load_10976_tech_nap_merch DAG Description",
...
with DAG(
    ...
    merch_on_order_fact_snapshot_truncate = open(
        os.path.join(sql_path, "merch_on_order_fact_snapshot_truncate.sql"), "r"
    ).read()

    launch_snap_stage_truncate_job = BigQueryInsertJobOperator(
        task_id="snap_stage_truncate_job",
        configuration={
            "query": {
                "query": merch_on_order_fact_snapshot_truncate,
                "useLegacySql": False,
            }
        },
    ...
    merch_on_order_fact_snapshot_load = open(
        os.path.join(sql_path, "merch_on_order_fact_snapshot_load.sql"), "r"
    ).read()

    launch_snap_stage_snap_job = BigQueryInsertJobOperator(
        task_id="snap_stage_snap_job",
        configuration={
            "query": {"query": merch_on_order_fact_snapshot_load, "useLegacySql": False}
        },
        project_id=project_id,
        gcp_conn_id=gcp_conn,
        params=sql_config_dict,
    ...
    (
        ...
        >> [launch_snap_stage_truncate_job]
    )
    launch_snap_stage_truncate_job >> [launch_snap_stage_snap_job]
    launch_snap_stage_snap_job >> pelican_validator >> pelican_sensor
