...
dag_id = 'gcp_inventory_apt_week_fact_load_dly_10976_tech_nap_merch'
...
default_args = {
    ...
    'description': 'inventory_apt_week_fact_load_dly_10976_tech_nap_merch DAG Description',
...
with DAG(
    ...
    merch_inventory_sku_store_week_fct_load_dly = open(
    os.path.join(sql_path, 'merch_inventory_sku_store_week_fct_load_dly.sql'),
    "r").read()

    launch_inventory_apt_data_load_dly_inv_week_fct_load = BigQueryInsertJobOperator(
        task_id = "inventory_apt_data_load_dly_inv_week_fct_load",
        configuration = 
        {
            "query": 
            {
                "query": merch_inventory_sku_store_week_fct_load_dly,
                "useLegacySql": False
            }
        },
    ...
    os.path.join(sql_path, 'inv_dw_batch_update.sql'),
    "r").read()

    launch_inventory_apt_data_load_dly_inv_dw_batch_update = BigQueryInsertJobOperator(
        task_id = "inventory_apt_data_load_dly_inv_dw_batch_update",
        configuration = 
        {
            "query": 
            {
                "query": inv_dw_batch_update,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    launch_other_appId_upstream_dependencies_sensor_3 >>[launch_inventory_apt_data_load_dly_inv_week_fct_load]
    launch_inventory_apt_data_load_dly_inv_week_fct_load >>[launch_inventory_apt_data_load_dly_inv_dw_batch_update]
    launch_inventory_apt_data_load_dly_inv_dw_batch_update >> pelican_validator >> pelican_sensor