...
dag_id = "gcp_vfmd_columnar_view_data_load_10976_tech_nap_merch"
...
default_args = {
    ...
    "description": "vfmd_columnar_view_data_load_10976_tech_nap_merch DAG Description",
...
with DAG(
    ...
    vfmd_agg_fact_load_apt = open(
        os.path.join(sql_path, "vfmd_agg_fact_load_apt.sql"), "r"
    ).read()

    launch_vfmd_columnar_data_load_job_0 = BigQueryInsertJobOperator(
        task_id="vfmd_columnar_data_load_job_0",
        configuration={
            "query": {"query": vfmd_agg_fact_load_apt, "useLegacySql": False}
        },
    ...
    vfmd_batch_date_update = open(
        os.path.join(sql_path, "vfmd_batch_date_update.sql"), "r"
    ).read()

    launch_vfmd_columnar_data_load_job_1 = BigQueryInsertJobOperator(
        task_id="vfmd_columnar_data_load_job_1",
        configuration={
            "query": {"query": vfmd_batch_date_update, "useLegacySql": False}
        },
        project_id=project_id,
        gcp_conn_id=gcp_conn,
        params=sql_config_dict,
    ...
    (
        ...
        >> [launch_vfmd_columnar_data_load_job_0]
    )
    launch_vfmd_columnar_data_load_job_0 >> [launch_vfmd_columnar_data_load_job_1]
    launch_vfmd_columnar_data_load_job_1 >> pelican_validator >> pelican_sensor
