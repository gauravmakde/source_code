...
CALL`{{params.gcp_project_id}}`.{{params.dbenv}}_NAP_UTL.DATA_TIMELINESS_METRIC_FACT_LD('MERCH_COST_VARIANCE_LEDGER_ALLOCATION_WRK', '{{params.database_name_staging}}',  'mfp_costvarianceledger_kafka_to_teradata',  'load_to_allocation_fact_03',  0,  'LOAD_START',  'Clearing WRK table',  CAST(FORMAT_TIMESTAMP('%F %H:%M:%E6S',current_datetime('PST8PDT')) AS DATETIME),  '');


TRUNCATE TABLE `{{params.gcp_project_id}}`.{{params.database_name_staging}}.merch_cost_variance_ledger_allocation_wrk;


CALL`{{params.gcp_project_id}}`.{{params.dbenv}}_NAP_UTL.DATA_TIMELINESS_METRIC_FACT_LD('MERCH_COST_VARIANCE_LEDGER_ALLOCATION_WRK',  '{{params.database_name_staging}}',  'mfp_costvarianceledger_kafka_to_teradata',  'load_to_allocation_fact_03',  1,  'INTERMEDIATE',  'Populating WRK table',  CAST(FORMAT_TIMESTAMP('%F %H:%M:%E6S',current_datetime('PST8PDT')) AS DATETIME),  '');


INSERT INTO `{{params.gcp_project_id}}`.{{params.database_name_staging}}.merch_cost_variance_ledger_allocation_wrk (location_id, last_updated_time_in_millis,
 last_updated_time,last_updated_time_tz, sku_id, tran_date, event_id, event_time, correlation_id, allocation_num, tran_code, quantity,
...
(SELECT location_id,
  ...
  correlation_id,
  reference_id AS allocation_num,
  CAST(trunc(cast(CASE
    WHEN tran_code = ''
    THEN '0'
    ELSE tran_code
 ...
 WHERE LOWER(event_type) = LOWER('ALLOCATION'));


CALL`{{params.gcp_project_id}}`.{{params.dbenv}}_NAP_UTL.DATA_TIMELINESS_METRIC_FACT_LD('MERCH_COST_VARIANCE_LEDGER_ALLOCATION_FCT',  ' {{params.database_name_fact}}',  'mfp_costvarianceledger_kafka_to_teradata',  'load_to_allocation_fact_03',  2,  'INTERMEDIATE',  'Clearing fact table',  CAST(FORMAT_TIMESTAMP('%F %H:%M:%E6S',current_datetime('PST8PDT')) AS DATETIME),  '');


DELETE FROM `{{params.gcp_project_id}}`.{{params.database_name_fact}}.merch_cost_variance_ledger_allocation_fct AS tgt
WHERE EXISTS (SELECT *
    FROM `{{params.gcp_project_id}}`.{{params.database_name_staging}}.merch_cost_variance_ledger_allocation_wrk AS src
...
CALL`{{params.gcp_project_id}}`.{{params.dbenv}}_NAP_UTL.DATA_TIMELINESS_METRIC_FACT_LD('MERCH_COST_VARIANCE_LEDGER_ALLOCATION_FCT',  '{{params.database_name_fact}}',  'mfp_costvarianceledger_kafka_to_teradata',  'load_to_allocation_fact_03',  3,  'INTERMEDIATE',  'Filling fact table',  CAST(FORMAT_TIMESTAMP('%F %H:%M:%E6S',current_datetime('PST8PDT')) AS DATETIME),  '');


INSERT INTO `{{params.gcp_project_id}}`.{{params.database_name_fact}}.merch_cost_variance_ledger_allocation_fct (location_id, last_updated_time_in_millis,
 last_updated_time, last_updated_time_tz,sku_id, tran_date, event_id, event_time, correlation_id, allocation_num, tran_code, quantity,
 dept_num, class_num, subclass_num, sku_type, total_cost_currcycd, total_cost_amt, total_retail_currcycd,
 total_retail_amt, dw_sys_load_tmstp, dw_sys_updt_tmstp)
(SELECT location_id,
  ...
  dw_sys_load_tmstp,
  dw_sys_updt_tmstp
 FROM `{{params.gcp_project_id}}`.{{params.database_name_staging}}.merch_cost_variance_ledger_allocation_wrk);




CALL`{{params.gcp_project_id}}`.{{params.dbenv}}_NAP_UTL.DATA_TIMELINESS_METRIC_FACT_LD('MERCH_COST_VARIANCE_LEDGER_ALLOCATION_FCT',  '{{params.database_name_fact}}',  'mfp_costvarianceledger_kafka_to_teradata',  'load_to_allocation_fact_03',  4,  'LOAD_END',  '',  CAST(FORMAT_TIMESTAMP('%F %H:%M:%E6S',current_datetime('PST8PDT')) AS DATETIME),  '');
