...
INSERT INTO `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_stg.merch_allocation_fact_sku_store_wrk (store_num, rms_sku_num, week_num, business_unit_num,
...
(SELECT CAST(TRUNC(CAST(RPAD(CAST(jrthd.store_num AS STRING), 4, ' ') AS FLOAT64)) AS INTEGER) AS store_num,
 ...
 FROM (SELECT a.rms_sku_num,
   ...
   WHERE a.business_day_date BETWEEN (SELECT DATE_SUB(dcd.week_start_day_date, INTERVAL 60 DAY)
    ...
    AND a.dw_batch_date BETWEEN (SELECT dcd0.week_start_day_date
      FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.day_cal_454_dim AS dcd0
       INNER JOIN `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_base_vws.etl_batch_dt_lkup AS ebd0 ON dcd0.day_date = ebd0.dw_batch_dt
      WHERE LOWER(ebd0.interface_code) = LOWER('MERCH_ALLOCATION_DLY')) AND (SELECT dw_batch_dt
      FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_base_vws.etl_batch_dt_lkup
      WHERE LOWER(interface_code) = LOWER('MERCH_ALLOCATION_DLY'))
