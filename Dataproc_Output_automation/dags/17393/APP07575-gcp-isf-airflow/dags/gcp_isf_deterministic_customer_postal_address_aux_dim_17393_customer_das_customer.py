...
dag_id = 'gcp_isf_deterministic_customer_postal_address_aux_dim_17393_customer_das_customer'
...
default_args = {
    ...
    'description': 'isf_deterministic_customer_postal_address_aux_dim_17393_customer_das_customer DAG Description',
...
with DAG(
    ...
    timeout=7200,
    external_task_id='ELT_BATCH_END',
    execution_delta=timedelta(minutes=120))

    
        
    deterministic_profile_deterministic_customer_postal_address_aux_elt_control_activate = open( os.path.join(sql_path, 'deterministic_profile/deterministic_customer_postal_address_aux_elt_control_activate.sql'),"r").read()

    launch_customer_postal_address_aux_elt_control_activate = BigQueryInsertJobOperator(
        task_id = "customer_postal_address_aux_elt_control_activate",
        configuration = 
        {
            "query": 
            {
                "query": deterministic_profile_deterministic_customer_postal_address_aux_elt_control_activate,
                "useLegacySql": False
            }
        },
    ...
    deterministic_profile_deterministic_customer_postal_address_aux_wrk = open( os.path.join(sql_path, 'deterministic_profile/deterministic_customer_postal_address_aux_wrk.sql'),"r").read()

    launch_customer_postal_address_aux_wrk = BigQueryInsertJobOperator(
        task_id = "customer_postal_address_aux_wrk",
        configuration = 
        {
            "query": 
            {
                "query": deterministic_profile_deterministic_customer_postal_address_aux_wrk,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    deterministic_profile_deterministic_customer_postal_address_aux_dim = open( os.path.join(sql_path, 'deterministic_profile/deterministic_customer_postal_address_aux_dim.sql'),"r").read()

    launch_customer_postal_address_aux_dim = BigQueryInsertJobOperator(
        task_id = "customer_postal_address_aux_dim",
        configuration = 
        {
            "query": 
            {
                "query": deterministic_profile_deterministic_customer_postal_address_aux_dim,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    launch_customer_postal_address_aux_send_newrelic_metric=PythonOperator(
        task_id='customer_postal_address_aux_send_newrelic_metric',
    ...
    deterministic_profile_deterministic_customer_postal_address_aux_elt_control_deactivate = open( os.path.join(sql_path, 'deterministic_profile/deterministic_customer_postal_address_aux_elt_control_deactivate.sql'),"r").read()

    launch_customer_postal_address_aux_elt_control_deactivate = BigQueryInsertJobOperator(
        task_id = "customer_postal_address_aux_elt_control_deactivate",
        configuration = 
        {
            "query": 
            {
                "query": deterministic_profile_deterministic_customer_postal_address_aux_elt_control_deactivate,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    creds_setup >> launch_check_external_upstream_dependencies_sensor_0 >>[launch_customer_postal_address_aux_elt_control_activate]
    launch_customer_postal_address_aux_elt_control_activate >>[launch_customer_postal_address_aux_wrk]
    launch_customer_postal_address_aux_wrk >>[launch_customer_postal_address_aux_dim]
    launch_customer_postal_address_aux_dim >>[launch_customer_postal_address_aux_send_newrelic_metric]
    launch_customer_postal_address_aux_send_newrelic_metric >>[launch_customer_postal_address_aux_elt_control_deactivate]
    launch_customer_postal_address_aux_elt_control_deactivate >> pelican_validator >> pelican_sensor