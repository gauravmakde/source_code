...
dag_id = 'gcp_isf_customer_object_ldg_to_dim_17393_customer_das_customer'
...
default_args = {
    ...
    'description': 'isf_customer_object_ldg_to_dim_17393_customer_das_customer DAG Description',
...
with DAG(
    ...
    customer_object_ldg_to_wrk = open( os.path.join(sql_path, 'customer_object/customer_object_ldg_to_wrk.sql'),"r").read()

    launch_customer_object_ldg_to_wrk = BigQueryInsertJobOperator(
        task_id = "customer_object_ldg_to_wrk",
        configuration = 
        {
            "query": 
            {
                "query": customer_object_ldg_to_wrk,
                "useLegacySql": False
            }
        },
    ...
    customer_object_wrk_to_dim = open( os.path.join(sql_path, 'customer_object/customer_object_wrk_to_dim.sql'),"r").read()

    launch_customer_object_wrk_to_dim = BigQueryInsertJobOperator(
        task_id = "customer_object_wrk_to_dim",
        configuration = 
        {
            "query": 
            {
                "query": customer_object_wrk_to_dim,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    customer_object_elt_control_set_elt_from_and_to = open( os.path.join(sql_path, 'customer_object/customer_object_elt_control_set_elt_from_and_to.sql'),"r").read()

    launch_customer_object_elt_control_set_elt_from_and_to = BigQueryInsertJobOperator(
        task_id = "customer_object_elt_control_set_elt_from_and_to",
        configuration = 
        {
            "query": 
            {
                "query": customer_object_elt_control_set_elt_from_and_to,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    customer_object_dq_audit = open( os.path.join(sql_path, 'customer_object/customer_object_dq_audit.sql'),"r").read()

    launch_customer_object_dq_audit = BigQueryInsertJobOperator(
        task_id = "customer_object_dq_audit",
        configuration = 
        {
            "query": 
            {
                "query": customer_object_dq_audit,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    launch_customer_object_send_newrelic_metric=PythonOperator(
        task_id='customer_object_send_newrelic_metric',
        python_callable=main,
        op_kwargs={
        'newrelic_connection' : config.get('dag', 'newrelic_connection'),
        'hook' : BigQueryHook(gcp_conn_id=gcp_conn,location='us-west1', use_legacy_sql=False),
        'bigquery_environment' : sql_config_dict['db_env'],
        'metrics_yaml_file' : os.path.join(module_path,'yaml/customer_object.yaml'),
    ...
    customer_object_elt_control_deactivate = open( os.path.join(sql_path, 'customer_object/customer_object_elt_control_deactivate.sql'),"r").read()

    launch_customer_object_elt_control_deactivate = BigQueryInsertJobOperator(
        task_id = "customer_object_elt_control_deactivate",
        configuration = 
        {
            "query": 
            {
                "query": customer_object_elt_control_deactivate,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    creds_setup >> launch_customer_object_ldg_to_wrk >>[launch_customer_object_wrk_to_dim]
    launch_customer_object_wrk_to_dim >>[launch_customer_object_elt_control_set_elt_from_and_to]
    launch_customer_object_elt_control_set_elt_from_and_to >>[launch_customer_object_dq_audit]
    launch_customer_object_dq_audit >>[launch_customer_object_send_newrelic_metric]
    launch_customer_object_send_newrelic_metric >>[launch_customer_object_elt_control_deactivate]
    launch_customer_object_elt_control_deactivate >> pelican_validator >> pelican_sensor