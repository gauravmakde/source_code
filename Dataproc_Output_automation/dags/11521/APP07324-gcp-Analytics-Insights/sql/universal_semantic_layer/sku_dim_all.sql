...
CREATE TEMPORARY TABLE IF NOT EXISTS sku_dim_prep_all
AS
...
FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.product_sku_dim AS sk
 INNER JOIN (SELECT rms_sku_num
  ...
  WHERE order_date_pacific BETWEEN DATE '2021-01-31' AND (CURRENT_DATE('PST8PDT'))
 ...
 LEFT JOIN (SELECT dept_num,
  ...
  FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.department_dim AS ddF
  GROUP BY dept_num,
   subdivision_name,
   merch_persona,
   merch_type,
...
TRUNCATE TABLE `{{params.gcp_project_id}}`.{{params.usl_t2_schema}}.sku_dim;

INSERT INTO `{{params.gcp_project_id}}`.{{params.usl_t2_schema}}.sku_dim
SELECT DISTINCT *
 FROM sku_dim_prep_all
 EXCEPT DISTINCT
 SELECT *
