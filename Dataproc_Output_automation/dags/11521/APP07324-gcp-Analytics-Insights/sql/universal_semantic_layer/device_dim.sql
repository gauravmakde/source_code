 ...
 */
MERGE INTO {{params.gcp_project_id}}.{{params.usl_t2_schema}}.device_dim AS tgt USING (
   SELECT
      source_platform_code,
      REPLACE (
         LOWER(
            COALESCE(
               NULLIF(source_platform_code, 'unknown'),
               'unknown device'
            )
         ),
         'csr_',
...
) AS src ON LOWER(tgt.source_platform_code) = LOWER(src.source_platform_code) WHEN MATCHED THEN
UPDATE
SET
   device_name = src.device_name,
   dw_sys_load_tmstp = CURRENT_DATETIME('PST8PDT') WHEN NOT MATCHED THEN INSERT (
      source_platform_code,
      device_name,
      dw_sys_load_tmstp
   )
...
DELETE FROM {{params.gcp_project_id}}.{{params.usl_t2_schema}}.device_dim
WHERE
   ...
   AND device_id <> (
      SELECT
         MIN(device_id)
      FROM
         {{params.gcp_project_id}}.{{params.usl_t2_schema}}.device_dim
      WHERE
         source_platform_code IS NULL
   );

--COLLECT   STATISTICS COLUMN (DEVICE_ID, SOURCE_PLATFORM_CODE) on {{params.gcp_project_id}}.{{params.usl_t2_schema}}.device_dim