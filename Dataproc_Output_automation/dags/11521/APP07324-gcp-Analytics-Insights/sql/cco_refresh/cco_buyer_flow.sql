...
-- Task_Name=run_cco_job_2_cco_buyer_flow;'
...
BEGIN
SET _ERROR_CODE  =  0;
/********* Step I-B-ii: Get the dates needed for the CCO Buyer tables *********/
...
BEGIN
SET _ERROR_CODE  =  0;
/********* Step I-B-iii: Start & End months of past 5 years *********/
...
SELECT DISTINCT month_num AS current_mo,
  ...
  month_num - 400 AS yr1_start_mo,
   ...
   THEN month_num - 89
   ELSE month_num - 1
   END - 400 AS yr0_end_mo,
  month_num - 500 AS yr0_start_mo
...
BEGIN
SET _ERROR_CODE  =  0;
/********* Step I-B-iv: Start & End dates of past 5 years *********/
CREATE TEMPORARY TABLE IF NOT EXISTS date_lookup
AS
SELECT dp.r4yr0_start_dt AS yr0_start_dt,
 dp.r4yr0_end_dt AS yr0_end_dt,
 dp.r4yr1_start_dt AS yr1_start_dt,
 dl.yr1_end_dt,
 dl.yr2_start_dt,
 dl.yr2_end_dt,
 dl.yr3_start_dt,
...
FROM date_parameter_lookup AS dp
 INNER JOIN (SELECT MAX(CASE
   ...
   MAX(CASE
     ...
     ELSE NULL
     END) AS yr4_end_dt
  FROM realigned_calendar) AS dl ON TRUE;
EXCEPTION WHEN ERROR THEN
SET _ERROR_CODE  =  1;
SET _ERROR_MESSAGE  =  @@error.message;
END;
...
AS
SELECT MAX(CASE
   WHEN month_num = (SELECT yr0_end_mo
     FROM month_lookup)
   THEN SUBSTR(CASE
 ...
 MAX(CASE
   ...
   END) AS yr4_name
FROM realigned_calendar
WHERE month_num IN ((SELECT yr0_end_mo
    FROM month_lookup), (SELECT yr1_end_mo
...
SELECT DISTINCT dtl.acp_id,
  ...
  END AS channel,
  CASE
  WHEN cal.month_num BETWEEN (SELECT yr0_start_mo
    FROM month_lookup) AND (SELECT yr0_end_mo
    FROM month_lookup)
  THEN (SELECT yr0_name
   FROM month_names)
...
SELECT acp_id,
 ...
 channel,
  CASE
  WHEN LOWER(reporting_year_shopped) = LOWER((SELECT yr1_name
     FROM month_names))
  THEN (SELECT yr0_name
   FROM month_names)
  WHEN LOWER(reporting_year_shopped) = LOWER((SELECT yr2_name
     FROM month_names))
  THEN (SELECT yr1_name
   FROM month_names)
  WHEN LOWER(reporting_year_shopped) = LOWER((SELECT yr3_name
     FROM month_names))
  THEN (SELECT yr2_name
   FROM month_names)
  WHEN LOWER(reporting_year_shopped) = LOWER((SELECT yr4_name
     FROM month_names))
  THEN (SELECT yr3_name
   FROM month_names)
  ELSE NULL
...
SELECT a.acp_id,
 ...
 a.channel,
  CASE
  WHEN LOWER(a.acp_id) = LOWER(b.acp_id) AND LOWER(a.channel) = LOWER(b.channel) AND LOWER(a.ly) = LOWER(b.reporting_year_shopped
     )
  THEN 1
  ELSE 0
  END AS chan_ret_flag,
  CASE
  WHEN LOWER(a.acp_id) = LOWER(c.acp_id) AND LOWER(a.ly) = LOWER(c.reporting_year_shopped)
  THEN 1
  ELSE 0
  END AS jwn_ret_flag
FROM cust_year_chan_ly AS a
 LEFT JOIN cust_year_chan_ly AS b ON LOWER(a.acp_id) = LOWER(b.acp_id) AND LOWER(a.channel) = LOWER(b.channel) AND LOWER(a
    .ly) = LOWER(b.reporting_year_shopped)
 LEFT JOIN (SELECT DISTINCT acp_id,
   reporting_year_shopped
  FROM cust_year_chan_ly) AS c ON LOWER(a.acp_id) = LOWER(c.acp_id) AND LOWER(a.ly) = LOWER(c.reporting_year_shopped)
WHERE LOWER(a.reporting_year_shopped) IN (LOWER((SELECT yr1_name
     FROM month_names)), LOWER((SELECT yr2_name
     FROM month_names)), LOWER((SELECT yr3_name
     FROM month_names)), LOWER((SELECT yr4_name
     FROM month_names)));
EXCEPTION WHEN ERROR THEN
SET _ERROR_CODE  =  1;
SET _ERROR_MESSAGE  =  @@error.message;
...
SELECT DISTINCT a.acp_id,
 ...
 a.jwn_ret_flag,
  CASE
  WHEN LOWER(a.acp_id) = LOWER(c.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(c.reporting_year_shopped) AND a.jwn_ret_flag
     = 0
  THEN 1
  ELSE 0
  END AS ntn_jwn_flag,
  CASE
  WHEN LOWER(a.acp_id) = LOWER(b.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(b.reporting_year_shopped) AND LOWER(a
       .channel) = LOWER(b.channel) AND a.jwn_ret_flag = 0 AND CASE
     WHEN LOWER(a.acp_id) = LOWER(c.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(c.reporting_year_shopped) AND a
       .jwn_ret_flag = 0
     THEN 1
     ELSE 0
     END = 1
  ...
  CASE
  WHEN LOWER(a.acp_id) = LOWER(d.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(d.reporting_year_shopped) AND LOWER(a
      .channel) = LOWER(d.channel) AND (a.jwn_ret_flag = 1 OR CASE
       WHEN LOWER(a.acp_id) = LOWER(c.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(c.reporting_year_shopped) AND
         a.jwn_ret_flag = 0
       THEN 1
       ELSE 0
       END = 1)
  ...
  END AS act_flag,
  CASE
  WHEN LOWER(a.acp_id) = LOWER(e.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(e.reporting_year_shopped) AND LOWER(a
      .channel) = LOWER(e.channel) AND a.chan_ret_flag = 0
  THEN 1
  WHEN CASE
    WHEN LOWER(a.acp_id) = LOWER(c.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(c.reporting_year_shopped) AND a.jwn_ret_flag
       = 0
...
FROM cust_year_chan AS a
 LEFT JOIN customer_acquisition AS b ON LOWER(a.acp_id) = LOWER(b.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(b
     .reporting_year_shopped) AND LOWER(a.channel) = LOWER(b.channel)
 LEFT JOIN customer_acquisition_jwn AS c ON LOWER(a.acp_id) = LOWER(c.acp_id) AND LOWER(a.reporting_year_shopped) =
   LOWER(c.reporting_year_shopped)
 LEFT JOIN customer_activation AS d ON LOWER(a.acp_id) = LOWER(d.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(d.reporting_year_shopped
     ) AND LOWER(a.channel) = LOWER(d.channel)
 LEFT JOIN customer_new2channel AS e ON LOWER(a.acp_id) = LOWER(e.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(e
     .reporting_year_shopped) AND LOWER(a.channel) = LOWER(e.channel);
EXCEPTION WHEN ERROR THEN
SET _ERROR_CODE  =  1;
SET _ERROR_MESSAGE  =  @@error.message;
END;
...
FROM (SELECT acp_id,
 ...
 INNER JOIN (SELECT DISTINCT acp_id,
  ...
  WHERE LOWER(SUBSTR(channel, 1, 1)) IN (LOWER('5'), LOWER('6'))) AS b ON LOWER(a.acp_id) = LOWER(b.acp_id) AND LOWER(a
     .reporting_year_shopped) = LOWER(b.reporting_year_shopped) AND LOWER(a.channel) = LOWER(b.channel);
EXCEPTION WHEN ERROR THEN
SET _ERROR_CODE  =  1;
SET _ERROR_MESSAGE  =  @@error.message;
END;
...
FROM (SELECT acp_id,
 ...
 INNER JOIN (SELECT DISTINCT acp_id,
   ...
   ntc_flag
  FROM buyer_flow_all_views
  WHERE LOWER(SUBSTR(channel, 1, 1)) = LOWER('7')) AS b ON LOWER(a.acp_id) = LOWER(b.acp_id) AND LOWER(a.reporting_year_shopped
     ) = LOWER(b.reporting_year_shopped) AND LOWER(a.channel) = LOWER(b.channel);
EXCEPTION WHEN ERROR THEN
SET _ERROR_CODE  =  1;
SET _ERROR_MESSAGE  =  @@error.message;
END;
BEGIN
SET _ERROR_CODE  =  0;
TRUNCATE TABLE `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_buyer_flow;
EXCEPTION WHEN ERROR THEN
SET _ERROR_CODE  =  1;
SET _ERROR_MESSAGE  =  @@error.message;
END;
BEGIN
SET _ERROR_CODE  =  0;
INSERT INTO `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_buyer_flow
(SELECT acp_id,
  reporting_year_shopped,
  channel,
  buyer_flow,
...
SET _ERROR_MESSAGE  =  @@error.message;
END;
--collect statistics column (acp_id) on `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_buyer_flow;
--collect statistics column (reporting_year_shopped) on `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_buyer_flow;
--collect statistics column (channel) on `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_buyer_flow;
--collect statistics column (buyer_flow) on `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_buyer_flow;
--collect statistics column (acp_id,reporting_year_shopped,channel) on `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_buyer_flow;
