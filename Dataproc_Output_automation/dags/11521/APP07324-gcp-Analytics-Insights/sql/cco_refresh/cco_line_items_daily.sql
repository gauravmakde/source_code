...
FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.product_upc_dim AS a
 INNER JOIN (SELECT div_num,
   ...
   GROUP BY div_num,
    ...
    class_num,
    style_desc,
    prmy_supp_num) AS b ON LOWER(a.rms_sku_num) = LOWER(b.rms_sku_num) AND LOWER(a.channel_country) = LOWER(b.channel_country
    )
...
(SELECT DISTINCT business_day_date,
  global_tran_id,
  line_item_seq_num,
  CAST(intent_store_num AS INTEGER) AS intent_store_num,
 ...
 WHERE business_day_date BETWEEN DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 60 DAY) AND (CURRENT_DATE('PST8PDT'))
  AND order_date_pacific BETWEEN DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 60 DAY) AND (CURRENT_DATE('PST8PDT'))
...
(SELECT DISTINCT business_day_date,
 ...
 WHERE business_day_date BETWEEN DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 60 DAY) AND (CURRENT_DATE('PST8PDT'))
  AND COALESCE(order_date, tran_date) BETWEEN DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 60 DAY) AND (CURRENT_DATE('PST8PDT'))
  AND line_net_amt > 0
...
WHERE LOWER(remote_sell_swimlane) IN (LOWER('STYLEBOARD_ATTRIBUTED'), LOWER('STYLEBOARD_PRIVATE_ATTRIBUTED'), LOWER('PERSONAL_REQUEST_A_LOOK_ATTRIB'
 ...
 AND business_day_date BETWEEN DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 60 DAY) AND (CURRENT_DATE('PST8PDT'))
...
FROM `{{params.gcp_project_id}}`.t2dl_das_sales_returns.sales_and_returns_fact AS dtl
 ...
 INNER JOIN `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.store_dim AS st ON CASE
   ...
   END = st.store_num
WHERE dtl.business_day_date BETWEEN DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 60 DAY) AND (CURRENT_DATE('PST8PDT'))
...
WHERE aare_status_date BETWEEN (SELECT yr1_start_dt
   FROM date_lookup) AND (SELECT DATE_ADD(yr4_end_dt, INTERVAL 14 DAY)
   FROM date_lookup);
...
(SELECT DISTINCT dtl.business_day_date,
  ...
  CAST(COALESCE(FORMAT('%11d', upc.dept_num), r.dept_num) AS INTEGER) AS dept_num,
...
DELETE FROM `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_line_items
WHERE business_day_date BETWEEN DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 60 DAY) AND (CURRENT_DATE('PST8PDT'));


DELETE FROM `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_line_items
WHERE LOWER(data_source) = LOWER('2) NSEAM');
...
(SELECT DISTINCT dtl.date_shopped,
  ...
  CAST(NULL AS INTEGER) AS div_num,
  CAST(NULL AS STRING) AS div_desc,
  CAST(NULL AS INTEGER) AS subdiv_num,
  CAST(NULL AS STRING) AS subdiv_desc,
  CAST(NULL AS INTEGER) AS dept_num,
