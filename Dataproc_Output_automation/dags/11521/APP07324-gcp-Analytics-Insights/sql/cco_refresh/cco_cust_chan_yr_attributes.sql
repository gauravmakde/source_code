...
---   Task_Name=run_cco_job_6_cco_cust_chan_yr_attributes;'*/
BEGIN
SET _ERROR_CODE  =  0;
...
SET _ERROR_MESSAGE  =  @@error.message;
END;
--collect statistics column (acp_id,reporting_year_shopped,channel) on cco_strategy_line_item_extract;
BEGIN
SET _ERROR_CODE  =  0;
...
reporting_year_shopped STRING,
channel STRING,
cust_chan_gross_sales NUMERIC,
cust_chan_return_amt NUMERIC,
cust_chan_net_sales NUMERIC,
...
cust_chan_net_sales_ly NUMERIC,
cust_chan_net_sales_ny NUMERIC,
...
(SELECT acp_id,
  ...
  cust_chan_npg,
  cust_chan_div_count,
  LAG(cust_chan_net_sales) OVER (PARTITION BY acp_id, channel ORDER BY reporting_year_shopped) AS cust_chan_net_sales_ly
  ,
  LEAD(cust_chan_net_sales) OVER (PARTITION BY acp_id, channel ORDER BY reporting_year_shopped) AS
  cust_chan_net_sales_ny,
  LAG(cust_chan_anniversary) OVER (PARTITION BY acp_id, channel ORDER BY reporting_year_shopped) AS
  cust_chan_anniversary_ly,
  LAG(cust_chan_holiday) OVER (PARTITION BY acp_id, channel ORDER BY reporting_year_shopped) AS cust_chan_holiday_ly
 FROM cust_chan_level_derived_nopi);
EXCEPTION WHEN ERROR THEN
SET _ERROR_CODE  =  1;
SET _ERROR_MESSAGE  =  @@error.message;
END;
--collect statistics column (acp_id,reporting_year_shopped,channel) on cust_chan_level_derived;
BEGIN
SET _ERROR_CODE  =  0;
...
SELECT acp_id,
 reporting_year_shopped,
 cust_chan_net_sales,
 ROW_NUMBER() OVER (PARTITION BY reporting_year_shopped ORDER BY cust_chan_net_sales DESC) AS rank_record
FROM cust_chan_level_derived
WHERE LOWER(channel) = LOWER('7) JWN')
QUALIFY (ROW_NUMBER() OVER (PARTITION BY reporting_year_shopped ORDER BY cust_chan_net_sales DESC)) <= 500000;
EXCEPTION WHEN ERROR THEN
SET _ERROR_CODE  =  1;
SET _ERROR_MESSAGE  =  @@error.message;
END;
...
BEGIN
SET _ERROR_CODE  =  0;
TRUNCATE TABLE `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_cust_chan_yr_attributes;
EXCEPTION WHEN ERROR THEN
SET _ERROR_CODE  =  1;
SET _ERROR_MESSAGE  =  @@error.message;
END;
...
BEGIN
SET _ERROR_CODE  =  0;
INSERT INTO `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_cust_chan_yr_attributes
(SELECT DISTINCT a.acp_id,
  ...
  c.cust_clv_fp,
  c.cust_clv_op,
  CAST(h.mcv_net_1year_fp + h.mcv_net_1year_op AS NUMERIC) as ust_next_year_net_spend_jwn,
  ...
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_discretionary_spend_estimate_spend_on_apparel,
  CAST(CASE
    WHEN e.household_discretionary_spend_estimate_score = ''
    THEN '0'
    ELSE e.household_discretionary_spend_estimate_score
    END AS INTEGER) AS cust_current_household_discretionary_spend_estimate_score,
  CAST(CASE
    WHEN e.household_estimated_income_national_percentile = ''
    THEN '0'
    ELSE e.household_estimated_income_national_percentile
    END AS INTEGER) AS cust_current_household_estimated_income_national_percentile,
  CAST(CASE
    WHEN e.household_estimated_income_amount_in_thousands = ''
    THEN '0'
    ELSE e.household_estimated_income_amount_in_thousands
    END AS INTEGER) AS cust_current_household_estimated_income_amount_in_thousands,
   CASE
  ...
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_young_adult_clothing_shoppers_likelihood,
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_prestige_makeup_user_likelihood,
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_luxury_store_shoppers_likelihood,
  CAST(CASE
    WHEN e.household_buyer_loyalty_card_user_likelihood_rank_asc = ''
    THEN '0'
    ELSE e.household_buyer_loyalty_card_user_likelihood_rank_asc
    END AS INTEGER) AS cust_current_household_buyer_loyalty_card_user_likelihood,
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_online_overall_propensity,
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_retail_overall_propensity,
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_consumer_expenditure_apparel_propensity,
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_online_apparel_propensity,
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_retail_apparel_propensity,
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_consumer_expenditure_shoes_propensity,
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_online_shoes_propensity,
  CAST(CASE
    ...
    END AS INTEGER) AS cust_current_household_buyer_retail_shoes_propensity,
 ...
 FROM cust_chan_level_derived AS a
  LEFT JOIN `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_buyer_flow AS b ON LOWER(a.acp_id) = LOWER(b.acp_id) AND LOWER(a.channel) = LOWER(b.channel
      ) AND LOWER(a.reporting_year_shopped) = LOWER(b.reporting_year_shopped)
  LEFT JOIN `{{params.gcp_project_id}}`.{{params.cco_t2_schema}}.cco_customer_level_attributes AS c ON LOWER(a.acp_id) = LOWER(c.acp_id) AND LOWER(a.reporting_year_shopped
  ...
  LEFT JOIN cust_jwn_rank AS k ON LOWER(a.acp_id) = LOWER(k.acp_id) AND LOWER(a.reporting_year_shopped) = LOWER(k.reporting_year_shopped
     ));
EXCEPTION WHEN ERROR THEN
SET _ERROR_CODE  =  1;
SET _ERROR_MESSAGE  =  @@error.message;
END;
--collect statistics column  (acp_id), column  (reporting_year_shopped), column  (channel), column  (cust_gender), column  (cust_age), column  (cust_lifestage), column  (cust_age_group), column  (cust_NMS_market), column  (cust_dma), column  (cust_country), column  (cust_dma_rank), column  (cust_loyalty_type), column  (cust_loyalty_level), column  (cust_employee_flag), column  (acp_id, reporting_year_shopped, channel) on t2dl_das_strategy.cco_cust_chan_yr_attributes;
