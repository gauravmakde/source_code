...
CREATE OR REPLACE VIEW `{{params.gcp_project_id}}`.{{params.mta_t2_schema}}.mta_ssa_cost_fcst_agg_vw AS SELECT activity_date_pacific,
 year_num,
 month_short_desc,
 week_454_num,
 week_of_fyr,
...
FROM (SELECT d.day_date AS activity_date_pacific,
   ...
   FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.day_cal AS d
    INNER JOIN `{{params.gcp_project_id}}`.t2dl_das_mta.mta_ssa_cost_agg AS mpcs_ty ON d.day_date = mpcs_ty.activity_date_pacific
   UNION ALL
   SELECT d0.day_date AS activity_date_pacific,
    ...
    0 AS ly_fcst_cost,
    0 AS ly_fcst_traffic,
    0 AS ly_fcst_attributed_orders,
    0 AS ly_fcst_attributed_demand,
    0 AS ly_fcst_attributed_pred_net,
    0 AS ly_fcst_sessions,
    0 AS ly_fcst_session_orders,
    mpcs_ly.dw_sys_load_tmstp
   FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.day_cal AS d0
    INNER JOIN `{{params.gcp_project_id}}`.t2dl_das_mta.mta_ssa_cost_agg AS mpcs_ly ON d0.last_year_day_date_realigned = mpcs_ly.activity_date_pacific
     ) AS mpcs
WHERE activity_date_pacific <= (SELECT MAX(activity_date_pacific)
   FROM `{{params.gcp_project_id}}`.t2dl_das_mta.mta_ssa_cost_agg)
