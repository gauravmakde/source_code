...
/*
T2/Table Name: T2DL_DAS_SERVICE_ENG.cust_service_3mo
...
CLUSTER BY year_ending
AS
SELECT DISTINCT CASE
  WHEN MOD(CAST(TRUNC(floor(month_num)) AS INTEGER), 100) IN (1, 2)
  THEN month_num - 90
  ELSE month_num - 2
  END AS start_3mo,
...
CREATE TEMPORARY TABLE IF NOT EXISTS acp_id_service_3mo_temp
CLUSTER BY acp_id, three_mo_ending, service_name
AS
SELECT a.acp_id,
 b.year_ending AS three_mo_ending,
 a.service_name,
...
FROM `{{params.gcp_project_id}}`.{{params.service_eng_t2_schema}}.cust_service_month AS a
 INNER JOIN fiscal_months AS b ON a.month_num >= b.start_3mo AND a.month_num <= b.year_ending
GROUP BY a.acp_id,
 a.service_name,
 b.year_ending;


TRUNCATE TABLE `{{params.gcp_project_id}}`.{{params.service_eng_t2_schema}}.cust_service_3mo;


INSERT INTO `{{params.gcp_project_id}}`.{{params.service_eng_t2_schema}}.cust_service_3mo
(SELECT DISTINCT a.acp_id,
  CAST(a.three_mo_ending AS NUMERIC) AS three_mo_ending,
  a.service_name,
 ...
 FROM acp_id_service_3mo_temp AS a
  INNER JOIN (SELECT DISTINCT acp_id,
    three_mo_ending
   FROM acp_id_service_3mo_temp
   WHERE LOWER(service_name) = LOWER('Z_) Nordstrom')) AS b ON LOWER(a.acp_id) = LOWER(b.acp_id) AND a.three_mo_ending =
    b.three_mo_ending);


--COLLECT   STATISTICS COLUMN(acp_id),                    COLUMN(three_mo_ending),                    COLUMN(service_name) ON T2DL_DAS_SERVICE_ENG.cust_service_3mo