...
-- CLUSTER BY acp_id, business_unit_desc
AS
SELECT CASE
...
WHERE LOWER(channel) IN (LOWER('1) Nordstrom Stores'), LOWER('2) Nordstrom.com'), LOWER('3) Rack Stores'), LOWER('4) Rack.com'
 ...
 AND CAST(SUBSTR(fiscal_year_shopped, 2 * -1) AS FLOAT64) + 2000 IN (SELECT DISTINCT year_num
   ...
   WHERE day_date BETWEEN DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 30 DAY) AND (CURRENT_DATE('PST8PDT')));
...
WHERE execution_qtr_end_dt >= DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 30 DAY)
...
WHERE business_day_date BETWEEN DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 30 DAY) AND (CURRENT_DATE('PST8PDT'))
...
WHERE activity_date_pacific BETWEEN DATE_SUB(DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 30 DAY), INTERVAL 30 DAY) AND (CURRENT_DATE('PST8PDT')
   )
...
WHERE activity_date BETWEEN DATE_SUB(DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 30 DAY), INTERVAL 30 DAY) AND (CURRENT_DATE('PST8PDT'))
...
DELETE FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_dsa_ai_fct.finance_margin_fact
WHERE business_day_date BETWEEN DATE_SUB(CURRENT_DATE('PST8PDT'), INTERVAL 30 DAY) AND (CURRENT_DATE('PST8PDT'));
