--CREATE OR REPLACE PROCEDURE `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.raven_source_bteq_76()
BEGIN
DECLARE _ERROR_CODE INT64;
DECLARE _ERROR_MESSAGE STRING;
/*SET QUERY_BAND = 'App_ID=APP09442;
DAG_ID=nmn_brand_customer_metrics_11521_ACE_ENG;
---Task_Name=nmn_brand_customer_metrics;'*/
BEGIN
SET _ERROR_CODE  =  0;
CREATE TEMPORARY TABLE IF NOT EXISTS date_mapping
--CLUSTER BY day_date
AS
...
FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.day_cal_454_dim
WHERE month_idnt BETWEEN (SELECT month_start
   FROM date_mapping
   WHERE day_date = CURRENT_DATE('PST8PDT')) AND (SELECT month_end
   FROM date_mapping
...
WHERE business_day_date <= (SELECT end_date
   FROM `_variables`)
...
BEGIN
SET _ERROR_CODE  =  0;
CREATE TEMPORARY TABLE IF NOT EXISTS brand_product
--CLUSTER BY sku_num
AS
...
FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.product_sku_dim_vw AS p
 INNER JOIN `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.product_style_dim AS q ON p.epm_style_num = q.epm_style_num AND LOWER(p.channel_country) =
   LOWER(q.channel_country)
 INNER JOIN (SELECT *
  FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.vendor_label_dim AS v
  WHERE vendor_brand_code IN (SELECT DISTINCT CAST(brand_id AS STRING)
     FROM `{{params.gcp_project_id}}`.t2dl_das_nmn.nmn_head_brands)) AS t1 ON CAST(q.vendor_label_code AS FLOAT64) = t1.vendor_label_code
...
FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.product_sku_dim_vw AS p0
 INNER JOIN `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.product_style_dim AS q0 ON p0.epm_style_num = q0.epm_style_num AND LOWER(p0.channel_country
    ) = LOWER(q0.channel_country)
 INNER JOIN (SELECT *
  FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_usr_vws.vendor_label_dim AS v
  WHERE vendor_brand_code IN (SELECT DISTINCT CAST(brand_id AS STRING)
     FROM `{{params.gcp_project_id}}`.t2dl_das_nmn.nmn_torso_brands)) AS t6 ON CAST(q0.vendor_label_code AS FLOAT64) = t6.vendor_label_code
...
BEGIN
SET _ERROR_CODE  =  0;
CREATE TEMPORARY TABLE IF NOT EXISTS brand_visits
--CLUSTER BY acp_id
AS
SELECT acp_id,
 brand_name,
 ...
 RANK() OVER (PARTITION BY acp_id, brand_name, brand_tier ORDER BY business_day_date DESC) AS visit_rank
FROM (SELECT acp_id,
   brand_name,
   business_day_date,
   brand_tier,
   is_brand_day,
   MAX(is_brand_day) AS is_brand_day_0
  FROM (SELECT s.acp_id,
     np.brand_name,
     np.brand_tier,
      CASE
      WHEN r.business_day_date BETWEEN (SELECT start_date
        FROM `_variables`) AND (SELECT end_date
        FROM `_variables`)
      THEN (SELECT start_date
       FROM `_variables`)
    ...
    WHERE LOWER(r.transaction_type) = LOWER('retail')
     AND r.business_day_date <= (SELECT end_date
       FROM `_variables`)
...
BEGIN
SET _ERROR_CODE  =  0;
CREATE TEMPORARY TABLE IF NOT EXISTS new_visits
--CLUSTER BY acp_id
AS
SELECT acp_id,
 brand_name,
 ...
 MAX(CASE
  ...
  WHEN MAX(business_day_date) BETWEEN (SELECT start_date
     FROM `_variables`) AND (SELECT end_date
     FROM `_variables`) AND DATE_DIFF(MAX(business_day_date), MAX(CASE
       WHEN visit_rank = 2
       THEN business_day_date
       ELSE NULL
       END), DAY) < 365
  ...
  WHEN MAX(business_day_date) < (SELECT start_date
     FROM `_variables`) AND DATE_DIFF((SELECT start_date
      FROM `_variables`), MAX(business_day_date), DAY) BETWEEN 1 AND 365
  THEN 1
  ELSE 0
  END AS lapsed_flag
FROM brand_visits
...
FROM (SELECT brand_name,
   ...
   SELECT brand_name,
    ...
    0 AS lapsed
   FROM brand_visits
   WHERE business_day_date BETWEEN (SELECT start_date
      FROM `_variables`) AND (SELECT end_date
      FROM `_variables`)
   GROUP BY brand_name,
    brand_tier) AS a
...
BEGIN
SET _ERROR_CODE  =  0;
DELETE FROM `{{params.gcp_project_id}}`.t2dl_das_nmn.nmn_brand_customer_metrics
...
BEGIN
SET _ERROR_CODE  =  0;
INSERT INTO `{{params.gcp_project_id}}`.t2dl_das_nmn.nmn_brand_customer_metrics
(SELECT brand,
  month_idnt,
  brand_tier,
  ...
  CAST(FORMAT_TIMESTAMP('%F %H:%M:%E6S', CURRENT_DATE('PST8PDT')) AS DATETIME) AS dw_sys_load_tmstp
