...
dag_id = 'gcp_location_inventory_tracking_total_11521_ACE_ENG'
...
from modules import merch_transaction_upstream_check
...
default_args = {
    ...
    'description': 'location_inventory_tracking_total_11521_ACE_ENG DAG Description',
...
with DAG(
    ...
    location_inventory_tracking_location_inventory_tracking_total = open( os.path.join(sql_path, 'location_inventory_tracking/location_inventory_tracking_total.sql'),"r").read()

    launch_location_inventory_tracking_total_job_1_run_sql_to_update_t2_table = BigQueryInsertJobOperator(
        task_id = "location_inventory_tracking_total_job_1_run_sql_to_update_t2_table",
        configuration = 
        {
            "query": 
            {
                "query": location_inventory_tracking_location_inventory_tracking_total,
                "useLegacySql": False
            }
        },
    ...
    upstream_dep_stage_check_NAP_DW_SALES_FACT_operator = BranchPythonOperator(
    task_id='upstream_dep_stage_check_NAP_DW_SALES_FACT',
    python_callable=merch_transaction_upstream_check.callable_func_name,
    op_args=[bq_hook])

    creds_setup >> upstream_dep_stage_check_NAP_DW_SALES_FACT_operator >> launch_location_inventory_tracking_total_job_1_run_sql_to_update_t2_table >> pelican_validator >> pelican_sensor