...
dag_id = 'gcp_sales_fact_11521_ACE_ENG'
...
default_args = {
    ...
    'description': 'sales_fact_11521_ACE_ENG DAG Description',
...
with DAG(
    ...
    universal_semantic_layer_sales_fact = open( os.path.join(sql_path, 'universal_semantic_layer/sales_fact.sql'),"r").read()

    launch_sales_fact_job_1_run_sql_to_update_t2_table = BigQueryInsertJobOperator(
        task_id = "sales_fact_job_1_run_sql_to_update_t2_table",
        configuration = 
        {
            "query": 
            {
                "query": universal_semantic_layer_sales_fact,
                "useLegacySql": False
            }
        },
    ...
    sales_cust_fact = open( os.path.join(sql_path, 'universal_semantic_layer/sales_cust_fact.sql'),"r").read()

    launch_sales_cust_fact_job_1_run_sql_to_update_t2_table = BigQueryInsertJobOperator(
        task_id = "sales_cust_fact_job_1_run_sql_to_update_t2_table",
        configuration = 
        {
            "query": 
            {
                "query": sales_cust_fact,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    creds_setup >> launch_sales_fact_job_1_run_sql_to_update_t2_table >>[launch_sales_cust_fact_job_1_run_sql_to_update_t2_table]
    launch_sales_cust_fact_job_1_run_sql_to_update_t2_table >> pelican_validator >> pelican_sensor