...
dag_id = 'gcp_mta_email_performance_11521_ACE_ENG'
...
default_args = {
    ...
    'description': 'mta_email_performance_11521_ACE_ENG DAG Description',
...
with DAG(
    ...
    external_dag_id='gcp_mkt_utm_agg_11521_ACE_ENG',
    timeout=3600,
    execution_date_fn=get_most_recent_dag_run,
    poke_interval=120,
    retries=3)

    mta_mta_email_performance = open( os.path.join(sql_path, 'mta/mta_email_performance.sql'),"r").read()
    launch_run_terdata_sql_job_1_run_sql_to_update_t2_table = BigQueryInsertJobOperator(
        task_id = "run_terdata_sql_job_1_run_sql_to_update_t2_table",
        configuration =
        {
            "query":
            {
                "query": mta_mta_email_performance,
                "useLegacySql": False
            }
        },
    ...
    launch_run_terdata_sql_job_2_refresh_tableau_workbook = DummyOperator(task_id = 'run_terdata_sql_job_2_refresh_tableau_workbook')

    # launch_k8s_api_job_operator(
    # dag=dag,
    # connection_id='TECH_ACE_ENGINEERING_APP07324_K8S_GUMBO_CONN_PROD',
    # namespace='app07324',
    # task_id='run_terdata_sql_job_2_refresh_tableau_workbook',
    # job_name='run-terdata-sql-job-2-refresh-tableau-workbook-mta-email-perfor',
    # macros=None,
    # container_image='artifactory.nordstrom.com/docker/app08499/insights-framework/tableau-agent:0.1.6',
    # container_command=['python', '/home/nonroot/agent/main.py', '--source_type', 'workbook', '--project_name', 'Marketing - CRM/Loyalty', '--site', 'AS', '--workbook_name', '4-Box Email Performance', '--vault_path', 'data/application/APP07324/ace/prod/db/tableau/tableau_credentials', '--vault_host', 'https://vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com:8200'],
    # resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    # common_envs=[],
    # task_envs=[{'name': 'TIER', 'value': '3'}, {'name': 'STATSD_HOST', 'value': 'gostatsd.kube-system.svc.cluster.local.'}, {'name': 'STATSD_PORT', 'value': '8128'}, {'name': 'APP_ID', 'value': 'APP07324'}, {'name': 'NEW_RELIC_APP_NAME', 'value': 'onehop-insights-framework-APP07324-PROD'}, {'name': 'ENV', 'value': 'PROD'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}, {'name': 'DAG_ID', 'value': 'mta_email_performance_11521_ACE_ENG'}],
    ...
    creds_setup >> launch_check_external_upstream_dependencies_sensor_0 >>[launch_run_terdata_sql_job_1_run_sql_to_update_t2_table]
    launch_run_terdata_sql_job_1_run_sql_to_update_t2_table >>[launch_run_terdata_sql_job_2_refresh_tableau_workbook]
    launch_run_terdata_sql_job_2_refresh_tableau_workbook >> pelican_validator >> pelican_sensor