...
dag_id = "gcp_cust_service_11521_ACE_ENG"
...
default_args = {
    ...
    "description": "cust_service_11521_ACE_ENG DAG Description",
...
with DAG(
    ...
    cust_service_cust_service_month = open(
        os.path.join(sql_path, "cust_service/cust_service_month.sql"), "r"
    ).read()

    launch_run_bigquery_sql_stg_0_job_0_run_cust_service_month = (
        BigQueryInsertJobOperator(
            task_id="run_bigquery_sql_stg_0_job_0_run_cust_service_month",
            configuration={
                "query": {
                    "query": cust_service_cust_service_month,
                    "useLegacySql": False,
                }
            },
    ...
    cust_service_cust_service_3mo = open(
        os.path.join(sql_path, "cust_service/cust_service_3mo.sql"), "r"
    ).read()

    launch_run_bigquery_sql_stg_0_job_1_run_cust_service_3mo = (
        BigQueryInsertJobOperator(
            task_id="run_bigquery_sql_stg_0_job_1_run_cust_service_3mo",
            configuration={
                "query": {"query": cust_service_cust_service_3mo, "useLegacySql": False}
            },
            project_id=project_id,
            gcp_conn_id=gcp_conn,
            params=sql_config_dict,
    ...
    cust_service_cust_service_6mo = open(
        os.path.join(sql_path, "cust_service/cust_service_6mo.sql"), "r"
    ).read()

    launch_run_bigquery_sql_stg_0_job_2_run_cust_service_6mo = (
        BigQueryInsertJobOperator(
            task_id="run_bigquery_sql_stg_0_job_2_run_cust_service_6mo",
            configuration={
                "query": {"query": cust_service_cust_service_6mo, "useLegacySql": False}
            },
            project_id=project_id,
            gcp_conn_id=gcp_conn,
            params=sql_config_dict,
    ...
    cust_service_cust_service_yr = open(
        os.path.join(sql_path, "cust_service/cust_service_yr.sql"), "r"
    ).read()

    launch_run_bigquery_sql_stg_0_job_3_run_cust_service_yr = BigQueryInsertJobOperator(
        task_id="run_bigquery_sql_stg_0_job_3_run_cust_service_yr",
        configuration={
            "query": {"query": cust_service_cust_service_yr, "useLegacySql": False}
        },
        project_id=project_id,
        gcp_conn_id=gcp_conn,
        params=sql_config_dict,
    ...
    launch_run_bigquery_sql_stg_0_job_2_run_cust_service_6mo >> [
        launch_run_bigquery_sql_stg_0_job_3_run_cust_service_yr
    ]
    (
        launch_run_bigquery_sql_stg_0_job_3_run_cust_service_yr
        >> pelican_validator
