...
dag_id = 'gcp_mothership_clarity_finance_sales_demand_fact_11521_ACE_ENG'
...
from modules import mothership_fsdf_upstream_check
...
dag_sla = timedelta(minutes=int(115)) if int(115)>1 else None
...
default_args = {
    ...
    'description': 'mothership_clarity_finance_sales_demand_fact_11521_ACE_ENG DAG Description',
...
with DAG(
    ...
    upstream_dep_stage_check_clarity_demand_opgmv_operator = BranchPythonOperator(
    task_id='upstream_dep_stage_check_clarity_demand_opgmv',
    python_callable=mothership_fsdf_upstream_check.callable_func_name,
    op_args=[bq_hook])

    # upstream_dep_stage_check_clarity_demand_opgmv_operator = DummyOperator(
    # task_id='upstream_dep_stage_check_clarity_demand_opgmv')
    
        
    clarity_finance_sales_demand_fact = open( os.path.join(sql_path, 'clarity/clarity_finance_sales_demand_fact.sql'),"r").read()

    launch_run_clarity_finance_sales_demand_fact_job_0 = BigQueryInsertJobOperator(
        task_id = "run_clarity_finance_sales_demand_fact_job_0",
        configuration = 
        {
            "query": 
            {
                "query": clarity_finance_sales_demand_fact,
                "useLegacySql": False
            }
        },
    ...
    # launch_mothership_clarity_finance_sales_demand_price_band_agg_11521_ACE_ENG = TriggerDagRunOperator(
    # dag=dag,
    # task_id='trigger_dag_mothership_clarity_finance_sales_demand_price_band_agg_11521_ACE_ENG',
    # trigger_dag_id='gcp_mothership_clarity_finance_sales_demand_price_band_agg_11521_ACE_ENG',
    # execution_date='{{ execution_date }}')

    launch_mothership_clarity_finance_sales_demand_price_band_agg_11521_ACE_ENG =DummyOperator(
    task_id='trigger_dag_mothership_clarity_finance_sales_demand_price_band_agg_11521_ACE_ENG')


    creds_setup >> upstream_dep_stage_check_clarity_demand_opgmv_operator >>[launch_run_clarity_finance_sales_demand_fact_job_0]
    launch_run_clarity_finance_sales_demand_fact_job_0 >>[launch_mothership_clarity_finance_sales_demand_price_band_agg_11521_ACE_ENG] >> pelican_validator >> pelican_sensor