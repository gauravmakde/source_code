...
dag_id = 'gcp_mothership_clarity_finance_margin_fact_11521_ACE_ENG'
...
from modules import mothership_margin_upstream_check
...
dag_sla = timedelta(minutes=int(175)) if int(175)>1 else None
...
default_args = {
    ...
    'description': 'mothership_clarity_finance_margin_fact_11521_ACE_ENG DAG Description',
...
with DAG(
    ...
    upstream_dep_stage_check_clarity_margin_operator = BranchPythonOperator(
    task_id='upstream_dep_stage_check_clarity_margin',
    python_callable=mothership_margin_upstream_check.callable_func_name,
    op_args=[bq_hook])

    mothership_clarity_finance_margin_fact_11521_ACE_ENG_run_clarity_finance_margin_fact_clarity_finance_margin_fact = open( os.path.join(sql_path, 'clarity/mothership_clarity_finance_margin_fact_11521_ACE_ENG_run_clarity_finance_margin_fact_clarity_finance_margin_fact.sql'),"r").read()

    launch_run_clarity_finance_margin_fact_job_0 = BigQueryInsertJobOperator(
        task_id = "run_clarity_finance_margin_fact_job_0",
        configuration = 
        {
            "query": 
            {
                "query": mothership_clarity_finance_margin_fact_11521_ACE_ENG_run_clarity_finance_margin_fact_clarity_finance_margin_fact,
                "useLegacySql": False
            }
        },
    ...
    creds_setup >> upstream_dep_stage_check_clarity_margin_operator >>[launch_run_clarity_finance_margin_fact_job_0]
    launch_run_clarity_finance_margin_fact_job_0 >> pelican_validator >> pelican_sensor