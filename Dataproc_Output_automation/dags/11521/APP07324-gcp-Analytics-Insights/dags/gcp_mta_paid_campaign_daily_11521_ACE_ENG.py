...
dag_id = "gcp_mta_paid_campaign_daily_11521_ACE_ENG"
...
default_args = {
    ...
    "description": "mta_paid_campaign_daily_11521_ACE_ENG DAG Description",
...
with DAG(
    ...
    mta_mta_paid_campaign_daily = open(
        os.path.join(sql_path, "mta/mta_paid_campaign_daily.sql"), "r"
    ).read()

    launch_run_bigquery_sql_update_mta_paid_campaign_daily = BigQueryInsertJobOperator(
        task_id="run_bigquery_sql_update_mta_paid_campaign_daily",
        configuration={
            "query": {"query": mta_mta_paid_campaign_daily, "useLegacySql": False}
        },
    ...
    launch_mta_ssa_cost_11521_ACE_ENG = TriggerDagRunOperator(
        dag=dag,
        task_id="trigger_dag_mta_ssa_cost_11521_ACE_ENG",
        trigger_dag_id="gcp_mta_ssa_cost_11521_ACE_ENG",
        execution_date="{{ execution_date }}",
    )

    launch_refresh_mta_paid_campaign_daily_11521_ACE_ENG = TriggerDagRunOperator(
        dag=dag,
        task_id="trigger_dag_refresh_mta_paid_campaign_daily_11521_ACE_ENG",
        trigger_dag_id="gcp_refresh_mta_paid_campaign_daily_11521_ACE_ENG",
        execution_date="{{ execution_date }}",
    )
    ...
    (
        ...
        >> launch_run_bigquery_sql_update_mta_paid_campaign_daily
    )
    (
    ...
    (
        launch_run_bigquery_sql_update_mta_paid_campaign_daily
        >> launch_mta_ssa_cost_11521_ACE_ENG
    )
    (
        launch_mta_ssa_cost_11521_ACE_ENG
        >> launch_refresh_mta_paid_campaign_daily_11521_ACE_ENG
        >> pelican_validator
        >> pelican_sensor
    )
