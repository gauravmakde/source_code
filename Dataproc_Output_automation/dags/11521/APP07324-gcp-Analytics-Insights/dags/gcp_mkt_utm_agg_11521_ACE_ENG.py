...
dag_id = 'gcp_mkt_utm_agg_11521_ACE_ENG'
...
dag_sla = timedelta(minutes=int(170)) if int(170)>1 else None
...
default_args = {
    ...
    'description': 'mkt_utm_agg_11521_ACE_ENG DAG Description',
...
with DAG(
    ...
    external_dag_id='gcp_mta_11521_ACE_ENG',
    timeout=3600,
    ...
    external_dag_id='gcp_analytical_session_fact_11521_ACE_ENG',
    timeout=3600,
    execution_date_fn=get_most_recent_dag_run,
    poke_interval=120,
    retries=3)
    ...
    mta_mkt_utm_agg = open( os.path.join(sql_path, 'mta/mkt_utm_agg.sql'),"r").read()

    launch_run_bigquery_sql_job_0_mkt_utm_agg_table = BigQueryInsertJobOperator(
        task_id = "run_bigquery_sql_job_0_mkt_utm_agg_table",
        configuration = 
        {
            "query": 
            {
                "query": mta_mkt_utm_agg,
                "useLegacySql": False
            }
        },
    ...
    launch_mta_paid_campaign_daily_11521_ACE_ENG = TriggerDagRunOperator(
    dag=dag,
    task_id='trigger_dag_mta_paid_campaign_daily_11521_ACE_ENG',
    trigger_dag_id='gcp_mta_paid_campaign_daily_11521_ACE_ENG',
    execution_date='{{ execution_date }}')

    launch_refresh_mkt_utm_agg_11521_ACE_ENG = TriggerDagRunOperator(
    dag=dag,
    task_id='trigger_dag_refresh_mkt_utm_agg_11521_ACE_ENG',
    trigger_dag_id='gcp_refresh_mkt_utm_agg_11521_ACE_ENG',
    execution_date='{{ execution_date }}')

    creds_setup >> launch_check_external_upstream_dependencies_sensor_0 >>launch_run_bigquery_sql_job_0_mkt_utm_agg_table
    launch_check_external_upstream_dependencies_sensor_1 >>launch_run_bigquery_sql_job_0_mkt_utm_agg_table
    launch_run_bigquery_sql_job_0_mkt_utm_agg_table >>launch_mta_paid_campaign_daily_11521_ACE_ENG
    launch_mta_paid_campaign_daily_11521_ACE_ENG >>launch_refresh_mkt_utm_agg_11521_ACE_ENG >> pelican_validator >> pelican_sensor