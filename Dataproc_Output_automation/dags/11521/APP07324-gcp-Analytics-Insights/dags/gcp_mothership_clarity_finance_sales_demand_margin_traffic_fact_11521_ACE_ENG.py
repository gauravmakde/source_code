...
dag_id = 'gcp_mothership_clarity_finance_sales_demand_margin_traffic_fact_11521_ACE_ENG'
...
from modules import placer_store_traffic_upstream_check
from modules import mothership_fsdf_downstream_check
...
dag_sla = timedelta(minutes=int(140)) if int(140)>1 else None
...
default_args = {
    ...
    'description': 'mothership_clarity_finance_sales_demand_margin_traffic_fact_11521_ACE_ENG DAG Description',
...
with DAG(
    ...
    bq_hook = BigQueryHook(gcp_conn_id=gcp_conn, use_legacy_sql=False)
    ...
    external_dag_id='gcp_mothership_clarity_finance_sales_demand_fact_11521_ACE_ENG',
    timeout=3600,
    ...
    clarity_clarity_finance_sales_demand_margin_traffic_fact = open( os.path.join(sql_path, 'clarity/clarity_finance_sales_demand_margin_traffic_fact.sql'),"r").read()

    launch_run_clarity_finance_sales_demand_margin_traffic_fact_job_0 = BigQueryInsertJobOperator(
        task_id = "run_clarity_finance_sales_demand_margin_traffic_fact_job_0",
        configuration =
        {
            "query":
            {
                "query": clarity_clarity_finance_sales_demand_margin_traffic_fact,
                "useLegacySql": False
            }
        },
    ...
    fsdf_dq_check_check_clarity_demand_opgmv_operator = BranchPythonOperator(
    task_id='fsdf_dq_check_check_clarity_demand_opgmv',
    python_callable=mothership_fsdf_downstream_check.callable_func_name,
    op_args=[bq_hook])

    placer_dq_check_check_placer_data_operator = BranchPythonOperator(
    task_id='placer_dq_check_check_placer_data',
    python_callable=placer_store_traffic_upstream_check.callable_func_name,
    op_args=[bq_hook])

    creds_setup>> launch_check_external_upstream_dependencies_sensor_0 >>[launch_run_clarity_finance_sales_demand_margin_traffic_fact_job_0]
    launch_run_clarity_finance_sales_demand_margin_traffic_fact_job_0 >>[fsdf_dq_check_check_clarity_demand_opgmv_operator, placer_dq_check_check_placer_data_operator] >> pelican_validator >> pelican_sensor