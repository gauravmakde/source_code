...
dag_id = 'gcp_ssf_receipts_agg_11521_ACE_ENG'
...
default_args = {
    ...
    'description': 'ssf_receipts_agg_11521_ACE_ENG DAG Description',
...
with DAG(
    ...
    ssf_receipts_agg = open( os.path.join(sql_path, 'ssf/receipts_agg.sql'),"r").read()

    launch_run_receipts_agg_job_0 = BigQueryInsertJobOperator(
        task_id = "run_receipts_agg_job_0",
        configuration = 
        {
            "query": 
            {
                "query": ssf_receipts_agg,
                "useLegacySql": False
            }
        },
    ...
    creds_setup >>  launch_run_receipts_agg_job_0 >> pelican_validator >> pelican_sensor