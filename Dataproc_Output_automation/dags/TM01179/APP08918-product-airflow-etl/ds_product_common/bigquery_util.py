...
def etl_control_load(hook,subject_area,task_type):
    bq_hook = BigQueryHook(gcp_conn_id=config.get(env, 'gcp_conn'), use_legacy_sql=False)
    # jdbc_hook = JdbcHook(jdbc_conn_id=td_connection_audit)
    if task_type == 'START':
        query = "CALL {}_NAP_UTL.ELT_CONTROL_{}_LOAD('{}')".format(bigquery_environment, task_type, subject_area)
        logging.info(f"using connection {hook} to execute sql: {query}")
        try:
            result = bq_hook.get_records(sql=query)
            logging.info(f'calling etl_control_start_load {result})')
