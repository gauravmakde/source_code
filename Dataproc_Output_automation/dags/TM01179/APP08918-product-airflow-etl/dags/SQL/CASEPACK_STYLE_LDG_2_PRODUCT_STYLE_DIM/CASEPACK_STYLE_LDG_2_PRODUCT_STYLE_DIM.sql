...
TRUNCATE TABLE  {{params.gcp_project_id}}.{{params.dbenv}}_NAP_STG.CASEPACK_STYLE_DIM_VTW;
--.IF ERRORCODE <> 0 THEN .QUIT 1


INSERT INTO {{params.gcp_project_id}}.{{params.dbenv}}_NAP_STG.CASEPACK_STYLE_DIM_VTW
( epm_style_num
, rms_style_num_list
, channel_country
, style_desc
...
FROM (
	...
	,COALESCE( RANGE_INTERSECT (SRC0.eff_period,hier.eff_period), SRC0.eff_period ) AS eff_period
FROM (
	SELECT distinct rms_casepack_num, epm_casepack_num, channel_country, casepack_desc
	...
	FROM {{params.gcp_project_id}}.{{params.dbenv}}_NAP_BASE_VWS.CASEPACK_STYLE_LDG
	) SRC_2
...
WHERE EXISTS (SELECT *
 FROM {{params.gcp_project_id}}.{{params.dbenv}}_nap_stg.casepack_style_dim_vtw AS src
 WHERE epm_style_num = tgt.epm_style_num
  AND LOWER(channel_country) = LOWER(tgt.channel_country));
--IF ERRORCODE <> 0 THEN .QUIT 4

INSERT INTO {{params.gcp_project_id}}.{{params.dbenv}}_nap_dim.product_style_dim (epm_style_num, rms_style_num_list, channel_country, style_desc, sbclass_num,
...
(SELECT DISTINCT epm_style_num,
 ...
 FROM (SELECT epm_style_num,
   ...
   FROM {{params.gcp_project_id}}.{{params.dbenv}}_nap_stg.casepack_style_dim_vtw) AS t3
