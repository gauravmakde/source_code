                    ...
                    FROM (
                        ...
                        FROM (
                            SELECT
                                ...
                                CAST(jwn_udf.iso8601_tmstp(lastupdatedat) AS TIMESTAMP) AS eff_begin_tmstp
                            FROM 
...
FROM (
    ...
    FROM (
        -- Inner normalize
        SELECT 
            rms_sku_num,channel_country,channel_brand,selling_channel,is_online_purchasable,eff_period,range_group
        FROM (
            SELECT *,
                SUM(discontinuity_flag) OVER (
            ...
            FROM (
                ...
                FROM (
                    ...
                    LEFT JOIN `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_base_vws.product_online_purchasable_item_dim TGT ON 
                        ...
                        RANGE_OVERLAPS(SRC.eff_period, RANGE(CAST(TGT.eff_begin_tmstp AS TIMESTAMP), CAST(TGT.eff_end_tmstp AS TIMESTAMP)))
                    WHERE (
            ...
            GROUP BY 
                rms_sku_num,channel_country,channel_brand,selling_channel,is_online_purchasable,range_group
            ORDER BY  
                rms_sku_num,channel_country,channel_brand,selling_channel,is_online_purchasable
        )AS NRML;
