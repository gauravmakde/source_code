--SET QUERY_BAND='AppName=NAP-Merch-Dimension;AppRelease=1;AppFreq=Daily;AppPhase=ldg-dim;AppSubArea=PRODUCT_POR_ORDERS;' UPDATE FOR SESSION;

-- APPLY DAILY CHANGES FROM POR ORDERS
...
--COLLECT STATISTICS           COLUMN ( ORDER_DATE_UTC ) ,           COLUMN ( ORDER_NUM ) ,           COLUMN ( ACP_ID ) ,           COLUMN ( ORDER_DATE_PACIFIC ) ,           COLUMN ( BU_TYPE_CODE ) ,           COLUMN ( ORDER_LINE_ID ) ,           COLUMN ( RMS_SKU_NUM ) ,           COLUMN ( ORDER_NUM, ORDER_LINE_ID )       ON PRD_NAP_FCT.PRODUCT_ORDERS_RETURN_PROBABILITY_FACT