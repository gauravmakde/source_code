 ...
 FROM (SELECT selling_event_num,
   ...
   FROM (SELECT SRC.*
     FROM (SELECT id AS selling_event_num,
        ...
        `{{params.gcp_project_id}}`.jwn_udf.udf_time_zone(lastupdatedat) AS src_updated_tmstp_tz
...
INSERT INTO {{params.gcp_project_id}}.{{params.dbenv}}_nap_dim.product_selling_event_categories_dim (selling_event_num, categories, selling_event_status,
 src_updated_tmstp,src_updated_tmstp_tz, dw_batch_id, dw_batch_date, dw_sys_load_tmstp)
(SELECT DISTINCT selling_event_num,
  categories,
  selling_event_status,
  src_updated_tmstp,
