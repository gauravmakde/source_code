...
WHERE EXISTS (
  ...
  WHERE LOWER(SRC.asset_id) = LOWER(TGT.asset_id)
...
UPDATE  `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_dim.product_image_asset_dim AS TGT
...
FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_stg.product_image_asset_dim_vtw AS SRC
WHERE LOWER(SRC.asset_id) = (TGT.asset_id)
    AND LOWER(SRC.channel_country) = LOWER(TGT.channel_country)
	AND LOWER(SRC.channel_brand) = LOWER(TGT.channel_brand)
	AND LOWER(SRC.selling_channel) = LOWER(tgt.selling_channel) 
	AND LOWER(SRC.color_num) = LOWER(tgt.color_num) 
