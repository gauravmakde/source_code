            ...
            FROM (
                ...
                FROM (
                    SELECT DISTINCT
                        ...
                        COALESCE(
                            RANGE_INTERSECT(SRC.eff_period, RANGE(TGT.eff_begin_tmstp,TGT.eff_end_tmstp)), 
                            SRC.eff_period 
                        ) AS eff_period,
                        delete_flag,
                        SRC.eff_begin_tmstp,
...
WHERE EXISTS (
  SELECT 1
  FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_stg.supp_dept_map_dim_vtw AS SRC
  WHERE LOWER(SRC.dept_num) = LOWER(TGT.dept_num)
    AND LOWER(SRC.supplier_num) = LOWER(TGT.supplier_num)
	AND LOWER(SRC.banner) = LOWER(TGT.banner)
	AND LOWER(SRC.delete_flag) = LOWER('N')
    AND SRC.eff_begin_tmstp <= TGT.eff_begin_tmstp
    AND SRC.eff_end_tmstp >= TGT.eff_end_tmstp
);

UPDATE  `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_dim.supp_dept_map_dim AS TGT
...
FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_stg.supp_dept_map_dim_vtw AS SRC
WHERE LOWER(SRC.dept_num) = (TGT.dept_num)
    AND LOWER(SRC.supplier_num) = LOWER(TGT.supplier_num)
	AND LOWER(SRC.banner) = LOWER(TGT.banner)
	AND LOWER(SRC.delete_flag) = LOWER('N')
