...
WHERE
  vendor_num IN (
    ...
    FROM
      {{params.gcp_project_id}}.{{params.db_env}}_nap_base_vws.vendor_relationship_partner_ldg
  )
  AND LOWER(vendor_role_name) = LOWER('PARTNER');

--COLLECT STATS ON PRD_NAP_STG.VENDOR_RELATIONSHIP_PARTNER_LDG
INSERT INTO
  {{params.gcp_project_id}}.{{params.db_env}}_nap_dim.vendor_relationship_dim (
    ...
    SELECT DISTINCT
      ...
      (
        ...
        WHERE
          LOWER(subject_area_nm) = LOWER('VENDOR_RELATIONSHIP_PARTNER')
      ) AS dw_batch_id,
      (
        SELECT
          curr_batch_date
      ...
      'PARTNER' AS vendor_role_name
    FROM
      {{params.gcp_project_id}}.{{params.db_env}}_nap_base_vws.vendor_relationship_partner_ldg AS src
    WHERE
