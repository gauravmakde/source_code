...
WHERE
  vendor_num IN (
    SELECT
      vendornumber
    FROM
      {{params.gcp_project_id}}.{{params.db_env}}_nap_base_vws.vendor_site_pay_to_ldg
  )
  AND LOWER(vendor_role_name) = LOWER('PAY_TO');

--COLLECT STATS ON PRD_NAP_STG.VENDOR_SITE_PAY_TO_LDG
INSERT INTO
  {{params.gcp_project_id}}.{{params.db_env}}_nap_dim.vendor_site_dim (
    ...
    SELECT DISTINCT
      ...
      (
        ...
        WHERE
          LOWER(subject_area_nm) = LOWER('VENDOR_SITE_PAYTO')
      ) AS dw_batch_id,
      (
        SELECT
          curr_batch_date
    ...
    FROM
      {{params.gcp_project_id}}.{{params.db_env}}_nap_base_vws.vendor_site_pay_to_ldg AS src QUALIFY (
        DENSE_RANK() OVER (
          PARTITION BY
            vendornumber
          ORDER BY
