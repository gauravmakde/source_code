...
WHERE
  vendor_num IN (
    ...
    FROM
      {{params.gcp_project_id}}.{{params.db_env}}_nap_base_vws.vendor_relationship_pay_to_ldg
  )
  AND LOWER(vendor_role_name) = LOWER('PAY_TO');

--COLLECT STATS ON PRD_NAP_STG.VENDOR_RELATIONSHIP_PAY_TO_LDG
INSERT INTO
  {{params.gcp_project_id}}.{{params.db_env}}_nap_dim.vendor_relationship_dim (
    ...
    SELECT DISTINCT
      ...
      (
        ...
        WHERE
          LOWER(subject_area_nm) = LOWER('VENDOR_RELATIONSHIP_PAYTO')
      ) AS dw_batch_id,
      (
        SELECT
          curr_batch_date
      ...
      'PAY_TO' AS vendor_role_name
    FROM
      {{params.gcp_project_id}}.{{params.db_env}}_nap_base_vws.vendor_relationship_pay_to_ldg AS src
    WHERE
