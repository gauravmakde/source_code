...
TRUNCATE TABLE `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_stg.product_sku_dim_resync_ldg;
--.IF ERRORCODE <> 0 THEN .QUIT 1

INSERT INTO `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_stg.product_sku_dim_resync_ldg (epm_sku_num, rms_sku_num, channel_country)
(SELECT DISTINCT b.epm_sku_num,
  b.rms_sku_num,
  b.channel_country
 FROM `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_dim.product_style_dim AS a
  INNER JOIN `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_dim.product_sku_dim AS b ON a.epm_style_num = b.epm_style_num AND LOWER(a.channel_country) = LOWER(b
     .channel_country)
...
--COLLECT STATS ON `{{params.dataplex_project_id}}`.{{params.dbenv}}_NAP_STG.PRODUCT_SKU_DIM_RESYNC_LDG;
            ...
            SELECT 
                *,
                SUM(discontinuity_flag) OVER (
                    PARTITION BY rms_sku_num,epm_sku_num,channel_country,sku_short_desc,sku_desc,web_sku_num,rms_style_num,epm_style_num,partner_relationship_num,
                            partner_relationship_type_code,web_style_num,style_desc,epm_choice_num,supp_part_num,prmy_supp_num,manufacturer_num, sbclass_num,sbclass_desc,
                            class_num,class_desc,dept_num,dept_desc,grp_num,grp_desc,div_num,div_desc,cmpy_num,cmpy_desc,color_num,color_desc,nord_display_color,
                            nrf_size_code, size_1_num,size_1_desc,size_2_num,size_2_desc,supp_color, supp_size, brand_name,return_disposition_code,return_disposition_desc,selling_status_code,selling_status_desc,live_date,drop_ship_eligible_ind,sku_type_code,sku_type_desc,pack_orderable_code,pack_orderable_desc,
pack_sellable_code,pack_sellable_desc, pack_simple_code,pack_simple_desc,display_seq_1,display_seq_2,hazardous_material_class_desc,hazardous_material_class_code ,
fulfillment_type_code ,selling_channel_eligibility_list ,smart_sample_ind ,gwp_ind ,msrp_amt , msrp_currency_code , npg_ind , order_quantity_multiple ,fp_forecast_eligible_ind ,fp_item_planning_eligible_ind ,fp_replenishment_eligible_ind ,op_forecast_eligible_ind ,op_item_planning_eligible_ind ,op_replenishment_eligible_ind ,size_range_desc ,size_sequence_num , size_range_code  
...
fulfillment_type_code ,selling_channel_eligibility_list ,smart_sample_ind ,gwp_ind ,msrp_amt , msrp_currency_code , npg_ind , order_quantity_multiple ,fp_forecast_eligible_ind ,fp_item_planning_eligible_ind ,fp_replenishment_eligible_ind ,op_forecast_eligible_ind ,op_item_planning_eligible_ind ,op_replenishment_eligible_ind ,size_range_desc ,size_sequence_num , size_range_code 
                ...
                FROM (
                    SELECT DISTINCT
                        ...
                        src0.eff_begin_tmstp_utc, src0.eff_end_tmstp_utc,
                        COALESCE(
                            (CASE 
                                WHEN styl.epm_style_num > 0 THEN (
                                    SAFE.RANGE_INTERSECT(
                                        RANGE(src0.eff_begin_tmstp_utc, src0.eff_end_tmstp_utc), 
                                        RANGE(styl.eff_begin_tmstp_utc, styl.eff_end_tmstp_utc)
                                    )
                                )
                                WHEN dept.dept_num > 0 THEN (
                                    SAFE.RANGE_INTERSECT(
                                        RANGE(src0.eff_begin_tmstp_utc, src0.eff_end_tmstp_utc), 
                                        RANGE(dept.eff_begin_tmstp_utc, dept.eff_end_tmstp_utc)
                                    )
                                )
                                WHEN choice.epm_choice_num > 0 THEN (
                                    SAFE.RANGE_INTERSECT(
                                        RANGE(src0.eff_begin_tmstp_utc, src0.eff_end_tmstp_utc), 
                                        RANGE(choice.eff_begin_tmstp_utc, choice.eff_end_tmstp_utc)
                                    )
                                )
                                ELSE RANGE(src0.eff_begin_tmstp_utc, src0.eff_end_tmstp_utc) 
                            END)
                        ) AS eff_period
                    FROM 
                        `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_base_vws.product_sku_dim_hist src0
                    LEFT JOIN 
                        `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_base_vws.product_style_dim_hist styl
                    ON 
                        src0.epm_style_num = styl.epm_style_num
                    ...
                    LEFT JOIN 
                        `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_base_vws.department_dim_hist dept
                    ON 
                        dept.dept_num = CAST(TRUNC(CAST(TRIM(CAST(styl.dept_num AS STRING)) AS FLOAT64)) AS INT64)
                        AND RANGE_OVERLAPS(
                            RANGE(src0.eff_begin_tmstp_utc, src0.eff_end_tmstp_utc), 
                            RANGE(dept.eff_begin_tmstp_utc, dept.eff_end_tmstp_utc)
                        )
                    ...
                    LEFT JOIN 
                        `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_base_vws.product_choice_dim_hist choice
                    ON 
                        src0.epm_choice_num = choice.epm_choice_num
                        AND src0.channel_country = choice.channel_country
                        AND src0.sku_type_code = 'f'  -- Only Fashion SKUs get color definition from Choice record
                        AND RANGE_OVERLAPS(
                            RANGE(src0.eff_begin_tmstp_utc, src0.eff_end_tmstp_utc), 
                            RANGE(choice.eff_begin_tmstp_utc, choice.eff_end_tmstp_utc)
                        )
                    ...
                    JOIN 
                        `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_stg.product_sku_dim_resync_ldg r_sku
                    ON 
                        src0.epm_sku_num = r_sku.epm_sku_num
...
, eff_begin
, eff_end
,`{{params.dataplex_project_id}}.JWN_UDF.UDF_TIME_ZONE`(cast(eff_begin as string)) as eff_begin_tmstp_tz,
 `{{params.dataplex_project_id}}.JWN_UDF.UDF_TIME_ZONE`(cast(eff_end as string)) as eff_end_tmstp_tz
...
FROM (
  ...
  SELECT
      ...
      MAX(RANGE_END(eff_period))   AS eff_end
       from  (
    --inner normalize
            SELECT rms_sku_num,epm_sku_num,channel_country,sku_short_desc,sku_desc,web_sku_num,rms_style_num,epm_style_num,partner_relationship_num,
                            partner_relationship_type_code,web_style_num,style_desc,epm_choice_num,supp_part_num,prmy_supp_num,manufacturer_num, sbclass_num,sbclass_desc,
                            class_num,class_desc,dept_num,dept_desc,grp_num,grp_desc,div_num,div_desc,cmpy_num,cmpy_desc,color_num,color_desc,nord_display_color,
                            nrf_size_code, size_1_num,size_1_desc,size_2_num,size_2_desc,supp_color, supp_size, brand_name,return_disposition_code,return_disposition_desc,selling_status_code,selling_status_desc,live_date,drop_ship_eligible_ind,sku_type_code,sku_type_desc,pack_orderable_code,pack_orderable_desc,
pack_sellable_code,pack_sellable_desc, pack_simple_code,pack_simple_desc,display_seq_1,display_seq_2,hazardous_material_class_desc,hazardous_material_class_code ,
fulfillment_type_code ,selling_channel_eligibility_list ,smart_sample_ind ,gwp_ind ,msrp_amt , msrp_currency_code , npg_ind , order_quantity_multiple ,fp_forecast_eligible_ind ,fp_item_planning_eligible_ind ,fp_replenishment_eligible_ind ,op_forecast_eligible_ind ,op_item_planning_eligible_ind ,op_replenishment_eligible_ind ,size_range_desc ,size_sequence_num , size_range_code,eff_period,range_group
            FROM (
                SELECT *,SUM(discontinuity_flag) OVER (PARTITION BY rms_sku_num,epm_sku_num,channel_country,sku_short_desc,sku_desc,web_sku_num,rms_style_num,epm_style_num,partner_relationship_num,partner_relationship_type_code,web_style_num,style_desc,epm_choice_num,supp_part_num,prmy_supp_num,manufacturer_num, sbclass_num,sbclass_desc,class_num,class_desc,dept_num,dept_desc,grp_num,grp_desc,div_num,div_desc,cmpy_num,cmpy_desc,color_num,color_desc,nord_display_color,
nrf_size_code, size_1_num,size_1_desc,size_2_num,size_2_desc,supp_color, supp_size, brand_name,return_disposition_code,return_disposition_desc,selling_status_code,selling_status_desc,live_date,drop_ship_eligible_ind,sku_type_code,sku_type_desc,pack_orderable_code,pack_orderable_desc,
pack_sellable_code,pack_sellable_desc, pack_simple_code,pack_simple_desc,display_seq_1,display_seq_2,hazardous_material_class_desc,hazardous_material_class_code ,
fulfillment_type_code ,selling_channel_eligibility_list ,smart_sample_ind ,gwp_ind ,msrp_amt , msrp_currency_code , npg_ind , order_quantity_multiple ,fp_forecast_eligible_ind ,fp_item_planning_eligible_ind ,fp_replenishment_eligible_ind ,op_forecast_eligible_ind ,op_item_planning_eligible_ind ,op_replenishment_eligible_ind ,size_range_desc ,size_sequence_num , size_range_code ORDER BY eff_begin_tmstp_utc ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS range_group
                FROM
      ( 
         SELECT  rms_sku_num,epm_sku_num,channel_country,sku_short_desc,sku_desc,web_sku_num,rms_style_num,epm_style_num,partner_relationship_num,
                            partner_relationship_type_code,web_style_num,style_desc,epm_choice_num,supp_part_num,prmy_supp_num,manufacturer_num, sbclass_num,sbclass_desc,
                            ...
                            nrf_size_code, size_1_num,size_1_desc,size_2_num,size_2_desc,supp_color, supp_size, brand_name,return_disposition_code,return_disposition_desc,selling_status_code,selling_status_desc,live_date,drop_ship_eligible_ind,sku_type_code,sku_type_desc,pack_orderable_code,pack_orderable_desc,
pack_sellable_code,pack_sellable_desc, pack_simple_code,pack_simple_desc,display_seq_1,display_seq_2,hazardous_material_class_desc,hazardous_material_class_code ,
fulfillment_type_code ,selling_channel_eligibility_list ,smart_sample_ind ,gwp_ind ,msrp_amt , msrp_currency_code , npg_ind , order_quantity_multiple ,fp_forecast_eligible_ind ,fp_item_planning_eligible_ind ,fp_replenishment_eligible_ind ,op_forecast_eligible_ind ,op_item_planning_eligible_ind ,op_replenishment_eligible_ind ,size_range_desc ,size_sequence_num , size_range_code,eff_begin_tmstp_utc,eff_end_tmstp_utc,
            CASE 
                WHEN LAG(eff_end_tmstp_utc) OVER (PARTITION BY rms_sku_num,epm_sku_num,channel_country,sku_short_desc,sku_desc,web_sku_num,rms_style_num,epm_style_num,partner_relationship_num,partner_relationship_type_code,web_style_num,style_desc,epm_choice_num,supp_part_num,prmy_supp_num,manufacturer_num, sbclass_num,sbclass_desc,class_num,class_desc,dept_num,dept_desc,grp_num,grp_desc,div_num,div_desc,cmpy_num,cmpy_desc,color_num,color_desc,nord_display_color,
nrf_size_code, size_1_num,size_1_desc,size_2_num,size_2_desc,supp_color, supp_size, brand_name,return_disposition_code,return_disposition_desc,selling_status_code,selling_status_desc,live_date,drop_ship_eligible_ind,sku_type_code,sku_type_desc,pack_orderable_code,pack_orderable_desc,
pack_sellable_code,pack_sellable_desc, pack_simple_code,pack_simple_desc,display_seq_1,display_seq_2,hazardous_material_class_desc,hazardous_material_class_code ,
fulfillment_type_code ,selling_channel_eligibility_list ,smart_sample_ind ,gwp_ind ,msrp_amt , msrp_currency_code , npg_ind , order_quantity_multiple ,fp_forecast_eligible_ind ,fp_item_planning_eligible_ind ,fp_replenishment_eligible_ind ,op_forecast_eligible_ind ,op_item_planning_eligible_ind ,op_replenishment_eligible_ind ,size_range_desc ,size_sequence_num , size_range_code  ORDER BY eff_begin_tmstp_utc) >= 
                DATE_SUB(eff_begin_tmstp_utc, INTERVAL 1 DAY) 
                THEN 0
                ELSE 1
            END AS discontinuity_flag,
             COALESCE(SAFE.RANGE_INTERSECT(eff_period , RANGE(eff_begin_tmstp_utc,eff_end_tmstp_utc)), eff_period ) AS eff_period ,
             from (
SELECT DISTINCT
  SRC.rms_sku_num
, SRC.epm_sku_num
...
GROUP BY rms_sku_num,epm_sku_num,channel_country,sku_short_desc,sku_desc,web_sku_num,rms_style_num,epm_style_num,partner_relationship_num,
                            partner_relationship_type_code,web_style_num,style_desc,epm_choice_num,supp_part_num,prmy_supp_num,manufacturer_num, sbclass_num,sbclass_desc,
                            class_num,class_desc,dept_num,dept_desc,grp_num,grp_desc,div_num,div_desc,cmpy_num,cmpy_desc,color_num,color_desc,nord_display_color,
                            nrf_size_code, size_1_num,size_1_desc,size_2_num,size_2_desc,supp_color, supp_size, brand_name,return_disposition_code,return_disposition_desc,selling_status_code,selling_status_desc,live_date,drop_ship_eligible_ind,sku_type_code,sku_type_desc,pack_orderable_code,pack_orderable_desc,
pack_sellable_code,pack_sellable_desc, pack_simple_code,pack_simple_desc,display_seq_1,display_seq_2,hazardous_material_class_desc,hazardous_material_class_code ,
fulfillment_type_code ,selling_channel_eligibility_list ,smart_sample_ind ,gwp_ind ,msrp_amt , msrp_currency_code , npg_ind , order_quantity_multiple ,fp_forecast_eligible_ind ,fp_item_planning_eligible_ind ,fp_replenishment_eligible_ind ,op_forecast_eligible_ind ,op_item_planning_eligible_ind ,op_replenishment_eligible_ind ,size_range_desc ,size_sequence_num , size_range_code,range_group
ORDER BY  rms_sku_num,epm_sku_num,channel_country,sku_short_desc,sku_desc,web_sku_num,rms_style_num,epm_style_num,partner_relationship_num,
                            partner_relationship_type_code,web_style_num,style_desc,epm_choice_num,supp_part_num,prmy_supp_num,manufacturer_num, sbclass_num,sbclass_desc,
                            class_num,class_desc,dept_num,dept_desc,grp_num,grp_desc,div_num,div_desc,cmpy_num,cmpy_desc,color_num,color_desc,nord_display_color,
                            nrf_size_code, size_1_num,size_1_desc,size_2_num,size_2_desc,supp_color, supp_size, brand_name,return_disposition_code,return_disposition_desc,selling_status_code,selling_status_desc,live_date,drop_ship_eligible_ind,sku_type_code,sku_type_desc,pack_orderable_code,pack_orderable_desc,
pack_sellable_code,pack_sellable_desc, pack_simple_code,pack_simple_desc,display_seq_1,display_seq_2,hazardous_material_class_desc,hazardous_material_class_code ,
...
INSERT INTO `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_dim.product_sku_dim
( rms_sku_num
, epm_sku_num
, channel_country
, sku_short_desc
...
, eff_begin_tmstp
, eff_end_tmstp
,(SELECT BATCH_ID FROM `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_base_vws.elt_control WHERE Subject_Area_Nm ='NAP_PRODUCT') AS dw_batch_id
,(SELECT CURR_BATCH_DATE FROM `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_base_vws.elt_control WHERE Subject_Area_Nm ='NAP_PRODUCT') AS dw_batch_date
, CURRENT_DATETIME('PST8PDT') AS dw_sys_load_tmstp
, eff_begin_tmstp_tz
, eff_end_tmstp_tz
FROM `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_stg.product_sku_dim_vtw
;
...
--ON `{{params.dataplex_project_id}}`.{{params.dbenv}}_NAP_DIM.PRODUCT_SKU_DIM;
