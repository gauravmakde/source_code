...
TRUNCATE TABLE  `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_dim.product_sku_lkup; 
/*.IF ERRORCODE <> 0 THEN .QUIT 2*/
INSERT INTO
  `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_dim.product_sku_lkup 
  (
    rms_sku_num,
    epm_sku_num,
    channel_country,
...
FROM
  `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_base_vws.product_sku_dim AS sku
LEFT OUTER JOIN
  `{{params.dataplex_project_id}}`.{{params.dbenv}}_nap_base_vws.product_color_dim_vw AS colr
ON
