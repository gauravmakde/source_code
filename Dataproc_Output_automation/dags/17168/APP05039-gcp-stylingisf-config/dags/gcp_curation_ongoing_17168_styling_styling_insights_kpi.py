...
dag_id = "gcp_curation_ongoing_17168_styling_styling_insights_kpi"
...
default_args = {
    ...
    "description": "curation_ongoing_17168_styling_styling_insights_kpi DAG Description",
...
with DAG(
    ...
    fetch_batch_run_id_task = FetchKafkaBatchRunIdOperator(
        ...
        gcp_region=region,
        topic_name="customer-curation-analytical-avro",
        consumer_group_name="onix-customer-curation-analytical-avro_curation_ongoing_17168_styling_styling_insights_kpi_read_kafka_to_teradata_job_0",
    ...
    session_name = get_batch_id(dag_id=dag_id, task_id="read_kafka_to_bigquery_job_0")
    read_kafka_to_bigquery_job_0 = DataprocCreateBatchOperator(
        ...
        batch={
            ...
            "spark_batch": {
                ...
                "args": [
                    f"{user_config_path}/argument_curation_ongoing_17168_styling_styling_insights_kpi_read_kafka_to_bigquery_job_0.json",
    ...
    curation_stg_to_curation_fact = open(
        os.path.join(sql_path, "curation_stg_to_curation_fact.sql"), "r"
    ).read()

    launch_read_kafka_to_bigquery_job_1 = BigQueryInsertJobOperator(
        task_id="read_kafka_to_bigquery_job_1",
        configuration={
            "query": {"query": curation_stg_to_curation_fact, "useLegacySql": False}
        },
        project_id=project_id,
        gcp_conn_id=gcp_conn,
    ...
    curation_stg_to_order_fact = open(
        os.path.join(sql_path, "curation_stg_to_order_fact.sql"), "r"
    ).read()

    launch_read_kafka_to_bigquery_job_2 = BigQueryInsertJobOperator(
        task_id="read_kafka_to_bigquery_job_2",
        configuration={
            "query": {"query": curation_stg_to_order_fact, "useLegacySql": False}
        },
        project_id=project_id,
        gcp_conn_id=gcp_conn,
        params=sql_config_dict,
    ...
    curation_stg_to_curation_styling_interaction_fact = open(
        os.path.join(sql_path, "curation_stg_to_curation_styling_interaction_fact.sql"),
        "r",
    ).read()

    launch_read_kafka_to_bigquery_job_3 = BigQueryInsertJobOperator(
        task_id="read_kafka_to_bigquery_job_3",
        configuration={
            "query": {
                "query": curation_stg_to_curation_styling_interaction_fact,
                "useLegacySql": False,
            }
        },
        project_id=project_id,
    ...
    creds_setup >> fetch_batch_run_id_task >> [read_kafka_to_bigquery_job_0]
    read_kafka_to_bigquery_job_0 >> [launch_read_kafka_to_bigquery_job_1]
    launch_read_kafka_to_bigquery_job_1 >> [launch_read_kafka_to_bigquery_job_2]
    launch_read_kafka_to_bigquery_job_2 >> [launch_read_kafka_to_bigquery_job_3]
    launch_read_kafka_to_bigquery_job_3 >> pelican_validator >> pelican_sensor
