...
dag_id = "gcp_merch_main_rp_performance_17284_MERCH_ANALYTICS_insights_framework"
...
default_args = {
    ...
    "description": "merch_main_rp_performance_17284_MERCH_ANALYTICS_insights_framework DAG Description",
...
with DAG(
    ...
    rp_performance_ma_pipeline_main_rp_performance = open(
        os.path.join(sql_path, "rp_performance/ma_pipeline_main_rp_performance.sql"),
        "r",
    ).read()

    launch_run_bigquery_sql_stg_00_job_0 = BigQueryInsertJobOperator(
        task_id="run_bigquery_sql_stg_00_job_0",
        configuration={
            "query": {
                "query": rp_performance_ma_pipeline_main_rp_performance,
                "useLegacySql": False,
            }
        },
    ...
    # launch_run_bigquery_sql_stg_00_job_0_refresh_tableau = launch_k8s_api_job_operator(
    # dag=dag,
    # connection_id='TECH_ISF_ACE_MCH_SECURITY_K8S_GUMBO_CONN_PROD',
    # namespace='APP06792',
    # task_id='run_bigquery_sql_stg_00_job_0_refresh_tableau',
    # job_name='run-bigquery-sql-stg-00-job-0-refresh-tableau-merch-main-rp-per',
    # macros=None,
    # container_image='artifactory.nordstrom.com/docker/app08499/insights-framework/tableau-agent:0.1.7',
    # container_command=['python', '/home/nonroot/agent/main.py', '--source_type', 'datasource', '--project_name', 'RP Performance', '--site', 'AS', '--datasource_name', 'RP Performance Self Service', '--vault_path', 'data/application/APP06792/9850/merch_analytics/prod/db/tableau/tableau_credentials', '--vault_host', 'https://vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com:8200'],
    # resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    # common_envs=[],
    # task_envs=[{'name': 'TIER', 'value': '3'}, {'name': 'STATSD_HOST', 'value': 'gostatsd.kube-system.svc.cluster.local'}, {'name': 'STATSD_PORT', 'value': '8128'}, {'name': 'APP_ID', 'value': 'APP08499'}, {'name': 'NEW_RELIC_APP_NAME', 'value': 'onehop-insights-framework-APP08499-PROD'}, {'name': 'ENV', 'value': 'PROD'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}, {'name': 'DAG_ID', 'value': 'merch_main_rp_performance_17284_MERCH_ANALYTICS_insights_framework'}],
    ...
    launch_run_bigquery_sql_stg_00_job_0_refresh_tableau = DummyOperator(
        task_id="run_bigquery_sql_stg_00_job_0_refresh_tableau", dag=dag
    )

    (
        creds_setup
        >> launch_run_bigquery_sql_stg_00_job_0
        >> [launch_run_bigquery_sql_stg_00_job_0_refresh_tableau]
    )
    (
        launch_run_bigquery_sql_stg_00_job_0_refresh_tableau
        >> pelican_validator
