...
dag_id = "gcp_prc_main_apt_is_supplier_17284_MERCH_ANALYTICS_insights_framework"
...
default_args = {
    ...
    "description": "prc_main_apt_is_supplier_17284_MERCH_ANALYTICS_insights_framework DAG Description",
...
with DAG(
    ...
    apt_is_main_apt_is_supplier = open(
        os.path.join(sql_path, "apt_is/main_apt_is_supplier.sql"), "r"
    ).read()

    launch_run_bigquery_sql_stg_0_job_0 = BigQueryInsertJobOperator(
        task_id="run_bigquery_sql_stg_0_job_0",
        configuration={
            "query": {"query": apt_is_main_apt_is_supplier, "useLegacySql": False}
        },
    ...
    # job_name='run-teradata-sql-stg-0-job-0-refresh-tableau-prc-main-apt-is-su',
    # macros=None,
    # container_image='artifactory.nordstrom.com/docker/app08499/insights-framework/tableau-agent:0.1.7',
    # container_command=['python', '/home/nonroot/agent/main.py', '--source_type', 'datasource', '--project_name', 'APT Self Service', '--site', 'AS', '--datasource_name', 'APT Self Service - Supplier', '--vault_path', 'data/application/APP06792/9850/merch_analytics/prod/db/tableau/tableau_credentials', '--vault_host', 'https://vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com:8200'],
    # resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    # common_envs=[],
    # task_envs=[{'name': 'TIER', 'value': '3'}, {'name': 'STATSD_HOST', 'value': 'gostatsd.kube-system.svc.cluster.local'}, {'name': 'STATSD_PORT', 'value': '8128'}, {'name': 'APP_ID', 'value': 'APP08499'}, {'name': 'NEW_RELIC_APP_NAME', 'value': 'onehop-insights-framework-APP08499-PROD'}, {'name': 'ENV', 'value': 'PROD'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}, {'name': 'DAG_ID', 'value': 'prc_main_apt_is_supplier_17284_MERCH_ANALYTICS_insights_framework'}],
    ...
    (
        creds_setup
        >> launch_run_bigquery_sql_stg_0_job_0
        >> pelican_validator
        >> pelican_sensor
    )
