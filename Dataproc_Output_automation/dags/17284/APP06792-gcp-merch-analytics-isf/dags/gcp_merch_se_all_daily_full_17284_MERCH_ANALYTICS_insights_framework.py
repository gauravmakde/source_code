...
dag_id = 'gcp_merch_se_all_daily_full_17284_MERCH_ANALYTICS_insights_framework'
...
default_args = {
    ...
    'description': 'merch_se_all_daily_full_17284_MERCH_ANALYTICS_insights_framework DAG Description',
...
with DAG(
    ...
    scaled_events_ma_pipeline_prod_main_se_all_daily_full = open( os.path.join(sql_path, 'scaled_events/ma_pipeline_prod_main_se_all_daily_full.sql'),"r").read()

    launch_run_terdata_sql_00_job_1 = BigQueryInsertJobOperator(
        task_id = "run_terdata_sql_00_job_1",
        configuration = 
        {
            "query": 
            {
                "query": scaled_events_ma_pipeline_prod_main_se_all_daily_full,
                "useLegacySql": False
            }
        },
    ...
    scaled_events_ma_pipeline_prod_main_se_all_daily_clk_rms_full = open( os.path.join(sql_path, 'scaled_events/ma_pipeline_prod_main_se_all_daily_clk_rms_full.sql'),"r").read()

    launch_run_terdata_sql_00_job_2 = BigQueryInsertJobOperator(
        task_id = "run_terdata_sql_00_job_2",
        configuration = 
        {
            "query": 
            {
                "query": scaled_events_ma_pipeline_prod_main_se_all_daily_clk_rms_full,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    creds_setup >> launch_run_terdata_sql_00_job_1 >>[launch_run_terdata_sql_00_job_2]
    launch_run_terdata_sql_00_job_2 >> pelican_validator >> pelican_sensor