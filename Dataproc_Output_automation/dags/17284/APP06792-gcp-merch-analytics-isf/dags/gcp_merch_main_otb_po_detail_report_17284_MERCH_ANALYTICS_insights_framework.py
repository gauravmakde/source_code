...
dag_id = "gcp_merch_main_otb_po_detail_report_17284_MERCH_ANALYTICS_insights_framework"
...
default_args = {
    ...
    "description": "merch_main_otb_po_detail_report_17284_MERCH_ANALYTICS_insights_framework DAG Description",
...
with DAG(
    ...
    # external_dag_id='gcp_merch_main_otb_po_detail_base_17284_MERCH_ANALYTICS_insights_framework',
    # timeout=1800,
    # execution_date_fn=get_most_recent_dag_run)

    launch_check_external_upstream_dependencies_sensor_0 = DummyOperator(
        task_id="launch_check_external_upstream_dependencies_sensor_0", dag=dag
    )
    launch_check_external_upstream_dependencies_sensor_1 = DummyOperator(
        task_id="launch_check_external_upstream_dependencies_sensor_1", dag=dag
    )

    # launch_check_external_upstream_dependencies_sensor_1 = ExternalTaskSensor(
    # dag=dag,
    # task_id='check_external_upstream_dependencies_1',
    # external_dag_id='gcp_ast_prod_main_otbcm_17284_MERCH_ANALYTICS_insights_framework',
    # timeout=1800,
    # execution_date_fn=get_most_recent_dag_run)

    open_to_buy_ast_pipeline_prod_main_otb_po_detail_report = open(
        os.path.join(
            sql_path, "open_to_buy/ast_pipeline_prod_main_otb_po_detail_report.sql"
        ),
        "r",
    ).read()

    launch_run_create_stg_tbl_0_job_0 = BigQueryInsertJobOperator(
        task_id="run_create_stg_tbl_0_job_0",
        configuration={
            "query": {
                "query": open_to_buy_ast_pipeline_prod_main_otb_po_detail_report,
                "useLegacySql": False,
            }
        },
    ...
    (
        ...
        >> [launch_run_create_stg_tbl_0_job_0]
    )
    (
        creds_setup
    ...
    launch_run_create_stg_tbl_0_job_0 >> pelican_validator >> pelican_sensor
