...
dag_id = 'gcp_merch_main_cc_type_17284_MERCH_ANALYTICS_insights_framework'
...
default_args = {
    ...
    'description': 'merch_main_cc_type_17284_MERCH_ANALYTICS_insights_framework DAG Description',
...
with DAG(
    ...
    assortment_common_tables_ast_pipeline_prod_main_sku_cc_lkp = open( os.path.join(sql_path, 'assortment_common_tables/ast_pipeline_prod_main_sku_cc_lkp.sql'),"r").read()

    launch_run_teradata_sql_stg_0_sku_cc_lkp = BigQueryInsertJobOperator(
        task_id = "run_teradata_sql_stg_0_sku_cc_lkp",
        configuration = 
        {
            "query": 
            {
                "query": assortment_common_tables_ast_pipeline_prod_main_sku_cc_lkp,
                "useLegacySql": False
            }
        },
    ...
    assortment_common_tables_ast_pipeline_prod_main_cc_type = open( os.path.join(sql_path, 'assortment_common_tables/ast_pipeline_prod_main_cc_type.sql'),"r").read()

    launch_run_teradata_sql_stg_0_cc_type = BigQueryInsertJobOperator(
        task_id = "run_teradata_sql_stg_0_cc_type",
        configuration = 
        {
            "query": 
            {
                "query": assortment_common_tables_ast_pipeline_prod_main_cc_type,
                "useLegacySql": False
            }
        },
        project_id = project_id,
    ...
    creds_setup >> launch_check_external_upstream_dependencies_sensor_0 >>launch_run_teradata_sql_stg_0_sku_cc_lkp
    launch_run_teradata_sql_stg_0_sku_cc_lkp >>launch_run_teradata_sql_stg_0_cc_type
    launch_run_teradata_sql_stg_0_cc_type >> pelican_validator >> pelican_sensor