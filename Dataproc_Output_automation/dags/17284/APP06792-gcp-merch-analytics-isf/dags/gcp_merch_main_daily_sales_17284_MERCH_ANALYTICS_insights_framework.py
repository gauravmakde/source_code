...
dag_id = 'gcp_merch_main_daily_sales_17284_MERCH_ANALYTICS_insights_framework'
...
default_args = {
    ...
    'description': 'merch_main_daily_sales_17284_MERCH_ANALYTICS_insights_framework DAG Description',
...
with DAG(
    ...
    ma_pipeline_prod_main_daily_sales = open(
    os.path.join(sql_path, 'ma_pipeline_prod_main_daily_sales.sql'),
    "r").read()
    ...
    launch_run_teradata_sql_stg_00_job_0 = BigQueryInsertJobOperator(
        ...
        configuration = 
        {
            "query": 
            {
                "query": ma_pipeline_prod_main_daily_sales,
                "useLegacySql": False
            }
        },
    ...
    # job_name='run-teradata-sql-stg-00-job-0-refresh-tableau-merch-main-daily',
    # macros=None,
    # container_image='artifactory.nordstrom.com/docker/app08499/insights-framework/tableau-agent:0.1.7',
    # container_command=['python', '/home/nonroot/agent/main.py', '--source_type', 'workbook', '--project_name', 'Merchandising', '--site', 'AS', '--workbook_name', 'Daily Sales & Profitability', '--vault_path', 'data/application/APP06792/9850/merch_analytics/prod/db/tableau/tableau_credentials', '--vault_host', 'https://vpce-01f9458aff654a035-ng0ubact.vpce-svc-00627e41a40fb1e3c.us-west-2.vpce.amazonaws.com:8200'],
    # resources={'limits': {'cpu': 5, 'memory': '6Gi' },'requests': {'cpu': 3, 'memory': '2Gi'}},
    # common_envs=[],
    # task_envs=[{'name': 'TIER', 'value': '3'}, {'name': 'STATSD_HOST', 'value': 'gostatsd.kube-system.svc.cluster.local'}, {'name': 'STATSD_PORT', 'value': '8128'}, {'name': 'APP_ID', 'value': 'APP08499'}, {'name': 'NEW_RELIC_APP_NAME', 'value': 'onehop-insights-framework-APP08499-PROD'}, {'name': 'ENV', 'value': 'PROD'}, {'name': 'VAULT_ROLE_ID', 'valueFrom': {'secretKeyRef': {'key': 'roleid', 'name': 'vault'}}}, {'name': 'VAULT_SECRET', 'valueFrom': {'secretKeyRef': {'key': 'secret', 'name': 'vault'}}}, {'name': 'DAG_ID', 'value': 'merch_main_daily_sales_17284_MERCH_ANALYTICS_insights_framework'}],
