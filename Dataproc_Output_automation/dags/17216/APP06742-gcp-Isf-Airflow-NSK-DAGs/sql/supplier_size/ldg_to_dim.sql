...
MERGE INTO `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_dim.item_supplier_size_dim AS tgt
USING (SELECT ldg.npin_num, NULLIF(ldg.rms_sku_id, '') AS rms_sku_id, ldg.supplier_size_1, ldg.supplier_size_1_desc, NULLIF(ldg.supplier_size_2, '') AS supplier_size_2, NULLIF(ldg.supplier_size_2_desc, '') AS supplier_size_2_desc, CAST(TRUNC(cast(ldg.header_version as float64)) AS INTEGER) AS header_version,
 ...
 '+00:00' AS created_tmstp_tz,
  elt.batch_id, elt.curr_batch_date
    FROM `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_stg.item_supplier_size_ldg AS ldg
        INNER JOIN `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_base_vws.elt_control AS elt ON LOWER(elt.subject_area_nm) = LOWER('ITEM_SUPPLIER_SIZE_DATA_SERVICE')
        LEFT JOIN `{{params.gcp_project_id}}`.{{params.dbenv}}_nap_base_vws.item_supplier_size_dim AS dim ON LOWER(ldg.npin_num) = LOWER(dim.npin_num) AND LOWER(COALESCE(ldg.rms_sku_id, '')) = LOWER(COALESCE(dim.rms_sku_id, ''))
