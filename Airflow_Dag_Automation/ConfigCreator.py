...
def update_production_config(config_folder, dag_id, email_details, sql_env_vars, app_id, dag_id_script_mapping, pipeline_sql_path,target_dag_id):
...
def update_delivery_devlopment_config(config_folder, dag_id, email_details, sql_env_vars,app_id,dag_id_script_mapping,pipeline_sql_path,target_dag_id):
...
def config_dag_specific_creation():
# Ensure the output folder exists
    print("Dag specific config started")

    os.makedirs(Output_Folder_Path, exist_ok=True)

    dags_folders = find_dags_folders(main_folder_path)
    count = 0
        ...
        for file in os.listdir(dags_folder):
            if file.endswith('.py'):
                ...
                if dag_id and default_args['email'] is not None:
                    ...
                    update_production_config(config_folder, 'gcp_' + dag_id, default_args, sql_env_vars, app_id, dag_id_script_mapping, pipeline_sql_path,target_dag_id)
                    update_delivery_devlopment_config(config_folder, 'gcp_' + dag_id, default_args, sql_env_vars, app_id, dag_id_script_mapping, pipeline_sql_path,target_dag_id)
